...
void GroupMCastConnection::write(MemoryHandle mem, UInt32 size) {
  ...
  while (size) {
    ...
    if (_queue.waiting()) {
      ...
      _mcastSocket.sendTo(dgram->getBuffer(), dgram->getBufferSize(), _mcastAddress);
    }
#endif
    _queue.put(dgram);
    _lock->release();
...
bool GroupMCastConnection::sendQueue(void) {
  ...
  do {
    ...
    if (count < maxCount) {
      _lock->aquire();
      while (count < maxCount && (count == 0 || !_queue.empty())) {
    ...
    for (; send != end; send = (send + 1) % _windowSize) {
...
#endif
      {
        if (!dgram[send]->getEarlySend())
          _mcastSocket.sendTo(
              dgram[send]->getBuffer(), dgram[send]->getBufferSize(), _mcastAddress);
    ...
    while (
        count && ((readable = _mcastSocket.waitReadable(0)) || (count == maxCount && send == end) ||
      ...
      if (!readable && !_mcastSocket.waitReadable(ackTimeout)) {
        ...
        //                printf("%.10f timeout count %d %d missing
        ...
        _mcastSocket.sendTo(ackRequest.getBuffer(), ackRequest.getBufferSize(), _mcastAddress);
        ...
        if (_sendQueueThreadStop && getSystemTime() - lastAckTime > 1)
      ...
      // read response
      len = _mcastSocket.recvFrom(response.getBuffer(), response.getBufferCapacity(), fromAddress);
      lastAckTime = getSystemTime();
      ...
      if (!Dgram::less(response.getId(), dgram[ack]->getId())) {
        ...
        if (response.getResponseAck() == true) {
          ...
          for (m = ack; dgram[m]->getId() != response.getId(); m = (m + 1) % _windowSize)
            missing[m].erase(fromAddress);
          missing[m].erase(fromAddress);
        } else {
          if (response.getId() == lastNak && getSystemTime() - lastNakTime < 0.02)
            continue;
          lastNak     = response.getId();
          lastNakTime = getSystemTime();
          FDEBUG(("Nack %d from %s:%d\n", response.getId(), fromAddress.getHost().c_str(),
              fromAddress.getPort()));
          //                    printf("Nack %d from
          //                    %s:%d\n",response.getId(),fromAddress.getHost().c_str(),fromAddress.getPort());
          // retransmit
...
void GroupMCastConnection::sendQueueThread(void* arg) {
  ...
  try {
    the->sendQueue();
  } catch (SocketException& e) { FFATAL(("Writer Proc crashed %s\n", e.what())); }
...
void GroupMCastConnection::initialize() {
  ...
  for (index = 0; index < _sockets.size(); ++index) {
    ...
    SINFO << "Server:" << clientHost << " Port:" << clientPort << std::endl;
...
ConnectionType GroupMCastConnection::_type(&GroupMCastConnection::create, "Multicast");
