...
bool Plane::intersect(const Plane& pl, Line& is) const {
  ...
  case 0:
    pnt.setValues(0.f,
        (pl.getNormal()[2] * _distance - pl.getDistanceFromOrigin() * _normalVec[2]) / dir[0],
        (pl.getDistanceFromOrigin() * _normalVec[1] - pl.getNormal()[1] * _distance) / dir[0]);
    break;

  case 1:
    pnt.setValues(
        (pl.getDistanceFromOrigin() * _normalVec[2] - pl.getNormal()[2] * _distance) / dir[1], 0.f,
        (pl.getNormal()[0] * _distance - pl.getDistanceFromOrigin() * _normalVec[0]) / dir[1]);
    break;

  case 2:
    pnt.setValues(
        (pl.getNormal()[1] * _distance - pl.getDistanceFromOrigin() * _normalVec[1]) / dir[2],
        (pl.getDistanceFromOrigin() * _normalVec[0] - pl.getNormal()[0] * _distance) / dir[2], 0.f);
    break;
...
bool Plane::intersectInfinite(const Line& line, Real32& t) const {
  ...
  if (a != 0.0f) {
    t = _normalVec.dot(Pnt3f(_normalVec * _distance) - line.getPosition()) / a;
...
void Plane::transform(const Matrix& matrix) {
  ...
  if (trans[uiValPoint] > Eps || trans[uiValPoint] < -Eps) {
    if ((_normalVec[uiValNorm] < 0.f && trans[uiValPoint] < 0.f) ||
        (_normalVec[uiValNorm] > 0.f && trans[uiValPoint] > 0.f)) {
...
bool operator==(const Plane& p1, const Plane& p2) {
  return ((p1._distance == p2._distance) && (p1._normalVec == p2._normalVec));
}
