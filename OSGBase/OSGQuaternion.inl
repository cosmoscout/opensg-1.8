...
template <class ValueTypeT>
inline const QuaternionBase<ValueTypeT>& QuaternionBase<ValueTypeT>::identity(void) {
  return _identity;
}
...
template <class ValueTypeT>
inline QuaternionBase<ValueTypeT> QuaternionBase<ValueTypeT>::slerp(
...
template <class ValueTypeT>
inline QuaternionBase<ValueTypeT>::QuaternionBase(void) {
  _quat[0] = _quat[1] = _quat[2] = TypeTraits<ValueTypeT>::getZeroElement();

  _quat[3] = TypeTraits<ValueTypeT>::getOneElement();
}

template <class ValueTypeT>
inline QuaternionBase<ValueTypeT>::QuaternionBase(const QuaternionBase& source) {
...
template <class ValueTypeT>
inline QuaternionBase<ValueTypeT>::QuaternionBase(const MatrixType& matrix) {
  setValue(matrix);
}

template <class ValueTypeT>
inline QuaternionBase<ValueTypeT>::QuaternionBase(const VectorType& axis, const ValueTypeT angle) {
...
template <class ValueTypeT>
inline QuaternionBase<ValueTypeT>::QuaternionBase(
    const VectorType& rotateFrom, const VectorType& rotateTo) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setIdentity(void) {
  _quat[0] = _quat[1] = _quat[2] = TypeTraits<ValueTypeT>::getZeroElement();
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisRad(const ValueTypeT* valsP) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisDeg(const ValueTypeT* valsP) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsQuat(const ValueTypeT* valsP) {
  UInt32 i;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisRad(
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisDeg(
    const ValueTypeT x, const ValueTypeT y, const ValueTypeT z, const ValueTypeT w) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsQuat(
    const ValueTypeT x, const ValueTypeT y, const ValueTypeT z, const ValueTypeT w) {
  _quat[0] = x;
  _quat[1] = y;
  _quat[2] = z;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValue(const MatrixType& matrix) {
  Real64 tr;
  Real64 s;
  Real64 qt[3];
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisRad(
    const VectorType& axis, ValueTypeT angle) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValue(
    const VectorType& rotateFrom, const VectorType& rotateTo) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisRad(const Char8* str) {
  setValueAsQuat(str);

  if (osgfinite(_quat[0]) == 0 || osgfinite(_quat[1]) == 0 || osgfinite(_quat[2]) == 0 ||
      osgfinite(_quat[3]) == 0) {
    setIdentity();
  } else {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsAxisDeg(const Char8* str) {
  setValueAsQuat(str);
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValueAsQuat(const Char8* str) {
  UInt32 i;
  UInt32 numOfToken = 4;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::setValue(
    const ValueTypeT alpha, const ValueTypeT beta, const ValueTypeT gamma) {
...
template <class ValueTypeT>
inline const ValueTypeT* QuaternionBase<ValueTypeT>::getValues(void) const {
  return _quat;
}
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValueAsAxisRad(
    ValueTypeT& x, ValueTypeT& y, ValueTypeT& z, ValueTypeT& w) const {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValueAsAxisDeg(
    ValueTypeT& x, ValueTypeT& y, ValueTypeT& z, ValueTypeT& w) const {
  ValueTypeT len;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValueAsQuat(
    ValueTypeT& x, ValueTypeT& y, ValueTypeT& z, ValueTypeT& w) const {
  x = _quat[0];
  y = _quat[1];
  z = _quat[2];
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValueAsAxisRad(
    VectorType& axis, ValueTypeT& radians) const {
  ValueTypeT x;
  ValueTypeT y;
  ValueTypeT z;
  ValueTypeT w;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValueAsAxisDeg(
    VectorType& axis, ValueTypeT& degrees) const {
  ValueTypeT x;
  ValueTypeT y;
  ValueTypeT z;
  ValueTypeT w;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValue(MatrixType& matrix) const {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::getValuesOnly(MatrixType& matrix) const {
  matrix[0][0] = 1.0f - 2.0f * (_quat[Q_Y] * _quat[Q_Y] + _quat[Q_Z] * _quat[Q_Z]);
  matrix[0][1] = 2.0f * (_quat[Q_X] * _quat[Q_Y] + _quat[Q_Z] * _quat[Q_W]);
  matrix[0][2] = 2.0f * (_quat[Q_Z] * _quat[Q_X] - _quat[Q_Y] * _quat[Q_W]);

  matrix[1][0] = 2.0f * (_quat[Q_X] * _quat[Q_Y] - _quat[Q_Z] * _quat[Q_W]);
  matrix[1][1] = 1.0f - 2.0f * (_quat[Q_Z] * _quat[Q_Z] + _quat[Q_X] * _quat[Q_X]);
  matrix[1][2] = 2.0f * (_quat[Q_Y] * _quat[Q_Z] + _quat[Q_X] * _quat[Q_W]);

  matrix[2][0] = 2.0f * (_quat[Q_Z] * _quat[Q_X] + _quat[Q_Y] * _quat[Q_W]);
  matrix[2][1] = 2.0f * (_quat[Q_Y] * _quat[Q_Z] - _quat[Q_X] * _quat[Q_W]);
  matrix[2][2] = 1.0f - 2.0f * (_quat[Q_Y] * _quat[Q_Y] + _quat[Q_X] * _quat[Q_X]);
}

template <class ValueTypeT>
inline ValueTypeT QuaternionBase<ValueTypeT>::x(void) const {
  return _quat[0];
}

template <class ValueTypeT>
inline ValueTypeT QuaternionBase<ValueTypeT>::y(void) const {
  return _quat[1];
}

template <class ValueTypeT>
inline ValueTypeT QuaternionBase<ValueTypeT>::z(void) const {
  return _quat[2];
}

template <class ValueTypeT>
inline ValueTypeT QuaternionBase<ValueTypeT>::w(void) const {
  return _quat[3];
}
...
template <class ValueTypeT>
inline ValueTypeT QuaternionBase<ValueTypeT>::length(void) const {
  return osgsqrt(
      _quat[0] * _quat[0] + _quat[1] * _quat[1] + _quat[2] * _quat[2] + _quat[3] * _quat[3]);
}

//! Norm the quaternion to be of unit length

template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::normalize(void) {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::invert(void) {
  _quat[0] = -_quat[0];
  _quat[1] = -_quat[1];
  _quat[2] = -_quat[2];
  //    _quat[3] =  _quat[3];
...
template <class ValueTypeT>
inline const QuaternionBase<ValueTypeT> QuaternionBase<ValueTypeT>::inverse(void) const {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::multVec(const VectorType& src, VectorType& dst) const {
  ValueTypeT rx, ry, rz;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::scaleAngle(ValueTypeT scaleFactor) {
  VectorType axis;
  ValueTypeT radians;
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::slerpThis(
    const QuaternionBase& rot0, const QuaternionBase& rot1, const ValueTypeT t) {
  slerp(rot0, rot1, *this, t);
}

template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::mult(const QuaternionBase& other) {
  mult(_quat, other._quat);
}

template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::multLeft(const QuaternionBase& other) {
  mult(other._quat, _quat);
}

template <class ValueTypeT>
inline bool QuaternionBase<ValueTypeT>::equals(
    const QuaternionBase& rot, const ValueTypeT tolerance) const {
  bool returnValue = true;

  for (UInt32 i = 0; i < 4; i++) {
    returnValue &=
        ((_quat[i] - rot._quat[i] <= tolerance) && (rot._quat[i] - _quat[i] <= tolerance));
  }
...
template <class ValueTypeT>
inline ValueTypeT& QuaternionBase<ValueTypeT>::operator[](const UInt32 index) {
  return _quat[index];
}

template <class ValueTypeT>
inline const ValueTypeT& QuaternionBase<ValueTypeT>::operator[](const UInt32 index) const {
  return _quat[index];
}
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::operator*=(const QuaternionBase& other) {
  mult(_quat, other._quat);
}
...
template <class ValueTypeT>
inline const QuaternionBase<ValueTypeT>& QuaternionBase<ValueTypeT>::operator=(
...
template <class ValueTypeT>
inline bool QuaternionBase<ValueTypeT>::operator==(const QuaternionBase& other) const {
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::slerp(const QuaternionBase& rot0,
    const QuaternionBase& rot1, QuaternionBase& result, const ValueTypeT t) {
  ValueTypeT rot1q[4];
  ...
  cosom = rot0._quat[0] * rot1._quat[0] + rot0._quat[1] * rot1._quat[1] +
          rot0._quat[2] * rot1._quat[2] + rot0._quat[3] * rot1._quat[3];
...
template <class ValueTypeT>
inline void QuaternionBase<ValueTypeT>::mult(const ValueTypeT rVal1[4], const ValueTypeT rVal2[4]) {
...
template <class ValueTypeT>
inline std::ostream& operator<<(std::ostream& os, const QuaternionBase<ValueTypeT>& obj) {
#ifdef OSG_SGI_STL
  os << std::fixed << std::showpoint << std::setprecision(3) << std::setfill(' ');
#else
  os << std::setprecision(3) << std::setfill(' ');
#endif

  return os << std::setw(8) << obj.x() << " " << obj.y() << " " << obj.z() << " " << obj.w();
}
