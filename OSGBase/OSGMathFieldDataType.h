...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Matrix> : public FieldTraitsRecurseBase<Matrix> {
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Matrix4d> : public FieldTraitsRecurseBase<Matrix4d> {
  static DataType _type;
  ...
  static Matrix4d getDefault(void) {
    return Matrix4d();
  }

  static bool getFromString(Matrix4d& outVal, const Char8*& inVal) {
    outVal.setValue(inVal, false);
    return true;
  }

  static void putToString(const Matrix4d& inVal, std::string& outStr) {
  ...
  static UInt32 getBinSize(const Matrix4d*, UInt32 uiNumObjects) {
    return sizeof(Real64) * 16 * uiNumObjects;
  }

  static void copyToBin(BinaryDataHandler& pMem, const Matrix4d& oObject) {
    pMem.putValues(&oObject[0][0], 16);
  }

  static void copyFromBin(BinaryDataHandler& pMem, Matrix4d& oObject) {
    pMem.getValues(&oObject[0][0], 16);
  }

  static void copyToBin(
      BinaryDataHandler& pMem, const Matrix4d* pObjectStore, UInt32 uiNumObjects) {
    pMem.putValues(&pObjectStore[0][0][0], uiNumObjects * 16);
  }

  static void copyFromBin(BinaryDataHandler& pMem, Matrix4d* pObjectStore, UInt32 uiNumObjects) {
    pMem.getValues(&pObjectStore[0][0][0], uiNumObjects * 16);
  }
};
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Quaternion> : public FieldTraitsRecurseBase<Quaternion> {
  static DataType _type;
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const Quaternion* pObjectStore, UInt32 uiNumObjects) {
