...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Color3f> : public FieldTraitsRecurseBase<Color3f> {
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Color4f> : public FieldTraitsRecurseBase<Color4f> {
  static DataType _type;
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Color3ub> : public FieldTraitsRecurseBase<Color3ub> {
  static DataType _type;
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const Color3ub* pObjectStore, UInt32 uiNumObjects) {
    pMem.putValues(&pObjectStore[0][0], uiNumObjects * 3);
  }
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Color4ub> : public FieldTraitsRecurseBase<Color4ub> {
  static DataType _type;
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const Color4ub* pObjectStore, UInt32 uiNumObjects) {
    pMem.putValues(&pObjectStore[0][0], uiNumObjects * 4);
  }
...
struct OSG_BASE_DLLMAPPING FieldDataTraits<std::string>
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const std::string* pObjectStore, UInt32 uiNumObjects) {
    for (UInt32 i = 0; i < uiNumObjects; ++i) {
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits1<Time> : public FieldTraitsRecurseBase<Time> {
  static DataType _type;
...
struct OSG_BASE_DLLMAPPING FieldDataTraits<DynamicVolume>
  ...
  static const Char8* getSName(void) {
    return "SFDynamicVolume";
  }

  static const Char8* getMName(void) {
    return "MFDynamicVolume";
  }
  ...
  static bool getFromString(DynamicVolume& outVal, const Char8*& inVal) {
    ...
    Int16 count = sscanf(str, "%f %f %f %f %f %f", &valStore[0], &valStore[1], &valStore[2],
        &valStore[3], &valStore[4], &valStore[5]);

    if (count == 4) {
      outVal.setVolumeType(DynamicVolume::SPHERE_VOLUME);

      SphereVolume* pSVol = dynamic_cast<SphereVolume*>(&(outVal.getInstance()));
      ...
      BoxVolume* pBVol = dynamic_cast<BoxVolume*>(&(outVal.getInstance()));

      pBVol->setBounds(
          valStore[0], valStore[1], valStore[2], valStore[3], valStore[4], valStore[5]);
  ...
  static void putToString(const DynamicVolume& inVal, std::string& outStr) {
    ...
    case DynamicVolume::SPHERE_VOLUME:
      ...
      const SphereVolume& sVol = dynamic_cast<const SphereVolume&>(inVal.getInstance());
      ...
      outStr.append(TypeTrait::putToString(sVol.getCenter()[1]));
      outStr.append(" ");
      outStr.append(TypeTraits<Real32>::putToString(sVol.getRadius()));
      break;
    }
  }
  ...
  static void copyToBin(BinaryDataHandler& pMem, const DynamicVolume& oObject) {
    ...
    switch (type) {
    case DynamicVolume::BOX_VOLUME: {
      const BoxVolume* pBVol = dynamic_cast<const BoxVolume*>(&(oObject.getInstance()));
    ...
    case DynamicVolume::SPHERE_VOLUME: {
      const SphereVolume* pSVol = dynamic_cast<const SphereVolume*>(&(oObject.getInstance()));
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const DynamicVolume* pObjectStore, UInt32 uiNumObjects) {
    for (UInt32 i = 0; i < uiNumObjects; ++i) {
      copyToBin(pMem, pObjectStore[i]);
    }
  }
  ...
  static void copyFromBin(BinaryDataHandler& pMem, DynamicVolume& oObject) {
    ...
    pMem.getValue(type);
    DynamicVolume::Type volumeType = static_cast<DynamicVolume::Type>(type);
    oObject.setVolumeType(volumeType);

    switch (type) {
    case DynamicVolume::BOX_VOLUME: {
      BoxVolume* pBVol = dynamic_cast<BoxVolume*>(&(oObject.getInstance()));
    ...
    case DynamicVolume::SPHERE_VOLUME: {
      SphereVolume* pSVol = dynamic_cast<SphereVolume*>(&(oObject.getInstance()));
  ...
  static void copyFromBin(
      BinaryDataHandler& pMem, DynamicVolume* pObjectStore, UInt32 uiNumObjects) {
    for (UInt32 i = 0; i < uiNumObjects; ++i) {
      copyFromBin(pMem, pObjectStore[i]);
    }
  }
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits1<BitVector> : public FieldTraitsRecurseBase<BitVector> {
  static DataType _type;
  ...
  static void copyToBin(
      BinaryDataHandler& pMem, const BitVector* pObjectStore, UInt32 uiNumObjects) {
    for (UInt32 i = 0; i < uiNumObjects; ++i) {
      copyToBin(pMem, pObjectStore[i]);
    }
  }
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits1<GLenum> : public FieldTraitsRecurseBase<GLenum> {
  static DataType                  _type;
...
template <>
struct OSG_BASE_DLLMAPPING FieldDataTraits<Plane> : public FieldTraitsRecurseBase<Plane> {
  static DataType _type;
