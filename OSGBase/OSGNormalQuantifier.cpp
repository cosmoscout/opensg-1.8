...
UInt32 NormalQuantifier::getIndex(Vec3f& normal, UInt32 numberSubdivisions) const {
  UInt32 index;
  UInt32 nS;
  UInt32 octant;
  Int32  xoctant;
  ...
  octant = (((normal.x() >= 0) ? 0 : 1) << 2) | (((normal.y() >= 0) ? 0 : 1) << 1) |
           ((normal.z() >= 0) ? 0 : 1);
...
// ----------------------------------------------
bool NormalQuantifier::rayTriangle(
...
void NormalQuantifier::subdivide(
    Vec3f point1, Vec3f point2, Vec3f point3, UInt32 number, UInt32& index) {
  number--;
...
UInt32 NormalQuantifier::getSubIndex(
    Vec3f point, Vec3f point1, Vec3f point2, Vec3f point3, UInt32 number) const {
  ...
  if (rayTriangle(point, point1, newPoint1, newPoint2))
    intersect = 0;
  else if (rayTriangle(point, newPoint1, point2, newPoint3))
    intersect = 1;
  else if (rayTriangle(point, newPoint1, newPoint2, newPoint3))
    intersect = 2;
  else if (rayTriangle(point, newPoint2, newPoint3, point3))
    intersect = 3;
