...
bool intersect(const BoxVolume& box1, const BoxVolume& box2) {
  ...
  if (box1.isEmpty() == true || box2.isEmpty() == true) {
    ...
    retCode = (box1.getMin()[0] <= box2.getMax()[0] && box1.getMax()[0] >= box2.getMin()[0]) &&
              (box1.getMin()[1] <= box2.getMax()[1] && box1.getMax()[1] >= box2.getMin()[1]) &&
              (box1.getMin()[2] <= box2.getMax()[2] && box1.getMax()[2] >= box2.getMin()[2]);
  }
...
bool intersect(const BoxVolume& box, const CylinderVolume& cylinder) {
  ...
  if (box.isEmpty() == true || cylinder.isEmpty() == true) {
    ...
    u = p - c;
    u1 =
        (u[0] * adir[0] + u[1] * adir[1] + u[2] * adir[2]) / (adir.length() * adir.length()) * adir;
...
float dist3D_Segment_to_Segment(
    const Pnt3f& s1p, const Vec3f& s1d, const Pnt3f& s2p, const Vec3f& s2d) {
...
OSG_BASE_DLLMAPPING
bool intersect(const CylinderVolume& cyl1, const CylinderVolume& cyl2) {
  Pnt3f p1, p2;
  Vec3f v1, v2;
  cyl1.getAxis(p1, v1);
  cyl2.getAxis(p2, v2);
  return dist3D_Segment_to_Segment(p1, v1, p2, v2) <= (cyl1.getRadius() + cyl2.getRadius());
}
...
OSG_BASE_DLLMAPPING
bool intersect(
    const FrustumVolume& OSG_CHECK_ARG(frustum1), const FrustumVolume& OSG_CHECK_ARG(frustum2)) {
...
void extend(BoxVolume& srcVol, const BoxVolume& vol) {
  if ((!srcVol.isValid() && !srcVol.isEmpty()) || srcVol.isInfinite() || srcVol.isStatic()) {
    return;
  }
  ...
  srcVol.setBounds(osgMin(vol.getMin().x(), srcVol.getMin().x()),
      osgMin(vol.getMin().y(), srcVol.getMin().y()), osgMin(vol.getMin().z(), srcVol.getMin().z()),
      osgMax(vol.getMax().x(), srcVol.getMax().x()), osgMax(vol.getMax().y(), srcVol.getMax().y()),
...
void extend(BoxVolume& srcVol, const SphereVolume& vol) {
  ...
  srcVol.setBounds(osgMin(min.x(), srcVol.getMin().x()), osgMin(min.y(), srcVol.getMin().y()),
      osgMin(min.z(), srcVol.getMin().z()), osgMax(max.x(), srcVol.getMax().x()),
      osgMax(max.y(), srcVol.getMax().y()), osgMax(max.z(), srcVol.getMax().z()));
...
OSG_BASE_DLLMAPPING
void extend(BoxVolume& OSG_CHECK_ARG(srcVol), const FrustumVolume& OSG_CHECK_ARG(vol)) {
...
void extend(SphereVolume& srcVol, const BoxVolume& vol) {
  ...
  if (srcVol.isEmpty()) {
    if (vol.isEmpty()) {
      ...
      c = Pnt3f((min.x() + max.x()) * 0.5f, (min.y() + max.y()) * 0.5f, (min.z() + max.z()) * 0.5f);
  ...
  vol1.setBounds(osgMin(min.x(), min1.x()), osgMin(min.y(), min1.y()), osgMin(min.z(), min1.z()),
      osgMax(max.x(), max1.x()), osgMax(max.y(), max1.y()), osgMax(max.z(), max1.z()));
...
void extend(SphereVolume& srcVol, const SphereVolume& vol) {
  ...
  c = Pnt3f(
      (min2.x() + max2.x()) * 0.5f, (min2.y() + max2.y()) * 0.5f, (min2.z() + max2.z()) * 0.5f);
...
OSG_BASE_DLLMAPPING
void extend(SphereVolume& OSG_CHECK_ARG(srcVol), const FrustumVolume& OSG_CHECK_ARG(vol)) {
...
void extend(CylinderVolume& srcVol, const SphereVolume& vol) {
  ...
  if (srcVol.isEmpty()) {
    if (vol.isEmpty()) {
      ...
      adir = Vec3f(vol.getCenter().x() + r - apos.x(), vol.getCenter().y() + r - apos.y(),
...
OSG_BASE_DLLMAPPING
void extend(CylinderVolume& OSG_CHECK_ARG(srcVol), const FrustumVolume& OSG_CHECK_ARG(vol)) {
...
OSG_BASE_DLLMAPPING
void extend(FrustumVolume& OSG_CHECK_ARG(srcVol), const BoxVolume& OSG_CHECK_ARG(vol)) {
  FFATAL(("extend (frustum/box) is not impl.\n"));
  return;
}

OSG_BASE_DLLMAPPING
void extend(FrustumVolume& OSG_CHECK_ARG(srcVol), const SphereVolume& OSG_CHECK_ARG(vol)) {
  FFATAL(("extend (frustum/sphere) is not impl.\n"));
  return;
}

OSG_BASE_DLLMAPPING
void extend(FrustumVolume& OSG_CHECK_ARG(srcVol), const CylinderVolume& OSG_CHECK_ARG(vol)) {
  FFATAL(("extend (frustum/cylinder) is not impl.\n"));
  return;
}

OSG_BASE_DLLMAPPING
void extend(FrustumVolume& OSG_CHECK_ARG(srcVol), const FrustumVolume& OSG_CHECK_ARG(vol)) {
  FFATAL(("extend (frustum/frustum) is not impl.\n"));
  return;
}

OSG_BASE_DLLMAPPING
void extend(FrustumVolume& OSG_CHECK_ARG(srcVol), const Volume& OSG_CHECK_ARG(vol)) {
  FFATAL(("extend (frustum/volume) is not impl.\n"));
  return;
}
