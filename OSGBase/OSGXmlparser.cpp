  ...
  while (1 == 1) {
    ...
    case '!': {
      ...
      if (!token3.is_literal()) {
        ...
        if (token3.get_generic().at(0) == '-' && token3.get_generic().at(1) == '-')
    ...
    case '?': {
      ...
      if (ctxptr->handle_events())
        ctxptr->get_eventhandler().processing_instruction(pinode);
...
void xmlparser::parse_node(xmlnode& node, xmlcontextptr& ctxptr) {
  ...
  do {
    ...
    if (!token1.is_literal()) {
      ...
      if (handle)
        ctxptr->get_eventhandler().got_cdata(cdatanode.cdata);
  ...
  if (token3 == '/') {
    ...
    if (handle)
      ctxptr->get_eventhandler().end_node(node); // akira said so
  ...
  if (handle)
    ctxptr->get_eventhandler().end_node(node);
}
