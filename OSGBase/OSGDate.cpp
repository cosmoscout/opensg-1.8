...
bool Date::valid(void) {
  static const UInt32 daysPerMonth[12] = {31, 28, 31, 30, 31, 30, 30, 31, 30, 31, 30, 31};
...
void Date::set(UInt32 day, UInt32 month, Int32 year, UInt32 hour, UInt32 minute, UInt32 sec) {
  _day    = day;
  _month  = month;
  _year   = year;
  _hour   = hour;
...
std::ostream& OSG::operator<<(std::ostream& outStream, const Date& obj) {
  return outStream << std::setfill('0') << std::setw(2) << obj._day << '.' << std::setw(2)
                   << obj._month << '.' << obj._year << ' ' << std::setw(2) << obj._hour << ':'
...
std::istream& OSG::operator>>(std::istream& inStream, Date& obj) {
  ...
  return inStream >> obj._day >> c >> obj._month >> c >> obj._year >> obj._hour >> c >>
         obj._minute >> c >> obj._second;
}
