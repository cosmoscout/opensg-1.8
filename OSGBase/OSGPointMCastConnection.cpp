...
/*! connect to the given group. If timeout is reached, -1 is
    returned
 */
Connection::Channel PointMCastConnection::connectGroup(const std::string& address, Time timeout) {
...
Connection::Channel PointMCastConnection::selectChannel(Time timeout) {
  ...
  try {
    if (!_initialized)
      initialize();
    // todo
    if (isReadBufferEmpty() && !_lastDgram && _queue.empty()) {
      if (timeout == -1) {
        // wait for a dgram
        _lock->aquire();
        _queue.wait(_lock);
...
bool PointMCastConnection::recvNextDgram(Dgram* dgram) {
  ...
  if (selection.isSetRead(_responseSocket)) {
    length = _responseSocket.recvFrom(dgram->getBuffer(), dgram->getBufferCapacity(), from);
...
// ????
  ...
  if (selection.isSetRead(_mcastSocket)) {
    length = _mcastSocket.recvFrom(dgram->getBuffer(), dgram->getBufferCapacity(), from);
    dgram->setBufferSize(length);
...
void PointMCastConnection::combineAck(Dgram* dgram, SocketAddress from) {
  ...
  maxAck = _seqNumber - 1;
  for (std::map<SocketAddress, UInt16>::iterator aI = _combineAck.begin(); aI != _combineAck.end();
       ++aI) {
  ...
  if (Dgram::less(_maxAck, maxAck)) {
    ...
    _responseSocket.sendTo(response.getBuffer(), response.getBufferSize(), _ackDestination);
  }
}
  ...
  for (;;) {
    ...
    do {
      ...
      do {
        ...
        id = dgram->getId();
      } while (missing && id != _seqNumber && (getSystemTime() - ignoreT) < 0.01);
      ...
      if (!response.getResponseAck() || !_mcastSocket.waitReadable(0)) {
#ifdef TEST_LOST_DGRAM_RATE
        if (drand48() > TEST_LOST_DGRAM_RATE)
#endif
          if (response.getResponseAck()) {
            ...
            if (_combineAck.empty() || id == _maxAck)
              _responseSocket.sendTo(
                  response.getBuffer(), response.getBufferSize(), _ackDestination);
            else
              combineAck(NULL, _sender);
          } else {
            // send nak to sender
            _responseSocket.sendTo(response.getBuffer(), response.getBufferSize(), _sender);
          }
      }
...
ConnectionType PointMCastConnection::_type(&PointMCastConnection::create, "Multicast");
