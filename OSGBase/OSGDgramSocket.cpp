...
void DgramSocket::open() {
  ...
  int on = 1;
  if (::setsockopt(_sd, SOL_SOCKET, SO_BROADCAST, (SocketOptT*)&on, sizeof(on)) < 0) {
...
int DgramSocket::recvFrom(void* buf, int size, SocketAddress& from) {
  ...
  do {
#endif
    len = recvfrom(_sd, (char*)buf, size, 0, from.getSockAddr(), &addrLen);
#ifndef WIN32
...
int DgramSocket::peekFrom(void* buf, int size, SocketAddress& from) {
  ...
  len = recvfrom(_sd, (char*)buf, size, MSG_PEEK, from.getSockAddr(), &addrLen);
  if (len == -1) {
#if defined WIN32
    if (getError() == WSAECONNRESET) {
      throw SocketConnReset("recvfrom");
...
void DgramSocket::join(const SocketAddress& group, const SocketAddress& interf) {
  ...
  // group to join
  joinAddr.imr_multiaddr.s_addr = ((sockaddr_in*)group.getSockAddr())->sin_addr.s_addr;

  // interface that joins. (equal to bind address)
  joinAddr.imr_interface = ((struct sockaddr_in*)interf.getSockAddr())->sin_addr;
  rc = setsockopt(_sd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (SocketOptT*)&joinAddr, sizeof(joinAddr));
  if (rc < 0) {
...
void DgramSocket::leave(const SocketAddress& group, const SocketAddress& interf) {
  ...
  joinAddr.imr_multiaddr.s_addr = ((sockaddr_in*)group.getSockAddr())->sin_addr.s_addr;
  // interface that joins. (equal to bind address)
  joinAddr.imr_interface = ((sockaddr_in*)interf.getSockAddr())->sin_addr;
  rc = setsockopt(_sd, IPPROTO_IP, IP_DROP_MEMBERSHIP, (SocketOptT*)&joinAddr, sizeof(joinAddr));
  if (rc < 0) {
...
void DgramSocket::setTTL(unsigned char ttl) {
  int rc = setsockopt(_sd, IPPROTO_IP, IP_MULTICAST_TTL, (SocketOptT*)&ttl, sizeof(ttl));
  if (rc < 0) {
...
void DgramSocket::setMCastInterface(const SocketAddress& interf) {
  int rc = setsockopt(_sd, IPPROTO_IP, IP_MULTICAST_IF, (SocketOptT*)interf.getSockAddr(),
