...
enum LogType { LOG_NONE = 0, LOG_STDOUT = 1, LOG_STDERR = 2, LOG_FILE = 3, LOG_BUFFER = 4 };
  ...
  {                                                                                                \
    if (!condition) {                                                                              \
      ...
      OSG::osgLog().stream(OSG::LOG_FATAL)                                                         \
          ...
          << " FATAL ASSERT: " << (doExit ? "exit system" : "try to keep running") << std::flush   \
          << std::endl;                                                                            \
      OSG::osgLog().unlock();                                                                      \
      if (doExit)                                                                                  \
        exit(-1);                                                                                  \
  ...
  {                                                                                                \
    if (!condition) {                                                                              \
      ...
      OSG::osgLog().stream(OSG::LOG_FATAL)                                                         \
          ...
          << " FATAL ASSERT: " << (doExit ? "exit system" : "try to keep running") << std::flush   \
          << std::endl;                                                                            \
      OSG::osgLogP->doLog par OSG::osgLog().unlock();                                              \
      if (doExit)                                                                                  \
        exit(-1);                                                                                  \
    }                                                                                              \
  }
...
#define FPFPATAL(par)                                                                              \
  {                                                                                                \
    ...
    if (OSG::osgLogP->checkLevel(OSG::LOG_FPATAL)) {                                               \
      OSG::osgStartLog(false, OSG::LOG_FPATAL, OSG_LOG_MODULE, __FILE__, __LINE__);                \
      OSG::osgLogP->doLog par;                                                                     \
      OSG::osgLogP->unlock();                                                                      \
    }                                                                                              \
  }
...
#define FPINFPO(par)                                                                               \
  {                                                                                                \
    ...
    if (OSG::osgLogP->checkLevel(OSG::LOG_INFPO)) {                                                \
      OSG::osgStartLog(false, OSG::LOG_INFPO, OSG_LOG_MODULE, __FILE__, __LINE__);                 \
      OSG::osgLogP->doLog par;                                                                     \
      OSG::osgLogP->unlock();                                                                      \
    }                                                                                              \
  }
...
#define FPDEBUG(par)                                                                               \
  {                                                                                                \
    OSG::initLog();                                                                                \
    if (OSG::osgLogP->checkLevel(OSG::LOG_DEBUG)) {                                                \
      OSG::osgStartLog(false, OSG::LOG_DEBUG, OSG_LOG_MODULE, __FILE__, __LINE__);                 \
      OSG::osgLogP->doLog par;                                                                     \
      OSG::osgLogP->unlock();                                                                      \
    }                                                                                              \
  }
