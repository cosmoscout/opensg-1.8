...
bool CylinderVolume::intersect(const Line& line, Real32& enter, Real32& exit) const {
...
bool CylinderVolume::isOnSurface(const Pnt3f& point) const {
  ...
  return (onplane && dist <= _radius) || (!onplane && osgabs(dist - _radius) < Eps);
}
