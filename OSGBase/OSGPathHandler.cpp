...
std::string PathHandler::findFile(const Char8* fileName) {
  ...
  if (tmpList.size() != 0) {
    if ((pType & TypeMask) == AbsPath) {
      ...
      if (File::tstAttr(fileName, AccessFlags::IsReadable)) {
      ...
      if (_baseFilePath.empty() == false) {
        ...
        if (File::tstAttr(returnValue.c_str(), AccessFlags::IsReadable) == false) {
          returnValue.erase();
        } else {
          bFound = true;
        }
      ...
      if (bFound == false) {
        while (iter != listEnd) {
          ...
          if (File::tstAttr(returnValue.c_str(), AccessFlags::IsReadable) == true) {
            break;
          }
...
PathHandler::PathType PathHandler::analysePathList(const Char8* pathList) {
  ...
  if (returnValue == UnixPath) {
    ...
    if (returnValue == UnixPath) {
      if (uiSize >= 3) {
        if (pathList[1] == ':' && (pathList[2] == '/' || pathList[2] == '\\')) {
          returnValue = Win32Path;
        }
      }
    }
...
void PathHandler::expandWin32Path(std::string& path) {
  ...
  while (currPos < path.size()) {
    if (path[currPos] == '%') {
      ...
      while (path[currPos] != '%' && currPos < path.size()) {
        envVar += path[currPos++];
      }
...
void PathHandler::expandUnixPath(std::string& path) {
  ...
  while (currPos < path.size()) {
    if (path[currPos] == '$') {
      ...
      while (path[currPos] != ':' && path[currPos] != '/' && path[currPos] != '$' &&
             currPos < path.size()) {
        envVar += path[currPos++];
      }
...
void PathHandler::splitPathList(const Char8* pathList, const Char8 pathSep, PathList& result) {
  std::string::size_type currPos  = 0;
  std::string::size_type startPos = 0;
  ...
  if (currPos == std::string::npos) {
    ...
    while (currPos != std::string::npos) {
      result.push_back(workString.substr(startPos, currPos - startPos));
