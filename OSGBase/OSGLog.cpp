...
void LogBuf::setCallback(LogBuf::Callback cb, void* clientData, bool flushData) {
  std::list<LogBuf::Chunk*>::iterator cI;
  LogBuf::Chunk*                      chunk;
...
const Char8* Log::_levelName[] = {"LOG", "FATAL", "WARNING", "NOTICE", "INFO", "DEBUG", 0};

const Char8* Log::_levelColor[] = {0, // LOG
    "\x1b[31m",                       // FATAL
    "\x1b[33m",                       // WARNING
    0,                                // NOTICE
    0,                                // INFO
...
bool Log::colorHeader(LogLevel level, const char* sep) {
  ...
  if (hOutput == INVALID_HANDLE_VALUE || !GetConsoleScreenBufferInfo(hOutput, &csbiInfo)) {
    ok = false;
  } else {
...
void Log::setHeaderElem(UInt32 elemMask, bool force) {
  ...
  if (!force && (this == osgLogP) && (env = getenv("OSG_LOG_HEADER"))) {
    osgLog() << "Log::setHeaderElem: overriden by envvar OSG_LOG_HEADER '" << env << "'." << endLog;
...
void Log::addModuleHandling(LogModuleHandling handling, bool OSG_CHECK_ARG(force)) {
  _moduleHandling |= handling;
}

void Log::delModuleHandling(LogModuleHandling handling, bool OSG_CHECK_ARG(force)) {
  _moduleHandling &= ~handling;
}
...
bool Log::hasModule(const Char8* module) {
  ...
  if (module && *module) {
    for (mI = _moduleList.begin(); retCode || (mI != _moduleList.end()); ++mI) {
      retCode = (mI->isStatic) ? (module == mI->name) : (!strcmp(module, mI->name));
    }
  }
...
void Log::setLogType(LogType logType, bool force) {
  static Char8* typenames[] = {"none", "-", "stdout", "stderr", "file", "buffer", NULL};

  static LogType types[] = {LOG_NONE, LOG_STDOUT, LOG_STDOUT, LOG_STDERR, LOG_FILE, LOG_BUFFER};
  ...
  if (!force && (this == osgLogP) && (et = getenv("OSG_LOG_TYPE"))) {
    osgLog() << "Log::setLogType: overriden by envvar OSG_LOG_TYPE '" << et << "'." << std::endl;
...
void Log::setLogLevel(LogLevel logLevel, bool force) {
  static Char8* levelnames[] = {"log", "fatal", "warning", "notice", "info", "debug", NULL};

  static LogLevel levels[] = {LOG_LOG, LOG_FATAL, LOG_WARNING, LOG_NOTICE, LOG_INFO, LOG_DEBUG};
  ...
  if (!force && (this == osgLogP) && (el = getenv("OSG_LOG_LEVEL"))) {
    osgLog() << "OSGLog::setLogLevel: overriden by envvar OSG_LOG_LEVEL '" << el << "'."
             << std::endl;
...
void Log::setLogFile(const Char8* fileName, bool force) {
  ...
  if (!force && (this == osgLogP) && (name = getenv("OSG_LOG_FILE"))) {
    osgLog() << "Log::setLogFile: overriden by envvar OSG_LOG_FILE '" << name << "'." << std::endl;
  } else {
    name = fileName;
  }
