...
bool Line::intersect(const CylinderVolume& cyl, Real32& enter, Real32& exit) const {
  ...
  if (ln == 0) // IntersectionLine is parallel to CylinderAxis
  {
    ...
    if (isect) { // if ray hits cylinder
      ...
      s = osgabs((osgsqrt((radius * radius) - (dl * dl))) / (_dir.dot(O)));
...
bool Line::intersect(Real32 OSG_CHECK_ARG(angle), const BoxVolume& OSG_CHECK_ARG(box)) const {
  // TODO
  assert(false);
  return false;
}
...
bool Line::intersect(Real32 OSG_CHECK_ARG(angle), const Vec3f& OSG_CHECK_ARG(point)) const {
  // TODO
  assert(false);
  return false;
}
...
bool Line::intersect(Real32 OSG_CHECK_ARG(angle), const Vec3f& OSG_CHECK_ARG(v0),
    const Vec3f& OSG_CHECK_ARG(v1), Vec3f& OSG_CHECK_ARG(pt)) const {
  // TODO
  assert(false);
  return false;
}
...
bool Line::intersect(
    const Pnt3f& v0, const Pnt3f& v1, const Pnt3f& v2, Real32& t, Vec3f* norm) const {
