...
BinaryDataHandler::BinaryDataHandler(UInt32 zeroCopyThreshold, bool networkOrder)
    : _readBuffers()
    , _writeBuffers()
    , _zeroCopyBuffers()
...
void BinaryDataHandler::put(void const* src, UInt32 size) {
  ...
  if (_zeroCopyThreshold && size >= _zeroCopyThreshold) {
    if (_zeroCopyThreshold == 1) {
      ...
      _zeroCopyBuffers.push_back(MemoryBlock(const_cast<MemoryHandle>(data), size, size));
    }
  } else {
    UInt32 copySize;
    ...
    while (size != 0) {
      ...
      copySize = osgMin((_currentWriteBuffer->getSize() - _currentWriteBufferPos), size);

      memcpy(_currentWriteBuffer->getMem() + _currentWriteBufferPos, data, copySize);
...
void BinaryDataHandler::get(void* dst, UInt32 size) {
  ...
  if (_zeroCopyThreshold && size >= _zeroCopyThreshold) {
    ...
    while (size != 0) {
      ...
      // num bytes to copy
      copySize = osgMin((_currentReadBuffer->getDataSize() - _currentReadBufferPos), size);

      // no data in buffer ?
      if (copySize != 0) {
        memcpy(data, _currentReadBuffer->getMem() + _currentReadBufferPos, copySize);
...
void BinaryDataHandler::readBufAdd(MemoryHandle mem, UInt32 size, UInt32 dataSize) {
...
void BinaryDataHandler::writeBufAdd(MemoryHandle mem, UInt32 size, UInt32 dataSize) {
  MemoryBlock memBlock(mem, size, dataSize);
...
void BinaryDataHandler::readBuffer(void) {
  ...
  for (i = readBufBegin(); size != 0; ++i) {
    if (i == readBufEnd()) {
      SFATAL << "Read buffer is to small. " << size << "bytes missing" << std::endl;
...
void BinaryDataHandler::read(MemoryHandle OSG_CHECK_ARG(src), UInt32 OSG_CHECK_ARG(size)) {
  SWARNING << "BinaryDataHandler::read(MemoryHandle src,int size) called" << std::endl;
}
...
void BinaryDataHandler::write(MemoryHandle OSG_CHECK_ARG(src), UInt32 OSG_CHECK_ARG(size)) {
  SWARNING << "BinaryDataHandler::write(MemoryHandle mem,int size) called" << std::endl;
}
...
void BinaryDataHandler::pushBuffer() {
  ...
  for (i = _zeroCopyBuffers.begin(); i != _zeroCopyBuffers.end(); ++i) {
    write(i->getMem(), i->getDataSize());
  }
