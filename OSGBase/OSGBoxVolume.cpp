...
Real32 BoxVolume::getScalarVolume() const {
  return (isEmpty() == true) ? 0.0f
                             : (_max[0] - _min[0]) * (_max[1] - _min[1]) * (_max[2] - _min[2]);
}
...
//! set method
void BoxVolume::setBoundsByCenterAndSize(const Pnt3f& center, const Vec3f& size) {
  _min.setValues(
...
bool BoxVolume::intersect(const Pnt3f& pt) const {
  return (!isEmpty() && (_min[0] < pt[0] && _max[0] > pt[0]) &&
          (_min[1] < pt[1] && _max[1] > pt[1]) && (_min[2] < pt[2] && _max[2] > pt[2]));
}
...
bool BoxVolume::isOnSurface(const Pnt3f& point) const {
  if (((osgabs(point[0] - _min[0]) < Eps || osgabs(point[0] - _max[0]) < Eps) &&
          (point[1] >= _min[1] && point[1] <= _max[1] && point[2] >= _min[2] &&
              point[2] <= _max[2])) ||

      ((osgabs(point[1] - _min[1]) < Eps || osgabs(point[1] - _max[1]) < Eps) &&
          (point[0] >= _min[0] && point[0] <= _max[1] && point[2] >= _min[2] &&
              point[2] <= _max[2])) ||

      ((osgabs(point[2] - _min[2]) < Eps || osgabs(point[2] - _max[2]) < Eps) &&
          (point[1] >= _min[1] && point[1] <= _max[1] && point[0] >= _min[0] &&
              point[0] <= _max[0]))) {
    return true;
  } else {
    return false;
...
void BoxVolume::dump(UInt32 OSG_CHECK_ARG(uiIndent), const BitVector OSG_CHECK_ARG(bvFlags)) const {
  PLOG << "Box(" << _min << "|" << _max << ")";
}
...
bool operator==(const BoxVolume& b1, const BoxVolume& b2) {
  return ((b1._min[0] == b2._min[0]) && (b1._min[1] == b2._min[1]) && (b1._min[2] == b2._min[2]) &&
          (b1._max[0] == b2._max[0]) && (b1._max[1] == b2._max[1]) && (b2._max[2] == b2._max[2]));
}
