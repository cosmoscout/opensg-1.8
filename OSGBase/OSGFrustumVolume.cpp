...
Real32 FrustumVolume::getScalarVolume() const {
  const Int32 faces[6][4] = {
      {0, 1, 3, 2}, {4, 5, 7, 6}, {0, 4, 5, 1}, {2, 6, 7, 3}, {2, 6, 4, 0}, {1, 5, 7, 3}};
  ...
  for (Int32 i = 0; i < 6; i++) {
    ...
    height = _planeVec[i].getNormal().dot(center) - _planeVec[i].getDistanceFromOrigin();
...
void FrustumVolume::getBounds(Pnt3f& OSG_CHECK_ARG(minPnt), Pnt3f& OSG_CHECK_ARG(maxPnt)) const {
}

/*------------------------------ feature ----------------------------------*/

void FrustumVolume::setPlanes(const Plane& pnear, const Plane& pfar, const Plane& left,
    const Plane& right, const Plane& top, const Plane& bottom) {
  _planeVec[0] = pnear;
  _planeVec[1] = pfar;
  _planeVec[2] = left;
  _planeVec[3] = right;
...
void FrustumVolume::setPlanes(const Pnt3f& nlt, const Pnt3f& nlb, const Pnt3f& nrt,
    const Pnt3f& nrb, const Pnt3f& flt, const Pnt3f& flb, const Pnt3f& frt, const Pnt3f& frb) {
...
void FrustumVolume::setPlanes(const Matrix& objectClipMat) {
  ...
  for (Int32 i = 0; i < 6; i++) {
    vectorLength = osgsqrt(planeEquation[i][0] * planeEquation[i][0] +
...
bool FrustumVolume::intersect(const Pnt3f& point) const {
  ...
  for (Int32 i = 0; i < 6; i++) {
    if ((_planeVec[i].getNormal().x() * point.x() + _planeVec[i].getNormal().y() * point.y() +
            _planeVec[i].getNormal().z() * point.z() - _planeVec[i].getDistanceFromOrigin()) <
        0.f) {
      retCode = false;
      break;
    }
...
bool FrustumVolume::intersect(const Line& line, Real32& minDist, Real32& maxDist) const {
...
void FrustumVolume::dump(
    UInt32 OSG_CHECK_ARG(uiIndent), const BitVector OSG_CHECK_ARG(bvFlags)) const {
#if 1
  fprintf(stderr,
      "Frustum:(%f %f %f:%f)(%f %f %f:%f)(%f %f %f:%f)"
      "(%f %f %f:%f)(%f %f %f:%f)(%f %f %f:%f)\n",
      _planeVec[0].getNormal()[0], _planeVec[0].getNormal()[1], _planeVec[0].getNormal()[2],
      _planeVec[0].getDistanceFromOrigin(), _planeVec[1].getNormal()[0],
      _planeVec[1].getNormal()[1], _planeVec[1].getNormal()[2],
      _planeVec[1].getDistanceFromOrigin(), _planeVec[2].getNormal()[0],
      _planeVec[2].getNormal()[1], _planeVec[2].getNormal()[2],
      _planeVec[2].getDistanceFromOrigin(), _planeVec[3].getNormal()[0],
      _planeVec[3].getNormal()[1], _planeVec[3].getNormal()[2],
      _planeVec[3].getDistanceFromOrigin(), _planeVec[4].getNormal()[0],
      _planeVec[4].getNormal()[1], _planeVec[4].getNormal()[2],
      _planeVec[4].getDistanceFromOrigin(), _planeVec[5].getNormal()[0],
      _planeVec[5].getNormal()[1], _planeVec[5].getNormal()[2],
      _planeVec[5].getDistanceFromOrigin());

#else
  PLOG << "Frustum(" << _planeVec[0].getNormal() << ":" << _planeVec[0].getDistanceFromOrigin()
       << "|" << _planeVec[1].getNormal() << ":" << _planeVec[1].getDistanceFromOrigin() << "|"
       << _planeVec[2].getNormal() << ":" << _planeVec[2].getDistanceFromOrigin() << "|"
       << _planeVec[3].getNormal() << ":" << _planeVec[3].getDistanceFromOrigin() << "|"
       << _planeVec[4].getNormal() << ":" << _planeVec[4].getDistanceFromOrigin() << "|"
       << _planeVec[5].getNormal() << ":" << _planeVec[5].getDistanceFromOrigin() << "|"
       << ")";
#endif
}
...
bool operator==(const FrustumVolume& b1, const FrustumVolume& b2) {
  return ((static_cast<const Volume&>(b1) == b2) && (b1.getPlanes()[0] == b2.getPlanes()[0]) &&
          (b1.getPlanes()[1] == b2.getPlanes()[1]) && (b1.getPlanes()[2] == b2.getPlanes()[2]) &&
          (b1.getPlanes()[3] == b2.getPlanes()[3]) && (b1.getPlanes()[4] == b2.getPlanes()[4]) &&
