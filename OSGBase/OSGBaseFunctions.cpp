...
bool CompileConfig::compare(const CompileConfig& c) {
  FDEBUG(("CompileConfig::compare: lib: version %d.%d.%d, %sdebug, %sdll, "
          "%smt \n",
      _major, _minor, _release, _debug ? "" : "non-", _dll ? "" : "non-", _mt ? "" : "non-"));
  FDEBUG(("CompileConfig::compare: prog: version %d.%d.%d, %sdebug, %sdll, "
          "%smt \n",
      c._major, c._minor, c._release, c._debug ? "" : "non-", c._dll ? "" : "non-",
      c._mt ? "" : "non-"));
  ...
  if (_release != c._release) {
    FNOTICE(("The program is compiled against version %d.%d.%d headers, "
             ...
             "in your report.\n",
        _major, _minor, _release, c._major, c._minor, c._release));
  }
  if (_mt != c._mt) {
    FFATAL(("The program is linked against the %s-threaded runtime lib, "
        ...
        c._mt ? "multi" : "single", _mt ? "multi" : "single", _mt ? "multi" : "single",
  ...
  if (_debug != c._debug) {
    FFATAL(("The program is linked against the %sdebug runtime lib, "
        ...
        c._debug ? "" : "non-", _debug ? "" : "non-", _debug ? "" : "non-", _mt ? "D" : "L",
        _debug ? "d" : "", _mt ? "D" : "L", _debug ? "d" : ""));
    return true;
  }
#endif
...
bool osgInit(
    Int32, Char8**, UInt16 major, UInt16 minor, UInt16 release, bool debug, bool dll, bool mt) {
  CompileConfig prog(major, minor, release, debug, dll, mt),
      lib(OSG_MAJOR_VERSION, OSG_MINOR_VERSION, OSG_RELEASE_VERSION,
#ifdef _DEBUG
          true,
#else
          false,
  ...
  if (osgPreloadSharedObject != NULL) {
    for (UInt32 i = 0; i < osgPreloadSharedObject->size(); ++i) {
      SharedObjectHandler::the()->getOSGSharedObject((*osgPreloadSharedObject)[i].c_str());
    }
  }
  ...
  if (szEnvLibs != NULL) {
    ...
    while (libIt != libEnd) {
      SharedObjectHandler::the()->getOSGSharedObject((*libIt).c_str());
  ...
  if (szEnvPlugins != NULL) {
    ...
    std::vector<Char8*>* pPlugins = Directory::getEntries(szEnvPlugins, szEnvPluginPattern);
    ...
    if (pPlugins != NULL) {
      ...
      for (UInt32 i = 0; i < pPlugins->size(); ++i) {
        if ((*pPlugins)[i][0] == '.') {
          if ((*pPlugins)[i][1] == '\0') {
            continue;
          } else if ((*pPlugins)[i][1] == '.' && (*pPlugins)[i][2] == '\0') {
            continue;
          }
        }
        ...
        SharedObjectHandler::the()->getSharedObject(szPluginName.c_str());
      }
    }
...
string_token_iterator::string_token_iterator(const std::string& str, const char* separator)
