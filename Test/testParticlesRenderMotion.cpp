...
int matherr(struct math_exception* x) {
  ...
  switch (x->type) {
  case DOMAIN:
    strcpy(sterr, "domain error");
    break;
  case SING:
    strcpy(sterr, "singularity");
    break;
  case OVERFLOW:
    strcpy(sterr, "overflow");
    break;
  case UNDERFLOW:
    strcpy(sterr, "underflow");
    break;
  case TLOSS:
    strcpy(sterr, "total loss of significance");
    break;
  case PLOSS:
    strcpy(sterr, "partial loss of significance");
    break;
  }

  if (!strcmp(x->name, "atan2") || !strcmp(x->name, "atan2f") || !strcmp(x->name, "pow") ||
      !strcmp(x->name, "powf") || !strcmp(x->name, "modff") || !strcmp(x->name, "modf") ||
      !strcmp(x->name, "atan2") || !strcmp(x->name, "ldexp"))
    argc = 2;

  if (argc == 1)
    fprintf(stderr, "%s in %s(%lf), res %lf\n", sterr, x->name, x->arg1, x->retval);
  else
    fprintf(stderr, "%s in %s(%lf, %lf), res %lf\n", sterr, x->name, x->arg1, x->arg2, x->retval);
...
void enableFPE(void) {
  ...
  csr.fc_struct.en_divide0 = csr.fc_struct.en_overflow = csr.fc_struct.en_underflow = 1;
...
int main(int argc, char** argv) {
  ...
  if (!testSet) {
    ...
    Pnt3f tpos[] = {Pnt3f(.5, .5, .5), Pnt3f(.5, .5, .7), Pnt3f(.5, .5, .9), Pnt3f(.7, .5, .5),
        Pnt3f(.5, .7, .5), Pnt3f(-1000, -1000, -1000)};

    Pnt3f tsecpos[] = {
        Pnt3f(0, 0, 0), Pnt3f(0, 0, 0), Pnt3f(0, 0, 0), Pnt3f(0, 0, 0), Pnt3f(0, 0, 0)};

    Vec3f tvel[] = {Vec3f(0, 0, 0), Vec3f(0, 0, 0), Vec3f(0, 0, 0), Vec3f(0, 0, 0), Vec3f(0, 0, 0)};

    Color3f tcol[] = {Color3f(1, 0, 0), Color3f(0, 1, 0), Color3f(0, 0, 1), Color3f(1, 1, 0),
        Color3f(1, 0, 1), Color3f(0, 1, 1), Color3f(1, 1, 1)};

    Vec3f tsize[] = {
        Vec3f(.1, 0, 0), Vec3f(.1, 0, 0), Vec3f(.1, 0, 0), Vec3f(.1, 0, 0), Vec3f(.1, 0, 0)};
  ...
  UChar8   imgdata[] = {255, 255, 255, 255, 0, 0, 255, 0, 255, 255, 0, 0, 255, 0, 0, 255, 255, 255};
...
void idle(void) {
  if (doMotion) {
    ...
    beginEditCP(particles, Particles::PositionsFieldMask | Particles::SecPositionsFieldMask);
    particles->setPositions(secpnts);
    particles->setSecPositions(pnts);
    endEditCP(particles, Particles::PositionsFieldMask | Particles::SecPositionsFieldMask);
...
void display(void) {
  ...
  if (runBench) {
    ...
    if (--preframes < 0) {
      ...
      stop = getSystemTime();
      FLOG(("100 frames a %d parts in %.2f secs (%.0f parts/sec)\n", numParticles, stop - start,
          100 * numParticles / (stop - start)));
      exit(0);
    }
  }
...
class TestHandler : public OSG::BinaryDataHandler {
 public:
  TestHandler(FILE* file)
      : OSG::BinaryDataHandler(5)
      , _file(file) {
    unsigned int i;
    _memory.resize(7 * 2);
...
void keyboard(unsigned char k, int, int) {
  ...
  case 27:
    exit(1);
  case '0':
    beginEditCP(particles, Particles::ModeFieldMask);
  ...
  case '6':
    ...
    break;
  case 'q':
    beginEditCP(particles, Particles::DrawOrderFieldMask);
  ...
  case 'w':
    ...
    break;
  case 'a':
    beginEditCP(particles, Particles::DynamicFieldMask);
  ...
  case 'b': {
    ...
    // Bin
    std::cout << "BIN:" << FieldDataTraits<ParticleBSPTree>::getBinSize(bsp) << std::endl;
