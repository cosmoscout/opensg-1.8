...
NewActionTypes::ResultE enterDefault(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);
...
int main(int argc, char* argv[]) {
  ...
  IntersectActor::regDefaultClassEnter(osgTypedFunctionFunctor2CPtr<NewActionTypes::ResultE,
      NodeCorePtr, ActorBase::FunctorArgumentType&>(enterDefault));
  ...
  for (UInt32 i = 0; i < uiNumRays; ++i) {
    bool DFfastest =
        ((resultsDF[i]._time <= resultsDFS[i]._time) && (resultsDF[i]._time <= resultsP[i]._time) &&
    ...
    bool Pfastest =
        ((resultsP[i]._time <= resultsDF[i]._time) && (resultsP[i]._time <= resultsDFS[i]._time) &&
            (resultsP[i]._time <= resultsO[i]._time));
    bool Ofastest =
        ((resultsO[i]._time <= resultsDF[i]._time) && (resultsO[i]._time <= resultsDFS[i]._time) &&
            (resultsO[i]._time <= resultsP[i]._time));

    if ((resultsDF[i]._hit == resultsDFS[i]._hit) && (resultsDFS[i]._hit == resultsP[i]._hit) &&
      ...
      if ((osgabs(resultsDF[i]._dist - resultsDFS[i]._dist) >= 0.001) ||
        ...
        SINFO << "FAIL: df: " << resultsDF[i]._dist << " dfs: " << resultsDFS[i]._dist
              << " p: " << resultsP[i]._dist << " o: " << resultsO[i]._dist << endLog;
        SINFO << "FAIL: df: " << resultsDF[i]._tri << " dfs: " << resultsDFS[i]._tri
              << " p: " << resultsP[i]._tri << " o: " << resultsO[i]._tri << endLog;
      } else {
        ++passCount;
      }
    ...
    // SINFO << i << " \t" << (DFfastest  ? "D ->" : "    ") << " hit: " << resultsDF [i]._hit << "
    // time: " << resultsDF [i]._time << endLog; SINFO << "  \t"     << (DFSfastest ? "S ->" : " ")
    // << " hit: " << resultsDFS[i]._hit << " time: " << resultsDFS[i]._time << endLog; SINFO << "
    // \t"     << (Pfastest   ? "P ->" : "    ") << " hit: " << resultsP  [i]._hit << " time: " <<
    // resultsP  [i]._time << endLog; SINFO << "  \t"     << (Ofastest   ? "O ->" : "    ") << " hit:
    // " << resultsO  [i]._hit << " time: " << resultsO  [i]._time << endLog;
  }

  SINFO << " df total:  " << tDFTotal
        << (tDFTotal < tDFSTotal && tDFTotal < tPTotal && tDFTotal < tOTotal ? " *" : "  ")
        << " wins: " << DFwins << " ("
        << (static_cast<Real32>(DFwins) / static_cast<Real32>(passCount)) * 100.0 << "%)\t"
        << " wins on hit: " << DFwinsHit << " ("
        << (static_cast<Real32>(DFwinsHit) / static_cast<Real32>(hitCount)) * 100.0 << "%)\t"
        << " wins on miss: " << DFwinsMiss << " ("
        << (static_cast<Real32>(DFwinsMiss) / static_cast<Real32>(missCount)) * 100.0 << "%)"
        << endLog;

  SINFO << " dfs total: " << tDFSTotal
        << (tDFSTotal < tDFTotal && tDFSTotal < tPTotal && tDFSTotal < tOTotal ? " *" : "  ")
        << " wins: " << DFSwins << " ("
        << (static_cast<Real32>(DFSwins) / static_cast<Real32>(passCount)) * 100.0 << "%)\t"
        << " wins on hit: " << DFSwinsHit << " ("
        << (static_cast<Real32>(DFSwinsHit) / static_cast<Real32>(hitCount)) * 100.0 << "%)\t"
        << " wins on miss: " << DFSwinsMiss << " ("
        << (static_cast<Real32>(DFSwinsMiss) / static_cast<Real32>(missCount)) * 100.0 << "%)"
        << endLog;

  SINFO << " p total:   " << tPTotal
        << (tPTotal < tDFTotal && tPTotal < tDFSTotal && tPTotal < tOTotal ? " *" : "  ")
        << " wins: " << Pwins << " ("
        << (static_cast<Real32>(Pwins) / static_cast<Real32>(passCount)) * 100.0 << "%)\t"
        << " wins on hit: " << PwinsHit << " ("
        << (static_cast<Real32>(PwinsHit) / static_cast<Real32>(hitCount)) * 100.0 << "%)\t"
        << " wins on miss: " << PwinsMiss << " ("
        << (static_cast<Real32>(PwinsMiss) / static_cast<Real32>(missCount)) * 100.0 << "%)"
        << endLog;

  SINFO << " o total:   " << tOTotal
        << (tOTotal < tDFTotal && tOTotal < tDFSTotal && tOTotal < tPTotal ? " *" : "  ")
        << " wins: " << Owins << " ("
        << (static_cast<Real32>(Owins) / static_cast<Real32>(passCount)) * 100.0 << "%)\t"
        << " wins on hit: " << OwinsHit << " ("
        << (static_cast<Real32>(OwinsHit) / static_cast<Real32>(hitCount)) * 100.0 << "%)\t"
        << " wins on miss: " << OwinsMiss << " ("
        << (static_cast<Real32>(OwinsMiss) / static_cast<Real32>(missCount)) * 100.0 << "%)"
        << endLog;

  SINFO << "pass: " << passCount << " fail: " << failCount << " hit: " << hitCount
        << " miss: " << missCount << endLog;
...
NodePtr buildGraphRecurse(UInt32 depth, UInt32 maxDepth, GeometryPtr pGeo);

NodePtr buildGraph(void) {
  GeometryPtr pGeo = makeBoxGeo(0.08 * X_SIZE * (1.0 / (SCENE_DEPTH + 1.0)),
      0.08 * Y_SIZE * (1.0 / (SCENE_DEPTH + 1.0)), 0.08 * Z_SIZE * (1.0 / (SCENE_DEPTH + 1.0)), 1,
      1, 1);
...
void createRays(UInt32 uiNumRays, std::vector<Line>& rays) {
  ...
  for (UInt32 i = 0; i < uiNumRays; ++i) {
    ...
    rays.push_back(Line(Pnt3f(xPos, yPos, zPos), Vec3f(xDir, yDir, zDir)));
  }
