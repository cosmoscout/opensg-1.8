...
UInt32 primitiveCount(GeometryPtr geoPtr, UInt32& triangle, UInt32& line, UInt32& point) {
  UInt32                                                       lenI;
...
void displayInfo(int x, int y) {
  ...
  sprintf(text,
      ...
      "Triangles:  %12u\n"
      "Geometries: %12u",
      1.0 / frame_time, sum_positions, sum_triangles, sum_geometries);
...
void motion(int x, int y) {
  ...
  OSG::Real32 a = -2. * (lastx / w - .5), b = -2. * (.5 - lasty / h), c = -2. * (x / w - .5),
              d = -2. * (.5 - y / h);
...
void init(std::vector<std::string>& filenames) {
    ...
    for (x = -ca / 2.0; x < ca / 2.0; x++)
      for (y = -cb / 2.0; y < cb / 2.0; y++)
        for (z = -cc / 2.0; z < cc / 2.0; z++) {
          ...
          node->setCore(trans);
          trans->getMatrix().setTranslate(x * size[0] * 1.1, y * size[1] * 1.1, z * size[2] * 1.1);
  ...
  OSG::Vec3f pos(min[0] + ((max[0] - min[0]) * 0.5), min[1] + ((max[1] - min[1]) * 0.5),
...
int main(int argc, char** argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (strlen(argv[i]) > 1 && argv[i][0] == '-') {
      ...
      case 'g':
        opt = argv[i][2] ? argv[i] + 2 : argv[++i];
        if (sscanf(opt, "%d,%d,%d,%d", &winwidth, &winheight, &winx, &winy) != 4)
      ...
      case 'i':
        ...
        break;
      default:
        std::cout << argv[0] << "-ffile -m -rrows[,cols] -C -M" << std::endl;
  ...
  try {
    ...
    case 'L':
      ...
      if (!composerType.empty()) {
        FieldContainerPtr fcPtr =
            FieldContainerFactory::the()->createFieldContainer(composerType.c_str());
    ...
    glutMainLoop();
  } catch (OSG_STDEXCEPTION_NAMESPACE::exception& e) { SLOG << e.what() << std::endl; }
  return 0;
}
