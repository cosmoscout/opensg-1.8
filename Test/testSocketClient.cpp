...
void broadcastClient() {
  ...
  do {
    // write to server
    sock.sendTo("hallo", 6, SocketAddress(SocketAddress::BROADCAST, 23344));
...
void multicastClient() {
  ...
  SocketAddress   server;
  SocketSelection sel;
  SocketAddress   addr[3] = {SocketAddress("224.0.0.50", 23344), SocketAddress("224.0.0.53", 23344),
      SocketAddress("224.0.0.50", 23344)};
  char*           msg[3]  = {"msg:1", "msg:2", "_END_"};
  int             i;
...
int main(int argc, char** argv) {
  try {
    ...
    broadcastClient();
    multicastClient();
  } catch (exception& e) { SWARNING << e.what() << endl; }
  return 0;
}
