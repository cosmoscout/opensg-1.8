...
void save_rgb(const char* pcFileName, unsigned char* pucNormalImageData, unsigned int uiWidth,
    unsigned int uiHeight, int iNumColorChannels) {
  FILE* pt_output;
  ...
  // write data
  int i_bytes_written =
      fwrite(pucNormalImageData, 1, uiWidth * uiHeight * iNumColorChannels, pt_output);
...
void save_raw(const char* pcFileName, unsigned char* pucNormalImageData, unsigned int uiWidth,
    unsigned int uiHeight, int iNumColorChannels) {
  FILE* pt_output;
  int   i_num_bytes = 0;
...
bool raw_to_rgb(unsigned char*& rpucNormalImageData, unsigned int uiWidth, unsigned int uiHeight,
    int iNumColorChannels) {
  ...
  for (unsigned int ui_y = 0; ui_y < uiHeight; ui_y++)
    for (unsigned int ui_x = 0; ui_x < uiWidth; ui_x++)
      for (int i_channel = 0; i_channel < iNumColorChannels; i_channel++)
        //				pucTempData[i_channel*uiWidth*uiHeight+( uiHeight -
        //ui_y)*uiWidth+ui_x]= *(pucCurrentIndex++);
...
NodePtr makeChessboardNode(Real32 fSize) {
  ...
  */
  ...
  FLOG (("%d , %d\n", pcl_textimage_orig->getMipMapCount(),
  pcl_textimage_orig_2->getMipMapCount())); FLOG (("%d , %d\n", pcl_textimage_orig->getFrameCount(),
  pcl_textimage_orig_2->getFrameCount())); FLOG (("%d , %d\n", pcl_textimage_orig->getFrameDelay(),
  pcl_textimage_orig_2->getFrameDelay())); FLOG (("%d , %d\n", pcl_textimage_orig->getPixelFormat(),
  pcl_textimage_orig_2->getPixelFormat())); FLOG (("%d , %d\n", pcl_textimage_orig->getDataType(),
  pcl_textimage_orig_2->getDataType()));
...
pcl_mat->setMagFilter    (GL_NEAREST);
  ...
  endEditCP(pcl_mat);
  beginEditCP(pcl_geo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                           Geometry::PositionsFieldMask | Geometry::TexCoordsFieldMask |
  ...
  endEditCP(pcl_geo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                         Geometry::PositionsFieldMask | Geometry::TexCoordsFieldMask |
                         Geometry::MaterialFieldMask);
