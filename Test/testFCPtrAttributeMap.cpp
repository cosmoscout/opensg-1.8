...
int main(int argc, char* argv[]) {
  ...
  if (attr_map == attr_map2) {
    ...
    std::cerr << "Second call to OSG::fcptrAttributeMap() returned a "
              << "different attribute map!" << std::endl;
  }
  ...
  if (attr_map->getKeys().size() == 3) {
    ...
    std::cerr << "Incorrect number of attributes (" << attr_map->getKeys().size()
              << ") after adding 3" << std::endl;
  }

  if (attr_map->hasAttribute(test1_attr)) {
    std::cerr << "Attribute " << test1_attr << " added successfully" << std::endl;
  } else {
    std::cerr << "Failed to add attribute " << test1_attr << std::endl;
  }

  if (attr_map->hasAttribute(test2_attr)) {
    std::cerr << "Attribute " << test2_attr << " added successfully" << std::endl;
  } else {
    std::cerr << "Failed to add attribute " << test2_attr << std::endl;
  }

  if (attr_map->hasAttribute(test3_attr)) {
    std::cerr << "Attribute " << test3_attr << " added successfully" << std::endl;
  } else {
  ...
  if (test1_value == test1_value_check) {
    std::cerr << "[Attempt #1] Value for " << test1_attr << " is correct" << std::endl;
    ++success_count;
  } else {
    std::cerr << "[Attempt #1] Value for " << test1_attr << " is incorrect; " << test1_value
  ...
  if (test1_value == test1_value_check) {
    std::cerr << "[Attempt #2] Value for " << test1_attr << " is correct" << std::endl;
    ++success_count;
  } else {
    std::cerr << "[Attempt #2] Value for " << test1_attr << " is incorrect; " << test1_value
              << " expected but got " << test1_value_check << std::endl;
  }
  ...
  if (test1_value == test1_value_check) {
    std::cerr << "[Attempt #3] Value for " << test1_attr << " is correct" << std::endl;
    ++success_count;
  } else {
    std::cerr << "[Attempt #3] Value for " << test1_attr << " is incorrect; " << test1_value
              << " expected but got " << test1_value_check << std::endl;
  }

  if (success_count != 3) {
    std::cerr << 3 - success_count << " variation(s) of attribute value querying failed!!"
              << std::endl;
  }

  OSG::FieldContainerPtr test2_value_check = attr_map->getAttribute(test2_attr);

  if (test2_value == test2_value_check) {
    std::cerr << "Value for " << test2_attr << " is correct" << std::endl;
  } else {
    std::cerr << "Value for " << test2_attr << " is incorrect; " << test2_value
              << " expected but got " << test2_value_check << std::endl;
  }
  ...
  if (test3_value == test3_value_check) {
    ...
    std::cerr << "Value for " << test3_attr << " is incorrect; " << test3_value
              << " expected but got " << test3_value_check << std::endl;
  }
