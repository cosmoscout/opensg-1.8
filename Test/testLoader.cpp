...
int main(int argc, char** argv) {
  ...
  SceneFileHandler::the().getSuffixList(l, SceneFileType::OSG_READ_SUPPORTED);

  std::cout << "Formats supported for reading: ";
  std::copy(l.begin(), l.end(), std::ostream_iterator<std::string>(std::cout, ","));
  std::cout << std::endl;

  SceneFileHandler::the().getSuffixList(l, SceneFileType::OSG_WRITE_SUPPORTED);
  ...
  std::cerr << "Volume: center " << center << ", volume " << node->getVolume().getScalarVolume()
            << std::endl;
