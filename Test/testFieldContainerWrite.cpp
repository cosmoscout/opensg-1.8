...
class OSGWriter {
 ...
 private:
  ...
  struct SharedFCInfoHelper {
    ...
    bool               named;
    std::string        name;
    static std::string buildName(FieldContainerPtr fcptr, UInt32 num);
...
void OSGWriter::doListFC(FieldContainerPtr fieldConPtr) {
  ...
  if (_fcmap.insert(std::make_pair(fieldConPtr, SharedFCInfoHelper())).second == true) {
      ...
      if (!fDesc->isInternal()) {
        if (strstr(fType.getCName(), "Ptr") != NULL) {
          if (fieldPtr->getCardinality() == FieldType::SINGLE_FIELD) {
            doListFC(((SFFieldContainerPtr*)fieldPtr)->getValue());
          } else if (fieldPtr->getCardinality() == FieldType::MULTI_FIELD) {
            for (UInt32 j = 0; j < ((MFFieldContainerPtr*)fieldPtr)->size(); ++j) {
              doListFC((*(((MFFieldContainerPtr*)fieldPtr)))[j]);
            }
          }
        }
      }
    ...
    if ((*iter).second.named == false) {
      (*iter).second.name  = SharedFCInfoHelper::buildName((*iter).first, _sharedFCCount++);
...
void OSGWriter::doPrintListedFC(FieldContainerPtr fieldConPtr) {
  ...
  if ((*iter).second.printed == false) {
    ...
    _outstream << fieldConPtr->getTypeName() << " {" << std::endl;
    for (UInt32 i = 1; i <= fcType.getNumFieldDescs(); ++i) {
      FieldDescription* fDesc    = fcType.getFieldDescription(i);
      Field*            fieldPtr = fieldConPtr->getField(i);
      const FieldType&  fType    = fieldPtr->getType();
      if (!fDesc->isInternal()) {
        ...
        if (strstr(fType.getCName(), "Ptr") != NULL) {
          if (fieldPtr->getCardinality() == FieldType::SINGLE_FIELD) {
            ...
            doPrintListedFC(((SFFieldContainerPtr*)fieldPtr)->getValue());
            _indention -= _indentStep;
          } else if (fieldPtr->getCardinality() == FieldType::MULTI_FIELD) {
            _outstream << " [" << std::endl;
            _indention += _indentStep;
            for (UInt32 j = 0; j < ((MFFieldContainerPtr*)fieldPtr)->size(); ++j) {
              doPrintListedFC((*(((MFFieldContainerPtr*)fieldPtr)))[j]);
            }
            _indention -= _indentStep;
            indentLine();
            _outstream << "]" << std::endl;
    ...
    indentLine();
    _outstream << "USE " << (*iter).second.name << std::endl;
  }
}

std::string OSGWriter::SharedFCInfoHelper::buildName(FieldContainerPtr, UInt32 num) {
  std::string temp;
  temp.assign("FCName");
