...
void key(unsigned char key, int /*x*/, int /*y*/) {
  ...
  case 's':
    ...
    if (!outFileStream) {
      SLOG << "Can not open output stream to file: server.osg" << std::endl;
    } else {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strlen(argv[i]) > 1 && argv[i][0] == '-') {
      ...
      case 'a':
        ...
        break;
      case 'g':
        if (sscanf(&(argv[i][2]), "%d,%d,%d,%d", &width, &height, &x, &y) != 4) {
      ...
      case 'h':
        ...
        std::cout << "-aAddress Server network address" << std::endl;
        std::cout << "-A        run asynchron" << std::endl;
        return 0;
      }
    } else
      name = argv[i];
  ...
  try {
    ...
    */
    ...
    // start sync thread
    syncThread = dynamic_cast<Thread*>(ThreadManager::the()->getThread(NULL));
    if (syncThread == NULL) {
