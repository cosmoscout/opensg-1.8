...
void keyboard(unsigned char k, int, int) {
  switch (k) {
  case 'a':
    beginEditCP(fp, ProgramChunk::ProgramFieldMask);
    fp->read("simple.fp");
    endEditCP(fp, ProgramChunk::ProgramFieldMask);
    break;
  case 's':
    beginEditCP(fp, ProgramChunk::ParamValuesFieldMask);
    fp->setParameter("foo", Vec4f(osgrand(), osgrand(), osgrand(), osgrand()));
    endEditCP(fp, ProgramChunk::ParamValuesFieldMask);
    break;
  case 'd':
    beginEditCP(fp, ProgramChunk::ParamValuesFieldMask);
    fp->setParameter((int)1, Vec4f(osgrand(), osgrand(), osgrand(), osgrand()));
    endEditCP(fp, ProgramChunk::ParamValuesFieldMask);
    break;
...
int main(int argc, char** argv) {
  ...
  // create the fragment program chunk
  fp = FragmentProgramChunk::create();
  beginEditCP(fp);

  fp->read("simple.fp");

  fp->addParameter("foo", 0, Vec4f(1, 0, 0, 0));
  fp->addParameter("bar", 1, Vec4f(0, 1, 0, 0));
  ...
  UChar8   imgdata1[] = {255, 0, 0, 0, 255, 0, 0, 0, 255, 255, 0, 255};
