...
int main(int argc, char** argv) {
  ...
  if (argc == 1) {
    ...
    Real32    rres   = res;
    beginEditCP(imageP);
    imageP->set(Image::OSG_LA_PF, res, res, res, 1, 1, 0, 0, Image::OSG_UINT16_IMAGEDATA);
    ...
    traverse(scene, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(findVolume));
  }
...
void keyboard(unsigned char k, int OSG_CHECK_ARG(x), int OSG_CHECK_ARG(y)) {
  ...
  case 'a': {
    ...
    std::cout << "Sampling set to " << vol->getSampling() << std::endl;
    glutPostRedisplay();
  } break;
  case 'z': {
    OSG::beginEditCP(vol, OSG::DVRVolume::SamplingFieldMask);
