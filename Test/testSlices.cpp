...
void mouse(int button, int state, int x, int y) {
  if (state == 0) {
    ...
    case GLUT_MIDDLE_BUTTON:
      navigator.buttonPress(Navigator::MIDDLE_MOUSE, x, y);
      break;
    ...
    case GLUT_LEFT_BUTTON:
      ...
      break;
    case GLUT_MIDDLE_BUTTON:
      navigator.buttonRelease(Navigator::MIDDLE_MOUSE, x, y);
      break;
    case GLUT_RIGHT_BUTTON:
...
void key(unsigned char key, int x, int y) {
  ...
  case 'S': {
    ...
    std::cout << "obj size:      " << size.x() << "x" << size.y() << "x" << size.z() << std::endl;
  } break;
  case 'q':
    beginEditCP(slicesCore);
    {
...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    ...
    if (imageP->read(argv[1])) {
      ...
      size.setValues(imageP->getWidth(), imageP->getHeight(), imageP->getDepth());
      imageP->dump();
    ...
    for (Int16 z = 0; z < res; ++z)
      for (Int16 y = 0; y < res; ++y)
        for (Int16 x = 0; x < res; ++x) {
          ...
          int bx = (x < 2 || x >= res - 2), by = (y < 2 || y >= res - 2),
              bz = (z < 2 || z >= res - 2);
          if (bx + by + bz >= 2) {
            *d++ = 0;
            *d++ = 255;
  ...
  navigator.set(Pnt3f(dim.length() * 2, 0, 0), Pnt3f(0, 0, 0), Vec3f(0, 1, 0));
