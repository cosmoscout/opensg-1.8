...
StatElemDesc<OSG::StatStringElem> horiDirDesc(
    "horizontal direction", "The horizontal direction (left-to-right or right-to-left)");
StatElemDesc<OSG::StatStringElem> vertDirDesc(
    "vertical direction", "The vertical direction (top-to-bottom or bottom-to-top)");
  ...
  beginEditCP(geoPtr);
  {
    ...
    if (layoutParam.horizontal == true) {
      ...
      if (layoutParam.topToBottom == true) {
        ...
        case TextLayoutParam::ALIGN_MIDDLE:
          pos[1] =
              -(spacing * (layoutResult.lineBounds.size() - 1) + lineHeight) / 2.f + lineHeight;
      ...
      if (layoutParam.leftToRight == true) {
        ...
        case TextLayoutParam::ALIGN_MIDDLE:
          pos[0] = textPos[0] =
              -(spacing * (layoutResult.lineBounds.size() - 1) + lineHeight) / 2.f;
          break;
        case TextLayoutParam::ALIGN_END:
    ...
    else if (layoutParam.topToBottom == true)
      switch (layoutParam.majorAlignment) {
      case TextLayoutParam::ALIGN_BEGIN:
      case TextLayoutParam::ALIGN_FIRST:
        break;
...
void updateScene() {
  statfg->getCollector()
      .getElem(majorAlignDesc)
      ->set(alignmentToString(layoutParam.majorAlignment));
  statfg->getCollector()
      .getElem(minorAlignDesc)
      ->set(alignmentToString(layoutParam.minorAlignment));
  statfg->getCollector().getElem(dirDesc)->set(layoutParam.horizontal ? "Horizontal" : "Vertical");
  statfg->getCollector()
      .getElem(horiDirDesc)
      ->set(layoutParam.leftToRight ? "Left to right" : "Right to left");
  statfg->getCollector()
      .getElem(vertDirDesc)
      ->set(layoutParam.topToBottom ? "Top to bottom" : "Bottom to top");
  ...
  Real32   scale    = 2.f / (face->getHoriAscent() - face->getHoriDescent());

  // Put it all together into a Geometry NodeCore.
  GeometryPtr geo      = makePlaneGeo(width * scale, height * scale, 1, 1);
  ...
  Matrix       transMatrix;
  transMatrix.setTranslate(
      (offset.x() + width / 2.f) * scale, (offset.y() - height / 2.f) * scale, -0.03f);
  ...
  TextureTransformChunkPtr texTransChunk = TextureTransformChunk::create();
  beginEditCP(texTransChunk);
  {
    Real32 textureWidth  = osgnextpower2(static_cast<UInt32>(width));
    Real32 textureHeight = osgnextpower2(static_cast<UInt32>(height));
    Matrix matrix;
