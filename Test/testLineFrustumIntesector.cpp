...
NodePtr setFrustumPlanes(FrustumVolume& frustum, Line* view_rays, Pnt3f center, Real32 dist) {
  ...
  std::cout << "nleftbottom: " << nleftbottom[0] << " : " << nleftbottom[1] << " : "
            << nleftbottom[2] << std::endl;
  std::cout << "fleftbottom: " << fleftbottom[0] << " : " << fleftbottom[1] << " : "
            << fleftbottom[2] << std::endl;

  pnear.intersectInfinite(view_rays[2], nrightbottom);
  pfar.intersectInfinite(view_rays[2], frightbottom);

  std::cout << "nrightbottom: " << nrightbottom[0] << " : " << nrightbottom[1] << " : "
            << nrightbottom[2] << std::endl;
  std::cout << "frightbottom: " << frightbottom[0] << " : " << frightbottom[1] << " : "
            << frightbottom[2] << std::endl;

  pnear.intersectInfinite(view_rays[3], nrighttop);
  pfar.intersectInfinite(view_rays[3], frighttop);

  std::cout << "nrighttop: " << nrighttop[0] << " : " << nrighttop[1] << " : " << nrighttop[2]
            << std::endl;
  std::cout << "frighttop: " << frighttop[0] << " : " << frighttop[1] << " : " << frighttop[2]
            << std::endl;

  pnear.intersectInfinite(view_rays[4], nlefttop);
  pfar.intersectInfinite(view_rays[4], flefttop);

  std::cout << "nlefttop: " << nlefttop[0] << " : " << nlefttop[1] << " : " << nlefttop[2]
            << std::endl;
  std::cout << "flefttop: " << flefttop[0] << " : " << flefttop[1] << " : " << flefttop[2]
            << std::endl;
...
NodePtr makeGeoFrustumIntersectionWires(FrustumVolume& frustum, GeometryPtr geo) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if ((first = box.intersect(line1, l_en1, l_ex1))) {
        if (first && l_en1 >= enter1 && l_en1 <= exit1 && l_ex1 >= enter1 && l_ex1 <= exit1) {
          boxline = true;
        } else if (box.intersect(line2, l_en2, l_ex2) && l_en2 >= enter2 && l_en2 <= exit2 &&
                   l_ex2 >= enter2 && l_ex2 <= exit2) {
          boxline = true;
        }
      }
      ...
      if ((first = line1.intersect(frustum, l_en1, l_ex1))) {
        //                std::cout << "en-ex : l_en-l_ex :" << enter << ":" << exit << ":" << l_en
        //                << ":" << l_ex <<std::endl;
        ...
        if (first && l_en1 >= enter1 && l_en1 <= exit1 && l_ex1 >= enter1 && l_ex1 <= exit1) {
          ...
          len += 2;
        } else if (line2.intersect(frustum, l_en2, l_ex2) && l_en2 >= enter2 && l_en2 <= exit2 &&
                   l_ex2 >= enter2 && l_ex2 <= exit2) {
  ...
  GeometryPtr new_geo = Geometry::create();
  beginEditCP(new_geo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
  ...
  endEditCP(new_geo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                         Geometry::PositionsFieldMask | Geometry::MaterialFieldMask);
