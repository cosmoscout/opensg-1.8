...
int main(int argc, char** argv) {
    ...
    if (argv[i][0] == '-') {
      ...
      case 'g':
        if (argv[i][2] != '\0')
          ...
          str = argv[++i];
        if (sscanf(str, "%d,%d,%d,%d", &width, &height, &x, &y) != 4) {
          SWARNING << "Wrong args in -g. Use -gw,h,x,y" << std::endl;
          exit(0);
        }
        break;
      case 'p':
  ...
  try {
    ...
    glutInit(&argc, argv);
    if (doStereo)
      glutInitDisplayMode(GLUT_STEREO | GLUT_RGB | GLUT_DEPTH | GLUT_DOUBLE);
    else
    ...
    */
    ...
    window->setId(winid);
    window->init();
    server = new ClusterServer(window, name, connectionType, address, servicePort, serviceGroup);
    server->start();
    running = true;
    glutMainLoop();
  } catch (OSG_STDEXCEPTION_NAMESPACE::exception& e) {
