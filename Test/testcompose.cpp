...
bool test(
    Vec3f trans, Quaternion rotation, Vec3f scale, Quaternion scaleOrientation, Vec3f center) {
  Matrix m1;
  ...
  m2.getTransform(detrans2, derotation2, descale2, descaleOrientation2, decenter2);

  if (!m1.equals(m2, 1e-5)) {
    std::cerr << "Matrix 1: " << m1 << std::endl;
    std::cerr << "Matrix 2: " << m2 << std::endl;
    std::cerr << "Scale: " << scale << " : " << descale << std::endl;
    std::cerr << "ScaleOrientation: " << scaleOrientation << " : " << descaleOrientation
              << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  for (UInt32 i = 0; i < 100000; ++i) {
    ...
    scaleOrientation.setValueAsAxisRad(
        Vec3f(osgrand() * 2 - 1, osgrand() * 2 - 1, osgrand() * 2 - 1), osgrand() * 200);

    rotation.setValueAsAxisRad(
        Vec3f(osgrand() * 2 - 1, osgrand() * 2 - 1, osgrand() * 2 - 1), osgrand() * 200);
    ...
    std::cerr << i << " ";
    if (!test(Vec3f(0, 0, 0), rotation, scale, Quaternion(Vec3f(1, 0, 0), 0), Vec3f(0, 0, 0)))
      break;
  }
