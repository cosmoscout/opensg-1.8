...
NewActionTypes::ResultE enterGroupFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);
NewActionTypes::ResultE leaveGroupFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);

NewActionTypes::ResultE enterTransformFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);
NewActionTypes::ResultE leaveTransformFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);

NewActionTypes::ResultE enterDefaultFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);
NewActionTypes::ResultE leaveDefaultFunc(
    NodeCorePtr pNodeCore, ActorBase::FunctorArgumentType& funcArg);
...
int main(int argc, char* argv[]) {
  ...
  TestingExtendActor::regClassEnter(
      osgTypedFunctionFunctor2CPtr<NewActionTypes::ResultE, NodeCorePtr,
  ...
  TestingExtendActor::regDefaultClassEnter(osgTypedFunctionFunctor2CPtr<NewActionTypes::ResultE,
      NodeCorePtr, ActorBase::FunctorArgumentType&>(enterDefaultFunc));
  TestingExtendActor::regDefaultClassLeave(osgTypedFunctionFunctor2CPtr<NewActionTypes::ResultE,
      NodeCorePtr, ActorBase::FunctorArgumentType&>(leaveDefaultFunc));
...
NewActionTypes::ResultE enterGroupFunc(
  ...
  SINFO << "ENTER Group: (pass: " << funcArg.getPass() << ") "
        << (getName(pActor->getNode()) ? getName(pActor->getNode()) : "<unnamed>") << endLog;
...
NewActionTypes::ResultE leaveGroupFunc(
  ...
  SINFO << "LEAVE Group: (pass: " << funcArg.getPass() << ") "
        << (getName(funcArg.getNode()) ? getName(funcArg.getNode()) : "<unnamed>") << endLog;
...
NodePtr buildGraphRecurse(UInt32 depth, UInt32 maxDepth, UInt32& count, GeometryPtr pGeo);
...
NodePtr buildGraphRecurse(UInt32 depth, UInt32 maxDepth, UInt32& count, GeometryPtr pGeo) {
  ...
  if (depth == maxDepth) {
    ...
    beginEditCP(pNodeGroup, Node::CoreFieldId | Node::ChildrenFieldId | Node::TravMaskFieldId);
    ...
    endEditCP(pNodeGroup, Node::CoreFieldId | Node::ChildrenFieldId | Node::TravMaskFieldId);
