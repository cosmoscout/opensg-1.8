...
#define testMatrixField(name, set, get) testField(name, set, get, m1, m2, m1.equals(m2, Eps))

#define testVec3fField(name, set, get) testField(name, set, get, v31, v32, v31.equals(v32, Eps))

#define testVec4fField(name, set, get) testField(name, set, get, v41, v42, v41.equals(v42, Eps))

#define testColor4fField(name, set, get) testField(name, set, get, c41, c42, (c41 == c42))

#define testRealField(name, set, get) testField(name, set, get, r1, r2, r1 == r2)

#define testUInt32Field(name, set, get) testField(name, set, get, ui1, ui2, r1 == r2)

  // Transform Chunk

  TransformChunkPtr tchunk = TransformChunk::create();

  std::cerr << "Transform chunk class: " << tchunk->getClass()->getName() << ", id "
  ...
  std::cerr << "Material chunk class: " << mchunk->getClass()->getName() << ", id "
  ...
  std::cerr << "Light chunk class: " << lchunk->getClass()->getName() << ", id "
  ...
  testRealField(
      "LightChunk: linear attenuation", lchunk->setLinearAttenuation, lchunk->getLinearAttenuation);
  ...
  std::cerr << "Texture chunk class: " << xchunk->getClass()->getName() << ", id "
            << xchunk->getClassId() << ", numslots " << xchunk->getClass()->getNumSlots()
            << std::endl;

  testUInt32Field("TextureChunk: minFilter", xchunk->setMinFilter, xchunk->getMinFilter);
  testUInt32Field("TextureChunk: magFilter", xchunk->setMagFilter, xchunk->getMagFilter);
  testUInt32Field("TextureChunk: wrapR", xchunk->setWrapR, xchunk->getWrapR);
  testUInt32Field("TextureChunk: wrapS", xchunk->setWrapS, xchunk->getWrapS);
  testUInt32Field("TextureChunk: wrapT", xchunk->setWrapT, xchunk->getWrapT);
