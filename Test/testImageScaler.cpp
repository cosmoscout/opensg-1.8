...
int main(int argc, char** argv) {
  ...
  printf("Src: %dx%dx%d Sides: %d Frames %d\n", width, height, depth, pImageSrc->getSideCount(),
      pImageSrc->getFrameCount());
  if (argc > 3)
    width = atoi(argv[3]);
  if (argc > 4)
    height = atoi(argv[4]);
  ...
  if (argc > 6) {
    switch (argv[6][0]) {
    case 'b':
      filter = new ImageBoxFilter();
      break;
    case 'l':
      filter = new ImageLinearFilter();
      break;
    case 'g':
      filter = new ImageGaussianFilter();
      break;
    case 'h':
      filter = new ImageHammingFilter;
      break;
    case 'B':
      filter = new ImageBlackmanFilter;
      break;
    case 'L':
      filter = new ImageLanczos3Filter;
      break;
    }
  }
  if (!filter) {
