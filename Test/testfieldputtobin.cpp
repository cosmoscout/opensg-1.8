...
void runTest(bool write, BinaryDataHandler& pMem, const T& value) {
  ...
  if (write) {
    ...
    SLOG << SField<T>::SFieldTraits::getSName() << " size " << sfield.getBinSize() << std::endl;
    SLOG << MField<T>::MFieldTraits::getMName() << " size " << mfield.size() << " "
         << mfield.getBinSize() << std::endl;
  } else {
    ...
    if (mfield[0] == value && mfield[1] == value)
...
void runTest1(bool write, BinaryDataHandler& pMem, const T& value) {
  ...
  if (write) {
    ...
    mfield.push_back(value);
    mfield.copyToBin(pMem);
    SLOG << SField<T, 1>::SFieldTraits::getSName() << " size " << sfield.getBinSize() << std::endl;
    SLOG << MField<T, 1>::MFieldTraits::getMName() << " size " << mfield.size() << " "
         << mfield.getBinSize() << std::endl;
  } else {
    sfield.copyFromBin(pMem);
    if (sfield.getValue() == value)
...
void runTest2(bool write, BinaryDataHandler& pMem, const T& value) {
  ...
  if (write) {
    ...
    mfield.push_back(value);
    mfield.copyToBin(pMem);
    SLOG << SField<T, 2>::SFieldTraits::getSName() << " size " << sfield.getBinSize() << std::endl;
    SLOG << MField<T, 2>::MFieldTraits::getMName() << " size " << mfield.size() << " "
         << mfield.getBinSize() << std::endl;
  } else {
    sfield.copyFromBin(pMem);
    if (sfield.getValue() == value)
