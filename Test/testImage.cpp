...
int main(int argc, char** argv) {
  ...
  OSG::ImageFileHandler::the().getSuffixList(l, OSG::ImageFileType::OSG_READ_SUPPORTED);

  std::cout << "Formats supported for reading: ";
  std::copy(l.begin(), l.end(), std::ostream_iterator<std::string>(std::cout, ","));
  std::cout << std::endl;

  OSG::ImageFileHandler::the().getSuffixList(l, OSG::ImageFileType::OSG_WRITE_SUPPORTED);
  ...
  if (argc > 1 && pWRLFile != NULL) {
    ...
    pOutImage->set(OSG::Image::OSG_RGB_PF, // pImage->getPixelFormat(),
        pImage->getWidth(), pImage->getHeight(), pImage->getDepth(), 1, 1, 1.0, NULL);
    ...
    fprintf(pWRLFile, "}\n");
    fprintf(pWRLFile, "\n");
    fprintf(pWRLFile, "ROUTE_IMM BD_Sequencer.value TO BD_Switch.whichChoice\n");
    fprintf(pWRLFile, "\n");
    ...
    fprintf(pWRLFile, "}\n");
    fprintf(pWRLFile, "\n");
    fprintf(pWRLFile, "ROUTE_IMM TS.fraction_changed TO BD_Sequencer.fraction\n");
    fprintf(pWRLFile, "\n");
