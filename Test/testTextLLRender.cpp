...
void display(void) {
  if (currentNode == numNodes - 1) {
    ...
    beginEditCP(txfGeo, Geometry::PositionsFieldMask | Geometry::TexCoordsFieldMask);
    {
      GeoPositions3fPtr pos = GeoPositions3fPtr::dcast(txfGeo->getPositions());
      GeoTexCoords2fPtr tex = GeoTexCoords2fPtr::dcast(txfGeo->getTexCoords());
      ...
      fontText.fillTXFArrays(lineVec, &pos->getField()[0], &tex->getField()[0]);
    }

    endEditCP(txfGeo, Geometry::PositionsFieldMask | Geometry::TexCoordsFieldMask);
  }
