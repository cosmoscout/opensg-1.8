...
class Core1 : public NodeCore {
 public:
  ...
  Action::ResultE enter(Action* action) {
    std::cerr << "Core1::enter: " << this << " action " << action << std::endl;
    return Action::Continue;
  }

  Action::ResultE leave(Action* action) {
    std::cerr << "Core1::leave: " << this << " action " << action << std::endl;
    return Action::Continue;
  }
...
FieldContainerType Core1::_type("Core1", "NodeCore", 0, (PrototypeCreateF)&Core1::createEmpty);
...
FieldContainerType Core2::_type("Core2", "Core1", 0, (PrototypeCreateF)&Core2::createEmpty);
...
class Core1Action {
 public:
  ...
  Action::ResultE enter(CNodePtr& node, Action* action) {
    std::cerr << "Core1 enter: " << node << " action " << std::hex << action << " for Core1 "
              << this << std::endl;
    return Action::Continue;
  }

  Action::ResultE leave(CNodePtr& node, Action* action) {
    std::cerr << "Core1 leave: " << node << " action " << std::hex << action << " for Core1 "
              << this << std::endl;
    return Action::Continue;
  }
};
...
Action::ResultE Core2Enter(CNodePtr& node, Action* action) {
  std::cerr << "Core2 enter: " << node << " action " << std::hex << action << std::endl;

  return Action::Continue;
}

Action::ResultE Core2Leave(CNodePtr& node, Action* action) {
  std::cerr << "Core2 leave: " << node << " action " << std::hex << action << std::endl;
...
Action::ResultE defenter1(CNodePtr& node, Action* action) {
  std::cerr << "Default enter 1 called: " << node << " action " << std::hex << action << std::endl;

  return Action::Continue;
}

Action::ResultE defleave1(CNodePtr& node, Action* action) {
  std::cerr << "Default leave 1 called: " << node << " action " << std::hex << action << std::endl;

  return Action::Continue;
}

Action::ResultE defenter2(CNodePtr& node, Action* action) {
  std::cerr << "Default enter 2 called: " << node << " action " << std::hex << action << std::endl;

  return Action::Continue;
}

Action::ResultE defleave2(CNodePtr& node, Action* action) {
  std::cerr << "Default leave 2 called: " << node << " action " << std::hex << action << std::endl;
...
struct TravActor {
  ...
  Action::ResultE leave(NodePtr& n, Action::ResultE res) {
    std::cerr << "TravActor::leave " << this << " Node " << n << " Res " << res << std::endl;
    return Action::Continue;
  }
};
...
int main(int argc, char* argv[]) {
  ...
  Action::registerEnterDefault(Core1::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(defenter1));

  Action::registerLeaveDefault(Core1::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(defleave1));
  ...
  Action::registerEnterDefault(Core2::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(defenter2));
  Action::registerLeaveDefault(Core2::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(defleave2));
  ...
  act1->registerEnterFunction(Core1::getClassType(),
      osgTypedMethodFunctor2ObjPtrCPtrRef<Action::ResultE, Core1Action, CNodePtr, Action*>(
          &gf1, &Core1Action::enter));

  act1->registerLeaveFunction(Core1::getClassType(),
      osgTypedMethodFunctor2ObjPtrCPtrRef<Action::ResultE, Core1Action, CNodePtr, Action*>(
          &gf2, &Core1Action::leave));

  act1->registerEnterFunction(Core2::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(Core2Enter));
  act1->registerLeaveFunction(Core2::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(Core2Leave));
  ...
  act1->registerEnterFunction(Core1::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(firstOnly));
...

  {
    ...
    traverse(g1, osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, TravActor, NodePtr>(
                     &tact1, &TravActor::enter));
    ...
    traverse(g1->getMFChildren()->getValues(),
        osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, TravActor, NodePtr>(
            &tact2, &TravActor::enter));
    ...
    traverse(g1,
        ...
        osgTypedMethodFunctor2ObjPtrCPtrRef<Action::ResultE, TravActor, NodePtr, Action::ResultE>(
            &tact1, &TravActor::leave));
    ...
    traverse(g1->getMFChildren()->getValues(),
        osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, TravActor, NodePtr>(
            &tact2, &TravActor::enter),
        osgTypedMethodFunctor2ObjPtrCPtrRef<Action::ResultE, TravActor, NodePtr, Action::ResultE>(
            &tact2, &TravActor::leave));

    std::cerr << "Traverse(node,enter):" << std::endl;
    traverse(g1, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(traventer));

    std::cerr << "Traverse(list,enter):" << std::endl;
    traverse(g1->getMFChildren()->getValues(),
        osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(traventer));

    std::cerr << "Traverse(node,enter&leave):" << std::endl;
    traverse(g1, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(traventer),
        osgTypedFunctionFunctor2CPtrRef<Action::ResultE, NodePtr, Action::ResultE>(travleave));
