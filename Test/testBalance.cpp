...
void setFixCrossRegions(Int32 count, UInt32 x1, UInt32 y1, UInt32 x2, UInt32 y2, bool cutX,
...
void setFixSliceRegions(
    Int32 count, UInt32 width, UInt32 height, TileLoadBalancer::ResultT& region) {
  Int32 from = 0;
  Int32 to   = 0;
  Int32 i;
  for (i = 0; i < count; i++) {
 ...
 public:
  ...
  virtual void redraw(void) {
    ...
    if (animate && animPos.size() > 1) {
      ...
      _cart->getMatrix().setRotate(Quaternion::slerp(animOri[i], animOri[i + 1], a));
    } else {
    ...
    _win->frameInit();
    if (first) {
      tileLoadBalancer = new TileLoadBalancer(useFaceDistribution, !useCrossCut);
      first            = false;
    ...
    if (useFixCrossRegions) {
      setFixCrossRegions(
          serverCount, 0, 0, _win->getWidth() - 1, _win->getHeight() - 1, true, region);
    } else {
      if (useFixSliceRegions) {
        setFixSliceRegions(serverCount, _win->getWidth() - 1, _win->getHeight() - 1, region);
      } else {
    ...
    if (simulateRendering) {
      ...
      for (i = 0; i < region.size(); ++i) {
        ...
        beginEditCP(deco);
        beginEditCP(vp);
        vp->setSize(region[i].x1, region[i].y1, region[i].x2, region[i].y2);
        deco->setSize(region[i].x1 / (float)_win->getWidth(),
            region[i].y1 / (float)_win->getHeight(), region[i].x2 / (float)_win->getWidth(),
    ...
    if (setRegionStatistics) {
      tileLoadBalancer->setRegionStatistics(_win->getPort()[0], region);
      for (i = 0; i < region.size(); ++i) {
        if (i == 0) {
    ...
    glViewport(0, 0, _win->getWidth(), _win->getHeight());
    if (viewVolume)
    ...
    glPushMatrix();
    glLoadIdentity();
    gluOrtho2D(0, _win->getWidth(), 0, _win->getHeight());
    ...
    if (doSave) {
      ...
      pImage->set(Image::OSG_RGB_PF, w, h, 1, 1, 1, 0.0, NULL);
      ...
      glReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, pImage->getData());
  ...
  if (!navigate) {
    ...
    if (simulateRendering) {
      ...
      fprintf(resultio,
          ...
          serverCount, sum_tmin, sum_tmax, sum_tbalance, sum_faces, sum_culledFaces, sum_pixel,
          sum_facemax, sum_fcullmax, sum_pmax, sum_travTime, sum_drawTime);
...
void key(unsigned char key, int, int) {
  ...
  case 's': {
    ...
    Quaternion q(m);
    Real32     ax, ay, az, r;
    animPos.push_back(Vec3f(m[3][0], m[3][1], m[3][2]));
    animOri.push_back(q);
    q.getValueAsAxisRad(ax, ay, az, r);
    fprintf(file, "%f %f %f %f,%f %f %f\n", ax, ay, az, r, m[3][0], m[3][1], m[3][2]);
    fclose(file);
    break;
  }
  case 'S': {
