...
class OSG_SYSTEMLIB_DLLMAPPING BezierCurve2D {
 ...
 public:
  ...
  int midPointSubDivision(
      bezier2dvector& newbeziers); // subdivide curve at midpoint into two new curves
  int midPointSubDivision(
      BezierCurve2D& newcurve); // subdivide curve at midpoint into two new curves
  int subDivision(double t, bezier2dvector& newbeziers); // subdivide curvee at t into two new
                                                         // curves
  int subDivision(double t, BezierCurve2D& newcurve); // subdivide curvee at t into two new curves
  int intersection(
      DCTPdvector& res, Vec2d a, Vec2d b); // calculate intersection of curve with polyline (a, b)
  int intersection(
      DCTPdvector& res, double a, bool horiz); // calculate intersection of curve with line (a)
  int approximate(DCTPVec2dvector& vertices,
      double delta); // approximate curve linearly with given maximum tolerance
  //  int approximateLength( DCTPVec2dvector &vertices, double delta); // approximate curve linearly
  //  with given maximum tolerance
