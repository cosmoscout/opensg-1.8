...
static const Char8* suffixArray[] = {"exr"};

EXRImageFileType EXRImageFileType::_the(
    "image/x-exr", suffixArray, sizeof(suffixArray), OSG_READ_SUPPORTED | OSG_WRITE_SUPPORTED);
...
bool EXRImageFileType::read(ImagePtr& image, std::istream& is, const std::string& mimetype) {
  ...
  try {
    ...
    image->set(Image::OSG_RGBA_PF, width, height, 1, 1, 1, 0, 0, Image::OSG_FLOAT16_IMAGEDATA, true,
        numImg);
    image->clearHalf();
...
*/
UInt64 EXRImageFileType::restoreData(
...
*/
UInt64 EXRImageFileType::storeData(
...
*/
EXRImageFileType::EXRImageFileType(
