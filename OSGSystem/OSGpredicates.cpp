...
//#define OSG_USE_PREDICATES_INIT

#ifdef OSG_USE_PREDICATES_INIT
#include "predicates_init.h"
#endif /* OSG_USE_PREDICATES_INIT */

/* FPU control. We MUST have only double precision (not extended precision) */
#include "OSGrounding.h"
...
#define Eight_Four_Sum(a7, a6, a5, a4, a3, a2, a1, a0, b4, b3, b1, b0, x11, x10, x9, x8, x7, x6,   \
    x5, x4, x3, x2, x1, x0)                                                                        \
...
#ifndef OSG_USE_PREDICATES_INIT
...
#ifdef WIN32
/* added by MN */
/* computes a long random number for WIN32 */
long my_random() {
  long  res;
  int   i;
  char  c[8];
  char* res_addr = (char*)&res;
...
static int grow_expansion(int elen, REAL* e, REAL b, REAL* h) /* e and h can be the same. */
// int elen;
// REAL *e;
// REAL b;
// REAL *h;
...
static int grow_expansion_zeroelim(
    int elen, REAL* e, REAL b, REAL* h) /* e and h can be the same. */
// int elen;
// REAL *e;
// REAL b;
// REAL *h;
...
static int scale_expansion(int elen, REAL* e, REAL b, REAL* h) /* e and h cannot be the same. */
// int elen;
// REAL *e;
// REAL b;
// REAL *h;
...
static int scale_expansion_zeroelim(
    int elen, REAL* e, REAL b, REAL* h) /* e and h cannot be the same. */
// int elen;
// REAL *e;
// REAL b;
// REAL *h;
...
// REAL *pc;
{
  ...
  Two_Two_Product(acx, acxtail, bcy, bcytail, axby7, axby[6], axby[5], axby[4], axby[3], axby[2],
      axby[1], axby[0]);
  axby[7]    = axby7;
  negate     = -acy;
  negatetail = -acytail;
  Two_Two_Product(bcx, bcxtail, negate, negatetail, bxay7, bxay[6], bxay[5], bxay[4], bxay[3],
      bxay[2], bxay[1], bxay[0]);
  bxay[7] = bxay7;
...
REAL OSG::orient2d(REAL* pa, REAL* pb, REAL* pc)
// REAL *pa;
// REAL *pb;
// REAL *pc;
{
...
// REAL *pd;
{
  ...
  Two_Two_Product(adx, adxtail, bdy, bdytail, axby7, axby[6], axby[5], axby[4], axby[3], axby[2],
      axby[1], axby[0]);
  axby[7]    = axby7;
  negate     = -ady;
  negatetail = -adytail;
  Two_Two_Product(bdx, bdxtail, negate, negatetail, bxay7, bxay[6], bxay[5], bxay[4], bxay[3],
      bxay[2], bxay[1], bxay[0]);
  bxay[7] = bxay7;
  Two_Two_Product(bdx, bdxtail, cdy, cdytail, bxcy7, bxcy[6], bxcy[5], bxcy[4], bxcy[3], bxcy[2],
      bxcy[1], bxcy[0]);
  bxcy[7]    = bxcy7;
  negate     = -bdy;
  negatetail = -bdytail;
  Two_Two_Product(cdx, cdxtail, negate, negatetail, cxby7, cxby[6], cxby[5], cxby[4], cxby[3],
      cxby[2], cxby[1], cxby[0]);
  cxby[7] = cxby7;
  Two_Two_Product(cdx, cdxtail, ady, adytail, cxay7, cxay[6], cxay[5], cxay[4], cxay[3], cxay[2],
      cxay[1], cxay[0]);
  cxay[7]    = cxay7;
  negate     = -cdy;
  negatetail = -cdytail;
  Two_Two_Product(adx, adxtail, negate, negatetail, axcy7, axcy[6], axcy[5], axcy[4], axcy[3],
      axcy[2], axcy[1], axcy[0]);
  axcy[7] = axcy7;
...
REAL OSG::orient3d(REAL* pa, REAL* pb, REAL* pc, REAL* pd)
// REAL *pa;
// REAL *pb;
// REAL *pc;
// REAL *pd;
...
REAL OSG::incircle(REAL* pa, REAL* pb, REAL* pc, REAL* pd)
// REAL *pa;
// REAL *pb;
// REAL *pc;
// REAL *pd;
...
// REAL *pe;
{
  ...
  Two_Two_Product(aex, aextail, bey, beytail, axby7, axby[6], axby[5], axby[4], axby[3], axby[2],
      axby[1], axby[0]);
  axby[7]    = axby7;
  negate     = -aey;
  negatetail = -aeytail;
  Two_Two_Product(bex, bextail, negate, negatetail, bxay7, bxay[6], bxay[5], bxay[4], bxay[3],
      bxay[2], bxay[1], bxay[0]);
  bxay[7] = bxay7;
  ablen   = fast_expansion_sum_zeroelim(8, axby, 8, bxay, ab);
  Two_Two_Product(bex, bextail, cey, ceytail, bxcy7, bxcy[6], bxcy[5], bxcy[4], bxcy[3], bxcy[2],
      bxcy[1], bxcy[0]);
  bxcy[7]    = bxcy7;
  negate     = -bey;
  negatetail = -beytail;
  Two_Two_Product(cex, cextail, negate, negatetail, cxby7, cxby[6], cxby[5], cxby[4], cxby[3],
      cxby[2], cxby[1], cxby[0]);
  cxby[7] = cxby7;
  bclen   = fast_expansion_sum_zeroelim(8, bxcy, 8, cxby, bc);
  Two_Two_Product(cex, cextail, dey, deytail, cxdy7, cxdy[6], cxdy[5], cxdy[4], cxdy[3], cxdy[2],
      cxdy[1], cxdy[0]);
  cxdy[7]    = cxdy7;
  negate     = -cey;
  negatetail = -ceytail;
  Two_Two_Product(dex, dextail, negate, negatetail, dxcy7, dxcy[6], dxcy[5], dxcy[4], dxcy[3],
      dxcy[2], dxcy[1], dxcy[0]);
  dxcy[7] = dxcy7;
  cdlen   = fast_expansion_sum_zeroelim(8, cxdy, 8, dxcy, cd);
  Two_Two_Product(dex, dextail, aey, aeytail, dxay7, dxay[6], dxay[5], dxay[4], dxay[3], dxay[2],
      dxay[1], dxay[0]);
  dxay[7]    = dxay7;
  negate     = -dey;
  negatetail = -deytail;
  Two_Two_Product(aex, aextail, negate, negatetail, axdy7, axdy[6], axdy[5], axdy[4], axdy[3],
      axdy[2], axdy[1], axdy[0]);
  axdy[7] = axdy7;
  dalen   = fast_expansion_sum_zeroelim(8, dxay, 8, axdy, da);
  Two_Two_Product(aex, aextail, cey, ceytail, axcy7, axcy[6], axcy[5], axcy[4], axcy[3], axcy[2],
      axcy[1], axcy[0]);
  axcy[7]    = axcy7;
  negate     = -aey;
  negatetail = -aeytail;
  Two_Two_Product(cex, cextail, negate, negatetail, cxay7, cxay[6], cxay[5], cxay[4], cxay[3],
      cxay[2], cxay[1], cxay[0]);
  cxay[7] = cxay7;
  aclen   = fast_expansion_sum_zeroelim(8, axcy, 8, cxay, ac);
  Two_Two_Product(bex, bextail, dey, deytail, bxdy7, bxdy[6], bxdy[5], bxdy[4], bxdy[3], bxdy[2],
      bxdy[1], bxdy[0]);
  bxdy[7]    = bxdy7;
  negate     = -bey;
  negatetail = -beytail;
  Two_Two_Product(dex, dextail, negate, negatetail, dxby7, dxby[6], dxby[5], dxby[4], dxby[3],
      dxby[2], dxby[1], dxby[0]);
  dxby[7] = dxby7;
...
REAL OSG::insphere(REAL* pa, REAL* pb, REAL* pc, REAL* pd, REAL* pe)
// REAL *pa;
// REAL *pb;
// REAL *pc;
// REAL *pd;
