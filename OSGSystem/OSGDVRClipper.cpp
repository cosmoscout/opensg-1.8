...
static void OSG_APIENTRY vertexCombineCallback(
    GLdouble*, GLdouble**, GLfloat*, GLdouble**, void* data);
...
bool DVRClipper::setNumAddPerVertexAttr(DVRVolume* volume, UInt32 additionalPerVertexAttributes) {
  ...
  sliceVertexData = (GLdouble*)realloc(
      sliceVertexData, 6 * (6 + additionalPerVertexAttributes) * sizeof(GLdouble));
...
void DVRClipper::initialize(DVRVolume* volume) {
  ...
  if (myTess == NULL) {
    ...
    gluTessCallback(
        myTess, GLU_TESS_COMBINE_DATA, reinterpret_cast<OSGGLUfuncptr>(vertexCombineCallback));

    gluTessCallback(myTess, GLU_TESS_VERTEX_DATA, reinterpret_cast<OSGGLUfuncptr>(vertexCallback));

    gluTessCallback(myTess, GLU_TESS_BEGIN_DATA, reinterpret_cast<OSGGLUfuncptr>(beginCallback));

    gluTessCallback(myTess, GLU_TESS_END_DATA, reinterpret_cast<OSGGLUfuncptr>(endCallback));

    gluTessCallback(myTess, GLU_TESS_ERROR, reinterpret_cast<OSGGLUfuncptr>(errorCallback));
  }
}
...
void DVRClipper::clipSlice(DVRVolume* volume, DVRSlice& unclippedSlice, const Vec3f& slicingNormal,
    Real32 dist2RefPlane, DVRRenderSlice& clippedSlice) {
  ...
  if (clipObjects == NullFC) {
    ...
    for (UInt32 i = 0; i < unclippedSlice.getVertexCount(); i++) {
      ...
      sliceVertexData[idx + 3] =
          texScale[0] * unclippedSlice.getVertex(i).getValues()[0] + texTranslate[0];

      sliceVertexData[idx + 4] =
          texScale[1] * unclippedSlice.getVertex(i).getValues()[1] + texTranslate[1];

      sliceVertexData[idx + 5] =
          texScale[2] * unclippedSlice.getVertex(i).getValues()[2] + texTranslate[2];
  ...
  if (clipObjects->getDoContours()) {
    ...
    glBegin(GL_LINE_STRIP);
    {
      ...
      for (UInt32 i = 0; i < unclippedSlice.getVertexCount(); i++) {
        glColor3f(
            col % 3 == 0 ? 1.0f : 0.0f, col % 3 == 1 ? 1.0f : 0.0f, col % 3 == 2 ? 1.0f : 0.0f);
    ...
    for (UInt32 i = 0; i < clipObjects->count(); i++) {
      ...
      const DVRTriangleList& contours =
          clipObject->getContours(dist2RefPlane, !clipAwayOutside, slicingNormal);
      ...
      if (!contours.empty()) {
        ...
        for (contoursIt = contours.begin(); contoursIt != contours.end(); contoursIt++) {
          ...
          glBegin(GL_LINE_STRIP);
          {
            ...
            do {
              glColor3f(col % 3 == 0 ? 1.0f : 0.0f, col % 3 == 1 ? 1.0f : 0.0f,
                  col % 3 == 2 ? 1.0f : 0.0f);
...
void OSG_APIENTRY vertexCombineCallback(
    GLdouble* coords, GLdouble* vertexData[4], GLfloat weight[4], GLdouble** dataOut, void* data) {
  DVRRenderSlice* sliceData = (DVRRenderSlice*)data;
  ...
  if (sliceData->maxVertexCombineData <= sliceData->numVertexCombineData) {
    ...
    sliceData->vertexCombineData = (GLdouble*)realloc(sliceData->vertexCombineData,
  ...
  GLdouble* vertex = &(
      sliceData
          ->vertexCombineData[(6 + sliceData->numPerVertexData) * sliceData->numVertexCombineData]);
