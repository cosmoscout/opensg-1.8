  ...
  if (!getOptions().inlineTextures() && !img->getName().empty()) {
    ...
    if (getOptions().compressTextures()) {
      ...
      if (isImageTypeSupported(imageType)) {
        if (img->getDataType() == Image::OSG_UINT8_IMAGEDATA && img->hasCompressedData() == false &&
            img->getDepth() == 1 && img->getMipMapCount() == 1 && img->getFrameCount() == 1 &&
            img->getSideCount() == 1) {
          if (imageType == "jpeg" && (img->getBpp() == 1 || img->getBpp() == 3)) {
