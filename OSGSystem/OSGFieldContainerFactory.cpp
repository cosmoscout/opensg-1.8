...
FieldContainerFactory::TypeMapIterator FieldContainerFactory::_defaultTypeMapIt;
...
bool FieldContainerFactory::initializePendingTypes(void) {
  ...
  if (_pUnitTypesStore != NULL) {
    ...
    PINFO << "(" << returnValue << "|" << _pUnitTypesStore->size() << ")" << std::endl;
  }
...
void FieldContainerFactory::writeFCD(Char8* name, std::ostream* out) {
  ...
  if (name != NULL) {
    ...
    if (pType == NULL) {
      SWARNING << "FieldContainerFactory::writeFCD: type " << name << " is unknown!" << std::endl;
      return;
    }
  ...
  for (type = _pTypeIdMap->begin(); type != _pTypeIdMap->end(); ++type) {
    if (out != NULL) {
...
const FieldContainerFactory::FieldContainerStore* FieldContainerFactory::getFieldContainerStore(
    void) const {
...
bool FieldContainerFactory::initialize(void) {
  ...
  _pStoreLock = ThreadManager::the()->getLock("OSGFieldContainerFactory::_pStoreLock");

  addRefP(_pStoreLock);

  _pMapLock = ThreadManager::the()->getLock("OSGFieldContainerFactory::_pMaoLock");

  addRefP(_pMapLock);

  FDEBUG(("Got shore lock %p, Got map %p", _pStoreLock, _pMapLock));
...
bool FieldContainerFactory::terminate(void) {
  ...
  if (_pTypeIdMap != NULL) {
    int i = 0;
    for (typeIt = _pTypeIdMap->begin(); typeIt != _pTypeIdMap->end(); ++typeIt) {
...
void FieldContainerFactory::initTypeMap(void) {
  if (_pTypeIdMap == NULL && _pTypeNameMap == NULL) {
...
void FieldContainerFactory::writeSingleTypeFCD(std::ostream& out, const FieldContainerType* t) {
  ...
  out << "\tlibrary=\""
      << "???"
      << "\"" << std::endl;
  out << "\tstructure=\"" << (t->isAbstract() ? "abstract" : "concrete") << "\"" << std::endl;
  ...
  for (UInt32 i = parent ? parent->getNumFieldDescs() + 1 : 1; i <= t->getNumFieldDescs(); i++) {
    ...
    out << "\t\tcardinality=\"" << (ft->getCardinality() ? "multi" : "single") << "\"" << std::endl;

    out << "\t\tvisibility=\"" << (f->isInternal() ? "internal" : "external") << "\"" << std::endl;
