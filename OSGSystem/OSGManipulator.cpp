...
void Manipulator::dump(UInt32, const BitVector) const {
...
void Manipulator::onCreate(const Manipulator* source) {
  ...
  beginEditCP(_transHandleXC,
      ComponentTransform::TranslationFieldMask | ComponentTransform::RotationFieldMask);
...
Pnt2f Manipulator::calcScreenProjection(const Pnt3f& p, const ViewportPtr& port) {
  CameraPtr cam;
  ...
  if (port != NullFC) {
    ...
    cam->getProjectionTranslation(projtrans, port->getPixelWidth(), port->getPixelHeight());
...
void Manipulator::mouseMove(Int16 x, Int16 y) {
  ...
  if (getTarget() != NullFC) {
    ...
    if (t != NullFC) {
      ...
      t->getMatrix().getTransform(translation, rotation, scaleFactor, scaleOrientation);

      // calculate the camera coordinate of the center
      centerV            = translation;
      Pnt2f centerPixPos = calcScreenProjection(centerV.addToZero(), getViewport());

      // calculate the camera coordinate of the handle center
      handleCenterV            = centerV + coordVector[coord] * getLength()[coord];
      Pnt2f handleCenterPixPos = calcScreenProjection(handleCenterV.addToZero(), getViewport());
      ...
      doMovement(t, coord, s * getLength()[coord] * 0.01, translation, rotation, scaleFactor,
          scaleOrientation);
    } else
