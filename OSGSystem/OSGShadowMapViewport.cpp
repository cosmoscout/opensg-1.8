...
Erste Version des Shadowviewports f�r Shadowmapping
*/
...
void ShadowMapViewport::dump(UInt32, const BitVector) const {
...
void ShadowMapViewport::render(RenderActionBase* action) {
  ...
  if (!extensionCheck()) {
    ...
    _transparent.clear();
    traverse(
        getRoot(), osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, ShadowMapViewport, NodePtr>(
...
Action::ResultE ShadowMapViewport::findTransparent(NodePtr& node) {
  ...
  if (node->getCore() != NullFC) {
    ...
    if (node->getCore()->getType() == Geometry::getClassType()) {
      osg::GeometryPtr geo = osg::GeometryPtr::dcast(node->getCore());
      if (geo->getMaterial() != NullFC && geo->getMaterial()->isTransparent())
        _transparent.push_back(node);
  ...
  for (UInt32 i = 0; i < _lights.size(); ++i) {
    ...
    beginEditCP(_lightCamTrans[i]);
    {
      ...
      if (_lights[i]->getType() == SpotLight::getClassType()) {
        ...
        diff = (getSceneRoot()->getVolume().getMax() - getSceneRoot()->getVolume().getMin());
    ...
    if (_lightStates[i] != 0) {
      ...
      for (Real32 y = 0; y < 1.0; y += step) {
        ...
        for (Real32 x = 0; x < 1.0; x += step) {
          ...
          glBindTexture(GL_TEXTURE_2D, win->getGLObjectId(_texChunks[i]->getGLId()));
...
void ShadowMapViewport::projectShadowMaps(RenderActionBase* action) {
  ...
  for (UInt32 i = 0; i < _lights.size(); ++i) {
    if (_lightStates[i] != 0) {
      ...
      _lightCameras[i]->getViewing(
          viewMatrix, getPixelWidth() /*_mapRenderSize*/, getPixelHeight() /*_mapRenderSize*/);
