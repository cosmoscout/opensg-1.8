...
inline bool ShaderParameterAccess::setParameter(const char* name, const ValueType& value) {
  if (name == NULL)
    return false;
  ...
  if (it != _parametermap.end()) {
    ...
    FCPtr<ShaderParameterPtr, ParameterType> p =
        FCPtr<ShaderParameterPtr, ParameterType>::dcast(_parameters[(*it).second]);
    if (p == NullFC) {
...
template <class ParameterType, class ValueType>
inline bool ShaderParameterAccess::setMParameter(const char* name, const ValueType& value) {
  if (name == NULL)
    return false;
...
template <class ParameterType, class ValueType>
inline bool ShaderParameterAccess::getParameter(const char* name, ValueType& value) {
  if (name == NULL)
    return false;
...
#define OSGSHADERPARAMETERACCESS_INLINE_CVSID                                                      \
  "@(#)$Id: OSGShaderParameterAccess.inl,v 1.11 2007/03/09 18:11:48 a-m-z Exp $"
