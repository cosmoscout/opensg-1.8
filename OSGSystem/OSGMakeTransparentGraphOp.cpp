...
std::string MakeTransparentGraphOp::usage(void) {
  return "MakeTransparent: make used Materials transparent\n"
...
typename Chunk::Ptr getOrAddChunk(ChunkMaterialPtr cm, Type2Type<Chunk> = Type2Type<Chunk>()) {
...
void MakeTransparentGraphOp::applyTransparency(MaterialPtr m) {
  ...
  if (sm != NullFC) {
    ...
    beginEditCP(sm);
    sm->setTransparency(1.0f - (1.0f - sm->getTransparency()) * _transparency);
