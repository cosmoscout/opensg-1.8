...
void DVRVolumeTexture::changed(BitVector whichField, UInt32 origin) {
  ...
  if (whichField & ImageFieldMask) {
    ...
    if (sTatt) {
      ...
      _sfSliceThickness.setValue(Vec3f(Real32(sT[0]), Real32(sT[1]), Real32(sT[2])));
    } else {
    ...
    if (resAtt) {
      ...
      _sfResolution.setValue(Vec3f(Real32(sT[0]), Real32(sT[1]), Real32(sT[2])));
    } else {
      _sfResolution.setValue(
          Vec3f(getImage()->getWidth(), getImage()->getHeight(), getImage()->getDepth()));
    }
...
void DVRVolumeTexture::dump(UInt32, const BitVector) const {
