...
UInt32 RegisterCombinersChunk::_funcCombinerParameterfv      = Window::invalidFunctionID;
UInt32 RegisterCombinersChunk::_funcCombinerStageParameterfv = Window::invalidFunctionID;
UInt32 RegisterCombinersChunk::_funcCombinerInput            = Window::invalidFunctionID;
UInt32 RegisterCombinersChunk::_funcCombinerOutput           = Window::invalidFunctionID;
UInt32 RegisterCombinersChunk::_funcFinalCombinerInput       = Window::invalidFunctionID;
...
RegisterCombinersChunk::RegisterCombinersChunk(void)
    : Inherited() {
  _nvRegisterCombiners     = Window::registerExtension("GL_NV_register_combiners");
  _nvRegisterCombiners2    = Window::registerExtension("GL_NV_register_combiners2");
  ...
  _funcCombinerStageParameterfv = Window::registerFunction(
      OSG_DLSYM_UNDERSCORE "glCombinerStageParameterfvNV", _nvRegisterCombiners);
  _funcCombinerInput =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glCombinerInputNV", _nvRegisterCombiners);
  _funcCombinerOutput =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glCombinerOutputNV", _nvRegisterCombiners);
  _funcFinalCombinerInput =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glFinalCombinerInputNV", _nvRegisterCombiners);
...
void RegisterCombinersChunk::dump(UInt32, const BitVector) const {
...
void RegisterCombinersChunk::activate(DrawActionBase* action, UInt32) {
  ...
  void(OSG_APIENTRY * CombinerInput)(GLenum stage, GLenum portion, GLenum variable, GLenum input,
      GLenum mapping, GLenum component) =
      (void(OSG_APIENTRY*)(GLenum stage, GLenum portion, GLenum variable, GLenum input,
          GLenum mapping, GLenum component))win->getFunction(_funcCombinerInput);

  void(OSG_APIENTRY * CombinerOutput)(GLenum stage, GLenum portion, GLenum abOut, GLenum cdOut,
      GLenum sumOut, GLenum scale, GLenum bias, GLboolean abdot, GLboolean cddot,
      GLboolean muxSum) = (void(OSG_APIENTRY*)(GLenum stage, GLenum portion, GLenum abOut,
      GLenum cdOut, GLenum sumOut, GLenum scale, GLenum bias, GLboolean abdot, GLboolean cddot,
      GLboolean muxSum))win->getFunction(_funcCombinerOutput);

  void(OSG_APIENTRY * FinalCombinerInput)(GLenum variable, GLenum input, GLenum mapping,
      GLenum component) = (void(OSG_APIENTRY*)(GLenum variable, GLenum input, GLenum mapping,
      GLenum component))win->getFunction(_funcFinalCombinerInput);
  ...
  for (UInt16 i = 0; i < ncomb; i++) {
    if (getVariableArgb()[i * 3] != unused) {
      // RGB inputs
      CombinerInput(GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_A_NV, getVariableArgb()[i * 3],
          getVariableArgb()[i * 3 + 1], getVariableArgb()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_B_NV, getVariableBrgb()[i * 3],
          getVariableBrgb()[i * 3 + 1], getVariableBrgb()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_C_NV, getVariableCrgb()[i * 3],
          getVariableCrgb()[i * 3 + 1], getVariableCrgb()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_D_NV, getVariableDrgb()[i * 3],
          getVariableDrgb()[i * 3 + 1], getVariableDrgb()[i * 3 + 2]);

      // RGB output
      CombinerOutput(GL_COMBINER0_NV + i, GL_RGB, getOutputABrgb()[i], getOutputCDrgb()[i],
          getOutputSumrgb()[i], getScalergb()[i], getBiasrgb()[i], getDotABrgb()[i],
          getDotCDrgb()[i], getMuxSumrgb()[i]);

    } else {
      CombinerInput(
          GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_A_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);
      CombinerInput(
          GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_B_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);
      CombinerInput(
          GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);
      CombinerInput(
          GL_COMBINER0_NV + i, GL_RGB, GL_VARIABLE_D_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);

      CombinerOutput(GL_COMBINER0_NV + i, GL_RGB, GL_DISCARD_NV, GL_DISCARD_NV, GL_DISCARD_NV,
          GL_NONE, GL_NONE, GL_FALSE, GL_FALSE, GL_FALSE);
    }

    if (getVariableAalpha()[i * 3] != unused) {
      // Alpha inputs
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_A_NV, getVariableAalpha()[i * 3],
          getVariableAalpha()[i * 3 + 1], getVariableAalpha()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_B_NV, getVariableBalpha()[i * 3],
          getVariableBalpha()[i * 3 + 1], getVariableBalpha()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_C_NV, getVariableCalpha()[i * 3],
          getVariableCalpha()[i * 3 + 1], getVariableCalpha()[i * 3 + 2]);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_D_NV, getVariableDalpha()[i * 3],
          getVariableDalpha()[i * 3 + 1], getVariableDalpha()[i * 3 + 2]);

      // ALPHA output
      CombinerOutput(GL_COMBINER0_NV + i, GL_ALPHA, getOutputABalpha()[i], getOutputCDalpha()[i],
          getOutputSumalpha()[i], getScalealpha()[i], getBiasalpha()[i], GL_FALSE, GL_FALSE,
          getMuxSumalpha()[i]);
    } else {
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_A_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV,
          GL_ALPHA);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_B_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV,
          GL_ALPHA);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV,
          GL_ALPHA);
      CombinerInput(GL_COMBINER0_NV + i, GL_ALPHA, GL_VARIABLE_D_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV,
          GL_ALPHA);

      CombinerOutput(GL_COMBINER0_NV + i, GL_ALPHA, GL_DISCARD_NV, GL_DISCARD_NV, GL_DISCARD_NV,
          GL_NONE, GL_NONE, GL_FALSE, GL_FALSE, GL_FALSE);
    }

    if (getPerStageConstants()) {
      if (hasRC2) {
        CombinerStageParameterfv(GL_COMBINER0_NV + i, GL_CONSTANT_COLOR0_NV,
            (GLfloat*)getCombinerColor0()[i].getValuesRGBA());
        CombinerStageParameterfv(GL_COMBINER0_NV + i, GL_CONSTANT_COLOR1_NV,
  ...
  FinalCombinerInput(GL_VARIABLE_A_NV, getVariableArgb()[OSG_NUM_COMBINERS * 3],
      getVariableArgb()[OSG_NUM_COMBINERS * 3 + 1], getVariableArgb()[OSG_NUM_COMBINERS * 3 + 2]);

  glErr("RegisterCombinersChunk::final combiner var a setup");

  FinalCombinerInput(GL_VARIABLE_B_NV, getVariableBrgb()[OSG_NUM_COMBINERS * 3],
      getVariableBrgb()[OSG_NUM_COMBINERS * 3 + 1], getVariableBrgb()[OSG_NUM_COMBINERS * 3 + 2]);

  glErr("RegisterCombinersChunk::final combiner var b setup");

  FinalCombinerInput(GL_VARIABLE_C_NV, getVariableCrgb()[OSG_NUM_COMBINERS * 3],
      getVariableCrgb()[OSG_NUM_COMBINERS * 3 + 1], getVariableCrgb()[OSG_NUM_COMBINERS * 3 + 2]);

  glErr("RegisterCombinersChunk::final combiner var c setup");

  FinalCombinerInput(GL_VARIABLE_D_NV, getVariableDrgb()[OSG_NUM_COMBINERS * 3],
      getVariableDrgb()[OSG_NUM_COMBINERS * 3 + 1], getVariableDrgb()[OSG_NUM_COMBINERS * 3 + 2]);

  glErr("RegisterCombinersChunk::final combiner var d setup");

  FinalCombinerInput(GL_VARIABLE_E_NV, getVariableE()[0], getVariableE()[1], getVariableE()[2]);

  glErr("RegisterCombinersChunk::final combiner var e setup");

  FinalCombinerInput(GL_VARIABLE_F_NV, getVariableF()[0], getVariableF()[1], getVariableF()[2]);

  glErr("RegisterCombinersChunk::final combiner var f setup");

  FinalCombinerInput(GL_VARIABLE_G_NV, getVariableG()[0], getVariableG()[1], getVariableG()[2]);
...
void RegisterCombinersChunk::changeFrom(DrawActionBase* action, StateChunk* old, UInt32 idx) {
...
void RegisterCombinersChunk::setCombinerRGB(UInt16 which, GLenum ainput, GLenum amapping,
    GLenum acompusage, GLenum binput, GLenum bmapping, GLenum bcompusage, GLenum cinput,
    GLenum cmapping, GLenum ccompusage, GLenum dinput, GLenum dmapping, GLenum dcompusage,
    GLenum outputAB, GLenum outputCD, GLenum outputSum, GLenum scale, GLenum bias, GLboolean dotAB,
...
void RegisterCombinersChunk::setCombinerAlpha(UInt16 which, GLenum ainput, GLenum amapping,
    GLenum acompusage, GLenum binput, GLenum bmapping, GLenum bcompusage, GLenum cinput,
    GLenum cmapping, GLenum ccompusage, GLenum dinput, GLenum dmapping, GLenum dcompusage,
    GLenum outputAB, GLenum outputCD, GLenum outputSum, GLenum scale, GLenum bias,
    GLboolean muxSum) {
  ensureSizes();
...
void RegisterCombinersChunk::setFinalCombiner(GLenum ainput, GLenum amapping, GLenum acompusage,
    GLenum binput, GLenum bmapping, GLenum bcompusage, GLenum cinput, GLenum cmapping,
    GLenum ccompusage, GLenum dinput, GLenum dmapping, GLenum dcompusage, GLenum einput,
    GLenum emapping, GLenum ecompusage, GLenum finput, GLenum fmapping, GLenum fcompusage,
...
void RegisterCombinersChunk::setConstantColors(Color4f& color0, Color4f& color1) {
...
void RegisterCombinersChunk::setCombinerColors(UInt16 which, Color4f& color0, Color4f& color1) {
  RegisterCombinersChunkPtr tmpPtr(*this);
