...
int SimplePolygon::triangulate(DCTPVec2dvector& globalverts, simplepolygonvector& polylist) {
  ...
  case 3: {
    ...
    //				std::cerr << "adding polygon of 3 vertices into the list..." <<
    //std::endl; 				polylist.push_back( p ); // FIXME, this is not too elegant 				std::cerr <<
    //"triangulate out!!!" << std::endl;
  }
    return 0;
  case 4: {
    ...
    if (cd_sdist1 - cd_sdist2 < DCTP_EPS) {
      /*					const double cd_r1 = orient2d( ad_pa, ad_pb, ad_pc
         ); const double cd_r2 = orient2d( ad_pa, ad_pb, ad_pd ); if( ( ( cd_r1 <= 0.0 ) && ( cd_r2
         <= 0.0 ) ) || ( ( cd_r1 >= 0.0 ) && ( cd_r2 >= 0.0 ) ) )*/
      if ((orient2d(ad_pa, ad_pb, ad_pc) <= 0.0) || (orient2d(ad_pa, ad_pd, ad_pc) >= 0.0) ||
        ...
        p.vertices[2] = ci_v4;
        polylist.push_back(p);
        //						std::cerr << globalverts[ ci_v1 ] << globalverts[
        //ci_v2 ] << globalverts[ ci_v3 ] << globalverts[ ci_v4 ] << std::endl;
      } else {
        p.vertices[0] = ci_v1;
        p.vertices[1] = ci_v2;
        p.vertices[2] = ci_v3;
      ...
      //); 					const double cd_r2 = orient2d( ad_pc, ad_pd, ad_pb ); 					if( ( ( cd_r1 <= 0.0 ) && ( cd_r2
      //<= 0.0 ) ) || 						( ( cd_r1 >= 0.0 ) && ( cd_r2 >= 0.0 ) ) )
      if ((orient2d(ad_pb, ad_pc, ad_pd) <= 0.0) || (orient2d(ad_pb, ad_pa, ad_pd) >= 0.0) ||
 ...
 *          1 if inside <BR>
 */
int SimplePolygon::isInsideCircumCircle(
    DCTPVec2dvector& globalverts, int v1, int v2, int v3, int p) const {
...
//! Is the triangle formed by the vertices (v1, v2, v3) a Delaunay triangle (with regard to this
//! polygon ?
/*!
 *  \param globalverts global vertices vector
 *  \param v1 first vertex
...
int SimplePolygon::isDelaunay(DCTPVec2dvector& globalverts, int v1, int v2, int v3) {
  ...
  for (i = 0; i < vsize; ++i) {
    ...
    if (v3 != v4) {
      ...
      //      std::cerr << "Inside circle test " << v1 << "," << v2 << "," << v3 << "," << v4 << " =
      //      " << ret << std::endl;
      if (ret) {
        return ret;
      }
 ...
 *  \return zero on success, and a negative integer if some error occured.
 */
int SimplePolygon::removeLinearPoints(
    DCTPVec2dvector& globalverts, const Vec2d min, const Vec2d max) {
...
int SimplePolygon::splitPolygon(
    DCTPVec2dvector& globalverts, simplepolygonvector& polylist, const Vec2d min, const Vec2d max) {
  DCTPVec2dset                            vset;
  ...
  for (i1 = 0; i1 < oldNum; ++i1) {
    ...
    if (!siv.second) {
      ...
      //      std::cerr << oldNum << " split in " << vertices.size() << "," << poly.vertices.size()
      //      << std::endl;
  ...
  for (i2 = 2; i2 < oldNum; ++i2) {
    ...
    for (i1 = start; i1 < i2 - 1; ++i1) {
      ...
      if (min[1] - p4[1] > DCTP_EPS)
        test |= 0x08;
      if ((test & ptest) == 0) {
        //         std::cerr << i1 << "," << i1 + 1 << " overlaps " << i2 << "," << i2 + 1 <<
        //         std::endl;
        ...
        if ((orient2d(d1, d2, d3) == 0.0) && (orient2d(d1, d2, d4) == 0.0)) {
          ...
          if (doIntersect(globalverts, i1, i1 + 1, i2, (i2 + 1) % oldNum)) {
            ...
            //			std::cerr << p1[0] << "," << p1[1] << " - " << p2[0] << "," << p2[1] <<
            //std::endl; 			std::cerr << p3[0] << "," << p3[1] << " - " << p4[0] << "," << p4[1] <<
            //std::endl; 			std::cerr << globalverts[ iip ][0] << "," << globalverts[ iip ][1] << " - "
            //<< std::endl;
            intersect = true;
          }
        }
        if (intersect) {
          ...
          //          std::cerr << i1 << "," << i1 + 1 << " intersects " << i2 << "," << i2 + 1 <<
          //          std::endl;
          ...
          // copy iip's
          if (iip >= 0) {
            if ((iip != vertices[i1]) && (iip != vertices[(i2 + 1) % oldNum])) {
              newPoints[newNum] = iip;
              ++newNum;
            }
            if ((iip != vertices[i2]) && (iip != vertices[i1 + 1])) {
          ...
          //          std::cerr << "split in " << newNum << "," << poly.vertices.size() <<
          //          std::endl;
...
bool SimplePolygon::isConvex(DCTPVec2dvector& globalverts) {
  ...
  for (ui_next = 0; ui_next < cui_vertex_cnt; ++ui_next) {
    ...
    if (orient2d(ad_p, ad_v, ad_n) < 0.0) {
      ...
      //			std::cerr << ad_n[ 0 ] << " " << ad_n[ 1 ] << std::endl <<
      //std::endl;
      // not convex
      return false;
    }
