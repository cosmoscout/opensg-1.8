...
class OSG_SYSTEMLIB_DLLMAPPING BalancedMultiWindow : public BalancedMultiWindowBase {
 ...
 protected:
  ...
  void serverRender(WindowPtr window, UInt32 id, RenderActionBase* action);
 ...
 private:
  ...
  inline Real32 getVisibleLoad(Int32 const (&rect)[4], BBox& bbox);
  // get number of tiles for the given area
  UInt32 calcTileCount(Int32 const (&rect)[4]);
  // project group and calculate bounding box
  bool calculateProjectedBBox(VPort& port, LoadGroup& group, BBox& bbox, Matrix& proj);
  ...
  // split viewport
  void splitViewport(std::vector<Worker>& worker, VPort& port, Int32 const (&rect)[4], Real32 load);
  // sort bboxes
  void sortBBoxes(VPort& port, UInt32 axis, Int32 const (&rect)[4]);
  // split load at a given axis
  void splitAxis(Real32 const (&load)[2], VPort& port, Int32 const (&rect)[4], int axis,
      Real32 (&resultLoad)[2], Int32 (&resultRect)[2][4]);
  // render part of a viewport viewport
  void renderViewport(WindowPtr serverWindow, UInt32 id, RenderActionBase* action, UInt32 portId,
      Int32 const (&rect)[4]);
  // clear local ports
  void clearViewports(WindowPtr serverWindow, UInt32 id, RenderActionBase* action);
  // store viewport
  void storeViewport(Area& area, ViewportPtr vp, Int32 const (&rect)[4]);

  // do rendering and network transfer
  void drawSendAndRecv(WindowPtr serverWindow, RenderActionBase* action, UInt32 id);

  // preload display lists and textures
  void preloadCache(WindowPtr window, RenderActionBase* action);
...
#define OSGBALANCEDMULTIWINDOW_HEADER_CVSID                                                        \
