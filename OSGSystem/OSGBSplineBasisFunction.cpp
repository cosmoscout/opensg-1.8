  ...
  for (i_r = 0; i_r <= iP; ++i_r) {
    ...
    for (i_k = 1; i_k <= iDepth; ++i_k) {
      ...
      for (i_j = i_j1; i_j <= i_j2; ++i_j) {
        ppd_a[i_s2][i_j] =
            (ppd_a[i_s1][i_j] - ppd_a[i_s1][i_j - 1]) / ppd_ndu[i_pk + 1][i_rk + i_j];
...
int BSplineBasisFunction::CheckKnotPoints(const DCTPdvector& k) {
  if (osgabs(k[0] - k[k.size() - 1]) < DCTP_EPS) {
    std::cerr << "BSplineBasisFunction::CheckKnotPoints: first and last knot are equal: " << k[0]
              << std::endl;
  ...
  while (i != k.end()) {
    if (temp > *i) {
      std::cerr << "BSplineBasisFunction::CheckKnotPoints: illegal consecutive knots: " << temp
                << " " << *i << std::endl;
      return -2;
    }
    temp = *i;
...
int BSplineBasisFunction::findSpan(double& u) {
  ...
  --n;   // n holds the index of the last different knot before the equal ones at the of the knot
       // vector
