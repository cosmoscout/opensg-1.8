...
*/
void Camera::setup(DrawActionBase* OSG_CHECK_ARG(action), const Viewport& port) {
  Matrix m, t;
 ...
 */
void Camera::setupProjection(DrawActionBase* OSG_CHECK_ARG(action), const Viewport& port) {
  Matrix m, t;
 ...
 */
void Camera::draw(DrawAction* OSG_CHECK_ARG(action), const Viewport& OSG_CHECK_ARG(port)) {
}

/*! Get/calculate the projection matrix for this camera.
 */
void Camera::getProjection(
    Matrix& OSG_CHECK_ARG(result), UInt32 OSG_CHECK_ARG(width), UInt32 OSG_CHECK_ARG(height)) {
...
/*! Get/calculate the projection translation matrix for this camera. The
    default is identity.
*/
void Camera::getProjectionTranslation(
...
*/
void Camera::getViewing(Matrix& result, UInt32 OSG_CHECK_ARG(width), UInt32 OSG_CHECK_ARG(height)) {
  if (getBeacon() == NullFC) {
 ...
 */
void Camera::getFrustum(FrustumVolume& result, UInt32 width, UInt32 height) {
  Matrix mv, prt, pr;
...
bool Camera::calcViewRay(Line& line, Int32 x, Int32 y, const Viewport& port) {
  ...
  getProjectionTranslation(projtrans, port.getPixelWidth(), port.getPixelHeight());
  ...
  Real32 rx = (x - port.getPixelLeft()) / (Real32)port.getPixelWidth() * 2.f - 1.f,
         ry = 1.f - ((y - (port.getParent()->getHeight() - port.getPixelTop())) /
...
void Camera::dump(UInt32 OSG_CHECK_ARG(uiIndent), const BitVector OSG_CHECK_ARG(bvFlags)) const {
