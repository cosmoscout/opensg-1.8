...
typedef void(OSG_APIENTRY* CombinerParameteriNVFunc)(GLenum, const GLint);
typedef void(OSG_APIENTRY* CombinerInputNVFunc)(GLenum, GLenum, GLenum, GLenum, GLenum, GLenum);
typedef void(OSG_APIENTRY* CombinerOutputNVFunc)(
    GLenum, GLenum, GLenum, GLenum, GLenum, GLenum, GLenum, GLboolean, GLboolean, GLboolean);
typedef void(OSG_APIENTRY* FinalCombinerInputNVFunc)(GLenum, GLenum, GLenum, GLenum);

typedef void(OSG_APIENTRY* CombinerParameterfvNVFunc)(GLenum, GLfloat*);
...
typedef void(OSG_APIENTRY* MultiTexCoord2dARBFunc)(GLenum, GLdouble, GLdouble);
...
void DVRIsoShader::initCombiners_IsoSurfaceDiffuse(DrawActionBase* action) {
  ...
  CombinerInputNVFunc CombinerInputNV = (CombinerInputNVFunc)win->getFunction(_funcCombinerInputNV);
  ...
  CombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_B_NV, GL_CONSTANT_COLOR0_NV,
      GL_UNSIGNED_INVERT_NV,
      GL_ALPHA); // usage: Alpha

  // C <- scaled/biased gradients
  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_C_NV, GL_TEXTURE1_ARB, GL_EXPAND_NORMAL_NV, GL_RGB);

  // D <- interpolation factor
  CombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_D_NV, GL_CONSTANT_COLOR0_NV,
  ...
  //  A <- alpha values
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_A_NV, GL_TEXTURE0_ARB,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  // B <- interpolation factor
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_B_NV, GL_CONSTANT_COLOR0_NV,
      GL_UNSIGNED_INVERT_NV, GL_ALPHA);

  // C <- alpha values
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_C_NV, GL_TEXTURE1_ARB,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  // D <- interpolation factor
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_D_NV, GL_CONSTANT_COLOR0_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);
  ...
  CombinerOutputNV(GL_COMBINER0_NV, GL_ALPHA, GL_DISCARD_NV, GL_DISCARD_NV, GL_SPARE0_NV, GL_NONE,
      GL_NONE, GL_FALSE, GL_FALSE, GL_FALSE);
  ...
  CombinerInputNV(GL_COMBINER1_NV, GL_RGB, GL_VARIABLE_B_NV, GL_CONSTANT_COLOR0_NV,
  ...
  CombinerInputNV(GL_COMBINER1_NV, GL_RGB, GL_VARIABLE_D_NV, GL_CONSTANT_COLOR1_NV,
      GL_EXPAND_NORMAL_NV, GL_RGB);
...
void DVRIsoShader::initCombiners_Diffuse2Combiners(DrawActionBase* action) {
  ...
  FinalCombinerInputNV(GL_VARIABLE_A_NV, GL_TEXTURE0_ARB, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_B_NV, GL_PRIMARY_COLOR_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_E_NV, GL_TEXTURE1_ARB, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_F_NV, GL_SECONDARY_COLOR_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_D_NV, GL_E_TIMES_F_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
...
void DVRIsoShader::initCombiners_DiffuseMultiCombiners(DrawActionBase* action) {
  ...
  CombinerInputNV(GL_COMBINER2_NV, GL_RGB, GL_VARIABLE_B_NV, GL_CONSTANT_COLOR0_NV,
  ...
  CombinerInputNV(GL_COMBINER2_NV, GL_RGB, GL_VARIABLE_D_NV, GL_CONSTANT_COLOR1_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_RGB);
  ...
  FinalCombinerInputNV(GL_VARIABLE_A_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_B_NV, GL_SPARE1_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  FinalCombinerInputNV(
      GL_VARIABLE_D_NV, GL_SPARE0_PLUS_SECONDARY_COLOR_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
...
void DVRIsoShader::initCombiners_IsoSurfaceSpecular(DrawActionBase* action) {
  ...
  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_B_NV, GL_TEXTURE0_ARB, GL_EXPAND_NORMAL_NV, GL_RGB);

  // C <- interpolation factor
  CombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_C_NV, GL_CONSTANT_COLOR0_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  // D <- packed gradients (1)
  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_D_NV, GL_TEXTURE1_ARB, GL_EXPAND_NORMAL_NV, GL_RGB);
  ...
  // B <- alpha value (0)
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_B_NV, GL_TEXTURE0_ARB,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  // C <- interpolation factor
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_C_NV, GL_CONSTANT_COLOR0_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  // D <- alpha value (0)
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_D_NV, GL_TEXTURE1_ARB,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);
  ...
  CombinerOutputNV(GL_COMBINER0_NV, GL_ALPHA, GL_DISCARD_NV, GL_DISCARD_NV, GL_TEXTURE0_ARB,
...
void DVRIsoShader::initCombiners_Specular2Combiners(DrawActionBase* action) {
  ...
  // E <- <grad,halfway>
  FinalCombinerInputNV(GL_VARIABLE_E_NV, GL_SPARE1_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  // F <- <grad,halfway>
  FinalCombinerInputNV(GL_VARIABLE_F_NV, GL_SPARE1_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  // A <- <grad,halfway>^2
  FinalCombinerInputNV(GL_VARIABLE_A_NV, GL_E_TIMES_F_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  // B <- <grad,halfway>^2
  FinalCombinerInputNV(GL_VARIABLE_B_NV, GL_E_TIMES_F_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
  ...
  // Alpha component
  FinalCombinerInputNV(GL_VARIABLE_G_NV, GL_TEXTURE0_ARB, GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);
}
...
void DVRIsoShader::initCombiners_SpecularMultiCombiners(DrawActionBase* action) {
  ...
  FinalCombinerInputNV(GL_VARIABLE_A_NV, GL_SPARE1_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  // B <- (1,1,1)
  FinalCombinerInputNV(GL_VARIABLE_B_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV, GL_RGB);
...
void DVRIsoShader::setupCombinerParametersDiffuse(DVRVolume* volume, DrawActionBase* action) {
  ...
  CombinerParameterfvNVFunc CombinerParameterfvNV =
      (CombinerParameterfvNVFunc)win->getFunction(_funcCombinerParameterfvNV);
  ...
  CombinerStageParameterfvNVFunc CombinerStageParameterfvNV =
      (CombinerStageParameterfvNVFunc)win->getFunction(_funcCombinerStageParameterfvNV);
  ...
  if ((m_maxCombiners > 5) && m_hasPerStageConstants) {
    ...
    CombinerStageParameterfvNV(
        GL_COMBINER2_NV, GL_CONSTANT_COLOR0_NV, diffuseColor[0].getValuesRGBA());

    CombinerStageParameterfvNV(
        GL_COMBINER2_NV, GL_CONSTANT_COLOR1_NV, diffuseColor[1].getValuesRGBA());
    ...
    for (i = 0; i < (m_maxCombiners - 5) / 3; i++) {
      ...
      CombinerStageParameterfvNV(GLenum(GL_COMBINER3_NV + i * 3), GL_CONSTANT_COLOR0_NV,
          diffuseDir[2 + 2 * i].getValues());

      CombinerStageParameterfvNV(GLenum(GL_COMBINER3_NV + i * 3), GL_CONSTANT_COLOR1_NV,
          diffuseDir[3 + 2 * i].getValues());

      // set color for next two lights
      CombinerStageParameterfvNV(GLenum(GL_COMBINER3_NV + i * 3 + 1), GL_CONSTANT_COLOR0_NV,
          diffuseColor[2 + 2 * i].getValuesRGBA());

      CombinerStageParameterfvNV(GLenum(GL_COMBINER3_NV + i * 3 + 1), GL_CONSTANT_COLOR1_NV,
          diffuseColor[3 + 2 * i].getValuesRGBA());
    }

    // set direction for last two lights
    CombinerStageParameterfvNV(
        GLenum(GL_COMBINER3_NV + i * 3), GL_CONSTANT_COLOR0_NV, diffuseDir[2 + 2 * i].getValues());

    CombinerStageParameterfvNV(
        GLenum(GL_COMBINER3_NV + i * 3), GL_CONSTANT_COLOR1_NV, diffuseDir[3 + 2 * i].getValues());
...
void DVRIsoShader::setupCombinerParametersSpecular(DVRVolume* volume, DrawActionBase* action) {
  ...
  if ((m_maxCombiners >= 8) && m_hasPerStageConstants) {
    ...
    CombinerStageParameterfvNV(
        GL_COMBINER4_NV, GL_CONSTANT_COLOR0_NV, diffuseColor[1].getValuesRGBA());

    // set diffuse color light 2
    CombinerStageParameterfvNV(
        GL_COMBINER4_NV, GL_CONSTANT_COLOR1_NV, diffuseColor[2].getValuesRGBA());

    // set specular color light 1
    CombinerStageParameterfvNV(
        GL_COMBINER5_NV, GL_CONSTANT_COLOR0_NV, specularColor[1].getValuesRGBA());

    // set specular color light 2
    CombinerStageParameterfvNV(
        GL_COMBINER5_NV, GL_CONSTANT_COLOR1_NV, specularColor[2].getValuesRGBA());

    // set specular color light 0
    CombinerStageParameterfvNV(
        GL_COMBINER7_NV, GL_CONSTANT_COLOR0_NV, specularColor[0].getValuesRGBA());

    // set diffuse color light 0
    CombinerStageParameterfvNV(
        GL_COMBINER7_NV, GL_CONSTANT_COLOR1_NV, diffuseColor[0].getValuesRGBA());
  } else {
    ...
    glColor3f(0.5f + 0.5f * diffuseDir[0][0], 0.5f + 0.5f * diffuseDir[0][1],
...
void DVRIsoShader::renderSlice_NVRegisterCombinerShading(
  ...
  case DVRRenderSlice::XY:
    intFactor = Real32((*(*clippedSlice->begin())->vertices.begin())[5]);
    break;

  case DVRRenderSlice::XZ:
    intFactor = Real32((*(*clippedSlice->begin())->vertices.begin())[4]);
    break;

  case DVRRenderSlice::YZ:
    intFactor = Real32((*(*clippedSlice->begin())->vertices.begin())[3]);
    break;
