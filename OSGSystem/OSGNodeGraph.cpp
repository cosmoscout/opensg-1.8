...
NodeGraph::NodeGraph(const NodeGraph& obj)
    : _nodeVec(obj._nodeVec)
    , _edgeMapVec(obj._edgeMapVec) {
  return;
}
...
bool NodeGraph::verify(bool printInfo) {
  ...
  if (printInfo) {
    FNOTICE(("NodeDegree Count:  %d %d %d %d (%d)\n", nodeDegree[0], nodeDegree[1], nodeDegree[2],
        nodeDegree[3], nodeDegree[4]));
...
//
//----------------------------------------------------------------------
int NodeGraph::createPathVec(vector<Path>& pathVec, bool OSG_CHECK_ARG(createStrips),
    bool OSG_CHECK_ARG(createFans), int OSG_CHECK_ARG(minFanEdgeCount)) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (brotherEdge)
          if (brotherEdge->node->list &&
              (!nextNode || (nextNode->list->degree() > brotherEdge->node->list->degree()))) {
        ...
        while (listRI != path.path.rend()) {
          ...
          if (walkRight == true) {
            if (((entrySide + 1) % 3 == exitSide)) {
              ...
              primitive.push_back(pi);
              cost += 2;
            } else if (((entrySide + 2) % 3 == exitSide)) {
              walkRight = false;
