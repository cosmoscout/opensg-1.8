...
UInt32 DVRSimpleLUTShader::_sgiTexColorTable = Window::invalidExtensionID;

UInt32 DVRSimpleLUTShader::_extPalettedTexture = Window::invalidExtensionID;

UInt32 DVRSimpleLUTShader::_extSharedPalettedTexture = Window::invalidExtensionID;

UInt32 DVRSimpleLUTShader::_arbMultitexture = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_nvTextureShader2 = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_arbFragmentProgram = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_nvRegisterCombiners = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcColorTableSGI = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcColorTableEXT = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcActiveTextureARB = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcFinalCombinerInputNV = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcCombinerInputNV = Window::invalidFunctionID;

UInt32 DVRSimpleLUTShader::_funcCombinerOutputNV = Window::invalidFunctionID;
...
DVRSimpleLUTShader::DVRSimpleLUTShader(void)
  ...
  _sgiTexColorTable = Window::registerExtension("GL_SGI_texture_color_table");

  _extPalettedTexture = Window::registerExtension("GL_EXT_paletted_texture");

  _extSharedPalettedTexture = Window::registerExtension("GL_EXT_shared_texture_palette");

  _arbMultitexture = Window::registerExtension("GL_ARB_multitexture");
  ...
  _funcColorTableSGI =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glColorTableSGI", _sgiTexColorTable);

  _funcColorTableEXT =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glColorTableEXT", _extPalettedTexture);

  _funcActiveTextureARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glActiveTextureARB", _arbMultitexture);
...
void DVRSimpleLUTShader::getPaletteFormat(
    DrawActionBase* /*action*/, UInt8 mode, GLenum& internalFormat, GLenum& externalFormat) {
  switch (mode) {
  case LM_TABLE_SGI:
...
void DVRSimpleLUTShader::setupAlphaCorrectionRegisterCombiners(DrawActionBase* action) {
  ...
  FinalCombinerInputNV = (FinalCombinerInputNVFunc)win->getFunction(_funcFinalCombinerInputNV);

  CombinerInputNV = (CombinerInputNVFunc)win->getFunction(_funcCombinerInputNV);

  CombinerOutputNV = (CombinerOutputNVFunc)win->getFunction(_funcCombinerOutputNV);
  ...
  // Input: RGB-portion
  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_A_NV, GL_TEXTURE1_ARB, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  CombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_B_NV, GL_PRIMARY_COLOR_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  CombinerInputNV(
      GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_D_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_RGB);

  // Input: Alpha Portion
  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_A_NV, GL_TEXTURE1_ARB,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  CombinerInputNV(GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_B_NV, GL_PRIMARY_COLOR_NV,
      GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  CombinerInputNV(
      GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);

  CombinerInputNV(
      GL_COMBINER0_NV, GL_ALPHA, GL_VARIABLE_D_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV, GL_ALPHA);
  ...
  CombinerOutputNV(GL_COMBINER0_NV, GL_ALPHA,
      GL_SPARE0_NV,  // abOutput
      GL_DISCARD_NV, // cdOutput
      GL_DISCARD_NV, // sumOutput
...
void DVRSimpleLUTShader::dump(UInt32, const BitVector) const {
...
bool DVRSimpleLUTShader::initialize(DVRVolume* volume, DrawActionBase* action) {
  ...
  if (getLutMode() != LM_AUTO) {
    ...
    if (isModeSupported(action, getLutMode(), volume->getTextureMode(win))) {
      SWARNING << "DVRSimpleLUTShader - User specified lookup table "
               << "mode " << int(getLutMode()) << std::endl;

      m_nTexturePaletteMode = getLutMode();
    } else {
      SWARNING << "DVRSimpleLUTShader - Unsupported lookup table mode " << int(getLutMode())
               << " disabling LUT" << std::endl;
  ...
  m_nTextureId = volume->getTextureManager().registerTexture(vol->getImage(), // image
...
#define CHECK_GL_ERROR(msg)                                                                        \
  {                                                                                                \
    ...
    if ((e != 0) && (e != GL_NO_ERROR)) {                                                          \
      std::cout << std::endl                                                                       \
                << "OpenGL-ERROR <0x" << setfill('0') << hex << setw(4) << e << dec                \
...
typedef void(OSG_APIENTRY* ColorTableSGIFunc)(
    GLenum, GLenum, GLsizei, GLenum, GLenum, const GLvoid*);

typedef void(OSG_APIENTRY* ColorTableEXTFunc)(
    GLenum, GLenum, GLsizei, GLenum, GLenum, const GLvoid*);
...
void DVRSimpleLUTShader::activate(DVRVolume* volume, DrawActionBase* action) {
  ...
  if (win->hasExtension(_sgiTexColorTable)) {
    ColorTableSGI = (ColorTableSGIFunc)win->getFunction(_funcColorTableSGI);
  }

  ColorTableEXTFunc ColorTableEXT = NULL;

  if (win->hasExtension(_extSharedPalettedTexture)) {
    ColorTableEXT = (ColorTableEXTFunc)win->getFunction(_funcColorTableEXT);
  }
...
#if defined GL_NV_texture_shader2
  ActiveTextureARBFunc ActiveTextureARB =
      (ActiveTextureARBFunc)action->getWindow()->getFunction(_funcActiveTextureARB);
#endif
