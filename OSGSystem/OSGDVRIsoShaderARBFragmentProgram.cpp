  ...
  beginEditCP(m_pFragProg, ProgramChunk::ParamValuesFieldMask);
  {
    ...
    if (doSpecular) {
      ...
      for (i = 0; i < maxSpecularLights && ls != specularLights.end(); i++) {
        ...
        // halfway vector
        m_pFragProg->setParameter(firstSpecId + 2 * i, Vec4f(tmp));
        // specular color
        m_pFragProg->setParameter(firstSpecId + 2 * i + 1, cToV(ls->color));
    ...
    for (; i < maxSpecularLights; i++) {
      // specular direction
      m_pFragProg->setParameter(firstSpecId + 2 * i, Vec4f(0, 0, 0, 0));

      // specular color
      m_pFragProg->setParameter(firstSpecId + 2 * i + 1, Vec4f(0, 0, 0, 0));
    }
  }
...
char DVRIsoShader::_fragProg2D[] = "!!ARBfp1.0 \n"
...
char DVRIsoShader::_fragProg3D[] = "!!ARBfp1.0 \n"
                                   "PARAM scaleBias = {2, -1, 0, 0};\n"
                                   "TEMP volume, normal, intensity, color;\n"
                                   "\n"
