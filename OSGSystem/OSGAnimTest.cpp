...
Animation* addAnim(
    InterpolatorBase::InterpolType type, char* name, char* objname, int destNr, float duration) {
  std::cerr << "addAnim(type=" << InterpolatorBase::type2String(type) << ", name=" << name
            << ", target=" << objname << ", duration=" << duration << " )" << std::endl;
  ...
  if (interpol) {
    ...
    if (Animation::getAnim()) {
      ...
      std::cerr << "OSGAnimTest::addAnim()\t Animation::getAnim() is NULL\n";
    }
  }
