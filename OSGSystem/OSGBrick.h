...
struct BrickSet {
  ...
  //! Build 3D bricks
  void buildBricks3D(
      DVRVolume* volume, Vec3f brickSubdivision, Int32 overlap = 1, Orientation ori = UNDEF);
...
class Brick {
 public:
  ...
  void init(Int32 minVoxX, Int32 minVoxY, Int32 minVoxZ, Int32 resX, Int32 resY, Int32 resZ,
      Real32 xMin, Real32 yMin, Real32 zMin, Real32 xMax, Real32 yMax, Real32 zMax,

      Int32 overlap = 1, BrickSet::Orientation ori = BrickSet::UNDEF);
  ...
  //! Render slices
  void renderSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, TextureManager::TextureMode mode);
 ...
 private:
  ...
  //! Render slices
  void render3DSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, const Vec3f& sliceDir);

  void render2DSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, const Vec3f& sliceDir);

  void render2DMultiSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, bool btf, const Vec3f& sliceDir);

  void render2DSliceXY(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, const Vec3f& sliceDir, const Real32& zCoord);

  void render2DSliceXZ(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
      DVRClipper* clipper, const Vec3f& sliceDir, const Real32& yCoord);

  void render2DSliceYZ(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
