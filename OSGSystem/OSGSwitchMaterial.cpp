...
void SwitchMaterial::subMaterial(MaterialPtr mat) {
  ...
  SWARNING << "SwitchMaterial::subMaterial(" << this << ") has no material " << mat << std::endl;
}
...
StatePtr SwitchMaterial::makeState(void) {
  ...
  if (choice >= _mfMaterials.size()) {
    if (!_mfMaterials.empty())
      SWARNING << "SwitchMaterial::makeState: choice index (" << choice << ") out of range!"
               << std::endl;
...
void SwitchMaterial::rebuildState(void) {
  ...
  if (choice >= _mfMaterials.size()) {
    if (!_mfMaterials.empty())
      SWARNING << "SwitchMaterial::rebuildState: choice index (" << choice << ") out of range!"
               << std::endl;
...
void SwitchMaterial::dump(UInt32, const BitVector) const {
