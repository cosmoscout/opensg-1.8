...
ClusterNetwork::~ClusterNetwork(void) {
  ...
  _mainConnection = NULL;
  for (ConnectionsT::iterator c = _connection.begin(); c != _connection.end(); ++c) {
    if (*c)
      delete (*c);
    (*c) = NULL;
  }
 ...
 */
void ClusterNetwork::connectAllPointToPoint(UInt32 thisId, const std::string& connectionType) {
  bool                          isClient;
  UInt32                        servers;
  ...
  for (id = 0; id <= servers; ++id) {
    if (isClient && id == thisId)
      _connection[id] = ConnectionFactory::the().createGroup(connectionType);
    else
      _connection[id] = ConnectionFactory::the().createPoint(connectionType);
    if (id > thisId) {
 ...
 *
 */
void ClusterNetwork::connectAllGroupToPoint(UInt32 thisId, const std::string& connectionType) {
  UInt32                        id, toId;
  bool                          isClient;
  UInt32                        servers;
  std::map<UInt32, std::string> address;
