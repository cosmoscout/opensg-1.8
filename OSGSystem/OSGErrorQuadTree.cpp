    ...
    for (ui_face = 0; ui_face < ui_face_cnt; ++ui_face) {
      ...
      if (pcl_face->faceinfo) {
        ...
        while (f_act_error > fError) {
          ...
          {
#ifdef OSG_ARBITRARY_SPLIT
            pclMesh->SubdivideQuadEW(
                pcl_face, -((SFaceTreeCell*)pcl_face->faceinfo)->ptErrorCell->fSplitValue);
#else
            pclMesh->SubdivideQuadEW(pcl_face);
#endif
          } else {
#ifdef OSG_ARBITRARY_SPLIT
            pclMesh->SubdivideQuadNS(
                pcl_face, ((SFaceTreeCell*)pcl_face->faceinfo)->ptErrorCell->fSplitValue);
#else
        ...
        if (((SFaceTreeCell*)pcl_face->faceinfo)->ptErrorCell) {
          ...
          //					std::cerr << pcl_face->orig_quad[ 3 ]->coords <<
          //pcl_face->orig_quad[ 1 ]->coords << std::endl; 					std::cerr << f_act_error << ", " <<
          //f_prev_error << std::endl;
        }
#endif
    ...
    /*		if( pclMesh->faces.size( ) >= 1000 )
                    {
                            if( fError > m_fErrorCutoff )
                            {
                                    std::cerr << "reduce error cutoff: " << m_fErrorCutoff << " -> "
       << fError << std::endl; m_fErrorCutoff = fError;
                            }
                    }
                    if( fError <= m_fErrorCutoff )
                    {
  ...
  for (ui_face = 0; ui_face < ui_face_cnt; ++ui_face) {
    ...
    if (pcl_face->faceinfo) {
      ...
      while (f_act_error > fError) {
        ...
        {
#ifdef OSG_ARBITRARY_SPLIT
          const double cd_ratio =
              osgMin(-cpt_errcell->fSplitValue, 1.0f + cpt_errcell->fSplitValue);
#ifdef OSG_UNION_TRI_QUAD
          if ((pcl_face->orig_face[1]->coords[0] - pcl_face->orig_face[0]->coords[0]) * cd_ratio >
              DCTP_EPS * 100.0)
#else
          if ((pcl_face->orig_quad[1]->coords[0] - pcl_face->orig_quad[0]->coords[0]) * cd_ratio >
              DCTP_EPS * 100.0)
#endif
#else
#ifdef OSG_UNION_TRI_QUAD
          if ((pcl_face->orig_face[1]->coords[0] - pcl_face->orig_face[0]->coords[0]) * 0.5 >
              DCTP_EPS * 100.0)
#else
          if ((pcl_face->orig_quad[1]->coords[0] - pcl_face->orig_quad[0]->coords[0]) * 0.5 >
              DCTP_EPS * 100.0)
#endif
#endif
          {
...
#ifdef OSG_UNION_TRI_QUAD
          if ((pcl_face->orig_face[0]->coords[1] - pcl_face->orig_face[3]->coords[1]) * cd_ratio >
              DCTP_EPS * 100.0)
#else
          if ((pcl_face->orig_quad[0]->coords[1] - pcl_face->orig_quad[3]->coords[1]) * cd_ratio >
              DCTP_EPS * 100.0)
#endif
#else
#ifdef OSG_UNION_TRI_QUAD
          if ((pcl_face->orig_face[0]->coords[1] - pcl_face->orig_face[3]->coords[1]) * 0.5 >
              DCTP_EPS * 100.0)
#else
          if ((pcl_face->orig_quad[0]->coords[1] - pcl_face->orig_quad[3]->coords[1]) * 0.5 >
              DCTP_EPS * 100.0)
#endif
#endif
          {
  ...
  /*	if( pclMesh->faces.size( ) >= 1000 )
          {
                  if( fError > m_fErrorCutoff )
                  {
                          std::cerr << "reduce error cutoff: " << m_fErrorCutoff << " -> " << fError
     << std::endl; m_fErrorCutoff = fError;
                  }
          }
          if( fError <= m_fErrorCutoff )
          {
...
/*#ifdef OSG_ARBITRARY_SPLIT
void CErrorQuadTree::CalculatePoints( std::vector< Vec2d > *pvclInsert, std::vector< Vec2d >
*pvclDelete, BSplineTensorSurface *pclPatch, float fError, float &rfMinError, float &rfMaxError )
{
        ...
        if( ( fError >= m_fMaxError ) && ( m_fMaxError >= 0.0 ) )
        {
                ...
                for( ui_cell = 0; ui_cell < ui_cell_cnt; ++ui_cell )
                {
                        ...
                        while( f_act_error > fError )
                        {
                                ...
                                vt_build_cells[ ui_cell_cnt ].clMin = vt_build_cells[ ui_cell
].clMin; vt_build_cells[ ui_cell_cnt ].clMax = vt_build_cells[ ui_cell ].clMax; vt_build_cells[
ui_cell ].bOwnSurface = false; vt_build_cells[ ui_cell ].pclBSplineSurface = NULL; if(
vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue < 0.0 )
                                {
                                        vt_build_cells[ ui_cell ].clMax[0] =
                                                vt_build_cells[ ui_cell ].clMin[0]
                                                - ( vt_build_cells[ ui_cell ].clMax[0] -
vt_build_cells[ ui_cell ].clMin[0] )
                                                * vt_build_cells[ ui_cell
].ptErrorCell->fSplitValue; vt_build_cells[ ui_cell_cnt ].clMin[0] = vt_build_cells[ ui_cell
].clMax[0];
                                }
                                else
                                {
...
vt_build_cells[ ui_cell ].clMin[1] )
                                                * vt_build_cells[ ui_cell
].ptErrorCell->fSplitValue; vt_build_cells[ ui_cell_cnt ].clMin[1] = vt_build_cells[ ui_cell
].clMax[1];
                                }
                                // set new error cells
                                vt_build_cells[ ui_cell_cnt ].ptErrorCell = &( vt_build_cells[
ui_cell ].ptErrorCell->ptChildren[ 1 ] ); vt_build_cells[ ui_cell ].ptErrorCell = &( vt_build_cells[
ui_cell ].ptErrorCell->ptChildren[ 0 ] ); if( ( b_insert ) && ( f_act_error >= f_old_error ) )
                                {
                                        // insert new points
                                        if( scl_check.insert( vt_build_cells[ ui_cell ].clMax
).second )
                                        {
                                                pvclInsert->push_back( vt_build_cells[ ui_cell
].clMax );
                                        }
                                        if( scl_check.insert( vt_build_cells[ ui_cell_cnt ].clMin
).second )
                                        {
                                                pvclInsert->push_back( vt_build_cells[ ui_cell_cnt
].clMin );
                                        }
                                }
                                else
                        ...
                        if( !b_insert )
                        {
                                ...
                                while( f_act_error > f_old_error )
                                {
                                        ...
                                        vt_build_cells[ ui_cell_cnt ].bOwnSurface = false;
                                        vt_build_cells[ ui_cell_cnt ].pclBSplineSurface = NULL;
                                        vt_build_cells[ ui_cell_cnt ].clMin = vt_build_cells[
ui_cell ].clMin; vt_build_cells[ ui_cell_cnt ].clMax = vt_build_cells[ ui_cell ].clMax;
...
vt_build_cells[ ui_cell ].clMin[1] )
                                        ...
                                        // set new error cells
                                        vt_build_cells[ ui_cell_cnt ].ptErrorCell = &(
vt_build_cells[ ui_cell ].ptErrorCell->ptChildren[ 1 ] ); vt_build_cells[ ui_cell ].ptErrorCell = &(
vt_build_cells[ ui_cell ].ptErrorCell->ptChildren[ 0 ] ); if( scl_check.insert( vt_build_cells[
ui_cell ].clMax ).second )
                                        {
                                                pvclDelete->push_back( vt_build_cells[ ui_cell
].clMax );
                                        }
                                        if( scl_check.insert( vt_build_cells[ ui_cell_cnt ].clMin
).second )
                                        {
                                                pvclDelete->push_back( vt_build_cells[ ui_cell_cnt
].clMin );
                                        }
                                        ++ui_cell_cnt;		// one new face
                                        f_act_error = vt_build_cells[ ui_cell ].ptErrorCell->fError;
                ...
                while( f_act_error > fError )
                {
                        if( vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue < 0.0 )
                        {
                                vt_build_cells[ ui_cell ].pclBSplineSurface->subDivisionU(
                                        vcl_surfaces, -vt_build_cells[ ui_cell
].ptErrorCell->fSplitValue );
                        }
                        else
                        {
                        ...
                        *( vt_build_cells[ ui_cell ].pclBSplineSurface ) = vcl_surfaces[ 0 ];
                        std::cerr << vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue <<
std::endl; std::cerr << vt_build_cells[ ui_cell ].clMin << vt_build_cells[ ui_cell ].clMax <<
std::endl; if( vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue < 0.0 )
                        {
                                vt_build_cells[ ui_cell ].clMax[0] =
                                        vt_build_cells[ ui_cell ].clMin[0]
                                        - ( vt_build_cells[ ui_cell ].clMax[0] - vt_build_cells[
ui_cell ].clMin[0] )
                                        * vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue;
                                vt_build_cells[ ui_cell_cnt ].clMin[0] = vt_build_cells[ ui_cell
].clMax[0];
                        }
                        else
                        {
                                vt_build_cells[ ui_cell ].clMax[1] =
                                        vt_build_cells[ ui_cell ].clMin[1]
                                        + ( vt_build_cells[ ui_cell ].clMax[1] - vt_build_cells[
ui_cell ].clMin[1] )
                                        * vt_build_cells[ ui_cell ].ptErrorCell->fSplitValue;
                                vt_build_cells[ ui_cell_cnt ].clMin[1] = vt_build_cells[ ui_cell
].clMax[1];
                        }
                        std::cerr << vt_build_cells[ ui_cell ].clMin << vt_build_cells[ ui_cell
].clMax << std::endl; std::cerr << vt_build_cells[ ui_cell_cnt ].clMin << vt_build_cells[
ui_cell_cnt ].clMax << std::endl << std::endl;
                        // set new error cells
                        if( vt_build_cells[ ui_cell ].ptErrorCell->ptChildren == NULL )
                        {
                                vt_build_cells[ ui_cell ].ptErrorCell->ptChildren = new
SErrorTreeCell[ 2 ]; vt_build_cells[ ui_cell ].ptErrorCell->ptChildren[ 0 ].ptChildren = NULL;
                                vt_build_cells[ ui_cell ].ptErrorCell->ptChildren[ 0 ].fError =
-1.0; vt_build_cells[ ui_cell ].ptErrorCell->ptChildren[ 1 ].ptChildren = NULL; vt_build_cells[
ui_cell ].ptErrorCell->ptChildren[ 1 ].fError = -1.0;
                        }
                        vt_build_cells[ ui_cell_cnt ].ptErrorCell = &( vt_build_cells[ ui_cell
].ptErrorCell->ptChildren[ 1 ] ); vt_build_cells[ ui_cell ].ptErrorCell = &( vt_build_cells[ ui_cell
].ptErrorCell->ptChildren[ 0 ] );
                        // compute error
...
#endif
{
  ...
  pcl_face = pclMesh->AddQuad(
      acl_edge_points[0], acl_edge_points[1], acl_edge_points[2], acl_edge_points[3], 0.0);
  ...
  for (ui_face = 0; ui_face < ui_face_cnt; ++ui_face) {
    ...
    if (pcl_face->faceinfo) {
      /*			std::cerr << "quad:";
                              ...
                              std::cerr << ( ( SFaceTreeCell* ) pcl_face->faceinfo
         )->ptBPCell->uiLeft << " - "; std::cerr << ( ( SFaceTreeCell* ) pcl_face->faceinfo
         )->ptBPCell->uiRight << " , "; std::cerr << ( ( SFaceTreeCell* ) pcl_face->faceinfo
         )->ptBPCell->uiBottom << " - "; std::cerr << ( ( SFaceTreeCell* ) pcl_face->faceinfo
         )->ptBPCell->uiTop << std::endl;*/
#ifdef OSG_UNION_TRI_QUAD
      pclMesh->MoveVertex(pcl_face->orig_face[0],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiLeft],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiTop + 1], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_face[1],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiRight + 1],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiTop + 1], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_face[2],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiRight + 1],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiBottom], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_face[3],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiLeft],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiBottom], 0.0));
#else
      pclMesh->MoveVertex(pcl_face->orig_quad[0],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiLeft],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiTop + 1], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_quad[1],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiRight + 1],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiTop + 1], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_quad[2],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiRight + 1],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiBottom], 0.0));
      pclMesh->MoveVertex(pcl_face->orig_quad[3],
          Vec3d((*cpvdIntervalsU)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiLeft],
              (*cpvdIntervalsV)[((SFaceTreeCell*)pcl_face->faceinfo)->ptBPCell->uiBottom], 0.0));
#endif
      /*			std::cerr << "moved to:";
  ...
  if (pcl_old_node->ptErrorCell) {
    ...
    for (ui_child = 0; ui_child < 4; ++ui_child) {
      ...
      /*			std::cerr << pcl_new_node->ptBPCell->uiLeft << " - ";
                              ...
                              std::cerr << apcl_faces[ ui_child ]->orig_quad[ 3 ]->coords <<
         std::endl;*/
      pcl_new_node->pclBezierSurface = NULL;
      pcl_new_node->bOwnSurface = false;
      apcl_faces[ui_child]->faceinfo = (void*)pcl_new_node;
...
void CErrorQuadTree::SubdivideBuild(DCTPMesh* pclMesh, DCTPFace* pclFace) {
  ...
  if (pcl_old_node->ptErrorCell->fSplitValue < 0.0) {
    pcl_old_node->pclBSplineSurface->subDivisionU(
        vcl_surfaces, -pcl_old_node->ptErrorCell->fSplitValue);
  } else {
    pcl_old_node->pclBSplineSurface->subDivisionV(
        vcl_surfaces, pcl_old_node->ptErrorCell->fSplitValue);
  }
#else
...
#else
    (*pcl_new_node->pclBSplineSurface) =
        vvcl_surfaces[((ui_child + 1) >> 1) & 1][1 - (ui_child >> 1)];
#endif
#else
...
//#endif
{
  ...
  if (m_sbNormalApproximation) {
    ...
    /*
       ...
       ] ) ); cl_nbm0 = ( crvvcl_cps[ cui_m ][ 1 ] - crvvcl_cps[ cui_m ][ 0 ] ).cross( ( crvvcl_cps[
       cui_m - 1 ][ 0 ] - crvvcl_cps[ cui_m ][ 0 ] ) ); cl_nbmn = ( crvvcl_cps[ cui_m ][ cui_n ] -
       crvvcl_cps[ cui_m - 1 ][ cui_n ] ).cross( ( crvvcl_cps[ cui_m ][ cui_n ] - crvvcl_cps[ cui_m
       ][ cui_n - 1 ] ) );
    */
    ...
    if ((cl_nb00.squareLength() < DCTP_EPS * DCTP_EPS) ||
      ...
      d_quadcurv =
          osgMax(osgMax((cl_nb00 - cl_nb0n).squareLength() / (ccl_b00 - ccl_b0n).squareLength(),
      ...
      //			d_quadcurv *= 4.0;	// 0.5 pixel geometric, 1 pixel shading
      //error
    }
  ...
  for (ui_i = 0; ui_i <= cui_m; ++ui_i) {
    ...
    for (ui_j = 0; ui_j <= cui_n; ++ui_j) {
      ...
      if (osgabs(d_quad_size - ptCell->ptErrorCell->fError) < DCTP_EPS) {
        ...
        //				std::cerr << cd_old_x * cd_old_x + cd_old_y * cd_old_y << " " <<
        //cd_new_x * cd_new_x + cd_new_y * cd_new_y << std::endl;
        if (cd_old_x * cd_old_x + cd_old_y * cd_old_y <=
            cd_new_x * cd_new_x + cd_new_y * cd_new_y) {
          cl_worst = cl_uv;
        }
...
#ifdef OSG_DIFF_TO_BILIN
#ifdef OSG_CONSERVATIVE_ERROR
  ptCell->ptErrorCell->fError +=
      (float)sqrt((ccl_b00 - ccl_b0n + ccl_bmn - ccl_bm0).squareLength()) * 0.25;
#endif
#endif
...
//	std::cerr << ptCell->ptErrorCell->fError << std::endl;
  ...
  if ((osgabs(cl_worst[0] - cl_min[0]) <= osgMax(DCTP_EPS, 1e-4 * cl_add[0])) ||
    ...
    if ((osgabs(cl_worst[1] - cl_min[1]) <= osgMax(DCTP_EPS, 1e-4 * cl_add[1])) ||
      ...
      ptCell->ptErrorCell->fSplitValue = (cl_worst[1] - cl_min[1]) / cl_add[1];
             ...
             (osgabs(cl_worst[1] - (cl_min[1] + cl_add[1])) <=
                 osgMax(DCTP_EPS, 1e-4 * cl_add[1]))) {
    //		std::cerr << "x";
    ...
    if ((m_sbNormalApproximation) && (cl_ncij.squareLength() > DCTP_EPS * DCTP_EPS)) {
      ...
      /*   #ifdef OSG_EUCLID_ERRORS
                              ...
                              d_hdiff = sqrt( osgMax( ( cl_cij - cl_bij ).squareLength( ),
                                                      ( cl_ncij - cl_nbij ).squareLength( ) /
         d_quadcurv ) ); #endif*/
      d_hdiff += sqrt((cl_ci0 - cl_cin).squareLength()) * 0.0001;
    } else {
      d_hdiff =
          sqrt((cl_cij - cl_bij).squareLength()) + sqrt((cl_ci0 - cl_cin).squareLength()) * 0.0001;
    }
    ...
    /*		std::cerr.precision( 4 );
                    if( m_sbNormalApproximation )
                    {
                            std::cerr << cl_ci0 << cl_cij << cl_cin << cl_nci0 << cl_ncij << cl_ncin
       << d_hdiff << std::endl;
                    }
                    else
                    {
    ...
    if ((m_sbNormalApproximation) && (cl_ncij.squareLength() > DCTP_EPS * DCTP_EPS)) {
      ...
      /*   #ifdef OSG_EUCLID_ERRORS
                              d_vdiff = sqrt( ( cl_cij - cl_bij ).squareLength( )
                                                              + ( cl_ncij - cl_nbij ).squareLength(
         ) / d_quadcurv ); #else d_vdiff = sqrt( osgMax( ( cl_cij - cl_bij ).squareLength( ), (
         cl_ncij - cl_nbij ).squareLength( ) / d_quadcurv ) ); #endif*/
  ...
  //	const double	cd_hdiff = sqrt( ( cl_cij - cl_ci0 ).squareLength( ) ) + sqrt( ( cl_cij -
  //cl_cin ).squareLength( ) );
  const double cd_hdiff =
      computeDistToLine(cl_cij, cl_ci0, cl_cin) + sqrt((cl_ci0 - cl_cin).squareLength()) * 0.001;
  ...
  //	const double	cd_vdiff = sqrt( ( cl_cij - cl_ci0 ).squareLength( ) ) + sqrt( ( cl_cij -
  //cl_cin ).squareLength( ) );
  const double cd_vdiff =
      computeDistToLine(cl_cij, cl_ci0, cl_cin) + sqrt((cl_ci0 - cl_cin).squareLength()) * 0.001;
  ...
  /*	const double cd_size = 0.5 * sqrt( osgMax( osgMax( osgMax( ( ccl_b00 - ccl_b0n
     ).squareLength( ), ( ccl_b0n - ccl_bmn ).squareLength( ) ), osgMax( ( ccl_bmn - ccl_bm0
     ).squareLength( ), ( ccl_bm0 - ccl_b00 ).squareLength( ) ) ), osgMax( ( ccl_b00 - ccl_bmn
     ).squareLength( ), ( ccl_b0n - ccl_bm0 ).squareLength( ) ) ) );
...
void CErrorQuadTree::ComputeBPError(SBPETreeCell*  pclBPNode,
  ...
  //	std::cerr << ( *cpvdIntervalsU ).size( ) << " x " << ( *cpvdIntervalsV ).size( ) <<
  //std::endl;
  ...
  const Vec3d ccl_s0n =
      pcl_surface->getControlPointMatrix()[0][pcl_surface->getControlPointMatrix()[0].size() - 1];
  const Vec3d ccl_a0n = ccl_s0n - ccl_a00;

  pcl_surface = &((*pvvclPatches)[pclBPNode->uiLeft][pclBPNode->uiTop]);
  const Vec3d ccl_sm0 =
      pcl_surface->getControlPointMatrix()[pcl_surface->getControlPointMatrix().size() - 1][0];
  const Vec3d ccl_am0 = ccl_sm0 - ccl_a00;

  pcl_surface = &((*pvvclPatches)[pclBPNode->uiRight][pclBPNode->uiTop]);
  const Vec3d ccl_smn =
      pcl_surface->getControlPointMatrix()[pcl_surface->getControlPointMatrix().size() - 1]
                                          [pcl_surface->getControlPointMatrix()[0].size() - 1];
  ...
  for (ui_v_surf = pclBPNode->uiBottom; ui_v_surf <= pclBPNode->uiTop; ++ui_v_surf) {
    ...
    const double cd_mul_v =
        ((*cpvdIntervalsV)[ui_v_surf] - (*cpvdIntervalsV)[pclBPNode->uiBottom]) /
        ((*cpvdIntervalsV)[pclBPNode->uiTop + 1] - (*cpvdIntervalsV)[pclBPNode->uiBottom]);
    const double cd_mul_vn =
        ((*cpvdIntervalsV)[ui_v_surf + 1] - (*cpvdIntervalsV)[pclBPNode->uiBottom]) /
        ((*cpvdIntervalsV)[pclBPNode->uiTop + 1] - (*cpvdIntervalsV)[pclBPNode->uiBottom]);
    for (ui_u_surf = pclBPNode->uiLeft; ui_u_surf <= pclBPNode->uiRight; ++ui_u_surf) {
      ...
      const unsigned int                     cui_m      = crvvcl_cps.size() - 1;
      const unsigned int                     cui_n      = crvvcl_cps[0].size() - 1;
      const double                           cd_mul_u =
          ((*cpvdIntervalsU)[ui_u_surf] - (*cpvdIntervalsU)[pclBPNode->uiLeft]) /
          ((*cpvdIntervalsU)[pclBPNode->uiRight + 1] - (*cpvdIntervalsU)[pclBPNode->uiLeft]);
      const double cd_mul_un =
          ((*cpvdIntervalsU)[ui_u_surf + 1] - (*cpvdIntervalsU)[pclBPNode->uiLeft]) /
          ((*cpvdIntervalsU)[pclBPNode->uiRight + 1] - (*cpvdIntervalsU)[pclBPNode->uiLeft]);
      const Vec3d ccl_b00 =
          ccl_a00 + ccl_a0n * cd_mul_v + (ccl_am0 + ccl_amn * cd_mul_v) * cd_mul_u;
      const Vec3d ccl_b0n =
          ccl_a00 + ccl_a0n * cd_mul_vn + (ccl_am0 + ccl_amn * cd_mul_vn) * cd_mul_u;
      const Vec3d ccl_bm0 =
          ccl_a00 + ccl_a0n * cd_mul_v + (ccl_am0 + ccl_amn * cd_mul_v) * cd_mul_un;
      const Vec3d ccl_bmn =
          ccl_a00 + ccl_a0n * cd_mul_vn + (ccl_am0 + ccl_amn * cd_mul_vn) * cd_mul_un;
      const double cd_rn    = 1.0 / cui_n;
      const double cd_rm    = 1.0 / cui_m;
      const Vec3d  ccl_dcx0 = (ccl_bm0 - ccl_b00) * cd_rm;
      const Vec3d  ccl_dcxn = (ccl_bmn - ccl_b0n) * cd_rm;
      ...
      for (ui_i = 0; ui_i <= cui_m; ++ui_i) {
        ...
        for (ui_j = 0; ui_j <= cui_n; ++ui_j) {
          ...
          //					std::cerr << ccl_dcix << "x" << ccl_dcxj <<
          //std::endl;
          /*					cl_norm = ccl_dcix.cross( ccl_dcxj );
                                                  d_quad_size = cl_norm.squareLength( );
                                                  if( d_quad_size > 0.0 )
                                                  {
          //						std::cerr << "qs:" << d_quad_size <<
          std::endl; cl_norm *= 1.0 / sqrt( d_quad_size ); d_quad_size = osgabs( cl_bij.dot( cl_norm
          ) ); if( d_quad_size > pclBPNode->fError )
                                                          {
                                                                  pclBPNode->fError = ( float )
          d_quad_size;
                                                          }
                                                  }
                                                  else*/
          ...
          //					d_quad_size = osgMin( computeDistToTriangle( cl_bij, ccl_a00,
          //ccl_s0n, ccl_sm0 ), 									   computeDistToTriangle( cl_bij, ccl_smn, ccl_s0n, ccl_sm0 ) );
          if (d_quad_size > pclBPNode->fError) {
            pclBPNode->fError = (float)d_quad_size;
          }
          if (cl_uv[0] <= cl_uv[1]) {
          ...
          //					d_quad_size = osgMin( computeDistToTriangle( cl_bij, ccl_s0n,
          //ccl_a00, ccl_smn ), 									   computeDistToTriangle( cl_bij, ccl_sm0, ccl_a00, ccl_smn ) );
          if (d_quad_size > pclBPNode->fError) {
            pclBPNode->fError = (float)d_quad_size;
          }
//					std::cerr << ccl_a00 << ccl_a0n << ccl_am0 << ccl_amn << std::endl
//<< std::endl;
#endif
          cl_cij += ccl_dcix;
          cl_c0j += ccl_dc0x;
...
double CErrorQuadTree::computeDistToPlane(
...
double CErrorQuadTree::computeDistToTriangle(
    const Vec3d cclP, const Vec3d cclV1, const Vec3d cclV2, const Vec3d cclV3) {
  Vec3d       cl_norm;
  double      d_dist;
  Vec3d       cl_diff;
...
double CErrorQuadTree::computeDistToLine(const Vec3d cclP, const Vec3d cclV1, const Vec3d cclV2) {
  ...
  //	std::cerr << "c " << sqrt( ( ccl_d * ( d_temp / d_temp2 ) - ccl_diff ).squareLength( ) ) <<
  //std::endl;
