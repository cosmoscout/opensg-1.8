...
void ParallelComposer::initMethod(void) {
}
...
void ParallelComposer::open(void) {
  ...
  _clusterWindow->getNetwork()->connectAllPointToPoint(clusterId(), "StreamSock");
...
void ParallelComposer::composeViewport(ViewportPtr port) {
#ifdef OSG_WITH_PARALLEL
  // setup viewport
  GLint pl = port->getPixelLeft(), pr = port->getPixelRight(), pb = port->getPixelBottom(),
        pt = port->getPixelTop();
  ...
  glPushMatrix();
  glLoadIdentity();
  glOrtho(0, port->getPixelWidth(), 0, port->getPixelHeight(), -1, 1);
...
void ParallelComposer::changed(BitVector whichField, UInt32 origin) {
  Inherited::changed(whichField, origin);
}

void ParallelComposer::dump(UInt32, const BitVector) const {
...
void ParallelComposer::createCtx(ViewportPtr port) {
  ...
  attribs[i++] = PC_FRAME_WIDTH;
  attribs[i++] = _wWidth;
  attribs[i++] = PC_FRAME_HEIGHT;
  attribs[i++] = _wHeight;
  attribs[i++] = PC_COMPOSITE_TYPE;
  attribs[i++] = PC_COMP_DEPTH;
  attribs[i++] = PC_PIXEL_FORMAT;
  attribs[i++] = PC_PF_BGRA8 | PC_PF_Z32I;
  // maybe this will change?
  attribs[i++] = PC_OUTPUT_DEPTH;
  attribs[i++] = 1;
...
void ParallelComposer::endFrame(UInt32 id) {
  ...
  if (isClient()) {
    /* grab the results */
    err = pcFrameResult(_context, id, 0, 0, _wWidth, _wHeight, reinterpret_cast<void**>(&_bufRet),
        NULL, &_rowPixels);
    if (err) {
...
void ParallelComposer::renderRead(void) {
  ...
  glReadPixels(0, 0, _wWidth, _wHeight, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, _bufDepth);
#endif
}
