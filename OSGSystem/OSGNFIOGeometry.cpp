...
template <class VecType, class GeoPropType>
void NFIOGeometry::writeQuantizedVectors(const GeoPropType& prop, const std::string& fieldName,
    const std::string& fieldType, UInt8 res) {
  ...
  if (noe > 0) {
    ...
    UInt32 size = 0;
    if (res == Quantizer::QRES_8BIT) {
      size = sizeof(UInt8) * d * noe + sizeof(UInt8) + sizeof(Real32) + sizeof(Real32) +
             sizeof(UInt32);
    } else if (res == Quantizer::QRES_16BIT) {
      size = sizeof(UInt16) * d * noe + sizeof(UInt8) + sizeof(Real32) + sizeof(Real32) +
             sizeof(UInt32);
    } else if (res == Quantizer::QRES_24BIT) {
      size = (sizeof(UInt16) + sizeof(UInt8)) * d * noe + sizeof(UInt8) + sizeof(Real32) +
