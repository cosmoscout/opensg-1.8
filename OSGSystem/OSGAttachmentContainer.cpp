...
const BitVector AttachmentContainer::AttachmentsFieldMask =
    (TypeTraits<BitVector>::One << AttachmentContainer::AttachmentsFieldId);

FieldDescription* AttachmentContainer::_desc[] = {new FieldDescription(
    SFAttachmentMap::getClassType(), "attachments", OSG_FC_FIELD_IDM_DESC(AttachmentsField), false,
    (FieldAccessMethod)&AttachmentContainer::getSFAttachments)};

FieldContainerType AttachmentContainer::_type(
    "AttachmentContainer", "FieldContainer", 0, NULL, 0, _desc, sizeof(_desc));
...
void AttachmentContainer::addAttachment(const AttachmentPtr& fieldContainerP, UInt16 binding) {
  UInt32 key;
...
void AttachmentContainer::subAttachment(const AttachmentPtr& fieldContainerP, UInt16 binding) {
  UInt32 key;
...
AttachmentPtr AttachmentContainer::findAttachment(UInt32 groupId, UInt16 binding) {
...
void AttachmentContainer::changed(BitVector whichField, UInt32 origin) {
...
void AttachmentContainer::copyToBin(BinaryDataHandler& pMem, const BitVector& whichField) {
  if (FieldBits::NoField != (AttachmentsFieldMask & whichField)) {
    _attachmentMap.copyToBin(pMem);
  }
}

void AttachmentContainer::copyFromBin(BinaryDataHandler& pMem, const BitVector& whichField) {
  if (FieldBits::NoField != (AttachmentsFieldMask & whichField)) {
...
void AttachmentContainer::dump(UInt32 uiIndent, const BitVector bvFlags) const {
...
AttachmentContainer::AttachmentContainer(const AttachmentContainer& source)
  ...
  AttachmentMap::const_iterator fcEnd = source._attachmentMap.getValue().end();
...
void AttachmentContainer::executeSync(FieldContainer& other, const BitVector& whichField) {
  this->executeSyncImpl((AttachmentContainer*)&other, whichField);
}

void AttachmentContainer::executeSyncImpl(
    AttachmentContainer* pOther, const BitVector& whichField) {
...
#else
void AttachmentContainer::executeSync(
    FieldContainer& other, const BitVector& whichField, const SyncInfo& sInfo) {
  this->executeSyncImpl((AttachmentContainer*)&other, whichField, sInfo);
}

void AttachmentContainer::executeSyncImpl(
    AttachmentContainer* pOther, const BitVector& whichField, const SyncInfo& sInfo) {
