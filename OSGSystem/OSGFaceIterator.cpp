...
void FaceIterator::operator++() {
  ...
  switch (getType()) {
  case GL_TRIANGLES:
    _facePntIndex[0] = _actPrimIndex++;
    _facePntIndex[1] = _actPrimIndex++;
    _facePntIndex[2] = _actPrimIndex++;
    _facePntIndex[3] = -1;
    break;
  case GL_QUAD_STRIP:
    _facePntIndex[0] = _facePntIndex[3];
  ...
  case GL_TRIANGLE_STRIP:
    ...
    if (getPositionIndex(0) == getPositionIndex(1) || getPositionIndex(0) == getPositionIndex(2) ||
  ...
  case GL_QUADS:
    ...
    break;
  default:
    SWARNING << "FaceIterator::++: encountered "
...
void FaceIterator::startPrim(void) {
  ...
  while (!isAtEnd()) {
    ...
    case GL_TRIANGLE_FAN:
      ...
      break;
    case GL_POLYGON:
      switch (PrimitiveIterator::getLength()) {
      case 0:
      case 1:
      case 2:
        break;
      ...
      break;
    case GL_QUADS:
      if (PrimitiveIterator::getLength() >= 4) {
        _facePntIndex[3] = _actPrimIndex++;
        return;
      }
      break;
    case GL_QUAD_STRIP:
      ...
      break;
    default:
      SWARNING << "FaceIterator::startPrim: encountered "
               << "unknown primitive type " << getType() << ", ignoring!" << std::endl;
      break;
    }
