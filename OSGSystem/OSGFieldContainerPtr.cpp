...
void FieldContainerPtrBase::dump(UInt32 uiIndent, const BitVector bvFlags) const {
  if (this != NULL && _storeP != NULL) {
    ...
    indentLog(uiIndent, PLOG);
    PLOG << "Storage  : " << std::hex << (UInt32*)getFirstElemP() << std::endl;
...
std::ostream& OSG::operator<<(std::ostream& os, const FieldContainerPtr& fc) {
  if (fc == NullFC) {
    os << std::hex << "FieldContainerPtr 0x" << &fc << std::dec << ":NullFC";
  } else {
    os << std::hex << "FieldContainerPtr 0x" << &fc << std::dec << ":" << fc->getType().getName()
       << "Ptr(0x" << std::hex << (&(*fc)) << std::dec << ")";
  }
