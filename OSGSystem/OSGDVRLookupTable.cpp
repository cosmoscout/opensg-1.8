...
void DVRLookupTable::changed(BitVector whichField, UInt32 origin) {
    ...
    for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
      for (UInt32 j = 0; j < _mfSize[i]; j++) {
        for (UInt32 k = 0; k < _sfChannel.getValue(); k++) {
          ...
          if (k == 0) {
            _mfDataR[j] = (Real32)_mfData[iter++] / (Real32)TypeTraits<UInt8>::getMax();
          } else if (k == 1) {
            _mfDataG[j] = (Real32)_mfData[iter++] / (Real32)TypeTraits<UInt8>::getMax();
          } else if (k == 2) {
            _mfDataB[j] = (Real32)_mfData[iter++] / (Real32)TypeTraits<UInt8>::getMax();
          } else if (k == 3) {
            _mfDataA[j] = (Real32)_mfData[iter++] / (Real32)TypeTraits<UInt8>::getMax();
          }
        }
      }
    }
  ...
  if (whichField & DataRFieldMask) {
    ...
    for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
      for (UInt32 j = 0; j < _mfSize[i]; j++) {
        ...
        _mfData[iter] = UInt8(Real32(_mfDataR[j]) * Real32(TypeTraits<UInt8>::getMax()));
  ...
  if (whichField & DataGFieldMask) {
    ...
    for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
      for (UInt32 j = 0; j < _mfSize[i]; j++) {
        //!! setValue ( <value>, index )
        _mfData[iter] = UInt8(Real32(_mfDataG[j]) * Real32(TypeTraits<UInt8>::getMax()));
  ...
  if (whichField & DataBFieldMask) {
    ...
    for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
      for (UInt32 j = 0; j < _mfSize[i]; j++) {
        //!! setValue ( <value>, index )
        _mfData[iter] = UInt8(Real32(_mfDataB[j]) * Real32(TypeTraits<UInt8>::getMax()));
  ...
  if (whichField & DataAFieldMask) {
    ...
    for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
      for (UInt32 j = 0; j < _mfSize[i]; j++) {
        //!! setValue ( <value>, index )
        _mfData[iter] = UInt8(Real32(_mfDataA[j]) * Real32(TypeTraits<UInt8>::getMax()));
  ...
  if (whichField & (DimensionFieldMask | SizeFieldMask | ChannelFieldMask)) {
    SINFO << "DVRLookupTable::changed - DimensionFieldMask | "
          << "SizeFieldMask | ChannelFieldMask " << std::endl;
    ...
    if (whichField & SizeFieldMask) {
      SINFO << "DVRLookupTable::changed - size " << _mfSize[0] << std::endl;
    }
...
void DVRLookupTable::dump(UInt32 uiIndent, const BitVector) const {
...
void DVRLookupTable::commonConstructor(void) {
  ...
  for (UInt32 i = 0; i < _sfDimension.getValue(); i++) {
    for (UInt32 j = 0; j < _mfSize[i]; j++) {
      ...
      _mfDataR[singleIter] = (Real32)j / (Real32)TypeTraits<UInt8>::getMax();

      _mfDataG[singleIter] = (Real32)j / (Real32)TypeTraits<UInt8>::getMax();

      _mfDataB[singleIter] = (Real32)j / (Real32)TypeTraits<UInt8>::getMax();

      _mfDataA[singleIter] = (Real32)j / (Real32)TypeTraits<UInt8>::getMax();
