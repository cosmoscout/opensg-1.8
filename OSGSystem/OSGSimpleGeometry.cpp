...
GeometryPtr OSG::makePlaneGeo(Real32 xsize, Real32 ysize, UInt16 hor, UInt16 vert) {
  if (!hor || !vert) {
    SWARNING << "makePlane: illegal parameters hor=" << hor << ", vert=" << vert << std::endl;
    return NullFC;
  }
  ...
  geo->getIndexMapping().push_back(
      Geometry::MapPosition | Geometry::MapNormal | Geometry::MapTexCoords);
  geo->setTypes(types);
  geo->setLengths(lens);
  endEditCP(geo);
...
GeometryPtr OSG::makeConeGeo(
    Real32 height, Real32 botradius, UInt16 sides, bool doSide, bool doBottom) {
  return makeConicalFrustumGeo(height, 0, botradius, sides, doSide, false, doBottom);
}
...
GeometryPtr OSG::makeCylinderGeo(
    Real32 height, Real32 radius, UInt16 sides, bool doSide, bool doTop, bool doBottom) {
  return makeConicalFrustumGeo(height, radius, radius, sides, doSide, doTop, doBottom);
}
...
NodePtr OSG::makeConicalFrustum(Real32 height, Real32 topradius, Real32 botradius, UInt16 sides,
    bool doSide, bool doTop, bool doBottom) {
  GeometryPtr pGeo =
      makeConicalFrustumGeo(height, topradius, botradius, sides, doSide, doTop, doBottom);
...
GeometryPtr OSG::makeConicalFrustumGeo(Real32 height, Real32 topradius, Real32 botradius,
  ...
  if (height <= 0 || topradius < 0 || botradius < 0 || sides < 3) {
    SWARNING << "makeConicalFrustum: illegal parameters height=" << height
             << ", topradius=" << topradius << ", botradius=" << botradius << ", sides=" << sides
             << std::endl;
    return NullFC;
  }
...
GeometryPtr OSG::makeTorusGeo(Real32 innerRadius, Real32 outerRadius, UInt16 sides, UInt16 rings) {
  if (innerRadius <= 0 || outerRadius <= 0 || sides < 3 || rings < 3) {
    SWARNING << "makeTorus: illegal parameters innerRadius=" << innerRadius
             << ", outerRadius=" << outerRadius << ", sides=" << sides << ", rings=" << rings
             << std::endl;
    return NullFC;
  }
  ...
  for (a = 0, theta = 0.0; a <= rings; a++, theta += ringDelta) {
    ...
    for (b = 0, phi = 0; b <= sides; b++, phi += sideDelta) {
      ...
      n->push_back(Vec3f(cosTheta * cosPhi, -sinTheta * cosPhi, sinPhi));
      p->push_back(Pnt3f(cosTheta * dist, -sinTheta * dist, innerRadius * sinPhi));
...
void addPoint(Pnt3f v, UInt32 index, Real32 radius, GeoPositions3f::StoredFieldType* p,
    GeoNormals3f::StoredFieldType* n, GeoTexCoords2f::StoredFieldType* tx) {
...
void subdivideTriangle(UInt32 i1, UInt32 i2, UInt32 i3, Int32 depth,
    GeoPositions3f::StoredFieldType* p, GeoNormals3f::StoredFieldType* n,
    GeoTexCoords2f::StoredFieldType* tx, GeoIndicesUI32::StoredFieldType* i, UInt32& z,
    Real32 radius) {
  if (depth == 0) {
    i->push_back(i1);
    i->push_back(i1);
...
GeometryPtr OSG::makeSphereGeo(UInt16 depth, Real32 radius) {
  ...
  Vec3f v[12] = {Vec3f(-X, 0., Z), Vec3f(X, 0., Z), Vec3f(-X, 0., -Z), Vec3f(X, 0., -Z),
      Vec3f(0., Z, X), Vec3f(0., Z, -X), Vec3f(0., -Z, X), Vec3f(0., -Z, -X), Vec3f(Z, X, 0.),
      Vec3f(-Z, X, 0.), Vec3f(Z, -X, 0.), Vec3f(-Z, -X, 0.)};
  ...
  Int32 tr[20][3] = {{1, 4, 0}, {4, 9, 0}, {4, 5, 9}, {8, 5, 4}, {1, 8, 4}, {1, 10, 8}, {10, 3, 8},
      {8, 3, 5}, {3, 2, 5}, {3, 7, 2}, {3, 10, 7}, {10, 6, 7}, {6, 11, 7}, {6, 0, 11}, {6, 1, 0},
  ...
  z = 12;
  for (j = 0; j < 20; j++) {
    subdivideTriangle(tr[j][0], tr[j][1], tr[j][2], depth, p, n, tx, i, z, radius);
  }
...
GeometryPtr OSG::makeLatLongSphereGeo(UInt16 latres, UInt16 longres, Real32 radius) {
  if (radius <= 0 || latres < 4 || longres < 4) {
    SWARNING << "makeLatLongSphere: illegal parameters "
             << "latres=" << latres << ", longres=" << longres << ", radius=" << radius
             << std::endl;
    return NullFC;
  }
  ...
  for (a = 0, theta = -Pi / 2; a <= latres; a++, theta += latDelta) {
    ...
    for (b = 0, phi = -Pi; b <= longres; b++, phi += longDelta) {
      ...
      n->push_back(Vec3f(cosTheta * sinPhi, sinTheta, cosTheta * cosPhi));
      p->push_back(
          Pnt3f(cosTheta * sinPhi * radius, sinTheta * radius, cosTheta * cosPhi * radius));
      tx->push_back(Vec2f(b / (Real32)longres, a / (Real32)latres));
    }
  }
...
OSG_SYSTEMLIB_DLLMAPPING
GeometryPtr OSG::makeBoxGeo(
    Real32 xsize, Real32 ysize, Real32 zsize, UInt16 hor, UInt16 vert, UInt16 depth) {
  if (!hor || !vert || !depth) {
    SWARNING << "makeBox: illegal parameters hor=" << hor << ", vert=" << vert
  ...
  for (pl = 0; pl < 6; pl++) {
    ...
    for (y = 0; y <= res[inds[pl][1]]; y++) {
      for (x = 0; x <= res[inds[pl][0]]; x++) {
        Pnt3f pnt;
        pnt[inds[pl][0]] = (x * step[inds[pl][0]] - size[inds[pl][0]] / 2) * signs[pl][0];
        pnt[inds[pl][1]] = (y * step[inds[pl][1]] - size[inds[pl][1]] / 2) * signs[pl][1];
        ...
        n->push_back(norm);
        tx->push_back(Vec2f(x / (Real32)res[inds[pl][0]], y / (Real32)res[inds[pl][1]]));
      }
    }
  }
  ...
  for (pl = 0; pl < 6; pl++) {
    ...
    basepoint += UInt32((res[inds[pl][0]] + 1.f) * (res[inds[pl][1]] + 1.f));
  }
