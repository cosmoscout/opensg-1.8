...
class OSG_SYSTEMLIB_DLLMAPPING Text {
 ...
 public:
  ...
  virtual bool fillTXFGeo(Geometry& mesh, bool isNew, std::vector<std::string>& lineVec);

  virtual UInt32 getTXFNVertices(std::vector<std::string>& lineVec);

  virtual bool fillTXFArrays(std::vector<std::string>& lineVec, Pnt3f* points, Vec2f* texcoords);

  virtual bool fillImage(ImagePtr& image, std::vector<std::string>& lineVec, Color4ub* fg,
      Color4ub* bg, bool forcePower2, Real32* maxX, Real32* maxY, ImageCreationMode creationMode,
      MergeMode mergeMode, Int32 pixelDepth, bool bConvertToBitmap) const;

  virtual bool fillGeo(Geometry& mesh, std::vector<std::string>& lineVec, Real32 precision = 1.f,
      Real32 extFac = 0.f, Real32 maxExtend = 0.f, MeshCreationMode creationMode = FILL_MCM,
