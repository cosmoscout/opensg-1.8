...
Vec3d BezierTensorSurface::computewdeCasteljau(Vec2d uv, int& /*error*/) {
  ...
  for (ui_i = 0; ui_i < cui_u; ++ui_i) {
    for (ui_j = 0; ui_j < cui_v; ++ui_j) {
      ...
      //			std::cerr << pd_vx[ ui_j ] << " " << pd_vy[ ui_j ] << " " << pd_vz[
      //ui_j ] << std::endl;
    }
    ...
    pcl_u[ui_i] = pcl_v[0];
    //		std::cerr << pd_ux[ ui_i ] << " " << pd_uy[ ui_i ] << " " << pd_uz[ ui_i ] <<
    //std::endl;
  }
  ...
  /*  Vec3dmatrix::size_type n = control_points.size() - 1;
    ...
    if ( n <= m ) { //less expensive: first by u0 we get a (m+1) element vector
      ...
      for( Vec3dvector::size_type j = 0; j <= m; ++j ) {
        ...
        for( Vec2dvector::size_type k = 0; k <= n; ++k ) row[ k ] = control_points[ k ][ j ];
    //copied jth row BezierCurve3D curve_of_row; curve_of_row.setControlPointVector( row ); Q[ j ] =
    curve_of_row.computewdeCasteljau( uv[0], error ); } //OK - in Q now we have the points on the
    surface at u0 (uv.u ) BezierCurve3D column_at_u0; column_at_u0.setControlPointVector( Q );
