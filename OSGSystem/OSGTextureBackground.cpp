...
/*! \class osg::TextureBackground
  A textured background e.g for distortion corrected fast live video from a camera with known
...
void TextureBackground::updateGrid(void) {
  bool gridChanged = ((getHor() != _hor) || (getVert() != _vert));
...
void TextureBackground::clear(DrawActionBase* action, Viewport* OSG_CHECK_ARG(viewport)) {
  ...
  if (osgabs(getRadialDistortion()) < Eps) {
    if (getTexCoords().getSize() < 4) {
      ...
      glBegin(GL_QUADS);
      glTexCoord2f(getTexCoords()[0].getValues()[0], getTexCoords()[0].getValues()[1]);
      glVertex3f(0.0f, 0.0f, 0.0f);
      glTexCoord2f(getTexCoords()[1].getValues()[0], getTexCoords()[1].getValues()[1]);
      glVertex3f(1.0f, 0.0f, 0.0f);
      glTexCoord2f(getTexCoords()[2].getValues()[0], getTexCoords()[2].getValues()[1]);
      glVertex3f(1.0f, 1.0f, 0.0f);
      glTexCoord2f(getTexCoords()[3].getValues()[0], getTexCoords()[3].getValues()[1]);
      glVertex3f(0.0f, 1.0f, 0.0f);
      glEnd();
    }
  } else // map texture to distortion grid
  {
    ...
    if (_vertexCoordArray.size() == gridCoords && _textureCoordArray.size() == gridCoords &&
...
void TextureBackground::dump(UInt32, const BitVector) const {
