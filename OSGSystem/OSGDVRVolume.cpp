...
DVRVolume::~DVRVolume(void) {
  SINFO << "~DVRVolume this: " << this << std::endl
        << "~DVRVolume appearance: " << getAppearance() << std::endl
        << "~DVRVolume::subRefCP(Appearance)" << std::endl;
  ...
  SINFO << "~DVRVolume::subRefCP(RenderMaterial)" << std::endl
        << "~DVRVolume renderMaterial: " << getRenderMaterial() << std::endl;

  subRefCP(_sfRenderMaterial.getValue());

  SINFO << "~DVRVolume::subRefCP(TextureStorage)" << std::endl
        << "~DVRVolume textureStorage: " << getTextureStorage() << std::endl;
...
void DVRVolume::adjustVolume(Volume& volume) {
  ...
  if (tex != NullFC) {
    ...
    Vec3f minBB(-0.5f * res[0] * slice[0], -0.5f * res[1] * slice[1], -0.5f * res[2] * slice[2]);
...
void DVRVolume::initMethod(void) {
  DrawAction::registerEnterDefault(getClassType(),
      osgTypedMethodFunctor2BaseCPtrRef<Action::ResultE, DVRVolumePtr, CNodePtr, Action*>(
          &DVRVolume::doDraw));
...
void DVRVolume::dump(UInt32 uiIndent, const BitVector) const {
...
Action::ResultE DVRVolume::draw(DrawActionBase* action) {
  ...
  if (getShader() != NullFC) {
    ...
    Brick* first = textureManager.sortBricks(action, modelMat, eyePointWorld.subZero(), this, mode);
    ...
    if (first != NULL) {
      ...
      for (Brick* current = first; current != NULL; prev = current, current = current->getNext()) {
    ...
    // offset for slices
    Real32 offset[3] = {(max[0] - min[0]) / 2 / res[0], (max[1] - min[1]) / 2 / res[1],
        (max[2] - min[2]) / 2 / res[2]};
    // increment for slices
    Real32 inc[3] = {
        (max[0] - min[0]) / res[0], (max[1] - min[1]) / res[1], (max[2] - min[2]) / res[2]};
...
TextureManager::TextureMode DVRVolume::getTextureMode(Window* window) {
  ...
  default: // auto
    mode = window->hasExtension(_extTex3D) ? TextureManager::TM_3D : TextureManager::TM_2D;
  }
