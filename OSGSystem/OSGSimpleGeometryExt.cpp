...
GeometryPtr makeGridGeo(Real32 width, Real32 depth, Real32 hCellWidth, Color3f color) {
  Real32  height, widthHalf, depthHalf;
  Real32  vCellWidth = hCellWidth;
  Real32  w, d;
  Int32   pntCnt, nCols, nRows;
  Real32  x, z;
  Color3f darkerColor = Color3f(color[0] - 0.4, color[1] - 0.4, color[2] - 0.4);
 ...
 */
NodePtr makeGrid(Real32 width, Real32 depth, Real32 hCellWidth, Color3f color) {
...
MaterialChunkPtr getMaterialChunk(
    Color4f ambientColor, Color4f diffuseColor, Color4f specularColor, bool lit) {
...
ChunkMaterialPtr getSelectionMaterial() {
  if (_gayaSelectionMaterial == NullFC) {
    ...
    _gayaSelectionMaterial->addChunk(getMaterialChunk(Color4f(1, 1, 1, 0), Color4f(1, 1, 1, 0)));
  }
