...
StatElemDesc<StatTimeElem> DrawActionBase::statTravTime("travTime", "time for traversal");

StatElemDesc<StatIntElem> DrawActionBase::statCullTestedNodes("cullTestedNodes", "nodes tested");

StatElemDesc<StatIntElem> DrawActionBase::statCulledNodes(
    "culledNodes", "nodes culled from frustum");
...
Action::ResultE DrawActionBase::start(void) {
  if (getFrustumCulling() == true && getAutoFrustum() == true && getCamera() != NULL &&
      getViewport() != NULL) {
  ...
  if (_camera != NULL && getViewport() != NULL) {
    _camera->getViewing(
        _mCameraToWorld, getViewport()->getPixelWidth(), getViewport()->getPixelHeight());
