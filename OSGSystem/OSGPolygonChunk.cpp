...
void PolygonChunk::dump(UInt32 uiIndent, const BitVector bvFlags) const {
...
bool PolygonChunk::operator==(const StateChunk& other) const {
  ...
  if (getCullFace() != tother->getCullFace() || getFrontFace() != tother->getFrontFace() ||
      getSmooth() != tother->getSmooth() || getFrontMode() != tother->getFrontMode() ||
      getBackMode() != tother->getBackMode() || getOffsetPoint() != tother->getOffsetPoint() ||
      getOffsetLine() != tother->getOffsetLine() || getOffsetFill() != tother->getOffsetFill() ||
  ...
  if (getStipple().size() != 0 || tother->getStipple().size() != 0)
    return false;
