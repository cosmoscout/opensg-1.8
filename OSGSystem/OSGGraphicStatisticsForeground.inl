...
OSG_BEGIN_NAMESPACE inline void GraphicStatisticsForeground::processValue(
    Real32& value, const UInt32& elementID) {
  processOnlyValue(value, elementID);
  addValueToHistory(value, elementID);
}

/* */
inline void GraphicStatisticsForeground::processOnlyValue(Real32& value, const UInt32& elementID) {
...
/* */
inline void GraphicStatisticsForeground::addValueToHistory(Real32& value, const UInt32& elementID) {
  UInt32 flags = getFlags()[elementID];
  ...
  if (size > 0) {
    ...
    if ((flags & OSG_SMOOTH) || (flags & OSG_OVERFLOW_RESIZE) || (flags & OSG_UNDERFLOW_RESIZE)) {
