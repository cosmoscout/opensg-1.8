...
//         Copy Constructor
//----------------------------------------------------------------------
HalfEdgeGraph::HalfEdgeGraph(const HalfEdgeGraph& OSG_CHECK_ARG(obj)) {
...
bool HalfEdgeGraph::Triangle::verify(void) {
  ...
  if ((neighbor[0] && ((neighbor[0] == neighbor[1]) || (neighbor[0] == neighbor[2]))) ||
      (neighbor[1] && ((neighbor[1] == neighbor[0]) || (neighbor[1] == neighbor[2]))) ||
      (neighbor[2] && ((neighbor[2] == neighbor[0]) || (neighbor[2] == neighbor[1])))) {
  ...
  if ((halfEdgeVec[0].vertexStart() == halfEdgeVec[1].vertexStart()) ||
    ...
    SINFO << "HalfEdgeGraph::Triangle::verify: Invalid collapsed Triangle" << endl;
    retCode = false;
  }

  if ((halfEdgeVec[0].triangle != this) || (halfEdgeVec[1].triangle != this) ||
  ...
  if ((halfEdgeVec[0].next != &halfEdgeVec[1]) || (halfEdgeVec[1].next != &halfEdgeVec[2]) ||
      (halfEdgeVec[2].next != &halfEdgeVec[0])) {
    SINFO << "HalfEdgeGraph::Triangle::verify: Edge next link error" << endl;
    retCode = false;
  }
...
//
//----------------------------------------------------------------------
void HalfEdgeGraph::reserve(UInt32 vertexNum, UInt32 triangleNum, UInt32 reserveEdges) {
  UInt32 i;
...
bool HalfEdgeGraph::verify(bool verbose) {
  ...
  for (i = 0, triangle = _validTriangleBag.first; triangle; i++, triangle = triangle->next) {
    if (triangle->verify() && (triangle->state >= 0) || (triangle->state <= 3)) {
    ...
    if (verbose) {
      nt0 = triangle->halfEdgeVec[0].twin ? triangle->halfEdgeVec[0].twin->triangle : 0;
      nt1 = triangle->halfEdgeVec[1].twin ? triangle->halfEdgeVec[1].twin->triangle : 0;
      nt2 = triangle->halfEdgeVec[2].twin ? triangle->halfEdgeVec[2].twin->triangle : 0;

      FINFO(("HEG: Triangle %p: %d %d %d, %p %p %p: %s\n", triangle,
          triangle->halfEdgeVec[0].vertexStart(), triangle->halfEdgeVec[1].vertexStart(),
          triangle->halfEdgeVec[2].vertexStart(), nt0, nt1, nt2, (validTri ? "VALID" : "INVALID")));
    }
  }

  SINFO << "HEG: linked triangle count " << _validTriangleBag.countElem() << endl;
  SINFO << "HEG: invalid triangle " << invalidTriangles << endl;
  SINFO << "HEG: nonmanifold split: " << _invalidTriangleBag.countElem() << endl;

  SINFO << "HEG: triangle state: " << triangleState[0] << "/" << triangleState[1] << "/"
        << triangleState[2] << "/" << triangleState[3] << endl;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (verbose) {
      ...
      for (lI = _edgeLinkVec[i].begin(); lI != _edgeLinkVec[i].end(); ++lI) {
        FINFO(("HEG: HalfEdge %p: %d to %d, twin: %p\n", lI->second, lI->second->vertexStart(),
            lI->second->vertexEnd(), lI->second->twin));
      }
    }
  }
  for (connectionI = connectionMap.begin(); connectionI != connectionMap.end(); ++connectionI) {
    SINFO << "HEG: Connection: " << connectionI->first << '/' << connectionI->second << ' '
          << std::endl;
  }
...
//
//----------------------------------------------------------------------
UInt32 HalfEdgeGraph::calcOptPrim(
    UInt32 extIteration, bool doStrip, bool doFan, UInt32 minFanTriangles) {
  ...
  if (doFan) {
    ...
    // find fans
    for (i = 0; i < n; ++i) {
      if ((_edgeLinkVec[i].size() >= minFanTriangles) && (gateEdge = _edgeLinkVec[i][0].second) &&
        ...
        if (halfEdge == gateEdge) {
          ...
          for (halfEdge = gateEdge; !triangle || (halfEdge != gateEdge);
               halfEdge = halfEdge->next->next->twin) {
  ...
  if (doStrip && iteration) {
    ...
    for (iteration--; iteration >= 0; iteration--) {
      ...
      while (doMainLoop) {
        ...
        case START:
          ...
          if (triangle) {
            ...
            gateEdge = 0;
            for (i = 0; i < 3; ++i) {
              if ((twin = triangle->halfEdgeVec[i].twin) && (twin->triangle->state > 0)) {
                if (twin->next->next->twin && (twin->next->next->twin->triangle->state > 0)) {
                  gateEdge = &triangle->halfEdgeVec[i];
                  break;
                } else {
                  if (twin->next->twin && (twin->next->twin->triangle->state > 0)) {
                    gateEdge = &triangle->halfEdgeVec[i];
                  } else {
        ...
        case LEFT:
          ...
          if (triangle->state == DEGREE_0) {
            ...
            if ((twin = gateEdge->next->next->twin) && (twin->triangle->state > 0)) {
        ...
        case RIGHT:
          ...
          if (triangle->state == DEGREE_0) {
            ...
            if ((twin = gateEdge->next->twin) && (twin->triangle->state > 0)) {
              gateEdge = gateEdge->next;
              ++stripCost;
              walkCase = LEFT;
            } else {
        ...
        case FINISH:
          ...
          if (checkRevOrder && (revCost = calcStripCost(*fList, true)) && (revCost < stripCost)) {
      ...
      if (sample) {
        ...
        SINFO << " cost/best/worst: " << sampleCost << '/' << bestCost << '/' << worstCost << endl;
      }
  ...
  if (sample) {
    SWARNING << "range: " << bestCost << '/' << worstCost << ' '
             << float(100 * (worstCost - bestCost)) / float(bestCost) << '%' << endl;
  }
...
//
//----------------------------------------------------------------------
Int32 HalfEdgeGraph::calcStripCost(TriangleList& strip, bool rev) {
  ...
  if (triangle) {
    ...
    if ((nextTriangle = rev ? triangle->prev : triangle->next)) {
      ...
      ++cost;
      walkCase = LEFT;
      for (triangle = nextTriangle; (nextTriangle = (rev ? triangle->prev : triangle->next));
...
//
//----------------------------------------------------------------------
Int32 HalfEdgeGraph::fillIndexFromStrip(
    std::vector<HalfEdgeGraph::IndexT>& indexVec, TriangleList& strip, bool rev) {
  Triangle *            triangle = rev ? strip.last : strip.first, *nextTriangle;
  HalfEdge *            firstEdge, *halfEdge, *gate;
  WalkCase              walkCase;
  HalfEdgeGraph::IndexT vertex;
  Int32                 cost = 0;
...
//
//----------------------------------------------------------------------
Int32 HalfEdgeGraph::fillIndexFromFan(
    std::vector<HalfEdgeGraph::IndexT>& indexVec, HalfEdge& firstEdge) {
  Int32     count = 0;
  ...
  if (halfEdge) {
    ...
    for (gateEdge = halfEdge->next->next->twin; gateEdge != halfEdge;
...
//
//----------------------------------------------------------------------
Int32 HalfEdgeGraph::getPrimitive(vector<HalfEdgeGraph::IndexT>& indexVec, Int32 type) {
  UInt32                  i = 0, n = 0;
  Triangle*               triangle;
  ...
  // fan
  if (!bag && (n = _fanBag.size()) && ((type == GL_TRIANGLE_FAN) || !type)) {
    i   = n - 1;
    bag = &_fanBag;
    fillIndexFromFan(indexVec, *_edgeLinkVec[_fanBag[i].first][0].second);
    type = GL_TRIANGLE_FAN;
  }

  // strip
  if (!bag && (n = _stripBag.size()) && ((type == GL_TRIANGLE_STRIP) || !type)) {
    i   = n - 1;
    bag = &_stripBag;
    fillIndexFromStrip(indexVec, *_stripBag[i].second, _stripBag[i].first ? true : false);
    type = GL_TRIANGLE_STRIP;
  }

  // tri
  if (!bag && (n = _triBag.size()) && ((type == GL_TRIANGLES) || !type)) {
    bag = &_triBag;
    if (_triBag[0].second->empty() == false) {
      n = _triBag[0].second->countElem() * 3;
      indexVec.resize(n);
      i = 0;
      for (triangle = _triBag[0].second->first; triangle; triangle = triangle->next) {
...
//
//----------------------------------------------------------------------
Int32 HalfEdgeGraph::calcEgdeLines(vector<HalfEdgeGraph::IndexT>& indexVec, bool codeBorder) {
  ...
  for (i = 0; i < nN; ++i) {
    ...
    for (j = 0; j < nE; ++j) {
      ...
      if ((isBorder = (halfEdge->twin == 0)) || (startVertexIndex < endVertexIndex)) {
