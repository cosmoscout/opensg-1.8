...
TileGeometryLoad::TileGeometryLoad(UInt32 nodeId, bool useFaceDistribution)
    : _nodeId(nodeId)
    , _faces(0)
  ...
  if (_directions.size() == 0) {
    ...
    _directions.push_back(Plane(Vec3f(1, 0, 0), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(0, 1, 0), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(0, 0, 1), Pnt3f(0, 0, 0)));

    _directions.push_back(Plane(Vec3f(1, 1, 1) * (1 / osgsqrt(3.0f)), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(-1, 1, 1) * (1 / osgsqrt(3.0f)), Pnt3f(1, 0, 0)));
    _directions.push_back(Plane(Vec3f(1, -1, 1) * (1 / osgsqrt(3.0f)), Pnt3f(0, 1, 0)));
    _directions.push_back(Plane(Vec3f(1, 1, -1) * (1 / osgsqrt(3.0f)), Pnt3f(0, 0, 1)));

    _directions.push_back(Plane(Vec3f(1, 1, 0) * (1 / osgsqrt(2.0f)), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(1, -1, 0) * (1 / osgsqrt(2.0f)), Pnt3f(0, 1, 0)));
    _directions.push_back(Plane(Vec3f(0, 1, 1) * (1 / osgsqrt(2.0f)), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(0, -1, 1) * (1 / osgsqrt(2.0f)), Pnt3f(0, 1, 0)));
    _directions.push_back(Plane(Vec3f(1, 0, 1) * (1 / osgsqrt(2.0f)), Pnt3f(0, 0, 0)));
    _directions.push_back(Plane(Vec3f(-1, 0, 1) * (1 / osgsqrt(2.0f)), Pnt3f(1, 0, 0)));
  }
  updateGeometry();
}
...
void TileGeometryLoad::updateView(
    Matrix& viewing, Matrix& projection, Real32 rNear, UInt32 width, UInt32 height) {
  Vec3f   vol[2];
  Pnt3f   pnt;
  ...
  */
  ...
  for (int i = 0; i < 8; ++i) {
    p->multFullMatrixPnt(Pnt3f(vol[(i)&1][0], vol[(i >> 1) & 1][1], vol[(i >> 2) & 1][2]), pnt);
    if (i > 0) {
      if (minx > pnt[0])
        minx = pnt[0];
      if (miny > pnt[1])
  ...
  if (maxx < -1 || maxy < -1 || minx > 1 || miny > 1) {
    ...
    _areaSize = (Real32)(_max[0] - _min[0] + 1) * (Real32)(_max[1] - _min[1] + 1);
    /* Don't clip!
  ...
  if (_useFaceDistribution) {
    ...
    for (UInt32 d = 0; d < _directions.size(); ++d) {
      ...
      for (f = geo->beginTriangles(); f != geo->endTriangles(); ++f) {
        ...
        _faceDistribution[d * 2][(int)(ceil(min * (FACE_DISTRIBUTION_SAMPLING_COUNT - 1)))]++;
        _faceDistribution[d * 2 + 1]
                         [(int)(ceil((1 - max) * (FACE_DISTRIBUTION_SAMPLING_COUNT - 1)))]++;
      }
    }
    for (UInt32 d = 0; d < _directions.size(); ++d) {
...
void TileGeometryLoad::dump(void) {
  if (_visible) {
    SLOG << "Min/Max     :" << _min[0] << " " << _min[1] << " / " << _max[0] << " " << _max[1]
         << std::endl;
  } else {
 ...
 **/
Real32 TileGeometryLoad::getVisibleFraction(
  ...
  if (viswmin[0] == _min[0] && viswmin[1] == _min[1] && viswmax[0] == _max[0] &&
      viswmax[1] == _max[1]) {
    return 1;
  }
  ...
  if (_useFaceDistribution) {
    ...
    return (getFaceDistribution(_faceDistDirX ^ 1, 1.0f - (viswmin[0] - _min[0]) * x) +
               getFaceDistribution(_faceDistDirX, (viswmax[0] - _min[0] + 1) * x) - 1) *
           (getFaceDistribution(_faceDistDirY ^ 1, 1.0f - (viswmin[1] - _min[1]) * y) +
               getFaceDistribution(_faceDistDirY, (viswmax[1] - _min[1] + 1) * y) - 1);
  } else {
    return ((Real32)(viswmax[0] - viswmin[0] + 1) * (Real32)(viswmax[1] - viswmin[1] + 1)) /
           _areaSize;
  }
}
 ...
 **/
bool TileGeometryLoad::getVisibleArea(
    const Int32 wmin[2], const Int32 wmax[2], Int32 viswmin[2], Int32 viswmax[2]) {
  ...
  // not in region
  if (viswmin[0] > viswmax[0] || viswmin[1] > viswmax[1])
    return false;
  else
    return true;
}
...
bool TileGeometryLoad::checkRegion(Int32 min[2], Int32 max[2]) {
  if (min[0] > _max[0] || max[0] < _min[0] || min[1] > _max[1] || max[1] < _min[1])
    return false;
  else
    return true;
}
