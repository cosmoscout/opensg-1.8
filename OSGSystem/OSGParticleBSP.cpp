...
void ParticleBSPTree::dump(
    UInt32 OSG_CHECK_ARG(uiIndent), const BitVector OSG_CHECK_ARG(bvFlags)) const {
  PLOG << "ParticleBSPTree(";

  if (!_tree.empty()) {
    for (std::vector<ParticleBSPNode>::const_iterator i = _tree.begin() + 1; i != _tree.end();
         ++i) {
      i->dump();
    }
  }
  ...
  if (!_tree.empty()) {
    for (std::vector<ParticleBSPNode>::const_iterator i = _tree.begin() + 1; i != _tree.end();
      ...
      if (i->isLeaf()) {
        ...
        outVal.append(TypeTraits<Real32>::putToString(i->getSplitValue()));
      }
      outVal.append(";");
    }
  }
...
Int32* ParticleBSPTree::traverse(const Pnt3f& refPoint, UInt32& length, Int32* order) const {
  if (_tree.empty())
    return NULL;
...
UInt32 ParticleBSPTree::doTraverse(
    const Pnt3f& refPoint, UInt32 index, UInt32 length, Int32* order) const {
...
Int32* ParticleBSPTree::traverse(const Vec3f& refVec, UInt32& length, Int32* order) const {
  if (order == NULL) {
    order = new Int32[length];
  }
...
UInt32 ParticleBSPTree::doTraverse(
    const Vec3f& refVec, UInt32 index, UInt32 length, Int32* order) const {
  const ParticleBSPNode* n = &_tree[index];
...
UInt32 ParticleBSPTree::doBuild(std::vector<Int32>::iterator begin,
    std::vector<Int32>::iterator end, UInt32 nodeindex, GeoPositionsPtr pos) {
  // reached a leaf?
  ...
  return osgMax(doBuild(begin, mid, nodeindex * 2, pos), doBuild(mid, end, nodeindex * 2 + 1, pos));
}
...
DataType FieldDataTraits<ParticleBSPTree>::_type("ParticleBSPTree", "None");
