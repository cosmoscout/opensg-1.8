...
struct OSG_SYSTEMLIB_DLLMAPPING SScanLineEventLess {
  bool operator()(const SScanLineEvent* ptEvent1, const SScanLineEvent* ptEvent2) const {
    ...
    if ((cui_e1 == cui_e2) && (ptEvent1->bStart == ptEvent2->bStart)) {
      ...
      //			std::cerr << ptEvent1->ptEdge << " <-> " << ptEvent2->ptEdge <<
      //std::endl; 			std::cerr << ptEvent1->bStart << " <-> " << ptEvent2->bStart << std::endl;
      return false;
    }
...
/* #else
        bool StateTransition2( Vec2d &rclAct, Vec2d clNext, std::vector< DCTPVertex* > &el, Vec3d
 &rclActS, Vec3d clNextS, Vec3d &rclActN, Vec3d clNextN ); #endif*/
#else
  bool StateTransition2(
      Vec2d& rclAct, Vec2d clNext, std::vector<DCTPVertex*>& el, Vec3d& rclActS, Vec3d clNextS);
#endif
  ...
  bool      insertScanLineEvents(SScanLineEdge* ptEdge, ScanLineEventSet& rsptEvents,
           char cWhich = -1); // ucWhich = -1 for both
 ...
 public:
  ParSpaceTrimmer() { /*std::cerr << "pst constr" << std::endl;*/
    m_bDeleteVertexInfo = false;
  }
  ~ParSpaceTrimmer() { /*std::cerr << "pst destr" << std::endl;*/
    if (m_bDeleteVertexInfo)
      deleteVertexInfo();
  }

  void Initialize(DCTPMesh& m, bezier2ddequevector& c2d, bezier3ddequevector& c3d,
      std::vector<std::vector<Vec2d>>& vccrd, double err
#ifdef OSG_USE_SIMPLIFIER
      ,
  ...
  void Initialize(DCTPMesh& m, bezier2ddequevector& tc, std::vector<std::vector<Vec2d>>& vccrd
    ...
    for (dctpvertexset::iterator it_vertex = mesh->vertices.begin();
         it_vertex != mesh->vertices.end(); ++it_vertex) {
...
#ifdef OSG_FORCE_NO_T_VERTICES
  // #ifdef OSG_CREATE_NORMAL_MAPS
  void Initialize2(DCTPMesh& m, std::vector<std::vector<Vec2d>>& vccrd,
      std::vector<std::vector<Vec3d>>& vvclSewed, std::vector<bool>& rvbRev,
      std::vector<bool>& rvbUsed)
/* #else
        void Initialize2( DCTPMesh &m, std::vector< std::vector< Vec2d > > &vccrd, std::vector<
 std::vector< Vec3d > > &vvclSewed, std::vector< std::vector< Vec3d > > &vvclNormals, std::vector<
 bool > &rvbRev, std::vector< bool > &rvbUsed ) #endif*/
#else
  void Initialize2(DCTPMesh& m, std::vector<std::vector<Vec2d>>& vccrd,
      std::vector<std::vector<Vec3d>>& vvclSewed, std::vector<bool>& rvbRev,
      std::vector<bool>& rvbUsed)
...
#ifdef OSG_FORCE_NO_T_VERTICES
#ifdef OSG_KEEP_2D_POINTS
  int buildSurfaceGraph(DirectedGraph<Vec2d, unsigned char>* pGraph,
      std::vector<Vec3d>* pvclSewed = NULL, std::vector<Vec3d>* pvclNormals = NULL,
      std::vector<unsigned int>* pvclIdx = NULL);
#else
  int buildSurfaceGraph(DirectedGraph<Vec2d, unsigned char>* pGraph,
      std::vector<Vec3d>* pvclSewed = NULL, std::vector<Vec3d>* pvclNormals = NULL);
#endif
#else
  int  buildSurfaceGraph(
       DirectedGraph<Vec2d, unsigned char>* pGraph, std::vector<Vec3d>* pvclSewed = NULL);
