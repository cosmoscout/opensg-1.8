...
HDRImageFileType HDRImageFileType::_the(
    "image/x-hdr", suffixArray, sizeof(suffixArray), OSG_READ_SUPPORTED | OSG_WRITE_SUPPORTED);
...
*/
UInt64 HDRImageFileType::restoreData(
...
*/
UInt64 HDRImageFileType::storeData(
...
*/
HDRImageFileType::HDRImageFileType(
...
int HDRImageFileType::fwritecolrs(
    FILE* file, Real32* scan, RGBE* rgbe_scan, int width, int height) {
  // convert scanline
...
int HDRImageFileType::fwritecolrs(
    FILE* file, Real16* scan, RGBE* rgbe_scan, int width, int height) {
  // convert scanline
  for (unsigned int i = 0; i < width; ++i) {
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < width; j += cnt) { // find next run
      for (beg = j; beg < width; beg += cnt) {
        for (cnt = 1;
             (cnt < 127) && ((beg + cnt) < width) && (rgbe_scan[beg + cnt][i] == rgbe_scan[beg][i]);
             cnt++)
          ;
        if (cnt >= MINRUN)
          break; // long enough
...
int HDRImageFileType::fwritecolrs(
    std::ostream& os, Real32* scan, RGBE* rgbe_scan, int width, int height) {
  // convert scanline
  for (unsigned int i = 0; i < width; i++) {
    float2RGBE(scan, rgbe_scan[i]);
    scan += 3;
...
int HDRImageFileType::fwritecolrs(
    std::ostream& os, Real16* scan, RGBE* rgbe_scan, int width, int height) {
  // convert scanline
  for (unsigned int i = 0; i < width; i++) {
    half2RGBE(scan, rgbe_scan[i]);
    scan += 3;
