 ...
 *  \return zero on success, and a negative value when some error occured.
 */
int BezierCurve3D::approximate(
    std::vector<double>& vertices, double delta, unsigned char strategy) {
  vertices.resize(0);
...
int BezierCurve3D::approximate_sub(
...
bool BezierCurve3D::reduceDegree(double tol) {
  ...
  for (i = 1; i < n; ++i) {
    b_right[i] = (control_points[i] * n - b_right[i - 1] * i) * (1.0 / (Real64)(n - i));
  }

  // calculate b_left:
  b_left[n - 1] = control_points[n];
  for (i = n - 1; i > 0; --i) {
    b_left[i - 1] = (control_points[i] * n - b_left[i] * (n - i)) * (1.0 / (Real64)i);
  }
