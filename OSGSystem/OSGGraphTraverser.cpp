...
/***********************************************************/
void GraphTraverser::Initialize(DirectedGraph<Vec2d, unsigned char>& gg, bool usedelaunaytri) {
  g               = &gg;
  ...
  */
  ...
  if (err) {
    ...
    for (i = 0; i < k; ++i) {
      ...
      if (!siv.second) {
        std::cerr << "duplicate vertex passed: " << globalverts[nodes[i]] << " i: " << i
                  << std::endl;
        ;
      }
...
int GraphTraverser::getOtherEnd(int eid, int nid) {
  ...
  if (e.to == nid) {
    //			std::cerr << "following edge reversed " << eid << " -> " << e.from <<
    //std::endl;
    return e.from;
  } else {
...
int GraphTraverser::getOutGoingEdge(int nid) {
  ...
  for (DCTPivector::iterator i = g->nodes[nid].edges.begin(); i != ee; ++i)
    if (g->edges[*i].direction && g->edges[*i].from == nid)
      return *i;
  return -1;
}
...
int GraphTraverser::Traverse(void) {
  ...
  while (edgeID != -1) {
    ...
    while (edgeID != -1) {
      ...
      if (edgeID == -1)
        break;
      //			std::cerr << "edgeID: " << edgeID << " g->edges[edgeID]: " <<
      //(void*) &g->edges[ edgeID ] << std::endl; 			std::cerr << "g->edges[ edgeID ].direction: " <<
      //g->edges[ edgeID ].direction << std::endl;
      ...
      if (start_node == act_node) { // a polygon finished
        ...
        b_convex = true;
        node_ids.clear();
        //				std::cerr << " act_node: " << act_node << " start_node: " <<
        //start_node << " leave: " << leave << std::endl;
        start_node = leave;
        leave      = -1;
        if (start_node != -1) {
          node_ids.push_back(start_node);
...
int GraphTraverser::getNextEdge(const int nodeid, const int previd /*const double& in_angle*/) {
  ...
  if (en == 1) { // most common case, along trimming curves over faces
    ...
    /*                if ( g->edges[ e0 ].from == nodeid &&
                    ...
                    {
    //					std::cerr << "tracing along graph " << nodeid << " -> " << e0 <<
    std::endl; return e0;
                                    }*/
    if ((g->edges[e0].from == nodeid) || (g->edges[e0].to == nodeid)) {
      return e0;
    }
    std::cerr << "Error " << nodeid << " <-> " << g->edges[e0].from << " -> " << g->edges[e0].to
              << std::endl;
  ...
  for (DCTPivector::iterator iv = g->nodes[nodeid].edges.begin(); iv != ee; ++iv) {
  ...
  if (/*eid.size() != ea.size() ||*/ eid.size() == 0) {
    //          std::cerr << "eid.size(): " << eid.size() << " ea.size(): " << ea.size() <<
    //          std::endl; throw GraphTraverserError( ERR_ZERO_SIZE );
    return -1;
  }
  // END DEBUG
  ...
  for (unsigned int i = 1; i < eae; ++i) {
    ...
    if ((orient < 0.0) || (tmp > 0.0)) {
      if (((tmp > 0.0) && (orient <= 0.0)) || ((tmp == 0.0) && (orient < 0.0)) ||
          (orient2d(b, c, d) > 0.0)) {
  ...
  //); 		std::cerr << "leftmost: " << leftmost << "eae: " << eae << std::endl; 		std::cerr << "going
  //left on graph " << nodeid << " -> " << eid[ leftmost ] << std::endl;
  return eid[leftmost];
}
