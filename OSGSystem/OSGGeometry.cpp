...
void Geometry::initMethod(void) {
  ...
  IntersectAction::registerEnterDefault(getClassType(),
      osgTypedMethodFunctor2BaseCPtrRef<Action::ResultE, GeometryPtr, CNodePtr, Action*>(
          &Geometry::intersect));

  IntersectActor::regClassEnter(
      osgTypedMethodFunctor2BaseCPtr<NewActionTypes::ResultE, GeometryPtr, NodeCorePtr,
...
void Geometry::onCreate(const Geometry*) {
  ...
  if (Thread::getAspect() != _sfIgnoreGLForAspect.getValue()) {
    ...
    setGLId(
        Window::registerGLObject(osgTypedMethodVoidFunctor2ObjCPtrPtr<GeometryPtr, Window, UInt32>(
                                     tmpPtr, &Geometry::handleGL),
            2));
...
void Geometry::onDestroy(void) {
  ...
  if (_sfSecondaryColors.getValue() != NullFC) {
    ...
    endEditCP(_sfSecondaryColors.getValue(), Attachment::ParentsFieldMask);
...
void Geometry::handleGL(Window* win, UInt32 idstatus) {
  ...
  if (mode == Window::initialize || mode == Window::needrefresh || mode == Window::reinitialize) {
    ...
    if (!vbo_supported) {
      ...
      GeoPumpFactory::GeoPump p = GeoPumpFactory::the()->getGeoPump(win, ind);
      ...
      if (p)
        ...
        SWARNING << "Geometry::handleGL: no Pump found for geometry " << this << std::endl;
      }
      ...
      if (glGetError() == GL_OUT_OF_MEMORY) {
        SFATAL
            << "Geometry::handleGL: couldn't create display list (out of memory) skipping geometry "
            << this << std::endl;
    ...
    SWARNING << "Geometry(" << this << "::handleGL: Illegal mode: " << mode << " for id " << id
             << std::endl;
  }
}
...
void Geometry::dump(UInt32 uiIndent, const BitVector bvFlags) const {
  UInt32 i;
  ...
  PLOG << "GeoCore"
       << "(" << thisP.getFieldContainerId() << ") : " << getType().getName() << " "
...
Action::ResultE Geometry::drawPrimitives(DrawActionBase* action) {
  ...
  if (!vbo_supported) {
    if (getDlistCache() == true) {
      ...
      GeoPumpFactory::Index   ind = GeoPumpFactory::the()->getIndex(this);
      GeoPumpFactory::GeoPump p   = GeoPumpFactory::the()->getGeoPump(action->getWindow(), ind);
      ...
      if (p)
        ...
        SWARNING << "draw: no Pump found for geometry " << this << std::endl;
      }
    }
  }
...
Action::ResultE Geometry::intersect(Action* action) {
  ...
  for (; it != end; ++it) {
    if (ia->getLine().intersect(
            it.getPosition(0), it.getPosition(1), it.getPosition(2), t, &norm)) {
...
NewActionTypes::ResultE Geometry::intersectActor(ActorBase::FunctorArgumentType& funcArg) {
  ...
  for (; itTris != endTris; ++itTris) {
    if (pIA->getRay().intersect(itTris.getPosition(0), itTris.getPosition(1), itTris.getPosition(2),
            hitDist, &hitNormal) == true) {
      pIA->setHit(hitDist * scaleFactor, funcArg.getNode(), itTris.getIndex(), hitNormal);
    }
  }
...
*/
void Geometry::changed(BitVector whichField, UInt32 origin) {
  if (whichField & TypesFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        GeometryPtr thisP = getPtr();

        if (_sfTypes.getValue() != NullFC && _sfTypes.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & LengthsFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfLengths.getValue() != NullFC && _sfLengths.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & TexCoordsFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfTexCoords.getValue() != NullFC && _sfTexCoords.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & IndicesFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfIndices.getValue() != NullFC && _sfIndices.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & (IndicesFieldMask | LengthsFieldMask | TypesFieldMask | IndexMappingFieldMask)) {
    ...
    if (getDlistCache() || getVbo()) {
      if (getGLId() == 0) {
        ...
        setGLId(Window::registerGLObject(
            osgTypedMethodVoidFunctor2ObjCPtrPtr<GeometryPtr, Window, UInt32>(
                tmpPtr, &Geometry::handleGL),
            2));
...
bool Geometry::CompareMaterials(MaterialPtr m1, MaterialPtr m2) {
  ...
  if (statechunks1.size() != statechunks2.size())
    return false;
...
Int16 Geometry::MergeIndex(const GeometryPtr other) {
  ...
  if (!CompareMaterials(getMaterial(), other->getMaterial()))
    return -1;

  // compare the existing attributes
  if (((getNormals() != NullFC) ^ (other->getNormals() != NullFC)) ||
      ((getColors() != NullFC) ^ (other->getColors() != NullFC)) ||
      ((getSecondaryColors() != NullFC) ^ (other->getSecondaryColors() != NullFC)) ||
      ((getTexCoords() != NullFC) ^ (other->getTexCoords() != NullFC)) ||
      ((getTexCoords1() != NullFC) ^ (other->getTexCoords1() != NullFC)) ||
      ((getTexCoords2() != NullFC) ^ (other->getTexCoords2() != NullFC)) ||
      ((getTexCoords3() != NullFC) ^ (other->getTexCoords3() != NullFC)) ||
      ((getTexCoords4() != NullFC) ^ (other->getTexCoords4() != NullFC)) ||
      ((getTexCoords5() != NullFC) ^ (other->getTexCoords5() != NullFC)) ||
      ((getTexCoords6() != NullFC) ^ (other->getTexCoords6() != NullFC)) ||
      ((getTexCoords7() != NullFC) ^ (other->getTexCoords7() != NullFC)))
    return -1;
...
#define copyAllAttrib                                                                              \
  {                                                                                                \
    copyAttrib(pos, Positions2f, Positions) copyAttrib(pos, Positions3f, Positions) copyAttrib(    \
        pos, Positions4f, Positions) copyAttrib(pos, Positions2d, Positions) copyAttrib(pos,       \
        Positions3d, Positions) copyAttrib(pos, Positions4d, Positions) copyAttrib(pos,            \
        Positions2s, Positions) copyAttrib(pos, Positions3s, Positions) copyAttrib(pos,            \
        Positions4s, Positions) copyAttrib(type, PTypesUI8, Types) copyAttrib(length, PLengthsUI8, \
        Lengths) copyAttrib(length, PLengthsUI16, Lengths) copyAttrib(length, PLengthsUI32,        \
        Lengths) copyAttrib(normal, Normals3f, Normals) copyAttrib(normal, Normals3s,              \
        Normals) copyAttrib(normal, Normals3b, Normals) copyAttrib(color, Colors3f,                \
        Colors) copyAttrib(color, Colors4f, Colors) copyAttrib(color, Colors3ub,                   \
        Colors) copyAttrib(color, Colors4ub, Colors) copyAttrib(seccolor, Colors3f,                \
        SecondaryColors) copyAttrib(seccolor, Colors4f, SecondaryColors) copyAttrib(seccolor,      \
        Colors3ub, SecondaryColors) copyAttrib(seccolor, Colors4ub,                                \
        SecondaryColors) copyAttrib(texcoord, TexCoords1f, TexCoords) copyAttrib(texcoord,         \
        TexCoords2f, TexCoords) copyAttrib(texcoord, TexCoords3f, TexCoords) copyAttrib(texcoord,  \
        TexCoords4f, TexCoords) copyAttrib(texcoord1, TexCoords1f, TexCoords1)                     \
        copyAttrib(texcoord1, TexCoords2f, TexCoords1) copyAttrib(texcoord1, TexCoords3f,          \
            TexCoords1) copyAttrib(texcoord1, TexCoords4f, TexCoords1) copyAttrib(texcoord2,       \
            TexCoords1f, TexCoords2) copyAttrib(texcoord2, TexCoords2f, TexCoords2)                \
            copyAttrib(texcoord2, TexCoords3f, TexCoords2) copyAttrib(texcoord2, TexCoords4f,      \
                TexCoords2) copyAttrib(texcoord3, TexCoords1f, TexCoords3) copyAttrib(texcoord3,   \
                TexCoords2f, TexCoords3) copyAttrib(texcoord3, TexCoords3f, TexCoords3)            \
                copyAttrib(texcoord3, TexCoords4f, TexCoords3) copyAttrib(texcoord4, TexCoords1f,  \
                    TexCoords4) copyAttrib(texcoord4, TexCoords2f, TexCoords4)                     \
                    copyAttrib(texcoord4, TexCoords3f, TexCoords4) copyAttrib(texcoord4,           \
                        TexCoords4f, TexCoords4) copyAttrib(texcoord5, TexCoords1f, TexCoords5)    \
                        copyAttrib(texcoord5, TexCoords2f, TexCoords5) copyAttrib(                 \
                            texcoord5, TexCoords3f, TexCoords5) copyAttrib(texcoord5, TexCoords4f, \
                            TexCoords5) copyAttrib(texcoord6, TexCoords1f, TexCoords6)             \
...
void Geometry::merge0(const GeometryPtr other) {
  UInt32 posBase = 0, typeBase = 0, lengthBase = 0, normalBase = 0, colorBase = 0, seccolorBase = 0,
         texcoordBase = 0, texcoord1Base = 0, texcoord2Base = 0, texcoord3Base = 0,
         texcoord4Base = 0, texcoord5Base = 0, texcoord6Base = 0, texcoord7Base = 0;
  ...
  if (getIndices() != NullFC) {
    ...
    } else // multi-index
    {
      ...
      for (i = 0, j = 0; i < oind->getSize(); i++, j = (j + 1) % nmap) {
...
void Geometry::merge1(const GeometryPtr other) {
  UInt32 posBase, typeBase, lengthBase, normalBase, colorBase, seccolorBase, texcoordBase,
      texcoord1Base, texcoord2Base, texcoord3Base, texcoord4Base, texcoord5Base, texcoord6Base,
      texcoord7Base;
...
bool Geometry::updateLowHighIndices(void) {
  ...
  if (getLengths() == NullFC || getLengths()->size() < primcount || getIndices() == NullFC ||
      getIndices()->size() == 0 || getIndexMapping().size() > 1)
    return true;
  ...
  beginEditCP(
      self, LowindicesFieldMask | HighindicesFieldMask | MinindexFieldMask | MaxindexFieldMask);
