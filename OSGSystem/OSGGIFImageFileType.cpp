...
typedef enum { gif_image, gif_comment, gif_text } GIFStreamType;
...
static const Char8* suffixArray[] = {"gif"};

GIFImageFileType GIFImageFileType::_the("image/gif", suffixArray, sizeof(suffixArray));
    ...
    for (gifData = gifStream->data; gifData; gifData = gifData->next) {
      ...
      case gif_image:
        ...
        if (currentFrame) {
          ...
          if ((channel == image->getBpp()) && (width == image->getWidth()) &&
            ...
            case gif_keep_disposal:
              memcpy(destData, image->getData(0, currentFrame - 1),
                  image->getWidth() * image->getHeight() * channel);
              break;
            ...
            case gif_color_restore: {
              ...
              for (UInt32 pixel = image->getWidth() * image->getHeight(); pixel > 0;
                   --pixel, d += channel) {
                d[0] = r;
                d[1] = g;
                d[2] = b;
            ...
            case gif_image_restore:
              memcpy(destData, image->getData(0, (currentFrame >= 2) ? (currentFrame - 2) : 0),
                  image->getWidth() * image->getHeight() * channel);
              break;
            default:
...
std::string GIFImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "GIF8", 4) == 0 ? std::string(getMimeType()) : std::string();
}
...
*/
GIFImageFileType::GIFImageFileType(
    const Char8* mimeType, const Char8* suffixArray[], UInt16 suffixByteCount)
...
#else
#define INFO_MSG(fmt)                                                                              \
  { FINFO(("Info loading gif: '%s'!\n", fmt)); }
#define GIF_ERROR(str)                                                                             \
  {                                                                                                \
    FWARNING(("Error loading gif: '%s'!\n", str));                                                 \
    longjmp(setjmp_buffer, 1);                                                                     \
  }
#endif
#endif
  ...
  while (ReadOK(is, &c, 1) && c != ';') {
    ...
    if (c == '!') { /* Extension */
      ...
      if (BitSet(buf[8], LOCALCOLORMAP)) {
        if (readColorMap(is, cur->data.image.cmapSize, cur->data.image.cmapData)) {
      ...
      readImage(is, BitSet(buf[8], INTERLACE), cur->width, cur->height, cur->data.image.data);
...
/* */
static void readImage(std::istream& is, int interlace, int width, int height, unsigned char* data) {
  unsigned char *dp, c;
  ...
  */
  ...
  if (verbose) {
    INFO_MSG(("reading %d by %d%s GIF image", width, height, interlace ? " interlaced" : ""));
  }
  ...
  */
  {
    ...
    */
    qsort(vals, size, sizeof(vals[0]), (int (*)(const void*, const void*))cvalCMP);
  ...
  */
  ...
  for (cur = stream->data; !flag && cur != NULL; cur = cur->next) {
    if (cur->type == gif_text || cur->type == gif_comment) {
      ...
      */
      if (cur->info.transparent != -1 || cur->info.delayTime != 0 || cur->info.inputFlag != 0 ||
          cur->info.disposal != 0)
        flag = GIF_TRUE;
    }
  }
  ...
  */
  ...
  for (cur = stream->data; cur != NULL; cur = cur->next) {
    if (cur->type == gif_image) {
      ...
      putImage(fp, cur->data.image.interlaced, bpp, cur->width, cur->height, cur->data.image.data);
    } else if (cur->type == gif_comment) {
      PUTBYTE('!', fp);
      PUTBYTE(0xfe, fp);
      putDataBlocks(fp, cur->data.comment.len, (unsigned char*)cur->data.comment.text);
      ...
      putDataBlocks(fp, cur->data.text.len, (unsigned char*)cur->data.text.text);
    }
  }
...
static void putGif89Info(FILE* fp, GIF89info* info) {
  ...
  if (info->transparent == -1 && info->delayTime == 0 && info->inputFlag == 0 &&
      info->disposal == 0)
    return;
  ...
  PUTBYTE(0xf9, fp);
  PUTBYTE(4, fp);
  c = (info->inputFlag ? 0x02 : 0x00) | ((info->disposal & 0x07) << 2) |
...
static void putImage(
    FILE* fp, int interlaced, int bpp, int width, int height, unsigned char* data) {
...
static unsigned long masks[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F, 0x003F, 0x007F,
