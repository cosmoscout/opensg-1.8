...
void StateChunkClass::swap(StateChunkClass& other) {
  FINFO(("Swap %s(%d|%d) %s(%d|%d)\n", this->getName(), this->getId(), this->getNumSlots(),
      other.getName(), other.getId(), other.getNumSlots()));

  bool bNeighborSwap = false;

  if (this->getNumSlots() != other.getNumSlots()) {
    if (((this->getId() < other.getId() && this->getId() + this->getNumSlots() != other.getId()) ||
  ...
  for (UInt32 i = this->getId(); i < this->getId() + thisNumSlots; ++i) {
    (*_classNames)[i] = thisName;
    (*_numslots)[i]   = thisNumSlots;
  }

  for (UInt32 i = other.getId(); i < other.getId() + otherNumSlots; ++i) {
...
void StateChunk::activate(DrawActionBase* OSG_CHECK_ARG(action), UInt32 OSG_CHECK_ARG(index)) {
...
void StateChunk::deactivate(DrawActionBase* OSG_CHECK_ARG(action), UInt32 OSG_CHECK_ARG(index)) {
