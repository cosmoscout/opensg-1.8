...
void ClusterWindow::dump(UInt32, const BitVector) const {
...
void ClusterWindow::init(void) {
  ...
  if (connection == NULL) {
    SFATAL << "Unknown connection type " << getConnectionType() << std::endl;
    return;
  }
    ...
    for (id = 0; id < getServers().size(); ++id) {
      ...
      for (c = 0; c < autostart.length(); ++c) {
        if (autostart[c] == '%' && c + 1 < autostart.length())
          ...
          case '{':
            env = "";
            while (++c < autostart.length() && autostart[c] != '}')
              env += autostart[c];
            if (getenv(env.c_str()))
  ...
  for (s = getServers().begin(); s != getServers().end(); s++) {
    ...
    if (!getServiceInterface().empty()) {
      serviceSock.setMCastInterface(SocketAddress(getServiceInterface().c_str()));
    }
    ...
    while (retry) {
      try {
        ...
        if (_sfServiceAddress.getValue().size() != 0) {
          SINFO << "send request to:" << _sfServiceAddress.getValue() << std::endl;
          serviceSock.sendTo(
              msg, SocketAddress(_sfServiceAddress.getValue().c_str(), getServicePort()));
        }
        SINFO << "send request to:"
              << SocketAddress(SocketAddress::BROADCAST, getServicePort()).getHost().c_str()
              << std::endl;
        serviceSock.sendTo(msg, SocketAddress(SocketAddress::BROADCAST, getServicePort()));
        if (serviceSock.waitReadable(0.1)) {
          ...
          if (respServer == *s) {
            GroupSockConnection* pointSock = dynamic_cast<GroupSockConnection*>(connection);
            if (pointSock) {
              /* for all socket connections ignore the
                 incomming host and use the host from
                 the last response. */
              char port[16];
              if (sscanf(respAddress.c_str(), "%*[^:]:%15s", port) == 1)
      ...
      } catch (AsyncCancel&) { throw; } catch (OSG_STDEXCEPTION_NAMESPACE::exception& e) {
...
bool ClusterWindow::initAsync(connectioncbfp fp) {
  ...
  try {
    init();
    result = true;
  } catch (AsyncCancel& e) { result = false; }
...
bool ClusterWindow::loadCalibration(std::istream& in) {
  ...
  try {
    ...
    for (sI = servers.begin(); sI != servers.end(); ++sI) {
      ...
      // loop over children
      for (nI = (*sI)->get_nodelist().begin(); nI != (*sI)->get_nodelist().end(); ++nI) {
    ...
    SFATAL << "XML error line " << where.get_line() << " "
           << "at position " << where.get_pos() << ": error: " << errmsg.c_str() << std::endl;
    return false;
  }
  return true;
}
...
bool ClusterWindow::saveCalibration(std::ostream& out) {
  ...
  for (c = 0; c < getCalibration().size(); ++c) {
    ...
    out << "<colormatrix>\n" << calibPtr->getColorMatrix() << "</colormatrix>\n";
    out << "<gammaramp>\n";
    for (color = 0; color < calibPtr->getGammaRamp().size(); ++color)
      out << "<color>" << calibPtr->getGammaRamp()[color] << "</color>\n";
    out << "</gammaramp>\n";
    out << "<grid>\n";
    for (row = 0; row < calibPtr->getGridHeight(); ++row) {
      ...
      for (col = 0; col < calibPtr->getGridWidth(); ++col) {
        ...
        if (pos < calibPtr->getGrid().size())
          out << calibPtr->getGrid()[pos][0] << " " << calibPtr->getGrid()[pos][1];
        else
          out << col << " " << row;
        out << "</point>\n";
      }
...
bool ClusterWindow::loadFilter(std::istream& in) {
  ...
  try {
    ...
    for (sI = servers.begin(); sI != servers.end(); ++sI) {
      ...
      for (fI = (*sI)->get_nodelist().begin(); fI != (*sI)->get_nodelist().end(); ++fI) {
        if ((*fI)->get_name() == "colordisplayfilter") {
          ...
          for (nI = (*fI)->get_nodelist().begin(); nI != (*fI)->get_nodelist().end(); ++nI) {
            if ((*nI)->get_name() == "colormatrix") {
              nP = (*nI);
              do
                nP = nP->get_nodelist().front();
            ...
            if ((*nI)->get_name() == "gamma") {
              ...
              if (nP->get_type() == xmlpp::xml_nt_cdata)
                sscanf(nP->get_cdata().c_str(), "%f", &colorFilter->getGamma());
            }
            if ((*nI)->get_name() == "size") {
              nP = (*nI);
              do
              ...
              if (nP->get_type() == xmlpp::xml_nt_cdata)
                sscanf(nP->get_cdata().c_str(), "%d %d %d", &colorFilter->getWidth(),
                    &colorFilter->getHeight(), &colorFilter->getDepth());
            }
        ...
        if ((*fI)->get_name() == "resolutiondisplayfilter") {
          ...
          for (nI = (*fI)->get_nodelist().begin(); nI != (*fI)->get_nodelist().end(); ++nI) {
            if ((*nI)->get_name() == "downscale") {
              ...
              if (nP->get_type() == xmlpp::xml_nt_cdata)
                sscanf(nP->get_cdata().c_str(), "%f", &resolutionFilter->getDownScale());
            }
          }
        ...
        if ((*fI)->get_name() == "distortiondisplayfilter") {
          ...
          for (nI = (*fI)->get_nodelist().begin(); nI != (*fI)->get_nodelist().end(); ++nI) {
            if ((*nI)->get_name() == "rows") {
              ...
              if (nP->get_type() == xmlpp::xml_nt_cdata)
                sscanf(nP->get_cdata().c_str(), "%d", &distortionFilter->getRows());
            }
            if ((*nI)->get_name() == "cols") {
              nP = (*nI);
              do
              ...
              if (nP->get_type() == xmlpp::xml_nt_cdata)
                sscanf(nP->get_cdata().c_str(), "%d", &distortionFilter->getColumns());
            }
...
bool ClusterWindow::updateFilter(WindowPtr window, UInt32 id, RenderActionBase* action) {
  bool found = false;
    ...
    for (p = 0; p < window->getPort().size(); ++p) {
      ...
      for (c = 0; c < getFilter().size(); ++c) {
        ...
        if (filterFgnd->getServer() == name) {
          ...
          for (Int32 n = window->getPort()[p]->getForegrounds().size(), j = n - 1; j >= 0; j--) {
            MFForegroundPtr::iterator fgndIt = window->getPort()[p]->getForegrounds().begin() + j;
...
void ClusterWindow::serverInit(WindowPtr, UInt32) {
}
...
void ClusterWindow::serverRender(WindowPtr window, UInt32 id, RenderActionBase* action) {
  ...
  if (ract != NULL) {
    ...
    OSG::RenderOptionsPtr winRo =
        OSG::RenderOptionsPtr::dcast(window->findAttachment(OSG::RenderOptions::getClassType()));
 ...
 **/
void ClusterWindow::serverSwap(WindowPtr window, UInt32) {
  window->swap();
  window->frameExit();
}
