 ...
 */
std::string WebInterface::encodeUrl(const std::string& path, const ParameterT& param) {
...
*/
void WebInterface::decodeUrl(const std::string& url, std::string& path, ParameterT& param) {
 ...
 */
std::string WebInterface::createFCViewReference(FieldContainerPtr fcPtr, UInt32 fcId) {
  ...
  if (fcPtr == NullFC) {
    ...
    result << "<a href =\"fcview?id=" << fcPtr.getFieldContainerId() << "\">"
           << fcPtr->getType().getName().str() << " (" << fcPtr.getFieldContainerId() << ")"
           << "</a>";
  return result.str();
}
 ...
 */
void WebInterface::setParam(ParameterT& param, const char* name, const char* value) {
  if (!value)
 ...
 */
void WebInterface::treeViewNode(std::ostream& os, NodePtr node, ParameterT& param) {
  ParameterT::const_iterator pI;
  char                       idstr[32];
  std::string                folder;
  ...
  os << "&nbsp &nbsp &nbsp "
     << "<b>" << getNodeName(node) << "</b>&nbsp &nbsp &nbsp " << createFCViewReference(node);

  if (node->getCore() != NullFC) {
    os << "&nbsp &nbsp &nbsp Core: " << createFCViewReference(node->getCore());
  }
  os << "</li>";
...
/*! main page
 */
void WebInterface::rootHandler(std::ostream& os, const char*, ParameterT&) {
 ...
 */
void WebInterface::changelistHandler(std::ostream& os, const char*, ParameterT&) {
  ...
  os << "</tr>\n";
  for (col = 0, createdI = changeList->beginCreated(); createdI != changeList->endCreated();
       createdI++) {
  ...
  os << "</tr>\n";
  for (col = 0, changedI = changeList->beginChanged(); changedI != changeList->endChanged();
       changedI++) {
    ...
    if (!col)
      os << "<tr>";
    os << "<td>" << createFCViewReference(fcPtr) << "</td><td>" << mask << "</td>";
  ...
  for (col = 0; col < destroyedCols; ++col)
    os << "<th>FieldContainer</th>";
  os << "</tr>\n";
  for (col = 0, destroyedI = changeList->beginDestroyed(); destroyedI != changeList->endDestroyed();
       destroyedI++) {
 ...
 */
void WebInterface::fcViewHandler(std::ostream& os, const char*, ParameterT& param) {
  ...
  if (!getParam(param, "id")) {
    os << "Content-Type: text/html\r\n"
          "\r\n"
          "<html>"
       << _header << "id missing" << _footer << "</html>";
    return;
  }
  ...
  if (fcPtr == NullFC) {
    ...
    os << "<h1>" << fcPtr->getTypeName() << " " << getNodeName(fcPtr) << "</h1>"
    ...
    for (unsigned int i = 0; i < fcPtr->getType().getNumFieldDescs(); ++i) {
      ...
      os << "<tr><td>" << desc->getName().str() << "</td><td>"
         << desc->getFieldType().getName().str() << "</td><td>";
      if (type == "SFAttachmentMap") {
        ...
        if ((type.size() > 3) && (type.find("Ptr", type.size() - 3) != -1)) {
          os << "</td><td>\n";
          if (type.find("MF") == 0) {
          ...
          if (field) {
            ...
            os << "<form action=\"fcedit\">"
               ...
               << "<input type=\"hidden\" name=\"id\" value=\"" << fcPtr.getFieldContainerId()
               << "\">"
               << "<input type=\"hidden\" name=\"field\" value=\"" << desc->getFieldId() << "\">"
               << "</form>"
               << "</td><td>\n";
            os << value;
          }
...
/*! Edit field value
 */
void WebInterface::fcEditHandler(std::ostream& os, const char*, ParameterT& param) {
  ...
  if (fcPtr == NullFC) {
    os << "Content-Type: text/html\r\n"
          "\r\n"
          "<html>"
       << _header << "Unknown field container" << _footer << "</html>";
    return;
  }
  ...
  if (field == NULL) {
    os << "Content-Type: text/html\r\n"
          "\r\n"
          "<html>"
       << _header << "Unknown field in container" << _footer << "</html>";
    return;
  }
  ...
  os << "Content-Type: text/html\r\n"
        "\r\n"
        "<html>"
     << _header << "<h1>Edit " << fcPtr->getTypeName() << "." << desc->getName().str() << "</h1>\n"
     << "<form action=\"fcedit\">"
     << "<textarea name=\"value\" cols=\"50\" rows=\"10\">" << value << "</textarea><p>"
...
*/
void WebInterface::treeViewHandler(std::ostream& os, const char*, ParameterT& param) {
...
std::string WebInterface::getDefaultHeader(void) {
  ...
  header
      ...
      << "<td valign=center><a href=\"changelist\"><font "
         "color=\"#004faf\">Changelist</font></a></td>"
