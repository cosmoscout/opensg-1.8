...
static const Char8* pngSuffixArray[] = {"png"};
...
static void warningOutput(png_structp OSG_CHECK_ARG(png_ptr), const char* message) {
  FWARNING(("PNG: %s\n", message));
}

#endif

PNGImageFileType PNGImageFileType::_the(
    "image/png", pngSuffixArray, sizeof(pngSuffixArray), OSG_READ_SUPPORTED | OSG_WRITE_SUPPORTED);
...
bool PNGImageFileType::read(ImagePtr& OSG_PNG_ARG(image), std::istream& OSG_PNG_ARG(is),
  ...
  if (image->set(pixelFormat, width, height, 1, 1, 1, 0.0, 0, dataType)) {
    // set resolution png supports only pixel per meter,
    // so we do a conversion to dpi with some rounding.
    res_x =
        png_uint_32((Real32(res_x) / 39.37007874f) < 0.0f ? (Real32(res_x) / 39.37007874f) - 0.5f
                                                          : (Real32(res_x) / 39.37007874f) + 0.5f);
    res_y =
        png_uint_32((Real32(res_y) / 39.37007874f) < 0.0f ? (Real32(res_y) / 39.37007874f) - 0.5f
...
bool PNGImageFileType::write(const ImagePtr& OSG_PNG_ARG(img), std::ostream& OSG_PNG_ARG(os),
   ...
   */
  png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, 0, &errorOutput, &warningOutput);
  ...
  png_set_IHDR(png_ptr, info_ptr, img->getWidth(), img->getHeight(), bit_depth, ctype,
  ...
  if (img->getPixelFormat() == Image::OSG_BGR_PF || img->getPixelFormat() == Image::OSG_BGRA_PF) {
  ...
  for (Int32 k = 0; k < img->getHeight(); k++) {
    row_pointers[k] = img->getData() + (img->getHeight() - 1 - k) * img->getWidth() * img->getBpp();
  }
...
#else
  SWARNING << getMimeType() << " write is not compiled into the current binary " << endLog;
  return false;
#endif
}
...
std::string PNGImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "\x89PNG", 4) == 0 ? std::string(getMimeType()) : std::string();
}
...
/* */
UInt64 PNGImageFileType::restoreData(
    ImagePtr& OSG_PNG_ARG(image), const UChar8* OSG_PNG_ARG(buffer), Int32 OSG_CHECK_ARG(memSize)) {
#ifdef OSG_WITH_PNG
...
#else
  SWARNING << getMimeType() << " restoreData is not compiled into the current binary " << std::endl;
  return 0;
#endif
}
...
*/
UInt64 PNGImageFileType::storeData(
    const ImagePtr& OSG_PNG_ARG(image), UChar8* OSG_PNG_ARG(buffer), Int32 OSG_CHECK_ARG(memSize)) {
#ifdef OSG_WITH_PNG
  ...
  png_set_IHDR(png_ptr, info_ptr, image->getWidth(), image->getHeight(), bit_depth, ctype,
      PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
  ...
  for (Int32 k = 0; k < image->getHeight(); k++) {
    row_pointers[k] =
        image->getData() + (image->getHeight() - 1 - k) * image->getWidth() * image->getBpp();
  }
...
#else
  SWARNING << getMimeType() << " storeData is not compiled into the current binary " << std::endl;
  return 0;
#endif
}
...
*/
PNGImageFileType::PNGImageFileType(
