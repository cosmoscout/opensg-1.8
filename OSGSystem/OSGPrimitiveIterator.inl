 ...
 */
inline bool PrimitiveIterator::isAtEnd(void) const {
  return _ended;
}
...
*/
inline Int32 PrimitiveIterator::getIndex(void) const {
  return _primIndex;
}

/*! Return the length(i.e. number of used vertices) of the current primitive.
 */
inline UInt32 PrimitiveIterator::getLength(void) const {
  return _actPrimLength;
}

/*! Return the type of the current primitive.
 */
inline UInt32 PrimitiveIterator::getType(void) const {
  return _actPrimType;
}
...
*/
inline Int32 PrimitiveIterator::getPositionIndex(Int32 which) const {
  if (_geo->getIndices() != NullFC) {
    if (_positionIndex != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _positionIndex);
    } else {
...
/*! Return the position of a point in the current primitive.
    \a which is the point to access. Must be between 0 and getLength().
*/
inline Pnt3f PrimitiveIterator::getPosition(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getNormalIndex(Int32 which) const {
  if (_geo->getNormals() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_normalIndex != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _normalIndex);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec3f PrimitiveIterator::getNormal(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getColorIndex(Int32 which) const {
  if (_geo->getColors() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_colorIndex != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _colorIndex);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Color3f PrimitiveIterator::getColor(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getSecondaryColorIndex(Int32 which) const {
  if (_geo->getSecondaryColors() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_secondaryColorIndex != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _secondaryColorIndex);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Color3f PrimitiveIterator::getSecondaryColor(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex(Int32 which) const {
  if (_geo->getTexCoords() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex1(Int32 which) const {
  if (_geo->getTexCoords1() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex1 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex1);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords1(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex2(Int32 which) const {
  if (_geo->getTexCoords2() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex2 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex2);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords2(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex3(Int32 which) const {
  if (_geo->getTexCoords3() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex3 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex3);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords3(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex4(Int32 which) const {
  if (_geo->getTexCoords4() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex4 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex4);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords4(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex5(Int32 which) const {
  if (_geo->getTexCoords5() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex5 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex5);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords5(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex6(Int32 which) const {
  if (_geo->getTexCoords6() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex6 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex6);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords6(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getTexCoordsIndex7(Int32 which) const {
  if (_geo->getTexCoords7() == NullFC)
    return -1;

  if (_geo->getIndices() != NullFC) {
    if (_texcoordsIndex7 != -1) {
      return _indices->getValue((_actPointIndex + which) * _nmappings + _texcoordsIndex7);
    } else {
      return _indices->getValue(_actPointIndex * _nmappings + which);
    }
  } else {
...
*/
inline Vec2f PrimitiveIterator::getTexCoords7(Int32 which) const {
...
*/
inline Int32 PrimitiveIterator::getIndexIndex(Int32 which) const {
  if (_geo->getIndices() != NullFC) {
 ...
 */
inline GeometryPtr PrimitiveIterator::getGeometry(void) const {
  return _geo;
}
