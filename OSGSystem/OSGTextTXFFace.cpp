...
void TextTXFFace::addToGeom(GeometryPtr& geoPtr, const TextLayoutResult& layoutResult, Real32 scale,
  ...
  // Create color buffer: If Null container AND color is set && we have not potentially added text
  // before
...
// Author: pdaehne
//----------------------------------------------------------------------
GeometryPtr TextTXFFace::makeGeo(
    const TextLayoutResult& layoutResult, Real32 scale, Vec2f offset, Color3f color) {
...
static UInt32 readLong(istream& is, bool swap) {
  ...
  return swap ? (bytes[3] << 24) | (bytes[2] << 16) | (bytes[1] << 8) | bytes[0]
...
static UInt16 readShort(istream& is, bool swap) {
  ...
  return swap ? (bytes[1] << 8) | bytes[0] : (bytes[0] << 8) | bytes[1];
}
...
//----------------------------------------------------------------------
void TextTXFFace::layout(
    const wstring& text, const TextLayoutParam& param, TextLayoutResult& result) {
...
void TextTXFFace::prepareTexture(const TextTXFParam& param) {
  ...
  typedef multimap<UInt32, TextTXFGlyph*> HeightMap;
  HeightMap                               heightMap;
