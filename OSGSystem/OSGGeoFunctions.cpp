...
*/
{
  ...
  if (norms->getParents().size() > 1) {
    ...
    for (IndexSet::iterator i = used_indices.begin(); i != used_indices.end(); ++i) {
...
OSG_SYSTEMLIB_DLLMAPPING void OSG::calcVertexNormals(GeometryPtr geo, Real32 creaseAngle) {
  GeoNormalsPtr   norms;
  ...
  if (creaseAngle == 0) {
    ...
    for (TriangleIterator ti = geo->beginTriangles(); ti != geo->endTriangles(); ++ti) {
  ...
  for (ti = geo->beginTriangles(); ti != geo->endTriangles(); ++ti) {
    ...
    if (faceNorm.squareLength() != 0.0) {
      for (UInt16 i = 0; i < 3; ++i) {
        ...
        normset.clear();
        for (f = 0; f < fN; f++) {
          if (((pf = pntFaceDic[p][f]) == tind) || (faceNorm.dot(faceNormals[pf]) > cosCrease))
    ...
    if (geo->getIndexMapping().size() > 0) {
      ...
      for (UInt32 faceCnt = 0; faceIter != geo->endFaces(); ++faceIter, ++faceCnt) {
        if (faceIter.getLength() == 3) {
          // Face is a Triangle
          normal = (faceIter.getPosition(1) - faceIter.getPosition(0))
          ...
          if (normal.length() == 0) {
            ...
            normal = (faceIter.getPosition(1) - faceIter.getPosition(2))
...
Int32 OSG::setIndexFromVRMLData(GeometryPtr geoPtr, std::vector<Int32>& coordIndex,
    std::vector<Int32>& normalIndex, std::vector<Int32>& colorIndex,
    std::vector<Int32>& texCoordIndex, bool convex, bool ccw, bool normalPerVertex,
    bool colorPerVertex, bool createNormal, bool faceSet) {
  Int32 primitiveType = faceSet ? GL_POLYGON : GL_LINE_STRIP;

  return setIndexFromIndexedX3DData(geoPtr, coordIndex, normalIndex, colorIndex, texCoordIndex,
      primitiveType, convex, ccw, normalPerVertex, colorPerVertex, createNormal);
}
...
OSG_SYSTEMLIB_DLLMAPPING
Int32 OSG::setIndexFromIndexedX3DData(GeometryPtr geoPtr, std::vector<Int32>& coordIndex,
    std::vector<Int32>& normalIndex, std::vector<Int32>& colorIndex,
    std::vector<Int32>& texCoordIndex, Int32 primitiveType, bool OSG_CHECK_ARG(convex), bool ccw,
    bool normalPerVertex, bool colorPerVertex, bool OSG_CHECK_ARG(createNormal)) {
  /** define the bag type */
  ...
  static const char*   indexTypeStr[] = {"UNKNOWN_IT", "EMPTY_IT", "VERTEX_COORD_IT", "VERTEX_IT",
      "VERTEX_DUP_IT", "VERTEX_CREATE_IT", "PRIMTIVE_IT", "PRIMITIVE_INDEX_IT",
      "PRIMITIVE_CREATE_IT"};
  ...
  Int32 pN = 0, nN = 0, cN = 0, tN = 0, tN1 = 0, tN2 = 0, tN3 = 0, tN4 = 0, tN5 = 0, tN6 = 0,
        tN7 = 0;
  ...
  IndexBagP indexBag[4] = {&coordIndex, &normalIndex, &colorIndex, &texCoordIndex};
  ...
  if (nN) { // have normal elements
    if (normalPerVertex) {
      // normal per vertex
      if (niN >= piN) {
        // valid normal index number
        for (i = 0; i < piN; i++) { // check if normal index equals the coord index
...
*/
  ...
  colorIT = UNKNOWN_IT;
  ciN     = colorIndex.size();
  if (cN) { // have color elements
    if (colorPerVertex) {
      // color per vertex
      if (ciN >= piN) {
        // valid color index number
        for (i = 0; i < piN; i++) { // check if color index equals the coord index
  ...
  if (tN) { // have texture elemnts
    if (tiN >= piN) {
      // valid texture index number
      for (i = 0; i < piN; i++) { // check if texture index equals the coord index
  ...
  FNOTICE(("primitiveN:  %d, %d, 0/%d 1/%d 2/%d 3/%d 4/%d 5/%d\n", primitiveType, primitiveN,
      primitiveTypeCount[0], primitiveTypeCount[1], primitiveTypeCount[2], primitiveTypeCount[3],
      primitiveTypeCount[4], primitiveTypeCount[5]));

  FNOTICE(("IndexType: coord: %s, color: %s, normal: %s, texture: %s \n", indexTypeStr[coordIT],
      indexTypeStr[colorIT], indexTypeStr[normalIT], indexTypeStr[textureIT]));
    ...
    for (i = 0; ((i <= 3) && indexMap[i]); i++) {
      if (i == 0) {
        ...
        if (indexMap[i] & Geometry::MapTexCoords) {
          ...
          if (tN1 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords1);

            ++uiNumTextures;
          }

          if (tN2 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords2);

            ++uiNumTextures;
          }

          if (tN3 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords3);

            ++uiNumTextures;
          }

          if (tN4 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords4);

            ++uiNumTextures;
          }

          if (tN5 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords5);

            ++uiNumTextures;
          }

          if (tN6 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords6);

            ++uiNumTextures;
          }

          if (tN7 != 0) {
            geoPtr->getIndexMapping().push_back(Geometry::MapTexCoords7);
  ...
  for (pType = minPType; pType <= maxPType; pType++) {
    ...
    if (primitiveTypeCount[pType]) {
      ...
      for (i = 0; i <= piN; i++) {
        if (((i == piN) && (coordIndex[i - 1] >= 0)) || ((i < piN) && (coordIndex[i] < 0))) {
          len = i - beginIndex;
          if (ccw) {
            endIndex = i;
            step     = 1;
...
OSG_SYSTEMLIB_DLLMAPPING void OSG::calcVertexTexCoords(GeometryPtr geo, Int32 texIndex) {
  ...
  for (MFNodePtr::iterator pnI = geo->getMFParents()->begin(); pnI != geo->getMFParents()->end();
       ++pnI) {
    NodePtr node = *pnI;
  ...
  Int16 niTex = geo->calcMappingIndex(mapTex), niPos = geo->calcMappingIndex(Geometry::MapPosition);
...
OSG_SYSTEMLIB_DLLMAPPING void OSG::calcVertexTangents(
    GeometryPtr geo, Int32 srcTexIndex, Int32 dstAttribTan, Int32 dstAttribBin) {
  ...
  if (srcTexIndex < 0 || srcTexIndex > 3 || (dstAttribTan == -1 && dstAttribBin == -1)) {
  ...
  beginEditCP(geo, Geometry::IndexMappingFieldMask | Geometry::IndicesFieldMask);
  ...
  if (multiIndex) {
    ...
    if (niTan < 0 && niBin < 0) {
      status = 0;
    } else if (niBin < 0 && niTan >= 0 || niBin >= 0 && niTan < 0 || niBin >= 0 && niBin == niTan) {
      status = 1;
  ...
  endEditCP(geo, Geometry::IndexMappingFieldMask | Geometry::IndicesFieldMask);
  ...
  Int16              niPos = geo->calcMappingIndex(Geometry::MapPosition),
        niNorm = geo->calcMappingIndex(Geometry::MapNormal), niTex = geo->calcMappingIndex(mapTex);
  ...
  for (tI = geo->beginTriangles(), i = 0; tI != geo->endTriangles(); ++tI, ++i) {
    ...
    Int32 v0 = tI.getPositionIndex(0), v1 = tI.getPositionIndex(1), v2 = tI.getPositionIndex(2);
    ...
    edge1 = tI.getPosition(1) - tI.getPosition(0), edge2 = tI.getPosition(2) - tI.getPosition(0);
    tex1 = t1 - t0;
    tex2 = t2 - t0;
...
OSG_SYSTEMLIB_DLLMAPPING
Int32 OSG::createOptimizedPrimitives(GeometryPtr geoPtr, UInt32 iteration, bool createStrips,
    bool createFans, UInt32 minFanEdgeCount, bool OSG_CHECK_ARG(colorCode), bool stitchStrips) {
  if (geoPtr == NullFC) {
    return 0;
  }
  ...
  FDEBUG(("GeoOpt: start/tri cost: %d/%d imp: %d tri/line/point: %d/%d/%d\n", startCost, (triN * 3),
      indexMapSize, triN, lineN, pointN));
  ...
  if (remapIndex) {
    ...
    UInt32             triCount = 0;
    for (TriangleIterator tI = geoPtr->beginTriangles();
         (triCount < triN) && (tI != geoPtr->endTriangles()); ++tI) {
      Int32 v[3];
      for (Int32 i = 0; i < 3; i++) {
    ...
    for (TriangleIterator tI = geoPtr->beginTriangles();
         (triCount < triN) && (tI != geoPtr->endTriangles()); ++tI) {
      invalidTriCount +=
          graph.addTriangle(tI.getPositionIndex(0), tI.getPositionIndex(1), tI.getPositionIndex(2))
              ? 0
              : 1;
      ++triCount;
    }
  ...
  if (bestCost && (bestCost < startCost)) {
    // check/create the indexPtr/lengthsPtr/geoTypePtr
    OSG::beginEditCP(geoPtr, OSG::Geometry::LengthsFieldMask | OSG::Geometry::TypesFieldMask);
    ...
    OSG::endEditCP(geoPtr, OSG::Geometry::LengthsFieldMask | OSG::Geometry::TypesFieldMask);
    ...
    for (Int32 t = 0; t < typeN; ++t) {
      ...
      if (stitchStrips && totalTriStripLength && typeVec[t] == GL_TRIANGLE_STRIP) {
    ...
    if (cost != bestCost) {
      ...
      FINFO(("OptResult: %2g%%, Sampling (%di): cost %d/%d \n",
          double(double(bestCost) / double(startCost) * 100.0), iteration, bestCost, worstCost));
    }
  } else {
...
void OSG::createConvexPrimitives(GeometryPtr geoPtr) {
  ...
  OSGGLUfuncptr gluTessBeginDataCBPtr = reinterpret_cast<OSGGLUfuncptr>(gluTessBeginDataCB);

  OSGGLUfuncptr gluTessEdgeFlagCBPtr = reinterpret_cast<OSGGLUfuncptr>(gluTessEdgeFlagCB);

  OSGGLUfuncptr gluTessVertexDataCBPtr = reinterpret_cast<OSGGLUfuncptr>(gluTessVertexDataCB);

  OSGGLUfuncptr gluTessEndCBPtr = reinterpret_cast<OSGGLUfuncptr>(gluTessEndCB);

  OSGGLUfuncptr gluTessErrorCBPtr = reinterpret_cast<OSGGLUfuncptr>(gluTessErrorCB);
  ...
  for (; primI != geoPtr->endPrimitives(); ++primI) {
    ...
    // Process GL_POLYGONs i.e. tesselate them
      ...
      for (UInt32 i = 0; i < primI.getLength(); i++) {
        ...
        positionV.push_back(static_cast<GLdouble>(primI.getPosition(i)[0]));
        positionV.push_back(static_cast<GLdouble>(primI.getPosition(i)[1]));
        positionV.push_back(static_cast<GLdouble>(primI.getPosition(i)[2]));
      ...
      FINFO((
          "Vertex count: %d/%d (before/after tesselation)\n", primI.getLength(), outIndexV.size()));
...
OSG_SYSTEMLIB_DLLMAPPING Int32 OSG::createSharedIndex(GeometryPtr geoPtr) {
  ...
  if (indexPtr != NullFC) {
    ...
    if (indexMapSize) {
      ...
      for (mapMask = 1, propMask = 0; mapMask && mapMask != Geometry::MapEmpty; mapMask <<= 1) {
    ...
    for (indexBlock = 0; propMask;) {
      ...
      if ((masterProp = geoPtr->getProperty(masterPropMask)) &&
        ...
        masterDSize =
            masterProp->getFormatSize() * masterProp->getDimension() + masterProp->getStride();

        // find and store slave property data and size
        slaveDataVec.clear();
        slaveDSizeVec.clear();
        for (mapMask = 1; mapMask && mapMask != Geometry::MapEmpty; mapMask <<= 1) {
            ...
            if (mmI == memMap.end()) {
              ...
              for (si = 0; si < sN; si++) {
                if (memcmp(slaveDataVec[si] + (index * slaveDSizeVec[si]),
                        slaveDataVec[si] + (mmI->second * slaveDSizeVec[si]), slaveDSizeVec[si]))
                  break;
              }

              if (si == sN) {
                // no or valid slave data; remap the index
                indexPtr->setValue(mmI->second, i * indexMapSize + indexBlock);
        ...
        FWARNING(
            ("Invalid masterProp %p, mask: %d, block: %d\n", masterProp, propMask, indexBlock));
      }

      FINFO(("Create sharedIndex: %d/%d pass; "
             "data/index remap: %d/%d \n",
          indexBlock, int(propMask), dataRemapCount, indexRemapCount));
...
OSG_SYSTEMLIB_DLLMAPPING Int32 OSG::createSingleIndex(GeometryPtr geoPtr) {
  ...
  if ((geoPtr != NullFC) && (indexMapSize = (geoPtr->getIndexMapping().size()))) {
      ...
      for (i = 0; i < indexMapSize; i++) {
        ...
        for (maskID = 1; maskID; maskID <<= 1) {
          if ((mask & maskID) && (pP = geoPtr->getProperty(maskID))) {
            valueSize = pP->getFormatSize() * pP->getDimension() + pP->getStride();
            finalMask |= maskID;
            ...
            for (j = 0; j < vCount; j++) {
              ...
              memcpy(pData + (valueSize * j), data + (valueSize * index), valueSize);
            }
...
*/
OSG_SYSTEMLIB_DLLMAPPING UInt32 OSG::calcPrimitiveCount(
    GeometryPtr geoPtr, UInt32& triangle, UInt32& line, UInt32& point) {
...
OSG_SYSTEMLIB_DLLMAPPING void OSG::mergeGeometries(
    std::vector<NodePtr>& nodes, std::vector<NodePtr>& results) {
  FFATAL(("Merge Geometries: Not implemented yet!\n"));
  results.clear();

  for (std::vector<NodePtr>::iterator n = nodes.begin(); n != nodes.end(); ++n) {
...
OSG_SYSTEMLIB_DLLMAPPING void OSG::separateProperties(GeometryPtr geo) {
  separateProperty(PTypes, Types) separateProperty(PLengths, Lengths) separateProperty(
      Positions, Positions) separateProperty(Normals, Normals) separateProperty(Colors, Colors)
      separateProperty(Colors, SecondaryColors) separateProperty(TexCoords, TexCoords)
          separateProperty(TexCoords, TexCoords1) separateProperty(TexCoords, TexCoords2)
              separateProperty(TexCoords, TexCoords3) separateProperty(TexCoords, TexCoords4)
                  separateProperty(TexCoords, TexCoords5) separateProperty(TexCoords, TexCoords6)
                      separateProperty(TexCoords, TexCoords7) separateProperty(Indices, Indices)
}
