...
OFFSceneFileType OFFSceneFileType::_the(
...
NodePtr OFFSceneFileType::read(std::istream& is, const Char8*) const {
  ...
  Real32            x, y, z, a;
  bool hasNormals = false, hasColors = false, hasTexCoords = false, has4DimPoints = false;
...

  {
    for (i = 0; (!is.eof()) && (i < vertexCount); i++) {
      ...
      if (hasColors) {
        is >> x >> y >> z >> a;
        color.setValuesRGBA(Real32(x), Real32(y), Real32(z), Real32(a));
...
//
//------------------------------
OFFSceneFileType::OFFSceneFileType(const Char8* suffixArray[], UInt16 suffixByteCount,
