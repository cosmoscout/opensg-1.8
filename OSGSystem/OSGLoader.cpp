...
void OSGLoader::initFieldTypeMapper(void) {
  setIntExtMapping(SFBool::getClassType().getId(), ScanParseSkel::OSGsfBool);
  ...
  */
  ...
  setIntExtMapping(SFReal32::getClassType().getId(), ScanParseSkel::OSGsfFloat);
  /*
      setIntExtMapping(SFImagePtr::getClassType().getId(),
                       ScanParseSkel::OSGsfString);
  */
  setIntExtMapping(SFInt32::getClassType().getId(), ScanParseSkel::OSGsfInt32);

  setIntExtMapping(SFQuaternion::getClassType().getId(), ScanParseSkel::OSGsfRotation);

  setIntExtMapping(SFString::getClassType().getId(), ScanParseSkel::OSGsfString);

  setIntExtMapping(SFTime::getClassType().getId(), ScanParseSkel::OSGsfTime);

  setIntExtMapping(SFVec2f::getClassType().getId(), ScanParseSkel::OSGsfVec2f);

  setIntExtMapping(SFVec3f::getClassType().getId(), ScanParseSkel::OSGsfVec3f);

  setIntExtMapping(SFVec4f::getClassType().getId(), ScanParseSkel::OSGsfVec4f);

  setIntExtMapping(SFColor4f::getClassType().getId(), ScanParseSkel::OSGsfColor4f);

  setIntExtMapping(SFColor4ub::getClassType().getId(), ScanParseSkel::OSGsfColor4i);

  setIntExtMapping(SFColor3f::getClassType().getId(), ScanParseSkel::OSGsfColor3f);
  ...
  */
  ...
  setIntExtMapping(MFReal32::getClassType().getId(), ScanParseSkel::OSGmfFloat);

  setIntExtMapping(MFInt32::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(MFQuaternion::getClassType().getId(), ScanParseSkel::OSGmfRotation);

  setIntExtMapping(MFString::getClassType().getId(), ScanParseSkel::OSGmfString);

  setIntExtMapping(MFTime::getClassType().getId(), ScanParseSkel::OSGmfTime);

  setIntExtMapping(MFVec2f::getClassType().getId(), ScanParseSkel::OSGmfVec2f);

  setIntExtMapping(MFVec3f::getClassType().getId(), ScanParseSkel::OSGmfVec3f);

  setIntExtMapping(MFVec4f::getClassType().getId(), ScanParseSkel::OSGmfVec4f);

  setIntExtMapping(MFColor4f::getClassType().getId(), ScanParseSkel::OSGmfColor4f);

  setIntExtMapping(MFColor4ub::getClassType().getId(), ScanParseSkel::OSGmfColor4i);

  setIntExtMapping(MFColor3f::getClassType().getId(), ScanParseSkel::OSGmfColor3f);

  setIntExtMapping(SFNodePtr::getClassType().getId(), ScanParseSkel::OSGsfNode);
  setIntExtMapping(MFNodePtr::getClassType().getId(), ScanParseSkel::OSGmfNode);

  /* To Base Type Mappings */

  setIntExtMapping(SFUInt8::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(MFUInt8::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(SFInt8::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(MFInt8::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(SFUInt16::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(MFUInt16::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(SFInt16::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(MFInt16::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(SFUInt32::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(MFUInt32::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  setIntExtMapping(SFGLenum::getClassType().getId(), ScanParseSkel::OSGsfInt32);
  setIntExtMapping(SFGLenum::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  /* To Node Mappings */

  setIntExtMapping(SFAttachmentMap::getClassType().getId(), ScanParseSkel::OSGmfNode);

  /* extended types */

  setIntExtMapping(SFMatrix::getClassType().getId(), ScanParseSkel::OSGsfMatrix);

  setIntExtMapping(SFPnt3f::getClassType().getId(), ScanParseSkel::OSGsfPnt3f);

  setIntExtMapping(MFPnt3f::getClassType().getId(), ScanParseSkel::OSGmfPnt3f);
  setIntExtMapping(SFDynamicVolume::getClassType().getId(), ScanParseSkel::OSGmfFloat);

  setIntExtMapping(SFPlane::getClassType().getId(), ScanParseSkel::OSGsfPlane);
  setIntExtMapping(MFPlane::getClassType().getId(), ScanParseSkel::OSGmfPlane);

  setIntExtMapping(SFDynamicVolume::getClassType().getId(), ScanParseSkel::OSGsfVolume);
}
  ...
  if (_pCurrentField != NULL && _pCurrentFC != NullFC && pNewNode != NullFC) {
    if (_pCurrentField == _pCurrentFC->getField("core")) {
      ...
      if (pNewNode->getType().isAttachment() == true) {
        AttachmentContainerPtr pAttContainer = AttachmentContainerPtr::dcast(_pCurrentFC);
        ...
        if (pAttContainer != NullFC) {
          ...
          SLOG << "FieldContainer " << _pCurrentFC->getType().getName()
               << "is no attachment container" << std::endl;
        }
      } else {
        SLOG << "FieldContainer : " << pNewNode->getType().getName() << " is no attachment"
             << std::endl;
      }
...
void OSGLoader::beginNode(const Char8* szNodeTypename, const Char8* szNodename) {
  ...
  pNewNode = FieldContainerFactory::the()->createFieldContainer(szNodeTypename);

  if (szNodename != NULL && pNewNode != NullFC) {
    AttachmentContainerPtr pAttCon = AttachmentContainerPtr::dcast(pNewNode);
  ...
  if (_fcStack.size() == 1) {
    ...
    if (_pCurrentFC->getType().isNode()) {
      ...
      SLOG << "Fieldcontainer " << szNodeTypename << "is neither Node nor NodeCore " << std::endl;
    }
...
Int32 OSGLoader::mapExtIntFieldType(const Char8* szFieldname, const Int32 iFieldTypeId) {
  Int32 returnValue = Inherited::mapExtIntFieldType(szFieldname, iFieldTypeId);
  ...
  if (returnValue < 0 && szFieldname != NULL && _pCurrentFC != NullFC) {
    ...
    if (pField != NULL) {
      ...
      */
      ...
      PINFO << oFieldType.getContentType().getCName() << " comparing with "
            << FieldDataTraits<FieldContainerPtr>::getType().getCName() << std::endl;

      if (strstr(oFieldType.getContentType().getCName(), "Ptr") != NULL) {
        PINFO << "FieldContainerPtr or derived class, "
              << "parsing as Node" << std::endl;
...
void OSGLoader::beginField(const Char8* szFieldname, const UInt32) {
  PINFO << "BeginField " << szFieldname << " " << _pCurrentField << std::endl;
  ...
  if (_pCurrentFC != NullFC) {
    ...
    _pCurrentFieldDesc = _pCurrentFC->getType().findFieldDescription(szFieldname);

    PINFO << "BF : " << szFieldname << " " << _pCurrentField << " " << _pCurrentFieldDesc
          << std::endl;
