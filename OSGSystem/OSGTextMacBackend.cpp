...
class TextMacVectorFace : public TextVectorFace {
 public:
  ...
  virtual void layout(
      const wstring& text, const TextLayoutParam& param, TextLayoutResult& layoutResult);
...
class TextMacVectorGlyph : public TextVectorGlyph {
 public:
  // Constructor
  TextMacVectorGlyph(
      Index glyphIndex, Real32 scale, ATSUStyle horiFontStyle, ATSUStyle vertFontStyle);
...
class TextMacPixmapGlyph : public TextPixmapGlyph {
 public:
  // Constructor
  TextMacPixmapGlyph(Index glyphIndex, UInt32 width, UInt32 height, Real32 horiAdvance,
      Int32 horiBearingX, Int32 horiBearingY, Real32 vertAdvance, Int32 vertBearingX,
      Int32 vertBearingY, UInt8* pixmap);
...
class TextMacTXFFace : public TextTXFFace {
 ...
 private:
  void createGlyphs(ATSUStyle horiFontStyle, ATSUStyle vertFontStyle, ATSUTextLayout horiTextLayout,
      ATSUTextLayout vertTextLayout, const TextTXFParam& param);
};
...
class TextMacTXFGlyph : public TextTXFGlyph {
 public:
  // Constructor
  TextMacTXFGlyph(Index glyphIndex, Real32 scale, const ATSGlyphScreenMetrics& horiMetrics,
      const ATSGlyphScreenMetrics& vertMetrics);
...
static string getFamilyName(ATSUFontID fontID) {
  ...
  OSStatus  result = ATSUFindFontName(fontID, kFontFamilyName, kFontNoPlatformCode,
      kFontNoScriptCode, kFontNoLanguageCode, 0, 0, 0, &fontNameIndex);
  if (result != noErr)
    return string();
  ByteCount actualNameLength;
  result = ATSUGetIndFontName(fontID, fontNameIndex, 0, 0, &actualNameLength, 0, 0, 0, 0);
  if ((result != noErr) || (actualNameLength == 0))
    return string();
  vector<char> name;
  name.resize(actualNameLength);
  FontPlatformCode fontNamePlatform;
  result = ATSUGetIndFontName(
      fontID, fontNameIndex, actualNameLength, &(name.front()), 0, 0, &fontNamePlatform, 0, 0);
  if ((result != noErr) || (actualNameLength == 0))
    return string();
  CFStringRef str = 0;
  ...
  if (str == 0)
    return string();
  CFDataRef data =
      CFStringCreateExternalRepresentation(kCFAllocatorDefault, str, kCFStringEncodingUTF8, ' ');
  CFRelease(str);
  if (data == 0)
    return string();
...
static ATSUStyle findFont(const string& family, TextFace::Style style, UInt32 size) {
  ...
  // Boolean bold = ((style == TextFace::STYLE_BOLD) || (style == TextFace::STYLE_BOLDITALIC)) ?
  // TRUE : FALSE; Boolean italic = ((style == TextFace::STYLE_ITALIC) || (style ==
  // TextFace::STYLE_BOLDITALIC)) ? TRUE : FALSE;
  Fixed            fontSize        = IntToFixed(size);
  ATSUAttributeTag attributeTags[] = {
      kATSUFontTag, /*kATSUQDBoldfaceTag, kATSUQDItalicTag,*/ kATSUSizeTag};
...
TextVectorFace* TextMacBackend::createVectorFace(const string& family, TextFace::Style style) {
  ...
  OSStatus                 result =
      ATSUSetAttributes(horiFontStyle, 1, &attributeTag, &attributeSize, &attributeValue);
...
// Author: pdaehne
//----------------------------------------------------------------------
TextPixmapFace* TextMacBackend::createPixmapFace(
...
// Author: pdaehne
//----------------------------------------------------------------------
TextTXFFace* TextMacBackend::createTXFFace(
...
// Author: pdaehne
//----------------------------------------------------------------------
static void getFaceInfo(ATSUStyle horiFontStyle, ATSUStyle vertFontStyle, string& family,
    TextFace::Style& style, Real32& horiAscent, Real32& horiDescent, Real32& vertAscent,
    Real32& vertDescent) {
  ...
  if ((result != noErr) && (result != kATSUNotSetErr))
    italic = false;
  style = (bold == TRUE) ? (italic == TRUE ? TextFace::STYLE_BOLDITALIC : TextFace::STYLE_BOLD)
  ...
  horiDescent =
      ((result == noErr) || (result == kATSUNotSetErr)) ? -FixedToFloat(measurement) : 0.f;
...
// Author: pdaehne
//----------------------------------------------------------------------
static void createATSUObjects(ATSUStyle horiFontStyle, ATSUStyle& vertFontStyle,
  ...
  if (result != noErr)
    ...
    result                                        = ATSUSetLayoutControls(
        vertTextLayout, 1, layoutAttributeTags, layoutAttributeSizes, layoutAttributeValues);
  }
}
...
TextMacVectorFace::TextMacVectorFace(ATSUStyle horiFontStyle)
  ...
  getFaceInfo(_horiFontStyle, _vertFontStyle, _family, _style, _horiAscent, _horiDescent,
      _vertAscent, _vertDescent);
...
void TextMacVectorFace::layout(
  ...
  ATSUSetLayoutControls(
      textLayout, 2, layoutAttributeTags, layoutAttributeSizes, layoutAttributeValues);

  // Set the text
  OSStatus result = ATSUSetTextPointerLocation(
      textLayout, &(utf16Text[0]), kATSUFromTextBeginning, kATSUToTextEnd, utf16Text.size());
  if (result != noErr)
    return;
  ...
  if (deltaYs != 0)
    ATSUDirectReleaseLayoutDataArrayPtr(
        0, kATSUDirectDataBaselineDeltaFixedArray, (void**)&deltaYs);
  ATSUDirectReleaseLayoutDataArrayPtr(
      0, kATSUDirectDataLayoutRecordATSLayoutRecordCurrent, (void**)&layoutRecords);
...
// Author: pdaehne
//----------------------------------------------------------------------
static OSStatus quadraticLineCallback(
...
// Author: pdaehne
//----------------------------------------------------------------------
static OSStatus quadraticCurveCallback(const Float32Point* pt1, const Float32Point* controlPt,
...
// Author: pdaehne
//----------------------------------------------------------------------
static OSStatus cubicCurveToCallback(const Float32Point* pt1, const Float32Point* pt2,
...
auto_ptr<TextVectorGlyph> TextMacVectorFace::createGlyph(TextGlyph::Index glyphIndex) {
  ...
  return auto_ptr<TextVectorGlyph>(
      new TextMacVectorGlyph(glyphIndex, _scale, _horiFontStyle, _vertFontStyle));
}
...
TextMacVectorGlyph::TextMacVectorGlyph(
  ...
  OSStatus              result =
      ATSUGlyphGetScreenMetrics(horiFontStyle, 1, &glyphID, 0, false, false, &glyphScreenMetrics);
  if (result == noErr) {
  ...
  result =
      ATSUGlyphGetScreenMetrics(vertFontStyle, 1, &glyphID, 0, false, false, &glyphScreenMetrics);
  if (result == noErr) {
    _width        = glyphScreenMetrics.width * scale;
    _height       = glyphScreenMetrics.height * scale;
  ...
  if (curveType == kATSQuadCurveType) {
    ...
    result = ATSUGlyphGetQuadraticPaths(horiFontStyle, glyphID, newPathProc, lineProc, curveProc,
        closePathProc, &userData, &callbackResult);
    ...
    UserData             userData(scale, Vec2f(_horiBearingX, _horiBearingY), _outline);
    OSStatus             callbackResult;
    ATSUGlyphGetCubicPaths(horiFontStyle, glyphID, moveToProc, lineToProc, curveToProc,
        closePathProc, &userData, &callbackResult);
...
static void drawGlyph(ATSUTextLayout textLayout, const ATSLayoutRecord& layoutRecord,
    ATSUTextMeasurement xPos, ATSUTextMeasurement yPos, UInt32 width, UInt32 height, UInt8* dst,
    UInt32 pitch) {
...
// Author: pdaehne
//----------------------------------------------------------------------
const TextPixmapGlyph& TextMacPixmapFace::getPixmapGlyph(
    const ATSLayoutRecord& layoutRecord, bool horizontal) {
  ...
  ATSGlyphScreenMetrics glyphScreenMetrics;
  OSStatus              result =
      ATSUGlyphGetScreenMetrics(_horiFontStyle, 1, &glyphID, 0, true, true, &glyphScreenMetrics);
  if (result != noErr)
    return _emptyGlyph;
  ...
  // Determine vertical glyph metrics
  result =
      ATSUGlyphGetScreenMetrics(_vertFontStyle, 1, &glyphID, 0, true, true, &glyphScreenMetrics);
  if (result != noErr)
    return _emptyGlyph;
  ...
  if (horizontal == false) {
    ...
    yPos =
        FloatToFixed(glyphScreenMetrics.topLeft.y) - ((layoutRecord.realPos + fixed1) & 0xffff0000);
  }
  ...
  auto_ptr<TextPixmapGlyph> glyph(new TextMacPixmapGlyph(glyphID, width, height, horiAdvance,
      horiBearingX, horiBearingY, vertAdvance, vertBearingX, vertBearingY, pixmap));
...
TextMacPixmapGlyph::TextMacPixmapGlyph(Index glyphIndex, UInt32 width, UInt32 height,
    Real32 horiAdvance, Int32 horiBearingX, Int32 horiBearingY, Real32 vertAdvance,
    Int32 vertBearingX, Int32 vertBearingY, UInt8* pixmap)
    : TextPixmapGlyph() {
  _glyphIndex = glyphIndex;
  _width = _pitch = width;
  _height         = height;
...
TextMacTXFFace::TextMacTXFFace(ATSUStyle horiFontStyle, const TextTXFParam& param)
  ...
  // Try to get information about the face
  getFaceInfo(horiFontStyle, vertFontStyle, _family, _style, _horiAscent, _horiDescent, _vertAscent,
      _vertDescent);
...
void TextMacTXFFace::createGlyphs(ATSUStyle horiFontStyle, ATSUStyle vertFontStyle,
    ATSUTextLayout horiTextLayout, ATSUTextLayout vertTextLayout, const TextTXFParam& param) {
  ...
  OSStatus result = ATSUSetTextPointerLocation(horiTextLayout, &(utf16Characters[0]),
      kATSUFromTextBeginning, kATSUToTextEnd, utf16Characters.size());
  if (result != noErr)
    return;
  ...
  result = ATSUGlyphGetScreenMetrics(horiFontStyle, numRecords, &(layoutRecords[0].glyphID),
      sizeof(ATSLayoutRecord), true, true, &(horiMetrics.front()));
  if (result != noErr) {
    ATSUDirectReleaseLayoutDataArrayPtr(
        0, kATSUDirectDataLayoutRecordATSLayoutRecordCurrent, (void**)&layoutRecords);
    return;
  ...
  result = ATSUGlyphGetScreenMetrics(vertFontStyle, numRecords, &(layoutRecords[0].glyphID),
      sizeof(ATSLayoutRecord), true, true, &(vertMetrics.front()));
  if (result != noErr) {
    ATSUDirectReleaseLayoutDataArrayPtr(
        0, kATSUDirectDataLayoutRecordATSLayoutRecordCurrent, (void**)&layoutRecords);
    return;
  ...
  for (j = 0; j < numRecords - 1; ++j) {
    ...
    _glyphMap.insert(
        GlyphMap::value_type(c, new TextMacTXFGlyph(c, _scale, horiMetrics[j], vertMetrics[j])));
  }
  ...
  for (j = 0; j < numRecords - 1; ++j) {
    ...
    // Create pixmap
    ATSUTextMeasurement xPos =
        FloatToFixed(-horiMetrics[j].topLeft.x) - (layoutRecords[j].realPos & 0xffff0000);
...
// Author: pdaehne
//----------------------------------------------------------------------
TextMacTXFGlyph::TextMacTXFGlyph(Index glyphIndex, Real32 scale,
    const ATSGlyphScreenMetrics& horiMetrics, const ATSGlyphScreenMetrics& vertMetrics)
    : TextTXFGlyph() {
  _glyphIndex = glyphIndex;
  _scale      = scale;
