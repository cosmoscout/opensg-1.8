...
#ifndef __sgi
OSG_BEGIN_NAMESPACE
template OSG_SYSTEMLIB_DLLMAPPING class ScanParseFieldTypeMapper<ScanParseSkel>;
template OSG_SYSTEMLIB_DLLMAPPING class VRMLNodeFactory<ScanParseFieldTypeMapper<ScanParseSkel>>;
OSG_END_NAMESPACE
#endif
...
void VRMLFile::scanStream(std::istream& is) {
  ...
  FINFO(("Full Time : %lf | Use Time %lf\n", getSystemTime() - startTime, useTime));
}
...
void VRMLFile::beginNode(const Char8* szNodeTypename, const Char8* szNodename) {
  ...
  pNewNode = _pCurrNodeDesc->beginNode(szNodeTypename, szNodename, _pCurrentFC);
  ...
  if (szNodename != NULL) {
    if (pNewNode != NullFC) {
      if (pNewNode->getType().isNode() == true) {
        ...
        NameContainerMap::iterator mIt = _nameFCMap.find(IDStringLink(szNodename));
...
#ifdef OSG_DEBUG_VRML
          indentLog(VRMLNodeDesc::getIndent(), PINFO);
          PINFO << "Fieldcontainer " << szNodename << " added to map " << std::endl;
#endif
        }
...
#ifdef OSG_DEBUG_VRML
        indentLog(VRMLNodeDesc::getIndent(), PINFO);
        PINFO << "Fieldcontainer " << szNodeTypename << " is neither node nor nodecore "
              << std::endl;
#endif
...
#ifdef OSG_DEBUG_VRML
      indentLog(VRMLNodeDesc::getIndent(), PINFO);
      PINFO << "Desc for " << szNodename << " added to map " << std::endl;
#endif
    } else {
#ifdef OSG_DEBUG_VRML
      indentLog(VRMLNodeDesc::getIndent(), PINFO);
      PINFO << "Fieldcontainer " << szNodeTypename << "is empty, save on end " << std::endl;
#endif
...
void VRMLFile::endNode(void) {
  ...
  if (_pCurrNodeDesc->getOnEndSave() == true) {
    SLOG << "Fieldcontainer " << _pCurrNodeDesc->getSavename() << " on end Save " << std::endl;

    NameContainerMap::iterator mIt = _nameFCMap.find(IDStringLink(_pCurrNodeDesc->getSavename()));

    if (mIt == _nameFCMap.end()) {
      _nameFCMap[IDString(_pCurrNodeDesc->getSavename())] = _pCurrNodeDesc->getSaveFieldContainer();

      SLOG << "Fieldcontainer " << _pCurrNodeDesc->getSavename() << " added to map " << std::endl;
    }
  ...
  if (_pCurrentFC != NullFC) {
    if (_pCurrentFC->getType().isNode() == true) {
      ...
      endEditCP(pNode->getCore(), _bvChanged.first,
          ChangedOrigin::Abstract | ChangedOrigin::AbstrIgnoreCore |
              ChangedOrigin::AbstrIgnoreChild | ChangedOrigin::AbstrCheckValid);
    }

    endEditCP(_pCurrentFC, _bvChanged.first,
        ChangedOrigin::Abstract | ChangedOrigin::AbstrIgnoreCore | ChangedOrigin::AbstrIgnoreChild |
...
void VRMLFile::beginField(const Char8* szFieldname, const UInt32 OSG_VRML_ARG(uiFieldTypeId)) {
  if (szFieldname == NULL)
    return;

#ifdef OSG_DEBUG_VRML
  indentLog(VRMLNodeDesc::getIndent(), PINFO);
  PINFO << "VRMLFile::beginField : looking for " << szFieldname << " (" << uiFieldTypeId << " | "
        << _pCurrentField << " | " << _pCurrentFieldDesc << ")" << std::endl;
...
void VRMLFile::beginFieldDecl(
  ...
  _pCurrNodeDesc->getFieldAndDesc(_pCurrentFC, szFieldName, _pCurrentField, _pCurrentFieldDesc);
}
...
#ifdef OSG_DEBUG_VRML
  indentLog(VRMLNodeDesc::getIndent(), PINFO);
  PINFO << "VRMLFile::getFieldType : Got Field and type " << returnValue << " " << _pCurrentField
        << " " << _pCurrentFieldDesc << " ";
...
void VRMLFile::use(const Char8* szName) {
...
#ifdef OSG_DEBUG_VRML
  indentLog(VRMLNodeDesc::getIndent(), PINFO);
  PINFO << "VRMLFile::use : looking for " << szName << std::endl;
  ...
  if (pUsedFC == NullFC) {
    PWARNING << "No fieldContainer with name found to use" << szName << std::endl;
  } else {
    // assign nodepointer to current sf|mf field

    VRMLNodeDesc* pDesc = NULL;

    NameDescriptionMap::iterator mIt = _nameDescMap.find(IDStringLink(szName));
...
PROTO CylinderSensor [
  ...
  beginProto("CylinderSensor");
  {
    ...
    beginEventOutDecl("SFRotation", Self::OSGsfRotation, "rotation_changed");
    endEventDecl();
...
PROTO ProximitySensor [
  ...
  beginProto("ProximitySensor");
  {
    ...
    beginEventOutDecl("SFRotation", Self::OSGsfRotation, "orientation_changed");
    endEventDecl();
...
static Action::ResultE modifyMaterial(NodePtr& node) {
  ...
  if (matc != NullFC) {
    ...
    if (!matc->isTransparent() && img != NullFC && img->getBpp() == 4) {
...
NodePtr VRMLFile::getRoot(void) {
  ...
  if (_pSceneRootNode != NullFC) {
    traverse(
        _pSceneRootNode, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(modifyMaterial));
  }
...
void VRMLFile::initIntExtFieldTypeMapper(void) {
  Self::setIntExtMapping(SFBool::getClassType().getId(), ScanParseSkel::OSGsfBool);

  Self::setIntExtMapping(SFColor3f::getClassType().getId(), ScanParseSkel::OSGsfColor);

  Self::setIntExtMapping(SFReal32::getClassType().getId(), ScanParseSkel::OSGsfFloat);

  Self::setIntExtMapping(SFImagePtr::getClassType().getId(), ScanParseSkel::OSGsfImage);

  Self::setIntExtMapping(SFInt32::getClassType().getId(), ScanParseSkel::OSGsfInt32);

  Self::setIntExtMapping(SFGLenum::getClassType().getId(), ScanParseSkel::OSGsfInt32);

  Self::setIntExtMapping(SFQuaternion::getClassType().getId(), ScanParseSkel::OSGsfRotation);

  Self::setIntExtMapping(SFString::getClassType().getId(), ScanParseSkel::OSGsfString);

  Self::setIntExtMapping(SFTime::getClassType().getId(), ScanParseSkel::OSGsfTime);

  Self::setIntExtMapping(SFVec2f::getClassType().getId(), ScanParseSkel::OSGsfVec2f);

  Self::setIntExtMapping(SFVec3f::getClassType().getId(), ScanParseSkel::OSGsfVec3f);

  Self::setIntExtMapping(MFColor3f::getClassType().getId(), ScanParseSkel::OSGmfColor);

  Self::setIntExtMapping(MFReal32::getClassType().getId(), ScanParseSkel::OSGmfFloat);

  Self::setIntExtMapping(MFInt32::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  Self::setIntExtMapping(MFGLenum::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  Self::setIntExtMapping(MFQuaternion::getClassType().getId(), ScanParseSkel::OSGmfRotation);

  Self::setIntExtMapping(MFString::getClassType().getId(), ScanParseSkel::OSGmfString);

  Self::setIntExtMapping(MFTime::getClassType().getId(), ScanParseSkel::OSGmfTime);

  Self::setIntExtMapping(MFVec3f::getClassType().getId(), ScanParseSkel::OSGmfVec3f);

  Self::setIntExtMapping(MFVec2f::getClassType().getId(), ScanParseSkel::OSGmfVec2f);

  Self::setIntExtMapping(SFNodePtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(MFNodePtr::getClassType().getId(), ScanParseSkel::OSGmfNode);

  /* To Base Type Mappings */

  Self::setIntExtMapping(SFUInt8::getClassType().getId(), ScanParseSkel::OSGsfInt32);

  Self::setIntExtMapping(MFUInt8::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  Self::setIntExtMapping(SFUInt32::getClassType().getId(), ScanParseSkel::OSGsfInt32);

  Self::setIntExtMapping(MFUInt32::getClassType().getId(), ScanParseSkel::OSGmfInt32);

  /* To Node Mappings */

  Self::setIntExtMapping(SFNodeCorePtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFMaterialPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoPTypesPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoIndicesPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoPLengthsPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoColorsPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoPositionsPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoNormalsPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFGeoTexCoordsPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(MFStateChunkPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  Self::setIntExtMapping(SFAttachmentMap::getClassType().getId(), ScanParseSkel::OSGmfNode);

  Self::setIntExtMapping(SFFieldContainerPtr::getClassType().getId(), ScanParseSkel::OSGsfNode);

  /* extended types */

  Self::setIntExtMapping(SFMatrix::getClassType().getId(), ScanParseSkel::OSGsfMatrix);

  Self::setIntExtMapping(SFPnt3f::getClassType().getId(), ScanParseSkel::OSGsfPnt3f);

  Self::setIntExtMapping(MFPnt3f::getClassType().getId(), ScanParseSkel::OSGmfPnt3f);
}

void VRMLFile::initExtIntFieldTypeMapper(void) {
  Self::setExtIntMapping(ScanParseSkel::OSGsfBool, SFBool::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfColor, SFColor3f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfFloat, SFReal32::getClassType().getId());
  ...
  Self::setExtIntMapping(ScanParseSkel::OSGsfImage, SFImagePtr::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfInt32, SFInt32::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfRotation, SFQuaternion::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfString, SFString::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfTime, SFTime::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfVec2f, SFVec2f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfVec3f, SFVec3f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfColor, MFColor3f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfFloat, MFReal32::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfInt32, MFInt32::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfRotation, MFQuaternion::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfString, MFString::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfTime, MFTime::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfVec3f, MFVec3f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfVec2f, MFVec2f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfNode, SFNodePtr::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfNode, MFNodePtr::getClassType().getId());

  /* extended types */

  Self::setExtIntMapping(ScanParseSkel::OSGsfMatrix, SFMatrix::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGsfPnt3f, SFPnt3f::getClassType().getId());

  Self::setExtIntMapping(ScanParseSkel::OSGmfPnt3f, MFPnt3f::getClassType().getId());
}

/* remove this, if there is a general methode to find containers */
FieldContainerPtr VRMLFile::findFCByName(const Char8* szName, NodePtr pNode) {
  ...
  pNodename = NamePtr::dcast(pNode->findAttachment(Name::getClassType().getGroupId()));
  ...
  if (pCore != NullFC) {
    pNodename = NamePtr::dcast(pCore->findAttachment(Name::getClassType().getGroupId()));
  ...
  for (i = pNode->getMFChildren()->begin(); i != pNode->getMFChildren()->end(); i++) {
...
FieldContainerPtr VRMLFile::findReference(const Char8* szName) {
  ...
  NameContainerMap::iterator mIt = _nameFCMap.find(IDStringLink(szName));
...
void VRMLFile::setContainerFieldValue(const FieldContainerPtr& pFC) {
  if (_pCurrentField != NULL && pFC != NullFC) {
    if (_pCurrentFC != NullFC && _pCurrentField == _pCurrentFC->getField("children")) {
#ifdef OSG_DEBUG_VRML
      indentLog(VRMLNodeDesc::getIndent(), PINFO);
      PINFO << "Add Child " << &(*_pCurrentFC) << " " << &(*pFC) << std::endl;
#endif
      NodePtr pNode      = NodePtr::dcast(_pCurrentFC);
      NodePtr pChildNode = NodePtr::dcast(pFC);

      pNode->addChild(pChildNode);
    } else if (_pCurrentFC != NullFC && _pCurrentField == _pCurrentFC->getField("core")) {
#ifdef OSG_DEBUG_VRML
      indentLog(VRMLNodeDesc::getIndent(), PINFO);
      PINFO << "Add Core " << &(*_pCurrentFC) << " " << &(*pFC) << std::endl;
#endif
