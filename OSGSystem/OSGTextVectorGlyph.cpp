...
// Author: afischle
//----------------------------------------------------------------------
static void evalCubicBezierCurve(const Vec2f& pt1, const Vec2f& pt2, const Vec2f& pt3,
    const Vec2f& pt4, const UInt32 level, vector<Vec2f>& coords) {
...
static void evalBezierCurve(const TextVectorGlyph::Contour& contour, UInt32& index,
  ...
  case TextVectorGlyph::Point::PT_QUAD:
    ...
    evalQuadraticBezierCurve(
        contour[index].pos, contour[index + 1].pos, contour[index + 2].pos, level, coords);
    index += 2;
    break;
  ...
  case TextVectorGlyph::Point::PT_CUBIC:
    ...
    evalCubicBezierCurve(contour[index].pos, contour[index + 1].pos, contour[index + 2].pos,
        contour[index + 3].pos, level, coords);
    index += 3;
    break;
  default:
    assert(false);
...
static void OSG_APIENTRY gluTessCombineDataCB(GLdouble coords[3], void* vertexData[4],
    GLfloat weight[4], void** outDatab, void* polygonData) {
  TextVectorGlyph::PolygonOutline* outline =
      reinterpret_cast<TextVectorGlyph::PolygonOutline*>(polygonData);
  assert(outDatab != 0);
...
const TextVectorGlyph::PolygonOutline& TextVectorGlyph::getLines(UInt32 level) const {
  ...
  PolygonOutline& newOutline =
      _polygonOutlineMap.insert(PolygonOutlineMap::value_type(level, PolygonOutline()))
          .first->second;
  ...
  gluTessCallback(tess, GLU_TESS_BEGIN_DATA, reinterpret_cast<OSGGLUfuncptr>(gluTessBeginDataCB));
  gluTessCallback(tess, GLU_TESS_END_DATA, reinterpret_cast<OSGGLUfuncptr>(gluTessEndDataCB));
  gluTessCallback(
      tess, GLU_TESS_COMBINE_DATA, reinterpret_cast<OSGGLUfuncptr>(gluTessCombineDataCB));
  gluTessCallback(tess, GLU_TESS_VERTEX_DATA, reinterpret_cast<OSGGLUfuncptr>(gluTessVertexDataCB));
...
const TextVectorGlyph::Normals& TextVectorGlyph::getNormals(UInt32 level) const {
  ...
  for (iIt = outline.contours.begin(); iIt != outline.contours.end(); ++iIt, ++oriIt) {
    ...
    Vec2f prevEdgeNormal =
        computeEdgeNormal(outline.coords[end - 1], outline.coords[start], (*oriIt) == CW);
    while (index < end) {
      ...
      Vec2f nextEdgeNormal =
          computeEdgeNormal(outline.coords[index], outline.coords[nextIndex], (*oriIt) == CW);
...
// Author: afischle
//----------------------------------------------------------------------
static Int32 calcWindingNumber(
    const vector<Vec2f>& coords, UInt32 start, UInt32 end, const Vec2f& point) {
...
// Author: afischle
//----------------------------------------------------------------------
static bool isInteriorPoint(
    const Vec2f& point, const TextVectorGlyph::PolygonOutline& outline, GLenum windingRule) {
