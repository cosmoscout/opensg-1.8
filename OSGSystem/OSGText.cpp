...
bool Text::fillTXFGeo(Geometry& mesh, bool isNew, std::vector<std::string>& lineVec) {
  Int32             numVerts;
...
bool Text::fillTXFArrays(std::vector<std::string>& lineVec, Pnt3f* points, Vec2f* texcoords) {
  Int32         j, k;
  UInt32        i;
  ...
  for (k = lStart; k != lStop; k += lStep) {
    ...
    for (i = sStart; i != sStop; i += sStep) {
      for (j = 0; j < 4; j++, points++, texcoords++) {
        ...
        points->setValues(currentGlyph->getVertexCoords(j)[0] / scale + xOff,
            currentGlyph->getVertexCoords(j)[1] / scale + yOff, 0.0);

        texcoords->setValues(
            currentGlyph->getTextureCoords(j)[0], currentGlyph->getTextureCoords(j)[1]);
      }
...
bool Text::fillImage(ImagePtr& image, std::vector<std::string>& lineVec, Color4ub* fg, Color4ub* bg,
    bool forcePower2, Real32* maxX, Real32* maxY, ImageCreationMode OSG_CHECK_ARG(creationMode),
    MergeMode OSG_CHECK_ARG(mergeMode), Int32 pixelDepth, bool bConvertToBitmap) const {
  ImageFontGlyph*** g;
  UChar8*           img = 0;
  const Int32*      res;
  const Char8*      text = 0;
  ...
  if (_fontInstance) {
    ...
    for (line = 0; line < (Int32)lineVec.size(); line++) {
      ...
      for (i = 0; i < (Int32)strlen(text); i++) {
        ...
        if (g[line][i]) {
          ...
          width += (i + 1 == (Int32)strlen(text) ? g[line][i]->getImageSize()[0]
                                                 : g[line][i]->getAdvance());
          tmpMinY =
              g[line][i]->getBoundingBox()[2] < tmpMinY ? g[line][i]->getBoundingBox()[2] : tmpMinY;
          tmpMaxY =
              g[line][i]->getBoundingBox()[3] > tmpMaxY ? g[line][i]->getBoundingBox()[3] : tmpMaxY;
        }
      }
      ...
      if (!tmpMinY && !tmpMaxY) { // TXF-character not present -> all blanks..
        tmpMaxY = (Int32)osgfloor((Real32)_fontInstance->getBaselineSkip());
      }
    ...
    for (i = 0; i < overallHeight; i++) {
      memcpy(imageBuffer + i * overallWidth * pixelDepth, row, overallWidth * pixelDepth);
    }
    ...
    for (i = 0; i != (Int32)strlen(lineVec[line].c_str()); i++) {
      ...
      tmpMaxY =
          g[line][i]->getBoundingBox()[3] > tmpMaxY ? g[line][i]->getBoundingBox()[3] : tmpMaxY;
      tmpWidth +=
          (i + 1 == (Int32)strlen(text) ? g[line][i]->getImageSize()[0] : g[line][i]->getAdvance());
    }
    ...
    for (line = 0; line < (Int32)lineVec.size(); line++) {
      ...
      for (i = strStart; i != strEnd; i += strStep) {
        ...
        for (j = res[1] - 1; j >= 0; j--) {
          ...
          dstPixel = imageBuffer + ((pen_y + j) * overallWidth * pixelDepth) + pen_x;
      ...
      if (line + 1 < (Int32)lineVec.size()) {
        ...
        if (_justifyMajor == MIDDLE_JT || _justifyMajor == END_JT) {
          for (i = 0; i != (Int32)strlen(lineVec[line + 1].c_str()); i++) {
            tmpWidth += (Int32)osgfloor(g[line + 1][i]->getAdvance());
          }
        }
        ...
        if (line + 1 == (Int32)lineVec.size() - 1) {
          ...
          yoff +=
              (_topToBottom ? -1 : 1) * (Int32)osgfloor((Real32)_fontInstance->getBaselineSkip() *
    ...
    bool retval = image->set(pixelFormat, // Image::OSG_RGB_PF,
        overallWidth, overallHeight, 1, 1, 1, 0.0, imageBuffer);
    delete[] imageBuffer;
    return retval;
  }
...
bool Text::fillGeo(Geometry& mesh, std::vector<std::string>& lineVec, Real32 precision,
    Real32 extFac, Real32 OSG_CHECK_ARG(maxExtend), MeshCreationMode creationMode,
  ...
  if (_fontInstance) {
    ...
    for (line = 0; line < (Int32)lineVec.size(); line++) {
      ...
      for (i = 0; i < (Int32)strlen(text); i++) {
        ...
        if (g[line][i]) {
          ...
          numVertices += g[line][i]->getNumPoints() * (extFac != 0.0 ? 2 : 1);
          if (extFac != 0.0) {
      ...
      if (numNormals) {
        ...
        if (!line) {
          ...
          zCoord = g[line][j]->getPoints()[0][2] != 0.0
                       ? g[line][j]->getPoints()[0][2] / fabs(g[line][j]->getPoints()[0][2])
                       : -1;
      ...
      for (strOff = strStart; strOff != strEnd; strOff += strStep) {
        ...
        if (extFac != 0.0) {
          ...
          for (i = 0; i < g[line][strOff]->getNumNormals(); i++, normalOff++) {
            normals->setValue(
                Vec3f((g[line][strOff]->getNormals())[i][0], (g[line][strOff]->getNormals())[i][1],
        ...
        if (creationMode == FILL_TEX_CHAR_MCM) {
          ...
          diff    = extFac == 0.0 ? vertOff : glyphOff + (vertOff - glyphOff) / 2;

          for (i = glyphOff; i < diff; i++) {
            texCoords->setValue(
                Vec2f((g[line][strOff]->getPoints())[i - glyphOff][0] / localWidth,
                    ((g[line][strOff]->getPoints())[i - glyphOff][1] + descent) / localHeight),
                i);
          }

          for (i = diff; i < vertOff; i++) {
            texCoords->setValue(
                Vec2f((g[line][strOff]->getPoints())[i - diff][0] / localWidth,
                    ((g[line][strOff]->getPoints())[i - diff][1] + descent) / localHeight),
                i);
          }
        }
    ...
    if (creationMode == FILL_TEX_CHAR_MCM || creationMode == FILL_TEX_ALL_MCM) {
      mesh.getIndexMapping().push_back(Geometry::MapPosition | Geometry::MapTexCoords);
    } else {
