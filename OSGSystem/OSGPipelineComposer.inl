...
UInt32 PipelineComposer::getMinMaxOcclude(DepthT& depth, ColorT& color, DepthInfo* result) {
  UInt32      gx, gy, x, y;
  TileBuffer* tile;
  DepthT      dmin, dmax;
  UInt32      count      = 0;
  ...
  for (y = 0; y < _composeTilesY; ++y) {
    for (x = 0; x < _composeTilesX; ++x) {
      ...
      if (tile->depth.min == (DepthT)-1 && tile->depth.max == (DepthT)-1) {
        if (!reuseEmpty) {
          reuseEmpty = result;
          result++;
          count++;
...
void PipelineComposer::setTransInfo(DepthT& depth, ColorT& color) {
  ...
  for (y = 0; y < _composeTilesY; ++y) {
    for (x = 0; x < _composeTilesX; ++x) {
      tile = getComposeTileBuffer(x, y);
      tile->trans =
          transInfo[x + y * _composeTilesX + clusterId() * _composeTilesX * _composeTilesY];
      // occluded
      if (tile->trans.empty)
        tile->empty = true;
    }
...
void PipelineComposer::calculateTransInfo(DepthT& depth, ColorT& color) {
  ...
  for (y = 0; y < _composeTilesY; ++y) {
    for (x = 0; x < _composeTilesX; ++x) {
      ...
      for (int id1 = 0; id1 < count; ++id1) {
        if (_groupInfo[id1]->depth.occlude) {
          for (int id2 = 0; id2 < count; ++id2) {
            if (id1 != id2 && _groupInfo[id1]->depth.max < _groupInfo[id2]->depth.min &&
      ...
      // sort
      std::sort(_groupInfo.begin(), _groupInfo.end(), GroupInfoOrder());
      ...
      for (id = count - 1; id >= 0; --id) {
        ...
        if (_groupInfo[id]->depth.min == (DepthT)(-1)) {
          ...
          if (nextGroup) {
            ...
            if (_groupInfo[id]->depth.min > nextGroup->depth.max) {
      ...
      for (id = 0; id < count; ++id) {
        transInfo[_groupInfo[id]->id * _composeTilesX * _composeTilesY + x + y * _composeTilesX] =
            _groupInfo[id]->trans;
      }
#if 0
            for(id = 0 ; id < count ; ++id)
...
void PipelineComposer::clientCompose(DepthT& depth, ColorT& color) {
  ...
  while (recvCount--) {
    ...
    dstLen = lzf_decompress(&src[0], srcLen, &(readTile->header), 512 * 512 * 8);
#else
...
#endif
    _statistics.bytesIn +=
        sizeof(readTile->header) + readTile->header.w * readTile->header.h * sizeof(ColorT);
    // draw
    glRasterPos2f(readTile->header.x * getTileSize(), readTile->header.y * getTileSize());

    glDrawPixels(readTile->header.w, readTile->header.h, _colorFormat, _colorType, readTile->data);
  }
}
  ...
  for (ty = 0; ty < _composeTilesY; ++ty) {
    for (tx = 0; tx < _composeTilesX; ++tx) {
      ...
      if (!tile->empty && tile->trans.first) {
        ...
        tile->dstConnection = clusterWindow()->getNetwork()->getConnection(tile->trans.sendTo);
...
//        printf("Ratio %f\n",srcLen*100.0/dstLen);
    ...
    if (readTile->header.depth) {
      ...
      // compose
      dstDepthEnd = (DepthT*)(tile->data + tile->dataSize);
...
#ifdef COMPRESS_IMAGES
      dst[0] = lzf_compress(
          &(tile->header), tile->dataSize + sizeof(tile->header), &dst[1], 512 * 512 * 8);
      tile->dstConnection->put(&dst[0], dst[0] + sizeof(UInt32));
      _statistics.bytesOut += dst[0] + sizeof(UInt32);
#else

      tile->dstConnection->put(&(tile->header), tile->dataSize + sizeof(tile->header));
#endif
    } else {
      tile->header.depth = false;
#ifdef COMPRESS_IMAGES
      dst[0] = lzf_compress(
          &(tile->header), tile->colorSize + sizeof(tile->header), &dst[1], 512 * 512 * 8);
      tile->dstConnection->put(&dst[0], dst[0] + sizeof(UInt32));
      _statistics.bytesOut += dst[0] + sizeof(UInt32);
#else
      tile->dstConnection->put(&(tile->header), tile->colorSize + sizeof(tile->header));
#endif
    }
...
template <class DepthT, class ColorT>
void PipelineComposer::readBuffer(DepthT& depth, ColorT& color, ViewportPtr port) {
  ...
  for (ty = 0; ty < _readTilesY; ++ty) {
    for (tx = 0; tx < _readTilesX; ++tx) {
...
#endif
      // read depth
      glReadPixels(x, y, w, h, GL_DEPTH_COMPONENT, _depthType, tile->data + tile->colorSize);
      ...
      depthPtr = (DepthT*)(tile->data + tile->colorSize);
      // new version
      occlude = checkDepth<DepthT, (DepthT)-1>(depthPtr, depthMin, depthMax, w * h);
#endif
      ...
      if (!tile->empty) {
        glReadPixels(x, y, w, h, _colorFormat, _colorType, tile->data);
      }
    }
  }
...
#define OSGPIPELINECOMPOSER_INLINE_CVSID "@(#)$Id: $"
