...
UInt32 DVRMtexLUTShader::_arbMultitexture = Window::invalidExtensionID;

UInt32 DVRMtexLUTShader::_nvRegisterCombiners = Window::invalidExtensionID;

UInt32 DVRMtexLUTShader::_funcMultiTexCoord2dARB = Window::invalidFunctionID;

UInt32 DVRMtexLUTShader::_funcCombinerParameteriNV = Window::invalidFunctionID;

UInt32 DVRMtexLUTShader::_funcCombinerParameterfvNV = Window::invalidFunctionID;

UInt32 DVRMtexLUTShader::_funcCombinerInputNV = Window::invalidFunctionID;

UInt32 DVRMtexLUTShader::_funcCombinerOutputNV = Window::invalidFunctionID;

UInt32 DVRMtexLUTShader::_funcFinalCombinerInputNV = Window::invalidFunctionID;
...
DVRMtexLUTShader::DVRMtexLUTShader(void)
  ...
  _funcMultiTexCoord2dARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glMultiTexCoord2dARB", _arbMultitexture);

  _funcCombinerParameteriNV =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glCombinerParameteriNV", _nvRegisterCombiners);
...
void DVRMtexLUTShader::dump(UInt32, const BitVector) const {
...
bool DVRMtexLUTShader::initialize(DVRVolume* volume, DrawActionBase* action) {
  ...
  if (getLutMode() != LM_AUTO) {
    ...
    if (isModeSupported(action, getLutMode(), volume->getTextureMode(win)) == true) {
      SWARNING << "DVRMtexLUTShader - User specified lookup table mode " << int(getLutMode())
               << std::endl;

      m_nTexturePaletteMode = getLutMode();
    } else {
      SWARNING << "DVRMtexLUTShader - Unsupported lookup table mode " << int(getLutMode())
...
void DVRMtexLUTShader::activate(DVRVolume* volume, DrawActionBase* action) {
  ...
  volume = volume;
  action = action; // calm down compiler
#else
...
typedef void(OSG_APIENTRY* CombinerParameterfvNVFunc)(GLenum, const GLfloat*);

typedef void(OSG_APIENTRY* MultiTexCoord2dARBFunc)(GLenum, GLdouble, GLdouble);

//! callback for rendering slices
void DVRMtexLUTShader::renderSlice(
  ...
  volume   = volume;
  action   = action;
  data     = data; // calm down compiler
  vertices = vertices;
  values   = values;
#else
  Window* win = action->getWindow();

  if (!win->hasExtension(_nvRegisterCombiners) || !win->hasExtension(_arbMultitexture)) {
    return;
  }
