...
static jas_stream_ops_t istream_ops = {readIStream, writeIStream, seekIStream, closeIStream};
...
static const Char8* jp2000SuffixArray[] = {"jp2"};

JP2ImageFileType JP2ImageFileType::_the(
    "image/jpeg2000", jp2000SuffixArray, sizeof(jp2000SuffixArray), OSG_READ_SUPPORTED);
...
std::string JP2ImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode + 4, "jP  ", 4) == 0 ? std::string(getMimeType()) : std::string();
}
...
*/
JP2ImageFileType::JP2ImageFileType(
