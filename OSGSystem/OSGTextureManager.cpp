...
TextureRecord::TextureRecord(
    ImagePtr img, UInt32 internal, UInt32 externalFormat, Int32 stage0, Int32 stage1)
...
//! Register texture
Int32 TextureManager::registerTexture(ImagePtr image, UInt32 internalFormat, UInt32 externalFormat,
    bool /*doBricking*/, Int32 textureStage0, Int32 textureStage1) {
  FDEBUG(
      ("TextureManager::registerTexture stage0: %d - stage1: %d\n", textureStage0, textureStage1));
  ...
  for (UInt32 i = 0; i < _registeredTextures.size(); i++) {
    ...
    if ((textureStage1 != -1) && (textureStage1 == _registeredTextures[i]->_textureStage0 ||
  ...
  TextureRecord* record =
      new TextureRecord(image, internalFormat, externalFormat, textureStage0, textureStage1);
...
void TextureManager::reloadTexture(Int32 id, DrawActionBase* action) {
  ...
  for (UInt32 k = 0; k < 3; k++) {
    _brickSets[k].reloadBrickTextures(action, _registeredTextures[id]->_textureStage0);

    _brickSets[k].reloadBrickTextures(action, _registeredTextures[id]->_textureStage1);
  }
}
...
void TextureManager::buildTextures(
    ChunkMaterialPtr material, DVRVolume* volume, TextureMode textureMode) {
  ...
  case TM_2D_Multi: {
    ...
    _brickSets[BrickSet::XY].buildBricks3D(volume, brickSizeZ, 1, BrickSet::XY);
    _brickSets[BrickSet::XZ].buildBricks3D(volume, brickSizeY, 1, BrickSet::XZ);
    _brickSets[BrickSet::YZ].buildBricks3D(volume, brickSizeX, 1, BrickSet::YZ);

    _brickSets[BrickSet::XY].buildBrickTextures(material, _registeredTextures, textureMode);
    _brickSets[BrickSet::XZ].buildBrickTextures(material, _registeredTextures, textureMode);
    _brickSets[BrickSet::YZ].buildBrickTextures(material, _registeredTextures, textureMode);
  ...
  case TM_3D: {
    // Ordinary 3D-Bricks
    Vec3f res              = vT->getResolution();
    Vec3f brickSubdivision = calcBrickSubdivision(
        int(res[0]), int(res[1]), int(res[2]), vT->getImage()->getBpp(), volume);

    Vec3f brickSize(Real32((int)(res[0] / brickSubdivision[0])),
        Real32((int)(res[1] / brickSubdivision[1])), Real32((int)(res[2] / brickSubdivision[2])));

    FDEBUG(("TextureManager::buildTextures - BrickSubdivision %d %d %d\n", brickSubdivision[0],
        brickSubdivision[1], brickSubdivision[2]));

    _brickSets[BrickSet::XY].buildBricks3D(volume, brickSize, volume->getBrickOverlap());
...
void TextureManager::dump(UInt32 /*uiIndent*/, const BitVector /*bvFlags*/) const {
  SLOG << "TextureManager:" << std::endl;

  for (UInt32 i = 0; i < _registeredTextures.size(); i++) {
    SLOG << "image:          " << _registeredTextures[i]->_image << std::endl;
    SLOG << "internalFormat: " << _registeredTextures[i]->_internalFormat << std::endl;
    SLOG << "textureStage0:  " << _registeredTextures[i]->_textureStage0 << std::endl;
    SLOG << "textureStage1:  " << _registeredTextures[i]->_textureStage1 << std::endl;
  }
}

Vec3f TextureManager::calcBrickSubdivision(
    Int32 resX, Int32 resY, Int32 resZ, Int32 dataSize, DVRVolume* volume) {
  ...
  case BRICK_SUBDIVIDE_ON_TEXTURE_MEMORY: {
    Real32 fMB_available = 1024.0F * 1024.0F * (Real32)volume->getBrickStaticMemoryMB();

    Real32 fMB_required = (Real32)(resX * resY * resZ * dataSize);

    Int32 nNumBricks     = (Int32)osgceil(fMB_required / fMB_available);
    ...
    while (nNumBricks > nRealNumBricks) {
      ...
      nRealNumBricks = (Int32)(subdivision[0] * subdivision[1] * subdivision[2]);
    }
    break;
  }
...
Brick* TextureManager::sortBricks(
    DrawActionBase* da, Matrix modelMat, Vec3f eyePoint, DVRVolume* /*volume*/, TextureMode mode) {
