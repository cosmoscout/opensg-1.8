  ...
  if (whichField & HistorySizeFieldMask) {
    ...
    for (i = 0; i < n; i++) {
      if (_history[i].size() == 0) {
        _history[i].resize(getHistorySize()[i], (getMaxValue()[i] - getMinValue()[i]) / 2.0f);
      }
    }
  }
}
...
/* */
void GraphicStatisticsForeground::addElement(StatElemDescBase& desc, UInt32 displayType, Vec2f pos,
    Vec2f size, Color4f highColor, Color4f lowColor, Color4f currentColor, Real32 minValue,
    Real32 maxValue, UInt32 flags, UInt32 historySize, std::string description) {
...
/*******************************************************/
void GraphicStatisticsForeground::drawAnalog(
    UInt32 elementID, StatElem* el, DrawActionBase* base, Viewport*) {
  // helper Var
  Real32 xdist = 0.0;
  ...
  if (getTextEnabled()) {
    ...
    drawString(
        base, real2String(value), TextLayoutParam::ALIGN_MIDDLE, TextLayoutParam::ALIGN_FIRST);
    glMatrixMode(GL_MODELVIEW);
    glPopMatrix();
...
/*******************************************************/
void GraphicStatisticsForeground::drawChart(
    UInt32 elementID, StatElem* el, DrawActionBase* base, Viewport* port) {
...
/*******************************************************/
void GraphicStatisticsForeground::drawBar(
    UInt32 elementID, StatElem* el, DrawActionBase* base, Viewport* port) {
  // The amount of the display to be used for the text on the bottom
  Real32 textHeight = 0.0;
  Real32 textWidth  = 0.0;
  ...
  else // if xdist > 0.5
  {
    ...
    glVertex2f(textWidth, textHeight);
    glVertex2f(textWidth, 1.0);
    glColor4f(CurrentColor[0], CurrentColor[1], CurrentColor[2], getColorCurrent()[elementID][3]);
    glVertex2f(0.5, 1.0);
    ...
    glVertex2f(0.5, textHeight);
    glVertex2f(0.5, 1.0);
    glColor4f(MaxColor[0] * ((xdist - 0.5f) * 2) + CurrentColor[0] * (1 - ((xdist - 0.5f) * 2)),
        MaxColor[1] * ((xdist - 0.5f) * 2) + CurrentColor[1] * (1 - ((xdist - 0.5f) * 2)),
        MaxColor[2] * ((xdist - 0.5f) * 2) + CurrentColor[2] * (1 - ((xdist - 0.5f) * 2)),
        getColorMax()[elementID][3]);
  ...
  if (_history[elementID].size() > 0) {
    ...
    glVertex2f((vsave - minV) / (maxV - minV) * (1.0f - 2 * textWidth) + textWidth, 1.0f);
    glVertex2f((vsave - minV) / (maxV - minV) * (1.0f - 2 * textWidth) + textWidth, textHeight);
    glEnd();
  }
...
// begin drawlinechart
void GraphicStatisticsForeground::drawLineChart(
    UInt32 elementID, StatElem* el, DrawActionBase* base, Viewport* port) {
  // The amount of the display to be used for the text on the bottom
  Real32 textHeight = 0.0;
  Real32 textWidth  = 0.0;
  ...
  if (getTextEnabled()) {
    // create some Strings to be drawn
    std::string valstr = getDescription()[elementID] + " " + real2String(realValue);
...
/* */
void GraphicStatisticsForeground::drawText(
    UInt32 elementID, StatElem* el, DrawActionBase* base, Viewport* port) {
  /* Get the current value and process it */
  Real32 value = Real32(el->getValue());
...
void GraphicStatisticsForeground::calcPosAndSize(
    const UInt32& id, Viewport* port, Vec2f* Position, Vec2f* Size) {
...
//! Draws a String
void GraphicStatisticsForeground::drawString(DrawActionBase* base, const std::string& text,
    TextLayoutParam::Alignment majorAlignment, TextLayoutParam::Alignment minorAlignment) {
