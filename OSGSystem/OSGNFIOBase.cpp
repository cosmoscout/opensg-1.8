...
bool NFIOBase::write(const NodePtr& node, std::ostream& os, const std::string& options) {
  _options.init(options);
...
FieldContainerPtr NFIOBase::readFC(std::istream& is, const std::string& options) {
  _options.init(options);
...
bool NFIOBase::writeFC(const FieldContainerPtr& fc, std::ostream& os, const std::string& options) {
  _options.init(options);
...
FieldContainerPtr NFIOBase::readFieldContainer(void) {
  ...
  for (std::list<fcInfo>::iterator i = _fieldList.begin(); i != _fieldList.end(); ++i) {
    chargeFieldPtr(*i);
  }
...
void NFIOBase::chargeFieldPtr(const fcInfo& info) {
  ...
  if (!info.isMultiField()) {
    ...
    if (fc != NullFC && fcType.isNode() && info._mask == Node::CoreFieldMask) {
    ...
    if (attContainer != NullFC && info._mask == AttachmentContainer::AttachmentsFieldMask) {
      attachment = true;
    }
...
std::string NFIOBase::readFCFields(
    const FieldContainerPtr& fc, const std::string& exclude, const std::string& endMarkers) {
  ...
  while (true) {
    ...
    if (fieldName.empty() ||
        (!endMarkers.empty() && endMarkers.find("'" + fieldName + "'") != std::string::npos)) {
    ...
    if (fDesc != NULL) {
      ...
      if (fieldType != field->getType().getCName()) {
        ...
        FWARNING(("NFIOBase::readFCPtr: found conflicting field types field '%s' with "
                  "types '%s' and '%s'!\n",
            fieldName.c_str(), fieldType.c_str(), field->getType().getCName()));

        if (size != fc->getBinSize(mask)) {
          // ok in this case we skip the field!
          FFATAL(("NFIOBase::readFCPtr: found conflicting field types with different sizes "
                  "skipping field '%s' with "
                  "types '%s' and '%s'!\n",
              fieldName.c_str(), fieldType.c_str(), field->getType().getCName()));
          _in->skip(size);
          continue;
...
void NFIOBase::readSFFieldContainerPtr(
...
void NFIOBase::readMFFieldContainerPtr(
...
void NFIOBase::readSFAttachmentMap(
    const FieldContainerPtr& fc, const BitVector& mask, Field* field, UInt32 noe) {
  _fieldList.push_back(fcInfo(fc, mask, field));
  fcInfo& info = _fieldList.back();
  UInt32  id;
...
void NFIOBase::getFCCount(const FieldContainerPtr& fc, UInt32& count) {
  ...
  for (UInt32 i = 1; i <= fcType.getNumFieldDescs(); ++i) {
    ...
    if (!fDesc->isInternal()) {
      // ignore node volume
      if (fcType == Node::getClassType() && fDesc->getFieldMask() == Node::VolumeFieldMask) {
        continue;
      }
...
void NFIOBase::writeFieldContainer(const FieldContainerPtr& fc) {
  ...
  for (std::list<FieldContainerPtr>::iterator i = _fcList.begin(); i != _fcList.end(); ++i) {
    lfc      = *i;
...
void NFIOBase::writeFCFields(
    const FieldContainerPtr& fc, const std::string& exclude, bool endMarker) {
  FieldContainerType& fcType = fc->getType();
    ...
    if (strstr(fieldType.c_str(), "Ptr") != NULL) {
      if (fieldType[0] == 'S' && fieldType[1] == 'F') // single field
      {
        AttachmentPtr attachment =
            AttachmentPtr::dcast(((SFFieldContainerPtr*)fieldPtr)->getValue());
      ...
      } else if (fieldType[0] == 'M' && fieldType[1] == 'F') // multi field
      {
        ...
        UInt32               noe    = mfield->size();
        for (UInt32 j = 0; j < noe; ++j) {
          AttachmentPtr attachment = AttachmentPtr::dcast((*(mfield))[j]);
          if (attachment != NullFC) {
            fc.setParentFieldPos(fDesc->getFieldId());
            beginEditCP(attachment, Attachment::ParentsFieldMask);
            attachment->getParents().clear();
...
void NFIOBase::fillAttachmentParents(const NodePtr& node) {
  ...
  traverse(node, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(clearAttachmentParent));
  _added_cores.clear();
  traverse(node, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(addAttachmentParent));
  _added_cores.clear();
}
...
NFIOBase::fcInfo::fcInfo(
    const FieldContainerPtr& fc, const BitVector& mask, UInt32 id, Field* field)
    : _fc(fc)
    , _mask(mask)
    , _id(id)
    , _field(field)
...
NFIOBase::fcInfo::fcInfo(const FieldContainerPtr& fc, const BitVector& mask, Field* field)
    : _fc(fc)
    , _mask(mask)
    , _id(0)
    , _field(field)
