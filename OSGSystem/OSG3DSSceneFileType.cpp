...
A3DSSceneFileType A3DSSceneFileType::_the(
    _suffixA, sizeof(_suffixA), false, 10, SceneFileType::OSG_READ_SUPPORTED);
 ...
 * \return
 */
A3DSSceneFileType::A3DSSceneFileType(const Char8* suffixArray[], UInt16 suffixByteCount,
...
NodePtr A3DSSceneFileType::createMesh(L3DS& scene, LMesh& mesh) const {
  ...
  for (UInt32 i = 0; i < mesh.GetTriangleCount(); ++i) {
    ...
    normals->getFieldPtr()->push_back(
        Vec3f(t.vertexNormals[0].x, t.vertexNormals[0].y, t.vertexNormals[0].z));
    normals->getFieldPtr()->push_back(
        Vec3f(t.vertexNormals[1].x, t.vertexNormals[1].y, t.vertexNormals[1].z));
    normals->getFieldPtr()->push_back(
        Vec3f(t.vertexNormals[2].x, t.vertexNormals[2].y, t.vertexNormals[2].z));
...
MaterialPtr A3DSSceneFileType::createMaterial(L3DS& scene, UInt32 id) const {
  ...
  beginEditCP(matc);
  matc->setAmbient(
      Color4f(m.GetAmbientColor().r, m.GetAmbientColor().g, m.GetAmbientColor().b, 1.0f - t));
  matc->setDiffuse(
      Color4f(m.GetDiffuseColor().r, m.GetDiffuseColor().g, m.GetDiffuseColor().b, 1.0f - t));
  matc->setSpecular(
      Color4f(m.GetSpecularColor().r, m.GetSpecularColor().g, m.GetSpecularColor().b, 1.0f - t));
  ...
  if (t > 0.0 || (image != NullFC && image->hasAlphaChannel())) {
