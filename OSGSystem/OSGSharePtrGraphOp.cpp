...
std::string SharePtrGraphOp::usage(void) {
  return "SharePtr: try to share FieldContainers in a subtree\n"
         ...
         "  includes (string, "
         "): comma-separated list of types to share\n"
         "  excludes (string, "
         "): comma-separated list of types not to share\n"
...
Action::ResultE SharePtrGraphOp::traverseLeave(NodePtr& /*node*/, Action::ResultE /*res*/) {
  return Action::Continue;
}
...
bool SharePtrGraphOp::isInList(const std::vector<std::string>& tlist, const FieldContainerPtr& fc) {
  for (UInt32 k = 0; k < tlist.size(); ++k) {
    FieldContainerType* fct = FieldContainerFactory::the()->findType(tlist[k].c_str());
    if (fct == NULL) {
  ...
  for (UInt32 i = 1; i <= fcount; ++i) {
    ...
    if (strstr(ftype.getCName(), "Ptr") != NULL) {
      if (fc_field->getCardinality() == FieldType::SINGLE_FIELD) {
        FieldContainerPtr ffc = ((SFFieldContainerPtr*)fc_field)->getValue();
 ...
 */
bool SharePtrGraphOp::isEqual(const osg::FieldContainerPtr& a, const osg::FieldContainerPtr& b) {
...
void SharePtrGraphOp::fillAttachmentParents(const NodePtr& node) {
  ...
  _added_cores.clear();
  OSG::traverse(
      node, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(clearAttachmentParent));
  _added_cores.clear();
  OSG::traverse(
      node, osgTypedFunctionFunctor1CPtrRef<Action::ResultE, NodePtr>(addAttachmentParent));
  _added_cores.clear();
}
