...
DVRIsoShader::DVRIsoShader(void)
  ...
  _ARB_multitexture = Window::registerExtension("GL_ARB_multitexture");

  _EXT_texture3D = Window::registerExtension("GL_EXT_texture3D");

  _EXT_secondary_color = Window::registerExtension("GL_EXT_secondary_color");

  _NV_register_combiners = Window::registerExtension("GL_NV_register_combiners");

  _NV_register_combiners2 = Window::registerExtension("GL_NV_register_combiners2");

  _SGI_color_matrix = Window::registerExtension("GL_SGI_color_matrix");

  _ARB_fragment_program = Window::registerExtension("GL_ARB_fragment_program");

  _funcActiveTextureARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glActiveTextureARB", _ARB_multitexture);

  _funcMultiTexCoord2dARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glMultiTexCoord2dARB", _ARB_multitexture);

  _funcTexImage3DEXT = Window::registerFunction(GL_FUNC_TEXIMAGE3D, _EXT_texture3D, 0x0102);

  _funcCombinerParameteriNV = Window::registerFunction(
      OSG_DLSYM_UNDERSCORE "glCombinerParameteriNV", _NV_register_combiners);

  _funcCombinerParameterfvNV = Window::registerFunction(
      OSG_DLSYM_UNDERSCORE "glCombinerParameterfvNV", _NV_register_combiners);

  _funcCombinerStageParameterfvNV = Window::registerFunction(
      OSG_DLSYM_UNDERSCORE "glCombinerStageParameterfvNV", _NV_register_combiners);

  _funcSecondaryColor3fEXT =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glSecondaryColor3fEXT", _EXT_secondary_color);

  _funcSecondaryColor3fvEXT =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glSecondaryColor3fvEXT", _EXT_secondary_color);

  _funcCombinerInputNV =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glCombinerInputNV", _NV_register_combiners);

  _funcCombinerOutputNV =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glCombinerOutputNV", _NV_register_combiners);

  _funcFinalCombinerInputNV = Window::registerFunction(
      OSG_DLSYM_UNDERSCORE "glFinalCombinerInputNV", _NV_register_combiners);
}
...
void DVRIsoShader::dump(UInt32, const BitVector) const {
...
bool DVRIsoShader::initialize(DVRVolume* volume, DrawActionBase* action) {
  ...
  if ((m_shadingMode == SM_REGISTER_COMBINERS_MULTI2D) ||
    ...
    m_textureId = volume->getTextureManager().registerTexture(
        m_gradientImage, nInternalFormat, nExternalFormat, true, 0, 1);
  } else {
    // init single texture
    m_textureId = volume->getTextureManager().registerTexture(
        m_gradientImage, nInternalFormat, nExternalFormat, true, 0, -1);
...
ImagePtr DVRIsoShader::createGradientImage(DVRVolumeTexturePtr volTex) {
    ...
    for (int y = 0; y < resY; y++) {
      for (int x = 0; x < resX; x++) {
        ...
        if (x == 0) {
          ...
          gradient =
              Vec3f((float)(dataXl - dataXr), (float)(dataYb - dataYt), (float)(dataZf - dataZn));

          if (gradient.length() != 0.0)
            gradient.normalize();
        }

        gradbuffer[4 * (z * zOff + y * yOff + x)] = (UChar8)(127.0 + gradient[0] * 127.0); // R

        gradbuffer[4 * (z * zOff + y * yOff + x) + 1] = (UChar8)(127.0 + gradient[1] * 127.0); // G

        gradbuffer[4 * (z * zOff + y * yOff + x) + 2] = (UChar8)(127.0 + gradient[2] * 127.0); // B

        gradbuffer[4 * (z * zOff + y * yOff + x) + 3] = volData[z * zOff + y * yOff + x]; // A
  ...
  m_gradientImage->set(Image::OSG_RGBA_PF, resX, resY, resZ, 1, 1, 0.0, gradbuffer);
...
void DVRIsoShader::getLightSources(
...
UInt8 DVRIsoShader::selectMode(DVRVolume* volume, DrawActionBase* action) {
  ...
  SWARNING << "DVRIsoShader - None of the implemented shading algorithms" << std::endl
           << "is supported by this hardware!" << std::endl;
...
bool DVRIsoShader::isModeSupported(DVRVolume* volume, DrawActionBase* action, UInt8 mode) {
  ...
  case SM_COLORMATRIX_2D:
    ...
    return numStencil > 0 && (win->hasExtension(_SGI_color_matrix) || checkGLVersion(1.2)) &&
           !forceTexture3D;

  case SM_COLORMATRIX_3D:
    glGetIntegerv(GL_STENCIL_BITS, &numStencil);

    return numStencil > 0 && win->hasExtension(_EXT_texture3D) &&
           (win->hasExtension(_SGI_color_matrix) || checkGLVersion(1.2)) && !forceTexture2D;

  case SM_REGISTER_COMBINERS_2D:
    return win->hasExtension(_NV_register_combiners) && !forceTexture3D;

  case SM_REGISTER_COMBINERS_MULTI2D:
    return win->hasExtension(_ARB_multitexture) && win->hasExtension(_NV_register_combiners) &&
           !forceTexture3D;

  case SM_REGISTER_COMBINERS_3D:
    return win->hasExtension(_EXT_texture3D) && win->hasExtension(_NV_register_combiners) &&
           !forceTexture2D;

  case SM_FRAGMENT_PROGRAM_2D:
    return win->hasExtension(_ARB_multitexture) && win->hasExtension(_ARB_fragment_program) &&
           !forceTexture3D;

  case SM_FRAGMENT_PROGRAM_3D:
    return win->hasExtension(_EXT_texture3D) && win->hasExtension(_ARB_fragment_program) &&
           !forceTexture2D;
