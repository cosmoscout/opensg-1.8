...
template <>
struct FieldDataTraits<StatCollector> : public FieldTraitsRecurseBase<StatCollector> {
  ...
  static Char8* getSName(void) {
    return "SFStatCollector";
  }
  static Char8* getMName(void) {
    return "MFStatCollector";
  }

  static StatCollector getDefault(void) {
    return StatCollector();
  }

  static bool getFromString(StatCollector& outVal, const Char8*& inVal) {
    return outVal.getFromString(inVal);
  }

  static void putToString(const StatCollector& inVal, std::string& outVal) {
