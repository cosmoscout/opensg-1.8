...
void TexGenChunk::dump(UInt32, const BitVector) const {
  SLOG << "Dump TexGenChunk NI" << std::endl;
}

/*------------------------------ State ------------------------------------*/

static inline void setGenFunc(
    GLenum coord, GLenum gen, GLenum func, Vec4f& plane, NodePtr beacon, Matrix& cameraMat) {
  if (beacon != NullFC) {
    Matrix beaconMat;
...
void TexGenChunk::activate(DrawActionBase* action, UInt32 idx) {
  ...
  if (vp != NULL && action->getCamera() != NULL) {
    action->getCamera()->getViewing(cameraMat, vp->getPixelWidth(), vp->getPixelHeight());
  }

  // genfuncs
  setGenFunc(GL_S, GL_TEXTURE_GEN_S, getGenFuncS(), getGenFuncSPlane(), getSBeacon(), cameraMat);
  glErr("TexGenChunk::activateS");
  setGenFunc(GL_T, GL_TEXTURE_GEN_T, getGenFuncT(), getGenFuncTPlane(), getTBeacon(), cameraMat);
  glErr("TexGenChunk::activateT");
  setGenFunc(GL_R, GL_TEXTURE_GEN_R, getGenFuncR(), getGenFuncRPlane(), getRBeacon(), cameraMat);
  glErr("TexGenChunk::activateR");
  setGenFunc(GL_Q, GL_TEXTURE_GEN_Q, getGenFuncQ(), getGenFuncQPlane(), getQBeacon(), cameraMat);
  glErr("TexGenChunk::activateQ");
}

static inline void changeGenFunc(GLenum oldfunc, NodePtr oldbeacon, GLenum coord, GLenum gen,
...
void TexGenChunk::changeFrom(DrawActionBase* action, StateChunk* old, UInt32 idx) {
  ...
  changeGenFunc(oldp->getGenFuncS(), oldp->getSBeacon(), GL_S, GL_TEXTURE_GEN_S, getGenFuncS(),
      getGenFuncSPlane(), getSBeacon(), cameraMat);
  changeGenFunc(oldp->getGenFuncT(), oldp->getTBeacon(), GL_T, GL_TEXTURE_GEN_T, getGenFuncT(),
      getGenFuncTPlane(), getTBeacon(), cameraMat);
  changeGenFunc(oldp->getGenFuncR(), oldp->getRBeacon(), GL_R, GL_TEXTURE_GEN_R, getGenFuncR(),
      getGenFuncRPlane(), getRBeacon(), cameraMat);
  changeGenFunc(oldp->getGenFuncQ(), oldp->getQBeacon(), GL_Q, GL_TEXTURE_GEN_Q, getGenFuncQ(),
      getGenFuncQPlane(), getQBeacon(), cameraMat);
...
bool TexGenChunk::operator==(const StateChunk& other) const {
  ...
  return getGenFuncS() == tother->getGenFuncS() && getGenFuncT() == tother->getGenFuncT() &&
         getGenFuncR() == tother->getGenFuncR() && getGenFuncQ() == tother->getGenFuncQ() &&
