...
OffCenterPerspectiveCamera::OffCenterPerspectiveCamera(const OffCenterPerspectiveCamera& source)
    : Inherited(source) {
}
...
void OffCenterPerspectiveCamera::draw(
    DrawAction* OSG_CHECK_ARG(action), const Viewport& OSG_CHECK_ARG(port)) {
}

bool OffCenterPerspectiveCamera::MatrixPerspectivePrincipalPnt(Matrix& result, Real32 rFovy,
    Real32 rAspect, Real32 rNear, Real32 rFar, Real32 rPrincipalPointX, Real32 rPrincipalPointY) {
  ...
  MatrixFrustum(result, -rNear * ct * rAspect * (Real32(1) + rPrincipalPointX),
      rNear * ct * rAspect * (Real32(1) - rPrincipalPointX),
      -rNear * ct * (Real32(1) + rPrincipalPointY), rNear * ct * (Real32(1) - rPrincipalPointY),
      rNear, rFar);

  return false;
}

void OffCenterPerspectiveCamera::getProjection(Matrix& result, UInt32 width, UInt32 height) {
  Real32 fov = getFov();
  ...
  if (principalPointX == Real32(0) && principalPointY == Real32(0)) {
    ...
    MatrixPerspectivePrincipalPnt(result, fov / 2, width / (Real32)height * getAspect(), getNear(),
        getFar(), principalPointX, principalPointY);
  }
}
