...
static void subdivide(const typename std::vector<VectorTypeT>& dataIn,
    typename std::vector<VectorTypeT>* dataOut, bool closed);

template <class VectorTypeT>
static void subdivide(const typename std::vector<VectorTypeT>& dataIn,
    typename std::vector<VectorTypeT>* dataOut, UInt32 nTimes, bool close);
...
ExtrusionSurface::ExtrusionSurface(const std::vector<Pnt2f>& crossSection,
    const std::vector<Quaternion>& orientation, const std::vector<Vec2f>& scale,
    const std::vector<Pnt3f>& spine, Real32 creaseAngle, bool beginCap, bool endCap, bool ccw,
    bool convex, bool buildNormal, bool buildTexCoord)
    :
  ...
  if (!orientation.empty()) {
    ...
    for (UInt32 i = 0; i < DEF_N_ORIENTATION_PARAMS; i++)
      _orientation.push_back(Quaternion(Vec3f(DEF_ORIENTATION[i]), DEF_ORIENTATION[i][3]));
  }
...
void ExtrusionSurface::determineTopology(void) {
  ...
  FDEBUG(("OSGExtrusion:checkInput: Topology: (Spine: %s / Cross-section: %s)\n",
      (_spineClosed ? "Closed" : "Open"), (_crossSectionClosed ? "Closed" : "Open")));
}
  ...
  for (; spineIt != _spine.end(); ++spineIt, ++tIt) {
    ...
    if (zAxis.length() > Eps) {
      ...
      } else // assign found z-axis to this basis _only_
      {
        ...
        Vec3f prevZAxis((*(tIt - 1))[2][0], (*(tIt - 1))[2][1], (*(tIt - 1))[2][2]);
...
void ExtrusionSurface::calcTransforms(void) {
  ...
  for (; spineIt != _spine.end(); ++spineIt, ++tIt) {
    ...
    if (rotIt + 1 != _orientation.end()) {
      ++rotIt;
    }
    if (scaleIt + 1 != _scale.end()) {
      ++scaleIt;
    }
  }

  FDEBUG(
      ("OSGExtrusion:calcTransforms: spine collinear: %s\n", (_spineCollinear ? "true" : "false")));
...
void ExtrusionSurface::calcSweepSurfacePositions(void) {
  ...
  for (Int32 i = 0; i < _spine.size(); i++, ++gridRowIt) {
    ...
    for (crossIt = _crossSection.begin(); crossIt != _crossSection.end(); ++crossIt, ++gridColIt) {
...
void ExtrusionSurface::calcSweepSurfaceFaceNormals(void) {
  ...
  for (UInt32 i = 0; i < _spine.size(); ++i) {
    ...
    for (UInt32 j = 0; j < _crossSection.size(); ++j) {
      ...
      _grid[i][j].adjFaceNormals[0] = calcQuadFaceNormal(_grid[i][j].position,
          _grid[i][jNext].position, _grid[iNext][jNext].position, _grid[iNext][j].position);

      // upper left face normal (2nd quadrant)
      _grid[i][j].adjFaceNormals[1] = calcQuadFaceNormal(_grid[i][j].position,
          _grid[iNext][j].position, _grid[iNext][jPrev].position, _grid[i][jPrev].position);

      // lower left face normal (3nd quadrant)
      _grid[i][j].adjFaceNormals[2] = calcQuadFaceNormal(_grid[i][j].position,
          _grid[i][jPrev].position, _grid[iPrev][jPrev].position, _grid[iPrev][j].position);

      // lower right face normal (4th quadrant)
      _grid[i][j].adjFaceNormals[3] = calcQuadFaceNormal(_grid[i][j].position,
          _grid[iPrev][j].position, _grid[iPrev][jNext].position, _grid[i][jNext].position);
    }
  }
}
...
// Author: afischle
//-----------------------------------------------------------------------------
void ExtrusionSurface::storeSweepSurfaceWithoutNormals(
    GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr) {
...
void ExtrusionSurface::storeSweepSurfaceWithNormals(
    GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr) {
  assert(indicesPtr != NullFC);
  assert(lensPtr != NullFC);
  assert(typesPtr != NullFC);
  ...
  if (_ccw) {
    ...
    for (UInt32 i = 0; i < spineEnd; i++) {
      ...
      for (UInt32 j = 0; j < cSEnd; j++) {
        ...
        if ((fN.enclosedAngle(fNNext) + Eps > _creaseAngle - Eps) || (j + 1 == cSEnd)) {
  ...
  } else // cw case
  {
    ...
    for (UInt32 i = 0; i < _spine.size() - 1; i++) {
      ...
      for (UInt32 j = cSEnd; j > 0; j--) {
        Vec3f fNNext = _grid[i][j - 1].adjFaceNormals[0];
        if ((fN.enclosedAngle(fNNext) + Eps > _creaseAngle - Eps) || (j == 1)) {
          calcVertexNormal(&_grid[i + 1][j - 1], 3);
          storeVertex(_grid[i + 1][j - 1], indicesPtr);
...
// Author: afischle
//-----------------------------------------------------------------------------
Real32 ExtrusionSurface::computeMinYAbs(const std::vector<Pnt2f>& contour, Real32 alpha) {
  const Real32 sinAlpha = osgsin(alpha);
  const Real32 cosAlpha = osgcos(alpha);

  Real32 minYAbs = 1.f;
  for (std::vector<Pnt2f>::const_iterator it = contour.begin(); it != contour.end(); ++it) {
...
// Author: afischle
//-----------------------------------------------------------------------------
Real32 ExtrusionSurface::calcBetterRotationAngle(
    const std::vector<Pnt2f>& contour, UInt32 nAngles) {
  Real32 maxMinY   = 0.f;
...
// Author: afischle
//-----------------------------------------------------------------------------
bool ExtrusionSurface::calcOptimizedContour(
    const std::vector<Pnt2f>& contour, const Pnt2f& point, std::vector<Pnt2f>* optimizedContour) {
  // copy the input
  ...
  for (std::vector<Pnt2f>::iterator it = coords.begin(); it != coords.end(); ++it) {
...
// Author: afischle
//----------------------------------------------------------------------
Int32 ExtrusionSurface::calcWindingNumber(const std::vector<Pnt2f>& contour, const Pnt2f& point) {
...
// Author: afischle
//-----------------------------------------------------------------------------
void ExtrusionSurface::storeConvexCap(UInt32 spineIndex, bool isBeginCap,
    GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr) {
  assert(indicesPtr != NullFC);
  assert(lensPtr != NullFC);
  assert(typesPtr != NullFC);
  ...
  FDEBUG(("OSGExtrusion:storeCap: Generating cap[%d], type=%s, _ccw=%s\n", spineIndex,
      (isBeginCap ? "begin cap" : "end cap"), (_ccw ? "TRUE" : "FALSE")));
  ...
  Int32 windNum = calcWindingNumber(capBorderCurveInXZPlane, baryCenterInXZPlane);
  ...
  for (cIt = capBorderCurveInXZPlane.begin(); cIt != capBorderCurveInXZPlane.end(); ++cIt) {
    if (xMin > (*cIt)[0])
      xMin = (*cIt)[0];
    if (xMax < (*cIt)[0])
      xMax = (*cIt)[0];
  ...
  if (reverseInsert) {
    ...
    std::vector<Pnt3f>::reverse_iterator sRevIt = capBorderCurveInSection.rbegin();
    std::vector<Pnt2f>::reverse_iterator cRevIt = capBorderCurveInXZPlane.rbegin();
...
// Author: afischle
//-----------------------------------------------------------------------------
void ExtrusionSurface::storeMaps(
    GeoPositions3fPtr posPtr, GeoNormals3fPtr normalsPtr, GeoTexCoords2fPtr texPtr) {
  ...
  for (PositionMap::iterator pMIt = _posMap.begin(); pMIt != _posMap.end(); ++pMIt) {
  ...
  if (_createNormals) {
    ...
    for (NormalMap::iterator nMIt = _normalMap.begin(); nMIt != _normalMap.end(); ++nMIt) {
  ...
  if (_createTexCoords) {
    ...
    for (TexCoordMap::iterator tMIt = _texCoordMap.begin(); tMIt != _texCoordMap.end(); ++tMIt) {
...
GeometryPtr ExtrusionSurface::createGeometry(UInt32 nSubdivs) {
  ...
  endEditCP(geoPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                        Geometry::IndicesFieldMask | Geometry::IndexMappingFieldMask |
                        Geometry::PositionsFieldMask | Geometry::NormalsFieldMask |
                        Geometry::TexCoordsFieldMask);

  SINFO << "OSGExtrusion:createGeometry: Stats: (" << _primitiveCount << '/' << _totalVertexCount
        << '/' << _posMap.size() << '/' << _normalMap.size() << '/' << _texCoordMap.size() << ')'
        << "(Prims/Verts/Pos/Norms/TexCoords)" << std::endl;
...
NodePtr OSG::makeExtrusion(const std::vector<Pnt2f>& crossSection,
    const std::vector<Quaternion>& orientation, const std::vector<Vec2f>& scale,
    const std::vector<Pnt3f>& spine, Real32 creaseAngle, bool beginCap, bool endCap, bool ccw,
    bool convex, bool buildNormal, bool buildTexCoord, UInt32 numOfSubdivision) {
  GeometryPtr pGeo = makeExtrusionGeo(crossSection, orientation, scale, spine, creaseAngle,
      beginCap, endCap, ccw, convex, buildNormal, buildTexCoord, numOfSubdivision);
  if (pGeo == NullFC) {
    return NullFC;
  }
...
GeometryPtr OSG::makeExtrusionGeo(const std::vector<Pnt2f>& crossSection,
    const std::vector<Quaternion>& orientation, const std::vector<Vec2f>& scale,
    const std::vector<Pnt3f>& spine, Real32 creaseAngle, bool beginCap, bool endCap, bool ccw,
    bool convex, bool buildNormal, bool buildTexCoord, UInt32 numSubdivs) {
  ...
  ExtrusionSurface surf(crossSection, orientation, scale, spine, creaseAngle, beginCap, endCap, ccw,
      convex, buildNormal, buildTexCoord);
...
static void subdivide(const typename std::vector<VectorTypeT>& dataIn,
  ...
  if (closed) {
    ...
    dataOut->push_back(
        apply4PtScheme<VectorTypeT>(dataIn.end() - 2, inputIt, inputIt + 1, inputIt + 2));
  }
  // Apply 3pt scheme (3/8, 6/8, -1/8) to the first point.
  else {
    dataOut->push_back(apply3PtScheme<VectorTypeT>(inputIt, inputIt + 1, inputIt + 2));
  }
  ...
  if (dataIn.size() > 3) {
    ...
    for (; (inputIt + 2) != dataIn.end(); ++inputIt) {
      ...
      dataOut->push_back(
          apply4PtScheme<VectorTypeT>(inputIt - 1, inputIt, inputIt + 1, inputIt + 2));
    }
  }
  ...
  if (closed) {
    dataOut->push_back(apply4PtScheme<VectorTypeT>(
        dataIn.end() - 3, dataIn.end() - 2, dataIn.begin(), dataIn.begin() + 1));
  } else {
    // Apply 3pt scheme (-1, 6, 3) by reversing the order of the
    // points for the boundary and applying (3, 6, -1)
    dataOut->push_back(
        apply3PtScheme<VectorTypeT>(dataIn.end() - 1, dataIn.end() - 2, dataIn.end() - 3));
  }
...
template <class VectorTypeT>
static void subdivide(const typename std::vector<VectorTypeT>& dataIn,
    typename std::vector<VectorTypeT>* dataOut, UInt32 nTimes, bool closed) {
...
// Author: afischle
//-----------------------------------------------------------------------------
void ExtrusionSurface::storeVertexNormalGeo(
    GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr) {
...
// Author: afischle
//-----------------------------------------------------------------------------
void ExtrusionSurface::storeFaceNormalGeo(
    GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr) {
  if (!_createNormals || !_createTexCoords)
    return;
