...
static const Char8* datSuffixArray[] = {"dat"};

DATImageFileType DATImageFileType::_the("image/x-dat", datSuffixArray, sizeof(datSuffixArray));

std::map<std::string, DATImageFileType::KeyType> DATImageFileType::_keyStrMap;

std::map<std::string, DATImageFileType::FormatDesc> DATImageFileType::_formatStrMap;
...
*/
bool DATImageFileType::read(ImagePtr& image, const Char8* fileName) {
  bool retCode = false;
  ...
  // read the data file
  for (lineBuffer[0] = 0; inDat.getline(lineBuffer, lineBufferSize); lineBuffer[0] = 0) {
  ...
  if (objectFileName.empty() == false) {
    if ((res[0] > 0) && (res[1] > 0) && (res[2] > 0)) {
      if (formatType != Image::OSG_INVALID_IMAGEDATATYPE) {
        inVolS.open(objectFileName.c_str(), std::ios::in | std::ios::binary);
        if (inVolS.fail() && ImageFileHandler::the().getPathHandler()) {
          ...
          inVolS.open(
              ph->findFile(objectFileName.c_str()).c_str(), std::ios::in | std::ios::binary);
        }
        if (inVolS.fail()) {
          // Maybe compressed and name not changed?
          std::string gzname = objectFileName + ".gz";
          inVolS.clear(); // reset the error state
          inVolS.open(gzname.c_str(), std::ios::in | std::ios::binary);
          if (inVolS.fail() && ImageFileHandler::the().getPathHandler()) {
            // Try to find the file in the search path
            inVolS.clear(); // reset the error state
            PathHandler* ph = ImageFileHandler::the().getPathHandler();
            inVolS.open(ph->findFile(gzname.c_str()).c_str(), std::ios::in | std::ios::binary);
          }
        }
        if (inVolS.good()) {
          ...
          if (isGZip(inVolS)) {
            ...
            inVol    = unzipper;
#else
            SFATAL << "Compressed streams are not supported! Configure with --enable-png "
                      "--with-png=DIR options."
                   << std::endl;
#endif
          } else {
            inVol = &inVolS;
...
*/
bool DATImageFileType::write(const ImagePtr& image, const Char8* fileName) {
  initTypeMap();
  ...
  if (!dat) {
    SWARNING << "DATImageFileType::write : Can not open output stream for file '" << fileName
             << "'!" << std::endl;
    return false;
  }
  ...
  if (!raw) {
    SWARNING << "DATImageFileType::write : Can not open output stream for file '" << basename
             << "'!" << std::endl;
    return false;
  }
...
*/
UInt64 DATImageFileType::restoreData(
...
*/
UInt64 DATImageFileType::storeData(
...
*/
DATImageFileType::DATImageFileType(
