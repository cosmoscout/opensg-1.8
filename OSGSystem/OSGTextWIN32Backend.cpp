...
class TextWIN32VectorGlyph : public TextVectorGlyph {
 public:
  // Constructor
  TextWIN32VectorGlyph(Index glyphIndex, Real32 scale, const GLYPHMETRICS& hpgm,
      const GLYPHMETRICS& vhpgm, LPTTPOLYGONHEADER lpHeader, DWORD size);
...
class TextWIN32PixmapGlyph : public TextPixmapGlyph {
 public:
  // Constructor
  TextWIN32PixmapGlyph(
      Index glyphIndex, const GLYPHMETRICS& hpgm, const GLYPHMETRICS& vpgm, UInt8* pixmap);
...
class TextWIN32TXFFace : public TextTXFFace {
 public:
  // Constructor
  TextWIN32TXFFace(
      const TextWIN32Backend* backend, HFONT hHoriFont, HFONT hVertFont, const TextTXFParam& param);
...
// Author: pdaehne
//----------------------------------------------------------------------
static int CALLBACK enumFamCallBack(
    ENUMLOGFONT* lpelf, NEWTEXTMETRIC* lpntm, DWORD fontType, LPARAM lParam) {
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextWIN32Backend::createFonts(
    const string& family, UInt32 size, TextFace::Style style, HFONT& hHoriFont, HFONT& hVertFont) {
  // Initialization
  ...
  if (size == 0) {
    ...
    EnumData enumData;
    EnumFontFamilies(_hDC, (LPCTSTR)(f.c_str()), (FONTENUMPROC)enumFamCallBack, (LPARAM)&enumData);
  ...
  hHoriFont =
      CreateFont(size, 0, 0, 0, weight, italic, FALSE, FALSE, DEFAULT_CHARSET, OUT_TT_ONLY_PRECIS,
          CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, f.c_str());

  // Create the the vertical font
  hVertFont = CreateFont(size, 0, -900, 0, weight, italic, FALSE, FALSE, DEFAULT_CHARSET,
      OUT_TT_ONLY_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
      f.c_str());
}
...
// Author: pdaehne
//----------------------------------------------------------------------
TextVectorFace* TextWIN32Backend::createVectorFace(const string& family, TextFace::Style style) {
...
// Author: pdaehne
//----------------------------------------------------------------------
TextPixmapFace* TextWIN32Backend::createPixmapFace(
...
// Author: pdaehne
//----------------------------------------------------------------------
TextTXFFace* TextWIN32Backend::createTXFFace(
...
// Author: pdaehne
//----------------------------------------------------------------------
static int CALLBACK enumFamCallBack2(
    ENUMLOGFONT* lpelf, NEWTEXTMETRIC* lpntm, DWORD fontType, LPARAM lParam) {
  // We are not interested in raster fonts
  if ((fontType & RASTER_FONTTYPE) != 0)
    return 1;
...
// Author: pdaehne
//----------------------------------------------------------------------
static void getFontInfo(
    HDC hDC, HFONT hFont, string& family, TextFace::Style& style, Real32& ascent, Real32& descent) {
...
TextWIN32VectorFace::TextWIN32VectorFace(
    TextWIN32Backend* backend, HFONT hHoriFont, HFONT hVertFont)
    : TextVectorFace()
    , _backend(backend)
    , _hHoriFont(hHoriFont)
    , _hVertFont(hVertFont) {
  // Get information about the font
  getFontInfo(_backend->_hDC, _hHoriFont, _family, _style, _horiAscent, _horiDescent);
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextWIN32VectorFace::layout(
    const wstring& text, const TextLayoutParam& param, TextLayoutResult& layoutResult) {
  ...
  DWORD result =
      GetCharacterPlacementW(_backend->_hDC, &(utf16Text[0]), len, nMaxExtent, &results, dwFlags);
  if (result != 0) {
...
auto_ptr<TextVectorGlyph> TextWIN32VectorFace::createGlyph(TextGlyph::Index glyphIndex) {
  ...
  DWORD        size = GetGlyphOutlineW(
      _backend->_hDC, glyphIndex, GGO_METRICS | GGO_GLYPH_INDEX, &vpgm, 0, 0, &mat2);
  if (size == GDI_ERROR) {
    SelectObject(_backend->_hDC, oldFont);
    return auto_ptr<TextVectorGlyph>();
  }
  ...
  size = GetGlyphOutlineW(
      _backend->_hDC, glyphIndex, GGO_NATIVE | GGO_GLYPH_INDEX, &hpgm, 0, 0, &mat2);
  if (size == GDI_ERROR) {
    SelectObject(_backend->_hDC, oldFont);
    return auto_ptr<TextVectorGlyph>();
  }
  ...
  size                       = GetGlyphOutlineW(
      _backend->_hDC, glyphIndex, GGO_NATIVE | GGO_GLYPH_INDEX, &hpgm, size, lpHeader, &mat2);
  ...
  auto_ptr<TextVectorGlyph> glyph(
      new TextWIN32VectorGlyph(glyphIndex, _scale, hpgm, vpgm, lpHeader, size));
  delete[] lpHeader;
  return glyph;
}
...
// Author: pdaehne
//----------------------------------------------------------------------
TextWIN32VectorGlyph::TextWIN32VectorGlyph(Index glyphIndex, Real32 scale, const GLYPHMETRICS& hpgm,
    const GLYPHMETRICS& vpgm, LPTTPOLYGONHEADER lpHeader, DWORD size)
    : TextVectorGlyph() {
    ...
    // type of contour.
    {
        ...
        if (lpCurve->wType == TT_PRIM_LINE) {
          ...
          for (i = 0; i < lpCurve->cpfx; ++i) {
            ...
            if (i >= lpCurve->cpfx - 1)
              ...
              _outline.back().push_back(
                  TextVectorGlyph::Point(p, TextVectorGlyph::Point::PT_CUBIC));
          }
        } else
      ...
      if (_outline.back().size() < 3)
        _outline.erase(_outline.end() - 1);
          ...
          if ((startPoint.x.value != endPoint->x.value) ||
              (startPoint.y.value != endPoint->y.value)) {
        p.setValues(startPoint.x.value, startPoint.y.value);
        p *= scale;
        p -= offset;
        _outline.back().push_back(TextVectorGlyph::Point(p, TextVectorGlyph::Point::PT_ON));
...
TextWIN32PixmapFace::TextWIN32PixmapFace(
    TextWIN32Backend* backend, HFONT hHoriFont, HFONT hVertFont, UInt32 size)
    : TextPixmapFace()
    , _backend(backend)
    , _hHoriFont(hHoriFont)
    , _hVertFont(hVertFont) {
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextWIN32PixmapFace::layout(
    const wstring& text, const TextLayoutParam& param, TextLayoutResult& layoutResult) {
  // Initialize return values
  layoutResult.clear();
  if (param.horizontal == true)
...
auto_ptr<TextPixmapGlyph> TextWIN32PixmapFace::createGlyph(TextGlyph::Index glyphIndex) {
  ...
  GLYPHMETRICS hpgm;
  size = GetGlyphOutlineW(
      _backend->_hDC, glyphIndex, GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &hpgm, 0, 0, &mat2);
  if (size == GDI_ERROR) {
    SelectObject(_backend->_hDC, oldFont);
    return auto_ptr<TextPixmapGlyph>();
  }
  ...
  if (size == 0)
    ...
    size   = GetGlyphOutlineW(
        _backend->_hDC, glyphIndex, GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &hpgm, size, buffer, &mat2);
    if (size == GDI_ERROR) {
      delete[] buffer;
      SelectObject(_backend->_hDC, oldFont);
      return auto_ptr<TextPixmapGlyph>();
...
TextWIN32TXFFace::TextWIN32TXFFace(
    ...
    : TextTXFFace() {
  // Get information about the font
  getFontInfo(backend->_hDC, hHoriFont, _family, _style, _horiAscent, _horiDescent);
  ...
  for (j = 0; j < results.nGlyphs; ++j) {
    ...
    MAT2         mat2 = {{0, 1}, {0, 0}, {0, 0}, {0, 1}};
    DWORD        size = GetGlyphOutlineW(
        backend->_hDC, results.lpGlyphs[j], GGO_METRICS | GGO_GLYPH_INDEX, &vpgm, 0, 0, &mat2);
    if (size == GDI_ERROR)
      continue;
    ...
    size = GetGlyphOutlineW(backend->_hDC, results.lpGlyphs[j],
        /*GGO_METRICS*/ GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &hpgm, 0, 0, &mat2);
    if (size == GDI_ERROR)
      continue;

    _glyphMap.insert(GlyphMap::value_type(param.getCharacters()[j],
        new TextWIN32TXFGlyph(param.getCharacters()[j], this, _scale, hpgm, vpgm)));
  }
  ...
  for (j = 0; j < results.nGlyphs; ++j) {
    ...
    MAT2         mat2 = {{0, 1}, {0, 0}, {0, 0}, {0, 1}};
    DWORD        size = GetGlyphOutlineW(
        backend->_hDC, results.lpGlyphs[j], GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &hpgm, 0, 0, &mat2);
...
TextWIN32TXFGlyph::TextWIN32TXFGlyph(Index glyphIndex, TextWIN32TXFFace* face, Real32 scale,
  ...
  if (glyphIndex == 32) {
    ...
    _vertBearingY =
        static_cast<Int32>(-(face->getHoriAscent() - face->getHoriDescent()) / _scale / 20.f);
    if (_vertBearingY > -1)
      _vertBearingY = -1;
