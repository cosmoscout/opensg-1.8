...
// setup functions
int BSplineTensorSurface::setKnotsAndDimension(
    const DCTPdvector& knots_u, int dim_u, const DCTPdvector& knots_v, int dim_v) {
  if (dim_u < 1 || dim_v < 1)
...
int BSplineTensorSurface::read(std::istream& infile) {
  ...
  if (strcmp(txtbuffer, ff_const_1) && strcmp(txtbuffer, ff_const_5)) {
...
int BSplineTensorSurface::write(std::ostream& outfile) {
  ...
  DCTPVec4dvector::size_type num_of_cps_v =
      control_points[0].size(); // FIXME:what if it's not initialized yet
...
void BSplineTensorSurface::RatSurfaceDerivs(
    DCTPVec4dmatrix& rclHomDerivs, DCTPVec3dmatrix& rclEuclidDerivs) {
  ...
  for (ui_k = 0; ui_k < rclHomDerivs.size(); ++ui_k) {
    for (ui_l = 0; ui_l < rclHomDerivs.size() - ui_k; ++ui_l) {
      ...
      for (ui_j = 1; ui_j <= ui_l; ++ui_j) {
        // cl_v -= Binomial(ui_l, ui_j) * rclHomDerivs[0][ui_j][3] *
        // rclEuclidDerivs[ui_k][ui_l-ui_j];
        cl_v -= rclHomDerivs[0][ui_j][3] * rclEuclidDerivs[ui_k][ui_l - ui_j];
      }
      for (ui_i = 1; ui_i <= ui_k; ++ui_i) {
        // cl_v -= Binomial(ui_k, ui_i) * rclHomDerivs[ui_i][0][3] *
        // rclEuclidDerivs[ui_k-ui_i][ui_l];
        cl_v -= rclHomDerivs[ui_i][0][3] * rclEuclidDerivs[ui_k - ui_i][ui_l];
        cl_v2 = Vec3d(0.0, 0.0, 0.0);
        for (ui_j = 1; ui_j <= ui_l; ++ui_j) {
          // cl_v2 += Binomial(ui_l, ui_j) * rclHomDerivs[ui_i][ui_j][3] *
          // rclEuclidDerivs[ui_k-ui_i][ui_l-ui_j];
 ...
 *
 */
Vec3d BSplineTensorSurface::computeNormalTex(Vec2d& rclUV, int& riError, Vec3d& rclPos,
    Vec2d& rclTexCoord, const std::vector<std::vector<Pnt2f>>* cpvvclTexCP) {
  int             i_uspan;
  int             i_vspan;
  double**        ppd_nu;
  double**        ppd_nv;
 ...
 * vpars[ j ]..vpars[ j + 1 ].
 */
int BSplineTensorSurface::makeBezier(
    beziersurfacematrix& beziers, DCTPdvector& upars, DCTPdvector& vpars) {
  ...
  for (i = 1; i < uknots.size(); i++) {
    ...
    if (actk == prevknot)
      ...
      for (int j = mult + 1; j < dimension_u;
           j++) { // each interior knot must have the multiplicity of dimension -1
  ...
  for (i = 1; i < vknots.size(); i++) {
    ...
    if (actk == prevknot)
      ...
      for (int j = mult + 1; j < dimension_v;
           j++) { // each interior knot must have the multiplicity of dimension -1
...
int BSplineTensorSurface::midPointSubDivision(
    std::vector<std::vector<BSplineTensorSurface>>& rvvcl_newbspline) {
  double                          d_min;
  double                          d_max;
  unsigned int                    ui_cnt;
  unsigned int                    ui_idx;
  ...
  while (rvvcl_newbspline[0][0].getKnotVector_U()[ui_cnt] ==
         rvvcl_newbspline[0][0].getKnotVector_U()[ui_idx]) {
  ...
  while (rvvcl_newbspline[0][0].getKnotVector_V()[ui_cnt] ==
         rvvcl_newbspline[0][0].getKnotVector_V()[ui_idx]) {
...
int BSplineTensorSurface::midPointSubDivisionU(std::vector<BSplineTensorSurface>& rvcl_newbspline) {
  ...
  for (ui_idx = 0; ui_idx <= dimension_u; ++ui_idx) {
    rvcl_newbspline[1].getKnotVector_U().push_back(rvcl_newbspline[0].getKnotVector_U()[i_span]);
  }
  for (ui_idx = i_span - 1; ui_idx < ui_cnt; ++ui_idx) {
  ...
  ui_idx = ui_cnt - dimension_u - 1;
  while (rvcl_newbspline[0].getKnotVector_U()[ui_cnt] ==
         rvcl_newbspline[0].getKnotVector_U()[ui_idx]) {
...
int BSplineTensorSurface::midPointSubDivisionV(std::vector<BSplineTensorSurface>& rvcl_newbspline) {
  ...
  for (ui_idx = 0; ui_idx <= dimension_v; ++ui_idx) {
    rvcl_newbspline[1].getKnotVector_V().push_back(rvcl_newbspline[0].getKnotVector_V()[i_span]);
  }
  ...
  ui_idx = ui_cnt - dimension_v - 1;
  while (rvcl_newbspline[0].getKnotVector_V()[ui_cnt] ==
         rvcl_newbspline[0].getKnotVector_V()[ui_idx]) {
...
int BSplineTensorSurface::subDivisionU(
    std::vector<BSplineTensorSurface>& rvcl_newbspline, double dSplit) {
  double                          d_min;
  double                          d_max;
  unsigned int                    ui_cnt;
  unsigned int                    ui_idx;
...
// subdivide surface at param into 2 bezier surfaces
int BSplineTensorSurface::subDivisionV(
    std::vector<BSplineTensorSurface>& rvcl_newbspline, double dSplit) {
  double                          d_min;
  double                          d_max;
  unsigned int                    ui_cnt;
...
void BSplineTensorSurface::compute(std::vector<Vec2d>& rvclUV, std::vector<Pnt3f>& rvclPos)
...
void BSplineTensorSurface::computeNormal(
    std::vector<Vec2d>& rvclUV, std::vector<Pnt3f>& rvclPos, std::vector<Vec3f>& rvclNorm) {
  const unsigned int cui_cnt = rvclUV.size();
  unsigned int       ui_idx;
  int                i_uspan = -1;
  int                i_vspan = -1;
  ...
  for (ui_idx = 0; ui_idx < cui_cnt; ++ui_idx) {
    ...
    if ((i_uspan < 0) || (i_vspan < 0)) {
      ...
      //// switched because of optimization! 			d_len = rvclNorm[ ui_idx ].squareLength( );
...
void BSplineTensorSurface::computeNormalforTrimming(
    std::vector<Vec2d>& rvclUV, std::vector<Vec3d>& rvclNorm, std::vector<Vec3d>* pvclPos) {
  const unsigned int cui_cnt = rvclUV.size();
  unsigned int       ui_idx;
  int                i_uspan = -1;
  int                i_vspan = -1;
...
void BSplineTensorSurface::computeNormalTex(std::vector<Vec2d>& rvclUV, std::vector<Pnt3f>& rvclPos,
    std::vector<Vec3f>& rvclNorm, std::vector<Pnt2f>& rvclTexCoord,
...
void BSplineTensorSurface::computeTex(std::vector<Vec2d>& rvclUV, std::vector<Pnt3f>& rvclPos,
    std::vector<Pnt2f>& rvclTexCoord, const std::vector<std::vector<Vec2d>>* cpvvclTexCP) {
  const unsigned int cui_cnt = rvclUV.size();
  unsigned int       ui_idx;
  double*            pd_nu;
  double*            pd_nv;
...
void BSplineTensorSurface::computeTexforTrimming(std::vector<Vec2d>& rvclUV,
    std::vector<Vec2d>& rvclTexCoord, const std::vector<std::vector<Vec2d>>* cpvvclTexCP) {
  const unsigned int cui_cnt = rvclUV.size();
  unsigned int       ui_idx;
  double*            pd_nu;
  double*            pd_nv;
...
void BSplineTensorSurface::correctDers(
    const Vec2d cclUV, const Vec3d cclPos, Vec3d& rclDU, Vec3d& rclDV) {
  return;
