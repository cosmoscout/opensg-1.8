...
static const Char8* tiffSuffixArray[] = {"tif", "tiff"};
...
bool TIFImageFileType::read(ImagePtr& OSG_TIF_ARG(image), std::istream& OSG_TIF_ARG(is),
...
#ifdef OSG_WITH_TIF
  TIFF*   in   = TIFFClientOpen("dummy", "rm", (thandle_t)&is, isReadProc, isWriteProc, isSeekProc,
      closeProc, isSizeProc, mapFileProc, unmapFileProc);
  ...
  if (in) {
    ...
    if (bpp == 4) { // accept unspecified extra samples as associated alpha
      TIFFGetFieldDefaulted(in, TIFFTAG_EXTRASAMPLES, &extrasamples, &sampleinfo);
    ...
    if (TIFFRGBAImageOK(in, errorMessage) && TIFFReadRGBAImage(in, w, h, (uint32*)data, 1))
      valid = true;
    else {
...
bool TIFImageFileType::write(const ImagePtr& OSG_TIF_ARG(image), std::ostream& OSG_TIF_ARG(os),
  ...
  TIFF* out = TIFFClientOpen("dummy", "wm", (thandle_t)&os, osReadProc, osWriteProc, osSeekProc,
      closeProc, osSizeProc, mapFileProc, unmapFileProc);
  ...
  if (out) {
    ...
    for (row = 0; row < image->getHeight(); row++) {
      ...
      if (TIFFWriteScanline(out, (tdata_t) const_cast<UChar8*>(data), row, 0) < 0)
        break;
    }
...
*/
TIFImageFileType::TIFImageFileType(
