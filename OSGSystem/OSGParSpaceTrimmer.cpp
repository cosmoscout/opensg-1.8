...
bool ParSpaceTrimmer::isOverQuad(DCTPFace* f, Vec2d& p) {
  ...
  if (p[0] >= f->orig_face[0]->coords[0] && p[0] <= f->orig_face[1]->coords[0] &&
      p[1] <= f->orig_face[0]->coords[1] && p[1] >= f->orig_face[3]->coords[1])
#else
  /*        std::cerr << "isoverface: " << f->orig_quad[0]->coords << " "
          ...
          std::cerr << " point: " << p << std::endl;*/
  if (p[0] >= f->orig_quad[0]->coords[0] && p[0] <= f->orig_quad[1]->coords[0] &&
      p[1] <= f->orig_quad[0]->coords[1] && p[1] >= f->orig_quad[3]->coords[1])
#endif
    return true;
  else
    return false;
...
bool ParSpaceTrimmer::isOnSection(Vec2d& p1, Vec2d& p2, Vec2d& p) {
  ...
  double dist;
  double par;
  double lab = sqrt((p1[0] - p2[0]) * (p1[0] - p2[0]) + (p1[1] - p2[1]) * (p1[1] - p2[1]));
...
void ParSpaceTrimmer::initializeStartState(bezier2ddeque& tc) {
  ...
  DCTPVec3dvector& cp =
      b.getControlPointVector(); // std::cerr << "got control point vector " << std::endl;
  Vec2d p;
...
bool ParSpaceTrimmer::setMinIntersectionWithFace(BezierCurve2D& b) {
  ...
  for (std::vector<DCTPEdge*>::iterator i = state.f->edges.begin(); i != eend; ++i) {
    DCTPdvector res;
    DCTPVertex *v1, *v2;
    (*i)->getVertices(v1, v2);
    Vec2d v1coords(v1->coords[0], v1->coords[1]);
    ...
    ///             if ( res.size() ) std::cerr << "minintersection res.size(): " << res.size() << "
    ///             res[0]: " << res[0] << std::endl; else std::cerr <<"minintersection nothing" <<
    ///             std::endl;
    ...
    if (res.size() != 0) {
      ...
      //				std::cerr << ccl_temp << cptemp[ 0 ] << res[ 0 ]<<
      //std::endl;
...
bool ParSpaceTrimmer::StoreCurveApproximation(
    BezierCurve2D& bc, double t, std::vector<DCTPVertex*>& el) {
...
bool ParSpaceTrimmer::goingOutOnEdge(
    BezierCurve2D& bc, DCTPVertex*& v, bool& feu, std::vector<DCTPVertex*>& el) {
  bool on_edge = false;
  ///        std::cerr <<"goingout in..." << std::endl;
  std::vector<DCTPEdge*>::iterator eend = state.v->edges.end();
  for (std::vector<DCTPEdge*>::iterator i = state.v->edges.begin(); i != eend; ++i) {
    DCTPVertex *v1, *v2;
    (*i)->getVertices(v1, v2);
    DCTPdvector res;
    //                std::cerr << "goin'outonedge calling intersection and houston ;)" <<
    //                std::endl; dumpcontrolpoints( bc ); std::cerr << "v1: " << v1->coords << " v2:
    //                " << v2->coords << std::endl;
    Vec2d v1coords(v1->coords[0], v1->coords[1]);
    Vec2d v2coords(v2->coords[0], v2->coords[1]);
    ...
    //                std::cerr << "intersection result: " << r << "resultvector size: " <<
    //                res.size() << std::endl; if ( res.size() ) std::cerr <<"first intersec: " <<
    //                res[0] << std::endl;
    if (res.size() == 2) {
    ...
    case 2: {
      ...
      //								std::cerr << " res[0]: " <<
      //res[ 0 ] << " res[1]: " << res[ 1 ] << std::endl; 								std::cerr << " lineseg: " << v1coords << "
      //" << v2coords << std::endl;

      bc.subDivision(res[1], nc);
      bc = nc[1]; // nc[0] holdz da 2nd part
      /////								DCTPVec3dvector &cp =
      ///bc.getControlPointVector();
      //		                        if ( cp[ 0 ] == v1coords ) v = v1;
      //		                        else v = v2;
      //	                            std::cerr << "cp[0]: " << cp[0] << " v1: " << v1c << "
      //v2: " << v2c << std::endl;
      // FIXME:direct orig edge into v
      DCTPVec3dvector& cp2 = nc[0].getControlPointVector();
      Vec2d            cpeucl;
      ...
      if (DCTPVecIsEqual(cpeucl, v1coords))
        ...
        v = v1;
      //								std::cerr << "el.push_back 3
      //" << ( void* ) v << std::endl;
      el.push_back(v);
      //	                            Vec3d cp203d( cp2[ 0 ][0], cp2[ 0 ][1] );
      /*			                    if ( mesh->directEdge( cp203d, v->coords ) ) {
                                                                              std::cerr <<
         "directEdge ERROR!" << std::endl; throw ParSpaceTrimmerError(
         ParSpaceTrimmerError::ERR_DIRECTEDGE );
                                                                      }*/
      feu = false;
      return true;
    }
...
bool ParSpaceTrimmer::goingOutOnEdge2(
    Vec2d clAct, Vec2d clNext, DCTPVertex*& v, bool& feu, std::vector<DCTPVertex*>& el) {
  bool on_edge = false;
  //	std::cerr <<"goingout in..." << std::endl;
  std::vector<DCTPEdge*>::iterator eend = state.v->edges.end();
  for (std::vector<DCTPEdge*>::iterator i = state.v->edges.begin(); i != eend; ++i) {
      ...
      if (doIntersect(clAct, clNext, v1coords, v2coords, res)) {
        if (res - 1.0 > DCTP_EPS) {
          ...
          if (v == NULL) {
            ...
            //						std::cerr << v1coords << cp3d << v2coords <<
            //std::endl;
          } else {
            //						el.push_back( v );
            feu = true;
          ...
          if ((cp3p - Vec3d(v1coords)).squareLength() < (cp3p - Vec3d(v2coords)).squareLength())
            v = v1;
          else
            v = v2;
          //					el.push_back( v );
          ...
          Vec3d cp3d(clNext[0], clNext[1]);
          if ((cp3d - Vec3d(v1coords)).squareLength() < (cp3d - Vec3d(v2coords)).squareLength())
            v = v1;
          else
            v = v2;
          //					el.push_back( v );
...
DCTPFace* ParSpaceTrimmer::getContinuingFace2(Vec2d clAct, Vec2d clNext) {
  ...
  //        std::cerr <<" state.v: " << state.v->coords << " " << state.v->faces.size() <<
  //        std::endl;
  for (std::vector<DCTPFace*>::iterator i = state.v->faces.begin(); i != fe; ++i) {
  ...
  if (state.state == TrimState::OVER_FACE) {
    ...
    if (state.f->norm < DCTP_EPS) {
      while (m_pclQuadTree->computeApproximationError(state.f) > m_pclQuadTree->error_epsilon) {
        ...
        //					std::cerr << "subdividing face " << ( void* ) state.f << "
        //because of error " << m_pclQuadTree->computeApproximationError( state.f ) << std::endl;
    ...
    if (setMinIntersectionWithFace(b)) { // intersection exists
      ...
      if (vp == NULL) {
        ...
        /*							std::cerr << "5-" << ie << " " << vp <<
           std::endl; if( ( ( unsigned int ) vp ) == 0x2ec94840 )
                                                                {
                                                                        std::cerr << p << std::endl;
                                                                }*/
      }
      ...
      for (unsigned int ui_face = 0; ui_face < state.v->faces.size(); ++ui_face) {
        ...
        if (pcl_face->norm < DCTP_EPS) {
          if (m_pclQuadTree->computeApproximationError(pcl_face) > m_pclQuadTree->error_epsilon) {
            ...
            //							std::cerr << "subdividing face " << ( void* ) pcl_face << " because of
            //error " << m_pclQuadTree->computeApproximationError( pcl_face ) << std::endl;
            ...
            if (state.v->edges.size() <= 1) {
              ...
              if (pcl_find) {
                //									std::cerr << "delete
                //vertex1:" << ( void* ) state.v << std::endl; 									delete state.v; 									std::cerr <<
                //pcl_find->edges.size( ) << std::endl;
                state.v = pcl_find;
              } else {
                ...
                if (!isNearQuad(pcl_face, cl_v2d)) {
                  ...
                  state.v->edges[0] =
                      (DCTPEdge*)isOnFaceBoundary(state.v->faces[ui_face], cl_v2d, b_dummy);
                }
                if (isNearQuad(mesh->faces[mesh->faces.size() - 3], cl_v2d)) {
                  state.v->faces.push_back(mesh->faces[mesh->faces.size() - 3]);
                  state.v->edges[0] = (DCTPEdge*)isOnFaceBoundary(
                      mesh->faces[mesh->faces.size() - 3], cl_v2d, b_dummy);
                }
                if (isNearQuad(mesh->faces[mesh->faces.size() - 2], cl_v2d)) {
                  state.v->faces.push_back(mesh->faces[mesh->faces.size() - 2]);
                  state.v->edges[0] = (DCTPEdge*)isOnFaceBoundary(
                      mesh->faces[mesh->faces.size() - 2], cl_v2d, b_dummy);
                }
                if (isNearQuad(mesh->faces[mesh->faces.size() - 1], cl_v2d)) {
                  state.v->faces.push_back(mesh->faces[mesh->faces.size() - 1]);
                  state.v->edges[0] = (DCTPEdge*)isOnFaceBoundary(
                      mesh->faces[mesh->faces.size() - 1], cl_v2d, b_dummy);
                }
              }
            }
          } else {
            pcl_face->norm =
                m_pclQuadTree->error_epsilon / m_pclQuadTree->computeBilinearNorm(pcl_face);
            //							std::cerr << ( void* ) pcl_face << ":" << pcl_face->norm <<
            //std::endl;
          }
        }
      }
    ...
    if (goingOutOnEdge(b, v, feu, el)) { // goin' out on edge
      ...
      //						std::cerr << "inserting vertex in edge loop
      //" << state.v->coords << std::endl; 						std::cerr << "el.push_back 4 " << ( void* ) v <<
      //std::endl; 						el.push_back( v ); 						v->vertexinfo = ( void* ) 1;
      if (feu)
...
bool ParSpaceTrimmer::StateTransition2(
    Vec2d& rclAct, Vec2d clNext, std::vector<DCTPVertex*>& el, Vec3d& rclActS, Vec3d clNextS)
/* #else
bool ParSpaceTrimmer::StateTransition2( Vec2d &rclAct, Vec2d clNext, std::vector< DCTPVertex* > &el,
Vec3d &rclActS, Vec3d clNextS, Vec3d &rclActN, Vec3d clNextN ) #endif*/
#else
bool ParSpaceTrimmer::StateTransition2(
    Vec2d& rclAct, Vec2d clNext, std::vector<DCTPVertex*>& el, Vec3d& rclActS, Vec3d clNextS)
#endif
{
  ...
  if (state.state == TrimState::OVER_FACE) {
    //		std::cerr << " now over face..." << std::endl;
    // note: setMinIntersection... sets ie & ip member varz
    if (setMinIntersectionWithFace2(rclAct, clNext)) { // intersection exists
      //			int err;
      ...
      if (vp == NULL) {
        ...
        //				std::cerr << v1->coords << " -> " << v2->coords <<
        //std::endl; 				std::cerr << " ip: " << ip << " p: " << p << std::endl;
        std::cerr << "Brutal error I. in: "
      ...
      //			if( Vec3d( rclAct ) != vp->coords )
      {
        ...
        //				std::cerr << "add edge: " << el[ el.size( ) - 1 ]->coords << "->"
        //<< vp->coords << std::endl; 				std::cerr << vp->vertexinfo << std::endl;
        el.push_back(vp);
      }
      ...
      //			std::cerr << "rclAct == clNext " << ( rclAct == clNext ) <<
      //std::endl;
    ...
    //		std::cerr << "add edge: " << el[ el.size( ) - 1 ]->coords << "->" << v->coords <<
    //std::endl; 		std::cerr << v->vertexinfo << std::endl;
    el.push_back(v);
    ...
    if (DCTPVecIsEqual(rclAct, clNext))
      return false;
    if (goingOutOnEdge2(rclAct, clNext, v, feu, el)) { // goin' out on edge
      // IN_VERTEX state remains, but...
      if (feu) {
        rclActS = clNextS;
      } else {
      ...
      state.v = v;
      // insert edge in edgeloop
      //			std::cerr << "inserting vertex in edge loop " << state.v->coords <<
      //std::endl; 			if( Vec3d( rclAct ) != v->coords )
      {
        //				std::cerr << "direct edge: " << el[ el.size( ) - 1 ]->coords <<
        //"->" << v->coords << std::endl;
...
///        std::cerr << "got startface " << std::endl;
        ...
        for( std::vector<TrimSegment>::iterator i = start_face->trimseg.begin();
             ...
             if ( !i->start ) { s = i; } //std::cerr << "ParSpaceTrimmer::mergeCurve start found: "
<< std::endl; } if ( !i->end ) { e = i; } //std::cerr << "ParSpaceTrimmer::mergeCurve end found: "
<< std::endl; }
        }
///        if ( e == s ) std::cerr <<" basszajba, GECZI !!! " << std::endl;
        ...
        e->end = s->end;
//        std::cerr << "e->trimbeziers.size: " << e->trimbeziers.size() << " s->trimbeziers.size():
" << s->trimbeziers.size() << std::endl; e->trimbeziers.insert( e->trimbeziers.end(),
s->trimbeziers.begin(), s->trimbeziers.end() ); start_face->trimseg.erase( s );
}*/
...
#ifdef OSG_FORCE_NO_T_VERTICES
    // #ifdef OSG_CREATE_NORMAL_MAPS
    if (!StateTransition2(cl_act, cl_next, el, cl_act_s, (*m_pvvclSewed)[uiLoop][ui_vertex]))
/* #else
                if( !StateTransition2( cl_act, cl_next, el,
                                                           cl_act_s, ( *m_pvvclSewed )[ uiLoop ][
 ui_vertex ], cl_act_n, ( *m_pvvclNormals )[ uiLoop ][ ui_vertex ] ) ) #endif*/
#else
    if (!StateTransition2(cl_act, cl_next, el, cl_act_s, (*m_pvvclSewed)[uiLoop][ui_vertex]))
#endif
    {
...
int ParSpaceTrimmer::PerformTrimming(void) {
    ...
    for (unsigned int l = 0; l < lc; ++l) {
      if (!(*tcs)[l].empty()) {
        ...
        for (unsigned int c = 0; c < cc; ++c) {
          ...
          bc3d.approximate(
              params, terr, BezierCurve3D::MIDPOINT_SUBDIVISION | BezierCurve3D::LINE_DISTANCE);
  ...
  int          i_err;
  const double cd_error_quad = terr * terr * (1.0 - OSG_SIMPLIFIER_TESS_ERR) *
                               (1.0 - OSG_SIMPLIFIER_TESS_ERR) /
  ...
  /*	for( i = 0; i < vcel.size( ); ++i )
          {
                  ...
                  if( ui_vert_cnt )
                  {
                          for( ui_vert = 0; ui_vert < ui_vert_cnt; ++ui_vert )
                          {
                                  ...
                                  std::cerr << cl_uv << m_pclNurbs->compute( cl_uv, i_err ) <<
     std::endl;
                          }
                          std::cerr << std::endl;
                  }
    ...
    if (ui_vert_cnt) {
      ...
      for (ui_vert = 0; ui_vert < ui_vert_cnt; ++ui_vert) {
        cl_uv[0] = (vcel[i][vt_vertices[ui_vert].uiPrev]->coords[0] +
                       vcel[i][vt_vertices[ui_vert].uiNext]->coords[0]) *
                   0.5;
        cl_uv[1] = (vcel[i][vt_vertices[ui_vert].uiPrev]->coords[1] +
                       vcel[i][vt_vertices[ui_vert].uiNext]->coords[1]) *
                   0.5;
        cl_midpos                           = m_pclNurbs->compute(cl_uv, i_err);
        vt_vertices[ui_vert].dSimplifyError = osgMax(
            DistToEdge(vt_vertices[ui_vert].clPos, vt_vertices[vt_vertices[ui_vert].uiPrev].clPos,
                vt_vertices[vt_vertices[ui_vert].uiNext].clPos),
            DistToEdge(cl_midpos, vt_vertices[vt_vertices[ui_vert].uiPrev].clPos,
                vt_vertices[vt_vertices[ui_vert].uiNext].clPos));
      ...
      while ((!vt_sim_sort.empty()) && (ui_remain_cnt > 3)) {
        ...
        //				std::cerr << "remove " << ui_vert << " next: " << ui_next << "
        //prev: " << ui_prev << " err: " << sqrt( ( *vt_sim_sort.begin( ) )->dSimplifyError ) <<
        //std::endl;
        ...
        if (!mesh->findVertex(vcel[i][ui_vert]->coords)) {
          //					std::cerr << "delete vertex4:" << ui_vert << " " << ( void* ) vcel[
          //i ][ ui_vert ] << std::endl;
        ...
        if ((ui_vert != ui_prev) && (ui_vert != ui_next)) {
          cl_uv[0] =
              (vcel[i][vt_vertices[ui_prev].uiPrev]->coords[0] + vcel[i][ui_next]->coords[0]) * 0.5;
          cl_uv[1] =
              (vcel[i][vt_vertices[ui_prev].uiPrev]->coords[1] + vcel[i][ui_next]->coords[1]) * 0.5;
          cl_midpos  = m_pclNurbs->compute(cl_uv, i_err);
          d_max_dist = DistToEdge(cl_midpos, vt_vertices[vt_vertices[ui_vert].uiPrev].clPos,
          ...
          while (ui_vert != ui_next) {
            d_act_dist = DistToEdge(vt_vertices[ui_vert].clPos,
                vt_vertices[vt_vertices[ui_prev].uiPrev].clPos, vt_vertices[ui_next].clPos);
          ...
          vt_vertices[ui_prev].dSimplifyError +=
              sqrt((vt_vertices[vt_vertices[ui_prev].uiPrev].clPos -
                    vt_vertices[vt_vertices[ui_prev].uiNext].clPos)
                       .squareLength()) *
              0.00001;
          if (vt_vertices[ui_prev].dSimplifyError < cd_error_quad) {
          ...
          cl_uv[0] =
              (vcel[i][ui_prev]->coords[0] + vcel[i][vt_vertices[ui_next].uiNext]->coords[0]) * 0.5;
          cl_uv[1] =
              (vcel[i][ui_prev]->coords[1] + vcel[i][vt_vertices[ui_next].uiNext]->coords[1]) * 0.5;
          cl_midpos  = m_pclNurbs->compute(cl_uv, i_err);
          d_max_dist = DistToEdge(cl_midpos, vt_vertices[ui_prev].clPos,
              vt_vertices[vt_vertices[ui_next].uiNext].clPos);

          ui_vert = (ui_prev + 1) % ui_vert_cnt;
          while (ui_vert != vt_vertices[ui_next].uiNext) {
            d_act_dist = DistToEdge(vt_vertices[ui_vert].clPos, vt_vertices[ui_prev].clPos,
                vt_vertices[vt_vertices[ui_next].uiNext].clPos);
            if (d_act_dist > d_max_dist) {
              d_max_dist = d_act_dist;
            }
      ...
      ui_new     = 1;
      for (ui_vert = vt_vertices[ui_first].uiNext; ui_vert != ui_first;
           ui_vert = vt_vertices[ui_vert].uiNext) {
        vcel[i][ui_new] = vcel[i][ui_vert];
        ++ui_new;
      }
      vcel[i].resize(ui_new);
      //			std::cerr << "simplified " << ui_vert_cnt << " to " << ui_new <<
      //std::endl;
    }
...
//	std::cerr << ui_tloops << " trimming loops" << std::endl;
  ...
  for (ui_i = 0; ui_i < ui_tloops; ++ui_i) {
    if (((*m_pvbUsed)[ui_i]) && ((*m_pvbReversed)[ui_i])) {
      unsigned int v; //, eid, oid;
      Vec2d        cl_temp_vec;
      ...
      if (pcl_inside_face) {
        if (cl_max[0] - cl_min[0] > cl_max[1] - cl_min[1]) {
          d_ratio =
              ((cl_min[0] + cl_max[0]) * 0.5 - pcl_inside_face->orig_face[0]->coords[0]) /
              (pcl_inside_face->orig_face[1]->coords[0] - pcl_inside_face->orig_face[0]->coords[0]);
          ...
          d_ratio =
              ((cl_min[1] + cl_max[1]) * 0.5 - pcl_inside_face->orig_face[3]->coords[1]) /
              (pcl_inside_face->orig_face[0]->coords[1] - pcl_inside_face->orig_face[3]->coords[1]);
...
#ifdef OSG_FORCE_NO_T_VERTICES
#ifdef OSG_KEEP_2D_POINTS
int ParSpaceTrimmer::buildSurfaceGraph(DirectedGraph<Vec2d, unsigned char>* psg,
    std::vector<Vec3d>* pvclSewed, std::vector<Vec3d>* pvclNormals,
    std::vector<unsigned int>* pvuiIdx)
#else
int ParSpaceTrimmer::buildSurfaceGraph(DirectedGraph<Vec2d, unsigned char>* psg,
    std::vector<Vec3d>* pvclSewed, std::vector<Vec3d>* pvclNormals)
#endif
#else
int ParSpaceTrimmer::buildSurfaceGraph(
    DirectedGraph<Vec2d, unsigned char>* psg, std::vector<Vec3d>* pvclSewed)
#endif
{
...
// 			std::cerr << "ids: " << pvclSewed->size( ) << " " << ( *i )->node_id <<
// std::endl;
#ifdef OSG_FORCE_NO_T_VERTICES
      if (pvclSewed)
        pvclSewed->push_back(((SPosNorm*)(*i)->vertexinfo)->clPos);
        /* #ifndef OSG_CREATE_NORMAL_MAPS
                                if( pvclNormals ) pvclNormals->push_back( ( ( SPosNorm* ) ( *i
         )->vertexinfo )->clNorm ); #endif*/
#ifdef OSG_KEEP_2D_POINTS
      //			std::cerr << pvuiIdx->size( ) << ": " << ( ( SPosNorm* ) ( *i
      //)->vertexinfo )->uiNum << std::endl;
      if (pvuiIdx)
        pvuiIdx->push_back(((SPosNorm*)(*i)->vertexinfo)->uiNum);
#endif
#else
      if (pvclSewed)
        pvclSewed->push_back(*((Vec3d*)(*i)->vertexinfo));
#endif
    }
  }
#ifdef OSG_KEEP_2D_POINTS
...
#endif
  {
    ...
    //		if( orient ) std::cerr << "adding edge " << v1->node_id << ", " << v2->node_id <<
    //std::endl;
 ...
 //						std::cerr << "inserting end node " << sn << ", " <<
 j->end->node_id << std::endl; psg->AddEdge( dummy, sn, j->end->node_id, true );
  ...
  for (unsigned int loop = 0; loop < vcel.size(); ++loop) {
    //		std::cerr << "edge loop " << loop << " of " << vcel.size( ) << " (" << vcel[ loop ].size(
    //) << ")" << std::endl;
    unsigned int v; //, eid, oid;
    ...
    if ((*m_pvbReversed)[loop]) {
      ...
      for (v = 0; v < vcel[loop].size(); ++v) {
        ...
        //				if( oid != nid )
        //				{
        //					std::cerr << "oid " << oid << ", nid " << nid <<
        //std::endl; 					std::cerr << "tvec " << tvec << std::endl; 					std::cerr << "inserted in loop." <<
        //std::endl;
        /*					eid = psg->FindEdge( oid, nid );
                                                if( eid == -1 )
                                                {
        //						std::cerr << "adding edge to graph: from "
        << oid << " to " << nid << std::endl; psg->AddEdge( dummy, oid, nid, true );
                                                }
                                                else
                                                {
        //						std::cerr << "directing edge in graph: from
        " << oid << " to " << nid << std::endl; psg->setEdgeDirection( eid, nid );
                                                }
                                        }*/
      }
      if (pcl_inside_face) {
        ...
        while ((!b_ok) && (--ui_exit > 0)) {
          //					std::cerr << "check " << pcl_nearest_ul->coords << " " <<
          //pcl_upper_left->coords << std::endl;
          b_ok = true;
          ...
          for (ui_check = 0; ui_check < vcel.size(); ++ui_check) {
            if (ui_check != loop) {
              ...
              if (pcl_act_point) {
                ...
                ui_check       = vcel.size();
                b_ok           = false;
                /// std::cerr << "o=> " << pcl_nearest_ul->coords << " " << pcl_upper_left->coords
                /// << std::endl;
              }
            }
          }
          ...
          if (pcl_act_point) {
            pcl_nearest_ul = pcl_act_point;
            b_ok           = false;
            /// std::cerr << "t=> " << pcl_nearest_ul->coords << " " << pcl_upper_left->coords <<
            /// std::endl;
          }
            ...
            if ((pcl_nearest_ul != vpcl_added_from[ui_check]) &&
              ...
              if (doIntersect(vpcl_added_from[ui_check]->coords, vpcl_added_to[ui_check]->coords,
                ...
                if (d_act_dist <= 1.0) {
                  ...
                  /// std::cerr << "e=> " << pcl_nearest_ul->coords << " " << pcl_upper_left->coords
                  /// << std::endl;
                }
              }
            }
        ...
        if (pcl_upper_left != pcl_nearest_ul) {
          ...
          psg->AddEdge(dummy, pcl_upper_left->node_id, pcl_nearest_ul->node_id, false);
        }
        ...
        ui_exit = 1000;
        while ((!b_ok) && (--ui_exit > 0)) {
          //					std::cerr << "check " << pcl_nearest_lr->coords << " " <<
          //pcl_lower_right->coords << std::endl;
          b_ok = true;
          // check if there is an intersection with other loops!
          for (ui_check = 0; ui_check < vcel.size(); ++ui_check) {
            if (ui_check != loop) {
              ...
              if (pcl_act_point) {
                ...
                ui_check        = vcel.size();
                b_ok            = false;
                /// std::cerr << "o=> " << pcl_nearest_lr->coords << " " << pcl_lower_right->coords
                /// << std::endl;
              }
            }
          }
          ...
          if (pcl_act_point) {
            pcl_nearest_lr = pcl_act_point;
            b_ok           = false;
            /// std::cerr << "t=> " << pcl_nearest_lr->coords << " " << pcl_lower_right->coords <<
            /// std::endl;
          }
          // check if there is an intersection with existing connect edges
          for (ui_check = 0; ui_check < vpcl_added_from.size(); ++ui_check) {
            if ((pcl_nearest_lr != vpcl_added_from[ui_check]) &&
              ...
              if (doIntersect(vpcl_added_from[ui_check]->coords, vpcl_added_to[ui_check]->coords,
                ...
                if (d_act_dist <= 1.0) {
                  ...
                  b_ok     = false;
                  ui_check = vpcl_added_from.size();
                  /// std::cerr << "e=> " << pcl_nearest_lr->coords << " " <<
                  /// pcl_lower_right->coords << std::endl;
                }
              }
            }
        ...
        if (pcl_lower_right != pcl_nearest_lr) {
          ...
          psg->AddEdge(dummy, pcl_lower_right->node_id, pcl_nearest_lr->node_id, false);
        }
...
void ParSpaceTrimmer::checkEdgeloops() {
  ...
  for (ui_loop = 0; ui_loop < vcel.size(); ++ui_loop) {
      ...
      if (DCTPVecIsNotEqual(cl_p, cl_pp)) {
        //				if( vcel[ ui_loop ][ ui_vertex ]->vertexinfo )
        {
          ...
          if (vpcl_new_points[ui_new_num] != vcel[ui_loop][ui_vertex]) {
            //						std::cerr << ( void* ) vpcl_new_points[ ui_new_num ] << "
            //"; 						std::cerr << vcel[ ui_loop ][ ui_vertex ]->edges.size( ) << " "; 						std::cerr <<
            //"delete vertex2:" << ( void* ) vcel[ ui_loop ][ ui_vertex ] << std::endl; 						std::cerr <<
            //vpcl_new_points[ ui_new_num ]->coords << " "; 						delete vcel[ ui_loop ][ ui_vertex ];
        ...
        /*				else
                                        {
                                                const Vec3d	ccl_pn = vcel[ ui_loop ][ (
           ui_vertex + 1 ) % ui_size ]->coords; double		d_d1[ 2 ], d_d2[ 2 ], d_d3[ 2 ];
                                                ...
                                                {
                                                        vpcl_new_points[ ui_new_num ] = vcel[
           ui_loop ][ ui_vertex ];
                                                        ++ui_new_num;
                                                        ui_last = ui_vertex;
                                                }
        ...
        if (!mesh->findVertex(cl_p)) {
          //					std::cerr << "delete vertex3:" << ( void* ) vcel[ ui_loop ][
          //ui_vertex ] << std::endl;
    ...
    //		if( ui_new_num != ui_size )
    {
      ...
      for (ui_vertex = 0; ui_vertex < ui_size; ++ui_vertex) {
        ...
        //				std::cerr << vpcl_new_points[ ui_vertex ]->coords <<
        //std::endl;
      }
    }
    ...
    if (ui_size > 2) {
      ...
      for (ui_vertex = 0; ui_vertex < ui_size; ++ui_vertex) {
        if (vcel[ui_loop][ui_last]->id != vcel[ui_loop][ui_vertex]->id) {
          ...
          //					std::cerr << "new edge " << vcel[ ui_loop ][ ui_last ]->id << " " <<
          //vcel[ ui_loop ][ ui_vertex ]->id << std::endl;
  ...
  while (!spt_events.empty()) {
    ...
    if (pt_act_edge->ptEntry == NULL) {
      ...
      if (!pt_act_edge->bInvalid) {
        ...
        while (pt_act_entry) {
          /*					std::cerr << pt_act_entry << std::endl;
                                                  std::cerr <<
             pt_act_entry->ptEdge->pclFromVertex->coords << " -> "; std::cerr <<
             pt_act_entry->ptEdge->pclToVertex->coords << std::endl;*/
          if (DCTPVecIsGreater(pt_act_entry->ptEdge->pclFromVertex->coords,
                  pt_act_entry->ptEdge->pclToVertex->coords)) {
            //						std::cerr << "+";
      ...
      if (!pt_act_edge->bInvalid) {
        if (DCTPVecIsGreater(
                pt_act_edge->pclFromVertex->coords, pt_act_edge->pclToVertex->coords)) {
          // edge goes down => must be closed
          //					std::cerr << "inside = " << i_inside << ", should be -1" <<
          //std::endl;
          b_valid = (i_inside == -1);
        } else {
          // edge goes up => rest must be ok
          //					std::cerr << "inside = " << i_inside << ", should be 0" <<
          //std::endl;
          b_valid = (i_inside == 0);
        }
        if (b_valid) {
          ...
          for (ui_loop = 0; ui_loop < ui_size; ++ui_loop) {
            if (pt_act_edge->pclToVertex == vpt_start[ui_loop]->pclFromVertex) {
              //							std::cerr << "appending edge
              //to polyline " << ui_loop << std::endl;
          ...
          if (ui_loop < ui_size) {
            ...
            if (vpt_start[ui_loop]->pclFromVertex == vpt_end[ui_loop]->pclToVertex) {
              ...
              if (ui_loop2 < ui_size) {
                //								std::cerr << "removing old
                //polyline " << ui_loop2 << std::endl;
            ...
            //						std::cerr << "starting new polyline " << ui_loop <<
            //std::endl;
            // start new polyline
          ...
          //					std::cerr << "delete edge " << pt_act_edge->pclFromVertex->id << " "
          //<< pt_act_edge->pclToVertex->id << std::endl;
          delete pt_act_edge;
        }
      } else {
        //				std::cerr << "delete edge " << pt_act_edge->pclFromVertex->id << "
      ...
      if ((pt_next_entry) && (pt_next_entry->ptPrev)) {
        if ((!pt_next_entry->ptEdge->bInvalid) && (!pt_next_entry->ptPrev->ptEdge->bInvalid)) {
          //					std::cerr << pt_next_entry << ", " << pt_next_entry->ptPrev <<
          //std::endl;
  ...
  /*	if( vpt_start.size( ) > 0 )
          {
                  ...
                  std::cerr << vpt_start[ 0 ]->pclFromVertex->coords << " <-> " << vpt_end[ 0
  ]->pclToVertex->coords << std::endl;
  //		exit( -1 );
          }*/
  ui_size = vpt_start.size();
  for (ui_loop = 0; ui_loop < ui_size; ++ui_loop) {
...
bool ParSpaceTrimmer::insertScanLineEvents(
    SScanLineEdge* ptEdge, ScanLineEventSet& rsptEvents, char cWhich) {
  ...
  /*	int											i_select =
     0;
          ...
          else if( ptEdge->pclFromVertex->coords[1] > ptEdge->pclToVertex->coords[1] ) i_select =
     -1; else if( ptEdge->pclFromVertex->coords[0] < ptEdge->pclToVertex->coords[0] ) i_select = 1;
          else if( ptEdge->pclFromVertex->coords[0] > ptEdge->pclToVertex->coords[0] ) i_select =
     -1;*/
  ...
  } else if (DCTPVecIsEqual(ptEdge->pclFromVertex->coords, ptEdge->pclToVertex->coords))
  //	else if( i_select == 0 )
  {
    return false;
  } else {
...
SScanLineEntry* ParSpaceTrimmer::insertInScanLine(
    SScanLineEntry* ptActEntry, SScanLineEntry* ptScanLine) {
...
void ParSpaceTrimmer::saveLoop(SScanLineEdge* ptEdge) {
  ...
  while (ptEdge) {
    ...
    //		std::cerr << "delete edge " << pt_del->pclFromVertex->id << " " << pt_del->pclToVertex->id
    //<< std::endl;
    delete pt_del;
  }
...
void ParSpaceTrimmer::checkSLIntersection(
    SScanLineEntry* ptActEntry, ScanLineEventSet& rsptEvents) {
...
void ParSpaceTrimmer::ScanLineIntersect(
    SScanLineEntry* ptEntry1, SScanLineEntry* ptEntry2, ScanLineEventSet& rsptEvents) {
  ...
  if (((cd_r1 < 0.0) && (cd_r2 < 0.0)) || ((cd_r1 > 0.0) && (cd_r2 > 0.0))) {
    return;
  }
  ...
  if (((cd_r3 < 0.0) && (cd_r4 < 0.0)) || ((cd_r3 > 0.0) && (cd_r4 > 0.0))) {
    return;
  }
  ...
  if (((cd_r1 == 0.0) && (cd_r2 == 0.0) && (cd_r3 == 0.0) && (cd_r4 == 0.0)) ||
    ...
    if (DCTPVecIsEqual(pcl_ip1->coords, pcl_ip2->coords)) {
      ...
      pcl_split_vertex = DCTPVecIsLesser(pcl_ip1->coords, pcl_ip2->coords) ? pcl_ip1 : pcl_ip2;
    } else {
      pcl_split_vertex = DCTPVecIsGreater(pcl_ip1->coords, pcl_ip2->coords) ? pcl_ip1 : pcl_ip2;
    }
  } else {
    ...
    if ((cd_v1x * cd_v1x + cd_v1y * cd_v1y) < (cd_v2x * cd_v2x + cd_v2y * cd_v2y)) {
      ...
      //			std::cerr << cd_r1 << " " << cd_r2 << " " << cd_r3 << " " << cd_r4
      //<< std::endl;
      ...
      if (d_q < DCTP_EPS) {
        ...
        //				cl_ip[0] = 1e-7 * floor( ( pt_edge1->pclFromVertex->coords[0] +
        //cd_v1x * d_q ) / 1e-7 ); 				cl_ip[1] = 1e-7 * floor( ( pt_edge1->pclFromVertex->coords[1] +
        //cd_v1y * d_q ) / 1e-7 );
        cl_ip[2]         = 0.0;
      ...
      if (d_q < DCTP_EPS) {
        ...
        //				cl_ip[0] = 1e-7 * floor( ( pt_edge2->pclFromVertex->coords[0] +
        //cd_v2x * d_q ) / 1e-7 ); 				cl_ip[1] = 1e-7 * floor( ( pt_edge2->pclFromVertex->coords[1] +
        //cd_v2y * d_q ) / 1e-7 );
        cl_ip[2]         = 0.0;
        pcl_split_vertex = mesh->AddVertex(cl_ip);
      }
  ...
  //	std::cerr << "edge1: " << pt_edge1->pclFromVertex->coords << " - " <<
  //pt_edge1->pclToVertex->coords << std::endl; 	std::cerr << "edge2: " <<
  //pt_edge2->pclFromVertex->coords << " - " << pt_edge2->pclToVertex->coords << std::endl;

  // now we have the point => split edges
  if ((pcl_split_vertex != pt_edge1->pclFromVertex) &&
      (pcl_split_vertex != pt_edge1->pclToVertex)) {
    // split edge1
    //		std::cerr << "spliting edge1: " << pt_edge1->pclFromVertex->coords << " - " <<
    //pt_edge1->pclToVertex->coords << std::endl;
    ...
    //		std::cerr << "new edge1: " << pt_edge1->pclFromVertex->coords << " - " <<
    //pt_edge1->pclToVertex->coords << std::endl; 		std::cerr << "new edge: " <<
    //pt_new_edge->pclFromVertex->coords << " - " << pt_new_edge->pclToVertex->coords << std::endl;
    pcl_split_vertex->vertexinfo = (void*)1;
  }
  if ((pcl_split_vertex != pt_edge2->pclFromVertex) &&
      (pcl_split_vertex != pt_edge2->pclToVertex)) {
    // split edge2
    //		std::cerr << "spliting edge2: " << pt_edge2->pclFromVertex->coords << " - " <<
    //pt_edge2->pclToVertex->coords << std::endl;
    pt_new_edge = new SScanLineEdge;
    ...
    //		std::cerr << "new edge2: " << pt_edge2->pclFromVertex->coords << " - " <<
    //pt_edge2->pclToVertex->coords << std::endl; 		std::cerr << "new edge: " <<
    //pt_new_edge->pclFromVertex->coords << " - " << pt_new_edge->pclToVertex->coords << std::endl;
    pcl_split_vertex->vertexinfo = (void*)1;
  }
}
...
void ParSpaceTrimmer::removeSLEntry(SScanLineEntry* ptEntry, ScanLineEventSet& rsptEvents) {
  ...
  if (DCTPVecIsLesser(
          ptEntry->ptEdge->pclFromVertex->coords, ptEntry->ptEdge->pclToVertex->coords)) {
...
DCTPVertex* ParSpaceTrimmer::intersectsLoop(
    DCTPVertex* pclVertex1, DCTPVertex* pclVertex2, unsigned int uiLoop) {
  unsigned int ui_vertex;
      ...
      if (((cd_r1 >= 0.0) || (cd_r2 >= 0.0)) && ((cd_r1 <= 0.0) || (cd_r2 <= 0.0))) {
        ...
        if (((cd_r3 >= 0.0) || (cd_r4 >= 0.0)) && ((cd_r3 <= 0.0) || (cd_r4 <= 0.0))) {
          if ((cd_r1 == 0.0) && (cd_r2 == 0.0) && (cd_r3 == 0.0) && (cd_r4 == 0.0)) {
            ...
            //						std::cerr << pclVertex1->coords << "->" << pclVertex2->coords <<
            //std::endl; 						std::cerr << pcl_prev_vertex->coords << "->" << pcl_act_vertex->coords <<
            //std::endl;
            cl_old = pclVertex1->coords - pclVertex2->coords;
            if ((Vec2d(pclVertex1->coords - pcl_prev_vertex->coords)).dot(Vec2d(cl_old)) -
                    (Vec2d(pclVertex1->coords - pcl_act_vertex->coords)).dot(Vec2d(cl_old)) <
                DCTP_EPS) {
              //							std::cerr << "=>" <<
              //pcl_prev_vertex->coords << std::endl;
              return pcl_prev_vertex;
            }
            //						std::cerr << "=>" << pcl_act_vertex->coords <<
            //std::endl;
            return pcl_act_vertex;
          }
        }
...
bool ParSpaceTrimmer::doIntersect(Vec2d clV1, Vec2d clV2, Vec2d clV3, Vec2d clV4, double& rdParam) {
  ...
  if ((cd_r1 == 0.0) && (cd_r2 == 0.0) && (cd_r3 == 0.0) && (cd_r4 == 0.0)) {
    ...
    rdParam = -1.0;
    d_tmp =
        ((clV4[0] - clV1[0]) * (clV2[0] - clV1[0]) + (clV4[1] - clV1[1]) * (clV2[1] - clV1[1])) /
        (clV2 - clV1).squareLength();
    //		std::cerr << d_tmp << " ";
    if (d_tmp > DCTP_EPS)
      rdParam = d_tmp;
    d_tmp =
        ((clV3[0] - clV1[0]) * (clV2[0] - clV1[0]) + (clV3[1] - clV1[1]) * (clV2[1] - clV1[1])) /
        (clV2 - clV1).squareLength();
...
bool ParSpaceTrimmer::isLoopValid(const unsigned int cuiLoop) {
  ...
  for (ui_loop = 0; ui_loop < cui_loop_cnt; ++ui_loop) {
    if (ui_loop != cuiLoop) {
      ...
      for (ui_vertex = 0; ui_vertex < ui_vertex_cnt; ++ui_vertex) {
        //				std::cerr << "check: " << ( *pvccrd )[ ui_loop ][ ui_vertex ] << "
        //-> " << ( *pvccrd )[ ui_loop ][ ( ui_vertex + 1 ) % ui_vertex_cnt ] << std::endl;
        if (intersectsRay((*pvccrd)[ui_loop][ui_vertex],
                (*pvccrd)[ui_loop][(ui_vertex + 1) % ui_vertex_cnt], cl_ray_start, cl_ray_end)) {
...
bool ParSpaceTrimmer::intersectsRay(
    const Vec2d cclV1, const Vec2d cclV2, const Vec2d cclStart, const Vec2d cclEnd) {
  double ad_v1[2];
  double ad_v2[2];
  double ad_start[2];
  double ad_end[2];
...
#ifdef OSG_USE_SIMPLIFIER
double ParSpaceTrimmer::DistToEdge(
    const Vec3d cclPoint, const Vec3d cclLine1, const Vec3d cclLine2) const {
