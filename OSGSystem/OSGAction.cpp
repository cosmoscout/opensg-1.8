...
void Action::registerEnterDefault(const FieldContainerType& type, const Action::Functor& func) {
...
void Action::registerLeaveDefault(const FieldContainerType& type, const Action::Functor& func) {
...
void Action::registerEnterFunction(const FieldContainerType& type, const Action::Functor& func) {
  while (type.getId() >= _enterFunctors.size()) {
    _enterFunctors.push_back(osgTypedFunctionFunctor2CPtrRef<ResultE, CNodePtr, Action*>(
        &Action::_defaultEnterFunction));
  }
...
void Action::registerLeaveFunction(const FieldContainerType& type, const Action::Functor& func) {
  while (type.getId() >= _leaveFunctors.size()) {
    _leaveFunctors.push_back(osgTypedFunctionFunctor2CPtrRef<ResultE, CNodePtr, Action*>(
        &Action::_defaultLeaveFunction));
  }
...
Action::ResultE Action::recurse(NodePtr node) {
  ...
  if (core == NullFC) {
    SWARNING << "recurse: core is Null,  don't know what to do!" << std::endl;
    return Quit;
  }
...
Action::ResultE Action::_defaultEnterFunction(
    CNodePtr& OSG_CHECK_ARG(node), Action* OSG_CHECK_ARG(action)) {
  return Continue;
}

Action::ResultE Action::_defaultLeaveFunction(
    CNodePtr& OSG_CHECK_ARG(node), Action* OSG_CHECK_ARG(action)) {
  return Continue;
}
...
inline Action::ResultE doCallEnter(NodePtr node, TraverseEnterFunctor& func) {
  return func.call(node);
}

inline Action::ResultE doCallLeave(NodePtr node, Action::ResultE res, TraverseLeaveFunctor& func) {
...
Action::ResultE OSG::traverse(NodePtr node, TraverseEnterFunctor func) {
  ...
  case Action::Continue:
    return traverse(node->getMFChildren()->getValues(), func);
  default:
    break;
  }
...
OSG_SYSTEMLIB_DLLMAPPING
Action::ResultE OSG::traverse(
    NodePtr node, TraverseEnterFunctor enter, TraverseLeaveFunctor leave) {
  Action::ResultE res = Action::Continue;
  ...
  case Action::Skip:
    ...
    break;
  case Action::Continue:
    res = traverse(node->getMFChildren()->getValues(), enter, leave);
  default:
    break;
  }
