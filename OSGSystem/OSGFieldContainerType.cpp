...
inline void FieldContainerType::registerType(const Char8* szGroupName) {
...
FieldContainerType::FieldContainerType(const Char8* szName, const Char8* szParentName,
    const Char8* szGroupName, PrototypeCreateF fPrototypeCreate, InitContainerF fInitMethod,
    FieldDescription** pDesc, UInt32 uiDescByteCounter, bool bDescsAddable)
...
UInt32 FieldContainerType::addDescription(const FieldDescription& desc) {
  ...
  if (desc.isValid()) {
    if (descIt == _mDescMap.end()) {
      ...
      descVIt = std::find(_vDescVec.begin(), _vDescVec.end(), pNullDesc);
      ...
      SWARNING << "ERROR: Double field description "
               << "in " << _szName.str() << " from " << desc.getCName()
               << " (id:" << desc.getTypeId() << ")" << std::endl;
    }
  } else {
    SWARNING << "ERROR: Invalid field description "
             << "in " << _szName.str() << " from " << desc.getTypeId() << std::endl;
  }
...
NodePtr FieldContainerType::createNode(void) const {
  ...
  if (isAbstract() == false && isNode() == true) {
...
NodeCorePtr FieldContainerType::createNodeCore(void) const {
  ...
  if (isAbstract() == false && isNodeCore() == true) {
...
AttachmentPtr FieldContainerType::createAttachment(void) const {
  ...
  if (isAbstract() == false && isAttachment() == true) {
...
void FieldContainerType::dump(
    UInt32 OSG_CHECK_ARG(uiIndent), const BitVector OSG_CHECK_ARG(bvFlags)) const {
  SLOG << "FieldContainerType: " << getCName() << ", Id: " << getId()
       << ", parentP: " << (_pParent ? _pParent->getCName() : "NONE") << ", groupId: " << _uiGroupId
       << ", abstract: " << ((_pPrototype != NullFC) ? "false" : "true") << " " << _vDescVec.size()
       << std::endl;
  ...
  for (i = 0; i < _uiDescByteCounter / sizeof(FieldDescription*); i++) {
    if (_pDesc[i]->isValid()) {
      ...
      if (descIt == _mDescMap.end()) {
        ...
        SWARNING << "ERROR: Double field description "
                 << "in " << _szName.str() << " from " << _pDesc[i]->getCName()
                 << " (id:" << _pDesc[i]->getTypeId() << ")" << std::endl;

        _bInitialized = false;
      }
    } else {
      SWARNING << "ERROR: Invalid field description "
               << "in " << _szName.str() << "from " << (_pDesc[i] ? _pDesc[i]->getTypeId() : 0)
               << std::endl;
...
bool FieldContainerType::initParentFields(void) {
  ...
  if (_szParentName.str() != NULL) {
    _pParent = FieldContainerFactory::the()->findType(_szParentName.str());

    if (_pParent == NULL) {
      _pParent = FieldContainerFactory::the()->findUninitializedType(_szParentName.str());
    }
    ...
    if (_pParent != NULL) {
      ...
      for (dPIt = _pParent->_mDescMap.begin(); dPIt != _pParent->_mDescMap.end(); ++dPIt) {
        if (_mDescMap.find((*dPIt).first) == _mDescMap.end()) {
          _mDescMap[(*dPIt).first] = (*dPIt).second;
        } else {
          SWARNING << "ERROR: Can't add field "
                   << "description a second time: " << (*dPIt).first.str() << std::endl;
        }
      }

      _vDescVec.insert(_vDescVec.end(), _pParent->_vDescVec.begin(), _pParent->_vDescVec.end());

    } else {
      SWARNING << "ERROR: Can't find type with "
               << "name " << _szParentName.str() << std::endl;
...
bool FieldContainerType::initialize(void) {
  ...
  if (_vDescVec.size() > sizeof(BitVector) * 8) {
    FWARNING(
        ("FCType %s has %d (>%d) fields!\n", getCName(), _vDescVec.size(), sizeof(BitVector) * 8));
  }
