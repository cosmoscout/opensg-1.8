...
void DisplayFilterForeground::dump(UInt32, const BitVector) const {
...
void DisplayFilterForeground::draw(DrawActionBase* action, Viewport* port) {
  ...
  for (fI = getFilter().begin(); fI != getFilter().end(); ++fI) {
    if ((*fI)->getEnabled())
...
DisplayFilterForeground::DisplayFilterGroup* DisplayFilterForeground::findReadbackGroup(
...
DisplayFilterForeground::DisplayFilterGroup* DisplayFilterForeground::findOverlayGroup(
    const std::string& name) {
  DisplayFilterGroup*                                  group;
  std::map<std::string, DisplayFilterGroup*>::iterator cI;
...
DisplayFilterForeground::DisplayFilterGroup::DisplayFilterGroup(
    bool readback, std::vector<ViewportPtr>& ports) {
