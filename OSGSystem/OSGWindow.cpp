...
void OSG::Window::insertPort(UInt32 portIndex, const ViewportPtr& portP) {
  ...
  if (portP != NullFC) {
    ...
    beginEditCP(portP, Viewport::ParentFieldMask);
    { (*(_mfPort.insert(portIt, portP)))->setParent(WindowPtr(*this)); }
    endEditCP(portP, Viewport::ParentFieldMask);
  }
}
...
void OSG::Window::replacePort(UInt32 portIndex, const ViewportPtr& portP) {
  if (portP != NullFC) {
    ...
    //            FieldContainer::getPtr<WindowPtr>(*this));
    _mfPort[portIndex]->setParent(WindowPtr(*this));
  }
}

void OSG::Window::replacePortBy(const ViewportPtr& portP, const ViewportPtr& newportP) {
  ...
  if (newportP != NullFC) {
    if (portIt != _mfPort.end()) {
      ...
      //            (*portIt)->setParent(
      //                FieldContainer::getPtr<WindowPtr>(*this));
      (*portIt)->setParent(WindowPtr(*this));
    }
  }
}
...
void OSG::Window::validateGLObject(UInt32 id) {
  ...
  {
    _lastValidate.insert(_lastValidate.end(), id + 1 - _lastValidate.size(), 0);
  }

  FDEBUG(("Window 0x%p (event %d,ri:%d,rf:%d): "
          "Validating object %d: last reinit:%d, last validate:"
          "%d last refresh: %d => %s\n",
      this, getGlObjectEventCounter(), _mfGlObjectLastReinitialize.size(),
      _mfGlObjectLastRefresh.size(), id,
      (_mfGlObjectLastReinitialize.size() > id) ? _mfGlObjectLastReinitialize[id] : 0xffffffff,
      _lastValidate[id],
      (_mfGlObjectLastRefresh.size() > id) ? _mfGlObjectLastRefresh[id] : 0xffffffff,
      (_mfGlObjectLastReinitialize[id] == 0)
          ? "init"
          : ((_mfGlObjectLastReinitialize[id] > _lastValidate[id])
                    ? "reinit"
...
void OSG::Window::refreshGLObject(UInt32 id) {
  ...
  for (it = _allWindows.begin(); it != _allWindows.end(); ++it) {
    beginEditCP(*it, GlObjectEventCounterFieldMask | GlObjectLastRefreshFieldMask);
    ...
    endEditCP(*it, GlObjectEventCounterFieldMask | GlObjectLastRefreshFieldMask);
  }
}
...
void OSG::Window::reinitializeGLObject(UInt32 id) {
  ...
  for (it = _allWindows.begin(); it != _allWindows.end(); ++it) {
    beginEditCP(*it, GlObjectEventCounterFieldMask | GlObjectLastReinitializeFieldMask);
    ...
    endEditCP(*it, GlObjectEventCounterFieldMask | GlObjectLastReinitializeFieldMask);
  }
}
...
void OSG::Window::doInitRegisterGLObject(UInt32 id, UInt32 num) {
  ...
  beginEditCP(win, GlObjectLastReinitializeFieldMask | GlObjectLastRefreshFieldMask);
  ...
  endEditCP(win, GlObjectLastReinitializeFieldMask | GlObjectLastRefreshFieldMask);
}
  ...
  for (it = _allWindows.begin(); it != _allWindows.end(); ++it) {
    ...
    if (id + num > (*it)->_mfGlObjectLastReinitialize.size()) {
      FWARNING(("Window::destroyGLObject: id %d + num %d exceed"
          ...
          id, num, (*it)->_mfGlObjectLastReinitialize.size()));
      return;
    }
#endif
...
UInt32 OSG::Window::registerExtension(const Char8* s) {
  ...
  i = std::find(_registeredExtensions.begin(), _registeredExtensions.end(), s);
...
bool OSG::Window::hasExtension(const Char8* s) {
  if (std::find(_ignoredExtensions.begin(), _ignoredExtensions.end(), s) !=
      _ignoredExtensions.end()) {
    return false;
  }

  if (std::find(_extensions.begin(), _extensions.end(), s) != _extensions.end()) {
    return true;
  }
...
void OSG::Window::ignoreExtensions(const Char8* s) {
  ...
  std::back_insert_iterator<std::vector<std::string>> extension_back_inserter(_ignoredExtensions);
  ...
  for (string_token_iterator ignit = string_token_iterator(toex, ",. ");
    ...
    if (std::find(_ignoredExtensions.begin(), _ignoredExtensions.end(), ignore.c_str()) !=
        _ignoredExtensions.end()) {
    ...
    regit = std::find(_registeredExtensions.begin(), _registeredExtensions.end(), ignore.c_str());
    ...
    for (winit = _allWindows.begin(); winit != _allWindows.end(); ++winit) {
      ...
      extit = std::find((*winit)->_extensions.begin(), (*winit)->_extensions.end(), ignore.c_str());
...
*/
UInt32 OSG::Window::registerFunction(const Char8* s, Int32 ext, UInt32 version) {
  if (s == NULL)
    return TypeTraits<UInt32>::getMax();
  ...
  i = std::find(_registeredFunctions.begin(), _registeredFunctions.end(), s);
...
void OSG::Window::registerConstant(GLenum val) {
  ...
  if (std::find(_registeredConstants.begin(), _registeredConstants.end(), val) ==
...
void OSG::Window::frameInit(void) {
  ...
  if (_registeredExtensions.size() > _availExtensions.size()) {
    ...
    while (_registeredExtensions.size() > _availExtensions.size()) {
      ...
      bool supported =
          std::binary_search(_extensions.begin(), _extensions.end(), _registeredExtensions[s]);

      /* Is this extension ignored? */
      bool ignored = std::binary_search(
          _ignoredExtensions.begin(), _ignoredExtensions.end(), _registeredExtensions[s]);
  ...
  while (_registeredConstants.size() > _numAvailConstants) {
    for (std::vector<GLenum>::iterator it = _registeredConstants.begin() + _numAvailConstants;
         it != _registeredConstants.end(); ++it) {
  ...
  while (st != en) {
    ...
    if (getGlObjectLastReinitialize()[i] != 0) {
      ...
      if ((rc = _glObjects[i]->decRefCounter()) <= 0) {
        ...
        _glObjects[i]->getFunctor().call(this, packIdStatus(i, finaldestroy));
      }
    }
...
#elif defined(__sgi) || defined(darwin) || defined(__hpux) || defined(__linux) || defined(__sun)
