...
bool MergeGraphOp::traverse(NodePtr& node) {
  ...
  SINFO << "MergeGraphOp::traverse: Number of nodes before merge: " << next << endLog;
  bool   result = true;
  UInt32 current;
  do {
    current = next;
  ...
  SINFO << "MergeGraphOp::traverse: Number of nodes after merge: " << current << endLog;
...
std::string MergeGraphOp::usage(void) {
  return "Merge: merge all geometries in a subtree\n"
         ...
         "  secondary_color_is_vector  (bool, false): transform secondary color as if it were a "
         "normal\n"
...
void MergeGraphOp::makeExcludeList(NodePtr& node) {
  ::traverse(node,
      osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, MergeGraphOp, NodePtr>(
          this, &MergeGraphOp::excludeListEnter),
      osgTypedMethodFunctor2ObjPtrCPtrRef<Action::ResultE, MergeGraphOp, NodePtr, Action::ResultE>(
          this, &MergeGraphOp::excludeListLeave));
}
...
bool MergeGraphOp::isLeaf(NodePtr& node) {
  if (node->getMFChildren()->getValues().begin() == node->getMFChildren()->getValues().end())
    return true;
  else
    return false;
}
  ...
  for (; it != en; ++it) {
    ...
    if ((*it)->getCore()->getType().isDerivedFrom(Geometry::getClassType())) {
      ...
      if (!special && !inSubList) {
        ...
        for (; it2 != en; ++it2) {
          if (!isInExcludeList(*it2) &&
              (*it2)->getCore()->getType().isDerivedFrom(Geometry::getClassType())) {
            ...
            if (geo->isMergeable(geo2)) {
              ...
              if (new_geo == NullFC) {
                ...
                if (new_geo->merge(geo))
                  ...
                  FWARNING(("MergeGraphOp: processGeometries problem 1\n"));

                if (new_geo->merge(geo2))
                  toSub.push_back(*it2);
                else
                  FWARNING(("MergeGraphOp: processGeometries problem 2\n"));
              } else {
                if (new_geo->merge(geo2))
                  toSub.push_back(*it2);
              }
