...
SceneFileType::SceneFileType(const Char8* suffixArray[], UInt16 suffixByteCount, bool override,
...
NodePtr SceneFileType::read(
    std::istream& OSG_CHECK_ARG(is), const Char8* OSG_CHECK_ARG(fileNameOrExtension)) const {
...
bool SceneFileType::write(const NodePtr& OSG_CHECK_ARG(node), std::ostream& OSG_CHECK_ARG(os),
...
#ifndef OSG_DISABLE_DEPRECATED
bool SceneFileType::writeFile(
    const NodePtr& OSG_CHECK_ARG(node), const Char8* OSG_CHECK_ARG(fileName)) const {
  FWARNING(("FILE INTERFACE NOT IMPLEMENTED!\n"));
  return false;
}
#endif
