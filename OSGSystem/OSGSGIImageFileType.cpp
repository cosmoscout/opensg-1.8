...
static const UInt32 zsize2pixelformat[] = {
    Image::OSG_L_PF, Image::OSG_LA_PF, Image::OSG_RGB_PF, Image::OSG_RGBA_PF};
...
static const Char8* suffixArray[] = {"rgb", "rgba", "sgi", "bw"};

SGIImageFileType SGIImageFileType::_the("image/x-sgi", suffixArray, sizeof(suffixArray));
...
std::string SGIImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "\x01\xda", 2) == 0 ? std::string(getMimeType()) : std::string();
}
...
*/
SGIImageFileType::SGIImageFileType(
