...
#ifdef WIN32
OSG_FC_ST_TYPE_FUNCTIONS_INL_TMPL_DEF(GenericAttDesc, DynFieldAttachment)
#endif

OSG_DYNFIELD_FC_DLLEXPORT_DEF(DynFieldAttachment, GenericAttDesc, OSG_SYSTEMLIB_DLLTMPLMAPPING);
...
void VRMLNodeDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname, Field*& pField,
  ...
  PINFO << "VRMLNodeDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
  ...
  if (pFC->getType().isNode() == true) {
    ...
    if (pField != NULL) {
      ...
      pTmpFC = pNode->findAttachment(GenericAtt::getClassType().getGroupId());
...
#endif
      if (pField == NULL) {
        pTmpFC = pNodeCore->findAttachment(GenericAtt::getClassType().getGroupId());
...
#ifdef OSG_DEBUG_VRML
        indentLog(getIndent(), PINFO);
        PINFO << "Got this from nodecore attachment : " << pField << std::endl;
#endif
      }
...
bool VRMLNodeDesc::prototypeAddField(
    const Char8* OSG_VRML_ARG(szFieldType), const UInt32 uiFieldTypeId, const Char8* szFieldName) {
  FieldDescription* pDesc = NULL;
  FieldType*        pType = NULL;

  Field* pField = getField(_pNodeProto, _pNodeCoreProto, _pGenAtt, szFieldName);

#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLNodeDesc::prototypeAddField | getField " << szFieldName << "  returned : " << pField
        << std::endl;
#endif
  ...
  if (pField == NULL) {
    ...
    if (pType == NULL) {
      PWARNING << "VRMLNodeDesc::prototypeAddField "
               << "Could not get fieldtype " << uiFieldTypeId << " " << szFieldName << " "
               << std::endl;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLNodeDesc::prototypeAddField | got fieldtype : " << uiFieldTypeId << " "
          << pType->getName() << " " << pType->getId() << std::endl;
#endif

    if (_pGenAtt != NullFC) {
      pDesc = new FieldDescription(
          *pType, szFieldName, 0, 0, false, (FieldIndexAccessMethod)&GenericAtt::getDynamicField);

      _pGenAtt->addField(*pDesc);

      _pCurrField = getField(_pNodeProto, _pNodeCoreProto, _pGenAtt, szFieldName);
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLNodeDesc::prototypeAddField | field added : " << szFieldType << " "
          << uiFieldTypeId << " " << szFieldName << " " << _pCurrField << std::endl;
#endif
    ...
    if ((_pGenAtt != NullFC) && (_pGenAtt->getField(szFieldName) != NULL)) {
      PWARNING << "VRMLNodeDesc::prototypeAddField | "
               << "Could not add field " << szFieldName << " a second time" << std::endl;
    }
...
FieldContainerPtr VRMLNodeDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
...
void VRMLNodeDesc::addFieldValue(Field* pField, const Char8* szFieldVal) {
  if (pField != NULL) {
...
void VRMLNodeDesc::dump(const Char8* szNodeName) {
  ...
  if (_pGenAtt != NullFC) {
    ...
    for (UInt32 i = 2; i <= _pGenAtt->getType().getNumFieldDescs(); i++) {
      FieldDescription* pFieldDesc = _pGenAtt->getType().getFieldDescription(i);
      Field*            pField     = _pGenAtt->getDynamicField(i);

      stringVal.erase(stringVal.begin(), stringVal.end());

      pField->getValueByStr(stringVal);

      fprintf(stderr, "\t\t%s %s %s\n", pFieldDesc->getCName(),
          pFieldDesc->getFieldType().getCName(), stringVal.c_str());
    }
  }
}
...
Field* VRMLNodeDesc::getField(FieldContainerPtr pFC1, FieldContainerPtr pFC2, GenericAttPtr pGenAtt,
...
bool VRMLShapeDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool returnValue = false;

#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLShapeDesc::prototypeAddField | add request : " << szFieldname << std::endl;
#endif
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLShapeDesc::prototypeAddField | request internal : " << szFieldname << " "
          << _pCurrField << std::endl;
#endif
  }
  ...
  if (_pCurrField == NULL) {
    returnValue = Inherited::prototypeAddField(szFieldType, uiFieldTypeId, szFieldname);
  }
...
void VRMLShapeDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname, Field*& pField,
    const FieldDescription*& pDesc) {
  if (szFieldname == NULL)
    return;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLShapeDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
  ...
  if (stringcasecmp("geometry", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLShapeDesc::getFieldAndDesc : request internal " << szFieldname << std::endl;
#endif
    ...
    if (pNode != NullFC) {
      if (pNode->getCore() != NullFC) {
        ...
        if (pField != NULL) {
          pDesc = pNode->getCore()->getType().findFieldDescription("material");
        }
      }
    } else {
      VRMLNodeDesc::getFieldAndDesc(pFC, szFieldname, pField, pDesc);
    }
  } else {
    VRMLNodeDesc::getFieldAndDesc(pFC, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLShapeDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  NodePtr     pNode     = NullFC;
  NodeCorePtr pNodeCore = NullFC;
...
bool VRMLGeometryDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "GeoDesc::prototypeAddField : internal " << szFieldname << std::endl;
#endif

    return true;
  } else {
    return Inherited::prototypeAddField(szFieldType, uiFieldTypeId, szFieldname);
  }
}

void VRMLGeometryDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "GeoDesc::getFieldAndDesc : request " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("coord", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "GeoDesc::getFieldAndDesc : internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pGeo, szFieldname, pField, pDesc);
  }
}
...
FieldContainerPtr VRMLGeometryDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
...
void VRMLGeometryDesc::endNode(FieldContainerPtr pFC) {
  ...
  Inherited::getFieldAndDesc(pFC, "coordIndex", pField, pDesc);

  if (pField != NULL) {
    pCoordIndex = static_cast<MFInt32*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "normalIndex", pField, pDesc);

  if (pField != NULL) {
    pNormalIndex = static_cast<MFInt32*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "colorIndex", pField, pDesc);

  if (pField != NULL) {
    pColorIndex = static_cast<MFInt32*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "texCoordIndex", pField, pDesc);

  if (pField != NULL) {
    pTexCoordIndex = static_cast<MFInt32*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "convex", pField, pDesc);

  if (pField != NULL) {
    pConvex = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "ccw", pField, pDesc);

  if (pField != NULL) {
    pCcw = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "normalPerVertex", pField, pDesc);

  if (pField != NULL) {
    pNormalPerVertex = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "colorPerVertex", pField, pDesc);

  if (pField != NULL) {
    pColorPerVertex = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "creaseAngle", pField, pDesc);
  ...
  if (_bIsFaceSet == true) {
    if (pCoordIndex != NULL && pCoordIndex->size() > 2 && pNormalIndex != NULL &&
        pColorIndex != NULL && pTexCoordIndex != NULL && pConvex != NULL && pCcw != NULL &&
        pNormalPerVertex != NULL && pColorPerVertex != NULL && pCreaseAngle != NULL) {
      beginEditCP(pGeo);
#ifdef OSG_DEBUG_VRML
      indentLog(getIndent(), PINFO);
      ...
      setIndexFromVRMLData(pGeo, pCoordIndex->getValues(), pNormalIndex->getValues(),
          pColorIndex->getValues(), pTexCoordIndex->getValues(), pConvex->getValue(),
          pCcw->getValue(), pNormalPerVertex->getValue(), pColorPerVertex->getValue(),
    ...
    if (pCoordIndex != NULL && pCoordIndex->size() > 1 && pColorIndex != NULL &&
      ...
      setIndexFromVRMLData(pGeo, pCoordIndex->getValues(), dummyVec, pColorIndex->getValues(),
          dummyVec, dummybool, dummybool, dummybool, pColorPerVertex->getValue(),
          false, // create normal; not yet :)
          false);
    } else {
#if 0 // What's the point of doing that?
...
void VRMLGeometryDesc::addFieldValue(Field* pField, const Char8* szFieldVal) {
  if (pField != NULL) {
    pField->pushValueByStr(szFieldVal);
  }
}
...
bool VRMLGeometryPointSetDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLGeometryPointSetDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "GeoDesc::getFieldAndDesc : request " << szFieldname << std::endl;
...
FieldContainerPtr VRMLGeometryPointSetDesc::beginNode(
    const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
void VRMLGeometryPointSetDesc::endNode(FieldContainerPtr pFC) {
  ...
  GeoPLengthsUI32::StoredFieldType* pLenField = pLengths->getFieldPtr();
  ...
  beginEditCP(pGeo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask);
  {
    pGeo->setLengths(pLengths);
    pGeo->setTypes(pTypes);
  }
  endEditCP(pGeo, Geometry::TypesFieldMask | Geometry::LengthsFieldMask);
...
void VRMLGeometryPointSetDesc::addFieldValue(Field* pField, const Char8* szFieldVal) {
  if (pField != NULL) {
    pField->pushValueByStr(szFieldVal);
  }
}
...
VRMLGeometryPartDesc::VRMLGeometryPartDesc(
    Char8* szVRMLPartname, Char8* szOSGPartname, Char8* szOSGProtoname)
    : Inherited()
    ,
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "GeoPartDesc::init : " << szName << " " << _szVRMLPartname << " " << _szOSGPartname
        << " " << _szOSGProtoname << " " << std::endl;
#endif

  _pNodeProto = FieldContainerFactory::the()->createFieldContainer(_szOSGProtoname);

  if (_pNodeProto == NullFC) {
    PWARNING << "ERROR no prototype available for " << _szOSGProtoname << std::endl;
  }
...
bool VRMLGeometryPartDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "GeoPartDesc::prototypeAddField : add part " << szFieldname << std::endl;
#endif
  ...
  if (bFound == true) {
    ...
    return VRMLNodeDesc::prototypeAddField(szFieldType, uiFieldTypeId, szFieldname);
  }
}

void VRMLGeometryPartDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLGeometryPartDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp(_szVRMLPartname, szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLGeometryPartDesc::getFieldAndDesc : request internal " << szFieldname
          << " return " << _szOSGPartname << std::endl;
#endif
    ...
    if (pField == NULL) {
      PWARNING << "VRMLGeometryPartDesc::getFieldAndDesc : could not"
               << " map : " << szFieldname << " to " << _szOSGPartname << std::endl;
    }
  } else {
    Inherited::getFieldAndDesc(pFC, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLGeometryPartDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr returnValue = NullFC;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "GeoObjDesc::init : " << szName << " " << _szVRMLObjectname << std::endl;
#endif
...
bool VRMLGeometryObjectDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  _pCurrField = NULL;

  bool rc = VRMLNodeDesc::prototypeAddField(szFieldType, uiFieldTypeId, szFieldname);

  return rc;
}

void VRMLGeometryObjectDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLGeometryObjectDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
...
FieldContainerPtr VRMLGeometryObjectDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr returnValue = NullFC;
  NodePtr           pNode       = NullFC;
  GenericAttPtr     pAtt        = NullFC;
...
void VRMLGeometryObjectDesc::endNode(FieldContainerPtr pFC) {
  ...
  if (stringcasecmp("Box", _szVRMLObjectname) == 0) {
    Inherited::getFieldAndDesc(pFC, "size", pField, pDesc);
...
#ifdef OSG_DEBUG_VRML
      indentLog(getIndent(), PINFO);
      PINFO << "VRMLGeometryObjectDesc::endNode : Create box" << std::endl;
#endif

      GeometryPtr pGeo =
          makeBoxGeo(pVec->getValue()[0], pVec->getValue()[1], pVec->getValue()[2], 1, 1, 1);
    ...
    Inherited::getFieldAndDesc(pFC, "bottomRadius", pField, pDesc);

    if (pField != NULL) {
      pBotRad = static_cast<SFReal32*>(pField);
    }

    Inherited::getFieldAndDesc(pFC, "height", pField, pDesc);

    if (pField != NULL) {
      pHeight = static_cast<SFReal32*>(pField);
    }

    Inherited::getFieldAndDesc(pFC, "side", pField, pDesc);

    if (pField != NULL) {
      pSide = static_cast<SFBool*>(pField);
    }

    Inherited::getFieldAndDesc(pFC, "bottom", pField, pDesc);

    if (pField != NULL) {
      pBottom = static_cast<SFBool*>(pField);
    }

    if (pBotRad != NULL && pHeight != NULL && pSide != NULL && pBottom != NULL) {
#ifdef OSG_DEBUG_VRML
      indentLog(getIndent(), PINFO);
      PINFO << "VRMLGeometryObjectDesc::endNode : Create cone" << std::endl;
#endif

      GeometryPtr pGeo = makeConeGeo(
          pHeight->getValue(), pBotRad->getValue(), 32, pSide->getValue(), pBottom->getValue());
    ...
    Inherited::getFieldAndDesc(pFC, "radius", pField, pDesc);
    ...
    Inherited::getFieldAndDesc(pFC, "top", pField, pDesc);

    if (pField != NULL) {
      pTop = static_cast<SFBool*>(pField);
    }

    if (pBottom != NULL && pHeight != NULL && pRadius != NULL && pSide != NULL && pTop != NULL) {
#ifdef OSG_DEBUG_VRML
      indentLog(getIndent(), PINFO);
      PINFO << "VRMLGeometryObjectDesc::endNode : Create cylinder" << std::endl;
#endif

      GeometryPtr pGeo = makeCylinderGeo(pHeight->getValue(), pRadius->getValue(), 32,
          pSide->getValue(), pTop->getValue(), pBottom->getValue());
...
#ifdef OSG_DEBUG_VRML
      indentLog(getIndent(), PINFO);
      PINFO << "VRMLGeometryObjectDesc::endNode : Create sphere" << std::endl;
#endif
...
bool VRMLAppearanceDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  _pCurrField = NULL;
...
void VRMLAppearanceDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {

#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLAppearanceDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("material", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLAppearanceDesc::getFieldAndDesc : request internal " << szFieldname << std::endl;
#endif
    ...
    Inherited::getFieldAndDesc(_pGenAtt, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLAppearanceDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr returnValue = NullFC;
  ...
  if (pFC != NullFC) {
    ...
    if (pChunkMat != NullFC) {
      ...
      if (pChunkMat->isTransparent() == true ||
          (tex != NullFC && tex->getImage() != NullFC && tex->getImage()->hasAlphaChannel())) {
        BlendChunkPtr pBlendChunk = OSG::BlendChunk::create();

        beginEditCP(pBlendChunk);
        {
          if (tex != NullFC && tex->getImage() != NullFC && tex->getImage()->isAlphaBinary()) {
...
bool VRMLMaterialDesc::prototypeAddField(const Char8*, const UInt32, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "MaterialDesc::prototypeAddField : add part " << szFieldname << std::endl;
#endif
    return true;
  } else {
    return false;
...
void VRMLMaterialDesc::endProtoInterface(void) {
  ...
  cCol.setValuesRGBA(_defaultDiffuseColor.getValue().red() * _defaultAmbientIntensity.getValue(),
      _defaultDiffuseColor.getValue().green() * _defaultAmbientIntensity.getValue(),
      _defaultDiffuseColor.getValue().blue() * _defaultAmbientIntensity.getValue(),
  ...
  cCol.setValuesRGBA(_defaultDiffuseColor.getValue()[0], _defaultDiffuseColor.getValue()[1],
      _defaultDiffuseColor.getValue()[2], 1.f - _defaultTransparency.getValue());

  pMatChunk->setDiffuse(cCol);

  cCol.setValuesRGBA(_defaultSpecularColor.getValue()[0], _defaultSpecularColor.getValue()[1],
      _defaultSpecularColor.getValue()[2], 1.f - _defaultTransparency.getValue());
  pMatChunk->setSpecular(cCol);

  pMatChunk->setShininess(_defaultShininess.getValue() * 128.f);

  cCol.setValuesRGBA(_defaultEmissiveColor.getValue()[0], _defaultEmissiveColor.getValue()[1],
      _defaultEmissiveColor.getValue()[2], 1.f - _defaultTransparency.getValue());
...
void VRMLMaterialDesc::getFieldAndDesc(
    FieldContainerPtr, const Char8* szFieldname, Field*& pField, const FieldDescription*& pDesc) {
  if (stringcasecmp("ambientIntensity", szFieldname) == 0) {
...
FieldContainerPtr VRMLMaterialDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  reset();
...
void VRMLMaterialDesc::endNode(FieldContainerPtr) {
  if (_pMat != NullFC) {
    ...
    cCol.setValuesRGBA(_diffuseColor.getValue().red() * _ambientIntensity.getValue(),
        _diffuseColor.getValue().green() * _ambientIntensity.getValue(),
        _diffuseColor.getValue().blue() * _ambientIntensity.getValue(),
    ...
    cCol.setValuesRGBA(_diffuseColor.getValue()[0], _diffuseColor.getValue()[1],
        _diffuseColor.getValue()[2], 1.f - _transparency.getValue());
    _pMat->setDiffuse(cCol);

    cCol.setValuesRGBA(_specularColor.getValue()[0], _specularColor.getValue()[1],
        _specularColor.getValue()[2], 1.f - _transparency.getValue());
    _pMat->setSpecular(cCol);

    _pMat->setShininess(_shininess.getValue() * 128.f);

    cCol.setValuesRGBA(_emissiveColor.getValue()[0], _emissiveColor.getValue()[1],
        _emissiveColor.getValue()[2], 1.f - _transparency.getValue());
...
bool VRMLTextureTransformDesc::prototypeAddField(
    const Char8*, const UInt32, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "TextureTransformDesc::prototypeAddField : add part " << szFieldname << std::endl;
#endif
...
void VRMLTextureTransformDesc::endNode(FieldContainerPtr pFC) {
  ...
  if (pTexTrans != NullFC) {
    ...
    Vec3f center(_center.getValue()[0], _center.getValue()[1], 0.0f);

    Quaternion rotation;
    rotation.setValueAsAxisRad(0.0f, 0.0f, 1.0f, _rotation.getValue());

    Vec3f scale(_scale.getValue()[0], _scale.getValue()[1], 1.0f);

    Vec3f translation(_translation.getValue()[0], _translation.getValue()[1], 0.0f);

    m.setTransform(translation, rotation, scale, Quaternion::identity(), center);

    beginEditCP(pTexTrans);
    { pTexTrans->setMatrix(m); }
    endEditCP(pTexTrans);
  } else {
    PWARNING << "VRMLTextureTransformDesc::endNode : Invalid texture transform ptr" << std::endl;
  }
  ...
  indentLog(getIndent(), PINFO);
  PINFO << "End TextureTransform " << _center.getValue() << " " << _rotation.getValue() << " "
        << _scale.getValue() << " " << _translation.getValue() << " " << &(*pFC) << std::endl;
#endif
}
...
bool VRMLImageTextureDesc::prototypeAddField(const Char8*, const UInt32, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "ImageTextureDesc::prototypeAddField : add part " << szFieldname << std::endl;
#endif
...
FieldContainerPtr VRMLImageTextureDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
...
void VRMLImageTextureDesc::endNode(FieldContainerPtr pFC) {
  ...
  if (pTexture != NullFC) {
#ifdef OSG_DEBUG_VRML
    PNOTICE << "VRMLImageTextureDesc::endNode : Reading texture " << _url[0].c_str() << std::endl;
#endif
    ...
    if (pImage != NullFC) {
      ...
      PWARNING << "VRMLImageTextureDesc::endNode : "
               << "Couldn't read texture " << _url[0].c_str() << " !!!" << std::endl;
    }
  } else {
    PWARNING << "VRMLImageTextureDesc::endNode : Invalid texture ptr" << std::endl;
  }
  ...
  indentLog(getIndent(), PINFO);
  PINFO << "End ImageTexture " << _url[0].c_str() << " " << _repeatS.getValue() << " "
        << _repeatT.getValue() << " " << &(*pFC) << std::endl;
#endif
}
...
bool VRMLPixelTextureDesc::prototypeAddField(const Char8*, const UInt32, const Char8* szFieldname) {
  bool bFound = false;
...
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "PixelTextureDesc::prototypeAddField : add part " << szFieldname << std::endl;
#endif
...
FieldContainerPtr VRMLPixelTextureDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  TextureChunkPtr returnValue = TextureChunk::create();
...
void VRMLPixelTextureDesc::endNode(FieldContainerPtr pFC) {
  ...
  if (pTexture != NullFC) {
    ...
    PWARNING << "VRMLPixelTextureDesc::endNode : Invalid texture ptr" << std::endl;
  }
  ...
  indentLog(getIndent(), PINFO);
  PINFO << "End PixelTexture " << _repeatS.getValue() << " " << _repeatT.getValue() << " "
        << &(*pFC) << std::endl;
#endif
}
...
void VRMLPixelTextureDesc::addFieldValue(Field* pField, const Char8* szFieldVal) {
  if (pField != NULL) {
...
bool VRMLLODDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLLODDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname, Field*& pField,
    const FieldDescription*& pDesc) {
  if (szFieldname == NULL)
    return;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLLODDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("level", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLLODDesc::getFieldAndDesc :  : request internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pLOD, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLLODDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
bool VRMLSwitchDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLSwitchDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
  if (szFieldname == NULL)
    return;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLSwitchDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("choice", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLSwitchDesc::getFieldAndDesc :  : request internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pSwitch, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLSwitchDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
bool VRMLGroupDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLGroupDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname, Field*& pField,
    const FieldDescription*& pDesc) {
  if (szFieldname == NULL)
    return;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLGroupDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("bboxCenter", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLGroupDesc::getFieldAndDesc :  : request internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pGroup, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLGroupDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
bool VRMLExtrusionDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  _pCurrField = NULL;
...
void VRMLExtrusionDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLExtrusionDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
...
FieldContainerPtr VRMLExtrusionDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr returnValue = NullFC;
  NodePtr           pNode       = NullFC;
  GenericAttPtr     pAtt        = NullFC;
...
void VRMLExtrusionDesc::endNode(FieldContainerPtr pFC) {
  ...
  Inherited::getFieldAndDesc(pFC, "beginCap", pField, pDesc);
  ...
  Inherited::getFieldAndDesc(pFC, "crossSection", pField, pDesc);

  if (pField != NULL) {
    pCrossSection = static_cast<MFPnt2f*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "endCap", pField, pDesc);

  if (pField != NULL) {
    pEndCap = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "orientation", pField, pDesc);

  if (pField != NULL) {
    pOrientation = static_cast<MFQuaternion*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "scale", pField, pDesc);

  if (pField != NULL) {
    pScale = static_cast<MFVec2f*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "solid", pField, pDesc);

  if (pField != NULL) {
    pSolid = static_cast<SFBool*>(pField);
  }

  Inherited::getFieldAndDesc(pFC, "spine", pField, pDesc);

  if (pField != NULL) {
    pSpine = static_cast<MFPnt3f*>(pField);
  }

  if (pBeginCap != NULL && pCcw != NULL && pConvex != NULL && pCreaseAngle != NULL &&
      pCrossSection != NULL && pEndCap != NULL && pOrientation != NULL && pScale != NULL &&
      pSolid != NULL && pSpine != NULL) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLExtrusionDesc::endNode" << std::endl;
#endif

    // TODO: creaseAngle/numOfSubdivision param/handling ?

    GeometryPtr pGeo = makeExtrusionGeo(pCrossSection->getValues(), pOrientation->getValues(),
        pScale->getValues(), pSpine->getValues(), pCreaseAngle->getValue(), pBeginCap->getValue(),
        pEndCap->getValue(), pCcw->getValue(), pConvex->getValue(),
        true, // buildNormal
        true, // buildTexCoord
        0     // numOfSubdivision
    );
...
bool VRMLInlineDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLInlineDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
  if (szFieldname == NULL)
    return;
...
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "VRMLInlineDesc::getFieldAndDesc : looking for " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("bboxCenter", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "VRMLInlineDesc::getFieldAndDesc :  : request internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pInline, szFieldname, pField, pDesc);
  }
...
FieldContainerPtr VRMLInlineDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
void VRMLInlineDesc::endNode(FieldContainerPtr pFC) {
  ...
  VRMLNodeDesc::getFieldAndDesc(pFC, "url", pField, pFieldDesc);

  MFString* pUrl = dynamic_cast<MFString*>(pField);

  FDEBUG(("Inline : %s\n", (*pUrl)[0].c_str()));

  std::string filename = SceneFileHandler::the().getPathHandler()->findFile((*pUrl)[0].c_str());

  // could be a real url with a relative inline path.
  if (filename.empty())
    filename = (*pUrl)[0];

  std::string path = SceneFileHandler::the().getPathHandler()->extractPath(filename.c_str());
...
void VRMLViewpointDesc::init(const Char8* OSG_VRML_ARG(szName)) {
  ...
  FieldDescription* pDesc = new FieldDescription(SFBool::getClassType(), "isViewpoint", 0, 0, false,
      (FieldIndexAccessMethod)&GenericAtt::getDynamicField);
...
bool VRMLViewpointDesc::prototypeAddField(
    const Char8* szFieldType, const UInt32 uiFieldTypeId, const Char8* szFieldname) {
  bool bFound = false;
...
void VRMLViewpointDesc::getFieldAndDesc(FieldContainerPtr pFC, const Char8* szFieldname,
    Field*& pField, const FieldDescription*& pDesc) {
#ifdef OSG_DEBUG_VRML
  indentLog(getIndent(), PINFO);
  PINFO << "ViewpointDesc::getFieldAndDesc : request " << szFieldname << std::endl;
#endif
  ...
  if (stringcasecmp("orientation", szFieldname) == 0) {
#ifdef OSG_DEBUG_VRML
    indentLog(getIndent(), PINFO);
    PINFO << "ViewpointDesc::getFieldAndDesc : internal " << szFieldname << std::endl;
#endif
    ...
    VRMLNodeDesc::getFieldAndDesc(pTransform, szFieldname, pField, pDesc);
  }
}
...
FieldContainerPtr VRMLViewpointDesc::beginNode(const Char8*, const Char8*, FieldContainerPtr) {
  FieldContainerPtr pFC       = NullFC;
  NodePtr           pNode     = NullFC;
  NodeCorePtr       pNodeCore = NullFC;
  GenericAttPtr     pAtt      = NullFC;
...
DataType FieldDataTraits<GenericAttPtr>::_type("GenericAttPtr", "AttachmentPtr");
