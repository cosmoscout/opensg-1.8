 ...
 */
inline Real32 BalancedMultiWindow::getVisibleLoad(Int32 const (&rect)[4], BBox& bbox) {
  LoadGroup& group = _cluster.loadGroups[bbox.groupId];
  // invisible
  if (bbox.rect[LEFT] > rect[RIGHT] || bbox.rect[RIGHT] < rect[LEFT] ||
      bbox.rect[BOTTOM] > rect[TOP] || bbox.rect[TOP] < rect[BOTTOM])
    return 0;
  ...
  Real32 area        = (bbox.rect[2] - bbox.rect[0] + 1) * (bbox.rect[3] - bbox.rect[1] + 1);
  // constant
...
#define OSGBALANCEDMULTIWINDOW_INLINE_CVSID "@(#)$Id: $"
