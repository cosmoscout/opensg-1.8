...
std::string VerifyGraphOp::usage(void) {
  return "Verify: Test validity of Geometries\n"
...
Action::ResultE VerifyGraphOp::traverseEnter(NodePtr& node) {
  ...
  if (NullFC == node_core) {
    ...
    _numErrors += 1;
    if (_verbose) {
      SINFO << "Node: [" << node_name << "] has NULL core." << endLog;
    }
    if (_repair) {
      if (_verbose)
...
Action::ResultE VerifyGraphOp::verifyGeometry(NodePtr& node) {
  ...
  if (norm_errors > 0) {
    norm_errors = 0;
    if (_verbose)
      SINFO << "removed corrupted normals!\n";
    beginEditCP(geo);
  ...
  if (col_errors > 0) {
    col_errors = 0;
    if (_verbose)
      SINFO << "removed corrupted colors!\n";
    beginEditCP(geo);
  ...
  if (tex0_errors > 0) {
    tex0_errors = 0;
    if (_verbose)
      SINFO << "removed corrupted tex coords0!\n";
    beginEditCP(geo);
    geo->setTexCoords(NullFC);
    endEditCP(geo);
  }

  _numErrors += (pos_errors + norm_errors + col_errors + col2_errors + tex0_errors + tex1_errors +
    ...
    for (UInt32 i = 0; i < _corruptedGeos.size(); ++i) {
      ...
      for (UInt32 j = 0; j < _corruptedGeos[i]->getParents().size(); ++j) {
        ...
        if (parent != NullFC) {
          ...
          if (_verbose) {
            SINFO << "Removing corrupted geom from node: " << nname << endLog;
          }
          nname += "_CORRUPTED";
