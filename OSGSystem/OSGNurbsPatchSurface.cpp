...
// FIXME: quick hack
void CNurbsPatchSurface::setSurface(
    BSplineTrimmedSurface* clSurface, std::vector<Pnt2f>& texturecps, bool bUseTextures) {
  ...
  if (m_bUseTextures) {
    ...
    if ((u_size * v_size) != texturecps.size()) {
      std::cerr << "setSurface: wrong number of texture control points, texturing disabled"
                << std::endl;
...
void CNurbsPatchSurface::getTessellation(std::vector<Pnt3f>& gverts, std::vector<Vec3f>& norms,
    std::vector<SimplePolygon>& tris, bool usedelaunay) {
#ifdef WRITE_STAT
  g_clTotalStart = osg::getSystemTime();
#endif
...
void CNurbsPatchSurface::getTessellation(std::vector<Pnt3f>& gverts, std::vector<Vec3f>& norms,
    std::vector<Pnt2f>& texturecoords, std::vector<SimplePolygon>& tris, bool usedelaunay) {
  ...
  if ((m_vtSurfaces[0].ucStatus & BEZIER_SURFACES_VALID) &&
    ...
    //			BuildNode( ui_surface );
    ////			calculateNormals( norms );
    calculatePointsNormalsAndTextureCoords(gverts, tris, norms, texturecoords);
    ComputeNormalCone(0, norms);
    m_vclParameterVertices.clear();
    m_vclGlobalVertices.clear();
    m_vclTriangles.clear();
...
void CNurbsPatchSurface::calculatePointsAndNormals(
    std::vector<Pnt3f>& gverts, std::vector<SimplePolygon>& tris, std::vector<Vec3f>& norms) {
  ...
  cl_surf.computeNormal(m_vclParameterVertices, gverts, norms);
#else
...
#endif
  /*
      std::cerr<<"haho"<< m_vclParameterVertices.size() << " " << gverts.size() << " " <<
     norms.size() << std::endl; for ( UInt32 i = 0; i < gverts.size(); ++i )
      {
  ...
  for (unsigned int ui_actloop = 0; ui_actloop < numloops; ++ui_actloop) {
    ...
    for (unsigned int ui_act = 0; ui_act < actlooplength; ++ui_act) {
      m_vtSurfaces[0].vvclEdgeLoopsNorm[ui_actloop][ui_act] =
          Vec3d(norms[ui_actvertidx][0], norms[ui_actvertidx][1], norms[ui_actvertidx][2]);
    ...
    cl_surf.computeNormalforTrimming(
        m_vtSurfaces[0].vvclEdgeLoops[ui_actloop], m_vtSurfaces[0].vvclEdgeLoopsNorm[ui_actloop]);
#else
    cl_surf.computeNormalforTrimming(m_vtSurfaces[0].vvclEdgeLoops[ui_actloop],
...
void CNurbsPatchSurface::calculatePointsNormalsAndTextureCoords(std::vector<Pnt3f>& gverts,
    std::vector<SimplePolygon>& tris, std::vector<Vec3f>& norms,
  ...
  cl_surf.computeTex(m_vclParameterVertices, dummyverts, texturecoords, &m_vvTextureControlPoints);
...
#ifdef OSG_KEEP_2D_POINTS
    for (unsigned int ui_act = 0; ui_act < actlooplength; ++ui_act) {
      m_vtSurfaces[0].vvclEdgeLoopsTex[ui_actloop][ui_act] = Vec3d(texturecoords[ui_actvertidx][0],
          texturecoords[ui_actvertidx][1], texturecoords[ui_actvertidx][2]);
      ui_actvertidx++;
    }
#else
    cl_surf.computeTexforTrimming(m_vtSurfaces[0].vvclEdgeLoops[ui_actloop],
        m_vtSurfaces[0].vvclEdgeLoopsTex[ui_actloop], &m_vvTextureControlPoints);
#endif
  }
}

void CNurbsPatchSurface::calculatePointsAndTextureCoords(std::vector<Pnt3f>& gverts,
    std::vector<SimplePolygon>& tris, std::vector<Pnt2f>& texturecoords) {
  BSplineTensorSurface cl_surf;

  cl_surf = m_vclBSplineSurfaces[0].getSurface();
  cl_surf.computeTex(m_vclParameterVertices, gverts, texturecoords, &m_vvTextureControlPoints);
  /*
      std::cerr<<"haho"<< m_vclParameterVertices.size() << " " << gverts.size() << " " <<
     norms.size() << std::endl; for ( UInt32 i = 0; i < gverts.size(); ++i )
      {
...
void CNurbsPatchSurface::SetError(const unsigned int cuiSurface, double dError) {
  ...
  //	std::cerr << "Setting error = " << dError << " for surface " << cuiSurface + 1 << "." <<
  //std::endl << std::endl;
...
void CNurbsPatchSurface::ConvertToBezier(unsigned int uiSurface) {
  ...
  for (unsigned int ui_loop = 0; ui_loop < ui_tloops; ++ui_loop) {
    ...
    for (ui_curve = 0; ui_curve < ui_curves; ++ui_curve) {
      ...
      DCTPVec3dvector vcl_cp  = rvvcl_trimming_loops[ui_loop][ui_prev].getControlPointVector();
      ...
      if (DCTPVecIsNotEqual(cl_prev, vcl_cp[0])) {
        ...
        //				std::cerr << "inserting linear trimming curve to close gap." <<
        //std::endl; 				std::cerr << vcl_temp[ 0 ] << " -> " << vcl_temp[ 1 ] << std::endl;
      ...
      if (rvvcl_trimming_loops[ui_loop][ui_curve].makeBezier(vcl_converted_tmp, vd_pars)) {
        ...
        //				SWARNING << " surface " << uiSurface + 1 << " to bezier form
        //failed!" << endLog;
      } else {
    ...
    //		std::cerr << "loop " << ui_loop + 1 << " contains " << vui_curves_per_loop[ ui_loop ] << "
    //curves." << std::endl;
    for (ui_curve = 0; ui_curve < vui_curves_per_loop[ui_loop]; ++ui_curve) {
      /*			Vec2dvector	vcl_cp = vcl_beziertrimmingcurves[ ui_curve
         ].getControlPointVector( );

                              std::cerr << vcl_cp[ 0 ] << " -> " << vcl_cp[ vcl_cp.size( ) - 1 ] <<
         std::endl;*/
      m_vtSurfaces[uiSurface].vvclBezierCurves[ui_loop].push_back(
          vcl_beziertrimmingcurves[ui_curve]);
    }
  }

  if (rcl_bspline_surface.makeBezier(
          m_vvvclBezierSurfaces[uiSurface], m_vvdUParams[uiSurface], m_vvdVParams[uiSurface])) {
    //		SWARNING << "Conversion of surface " << uiSurface + 1 << " to bezier form failed!" <<
    //endLog;
  }
...
void CNurbsPatchSurface::CalculateQuadTree(unsigned int uiSurface, bool bForEdges) {
  ...
  if (bForEdges) {
    ...
    if (m_pclQuadTree->setInitialLeaves(
            m_vvvclBezierSurfaces[uiSurface], m_vvdUParams[uiSurface], m_vvdVParams[uiSurface])) {
...
#endif
        m_vtSurfaces[uiSurface].dError, m_vtSurfaces[uiSurface].fMinError,
        m_vtSurfaces[uiSurface].fMaxError);
#else
    m_vtSurfaces[uiSurface].pclErrorTree->BuildMesh(&m_clMesh, &m_vvvclBezierSurfaces[uiSurface],
        &m_vvdUParams[uiSurface], &m_vvdVParams[uiSurface], m_vtSurfaces[uiSurface].dError,
        m_vtSurfaces[uiSurface].fMinError, m_vtSurfaces[uiSurface].fMaxError);
#endif
...
void CNurbsPatchSurface::CalculateTrimmingLoops(unsigned int uiSurface) {
  ...
  if (!(m_vtSurfaces[uiSurface].ucStatus & QUAD_TREE_VALID)) {
    ...
    m_vtSurfaces[uiSurface].clMin = Vec3d(m_vtSurfaces[uiSurface].dError * 100,
        m_vtSurfaces[uiSurface].dError * 100, m_vtSurfaces[uiSurface].dError * 100);
  ...
  if (m_vtSurfaces[uiSurface].vvcl3DCurves.size() != 0) {
    ...
    cl_trimmer.Initialize(m_clMesh, vvcl_curves, m_vtSurfaces[uiSurface].vvcl3DCurves,
        m_vtSurfaces[uiSurface].vvclEdgeLoops, m_vtSurfaces[uiSurface].dError,
        &m_vclBSplineSurfaces[uiSurface].getSurface());
#else
    cl_trimmer.Initialize(m_clMesh, vvcl_curves, m_vtSurfaces[uiSurface].vvcl3DCurves,
        m_vtSurfaces[uiSurface].vvclEdgeLoops, m_vtSurfaces[uiSurface].dError);
    ...
    cl_trimmer.Initialize(m_clMesh, vvcl_curves, m_vtSurfaces[uiSurface].vvclEdgeLoops,
        m_pclQuadTree, &m_vclBSplineSurfaces[uiSurface].getSurface());
#else
    cl_trimmer.Initialize(
        m_clMesh, vvcl_curves, m_vtSurfaces[uiSurface].vvclEdgeLoops, m_pclQuadTree);
#endif
#else
    cl_trimmer.Initialize(m_clMesh, vvcl_curves, m_vtSurfaces[uiSurface].vvclEdgeLoops);
#endif
  ...
  try {
    ...
    std::cerr << "Par space trimmer exception caught: " << cl_err.errtype << " (surface "
              << uiSurface + 1 << ")" << std::endl;
    m_vtSurfaces[uiSurface].clMin = Vec3d(m_vtSurfaces[uiSurface].dError * 100,
        m_vtSurfaces[uiSurface].dError * 100, m_vtSurfaces[uiSurface].dError * 100);
    m_vtSurfaces[uiSurface].clMax = Vec3d(0, 0, 0); // this is invalid and will always fail BB test
#ifndef OSG_ADAPTIVE_QUAD_TREE
  ...
  int                i_err;
  const unsigned int cui_size = m_vtSurfaces[uiSurface].vvclEdgeLoops.size();
  ...
  for (unsigned int ui_loop = 0; ui_loop < cui_size; ++ui_loop) {
    ...
    m_vclBSplineSurfaces[uiSurface].getSurface().compute(
        m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop], vcl_temp3d);
    for (ui_vertex = 0; ui_vertex < cui_size2; ++ui_vertex) {
      //			std::cerr << "vertex " << ui_vertex + 1 << " of " << cui_size2 <<
      //std::endl;
      /*			const Vec3d	ccl_vec = m_vclBSplineSurfaces[ uiSurface
         ].getSurface( ).compute( m_vtSurfaces[ uiSurface ].vvclEdgeLoops[ ui_loop ][ ui_vertex ],
                                      i_err );*/
      Vec3d ccl_vec;
      ...
      if ((ui_loop == 0) && (ui_vertex == 0)) {
        m_vtSurfaces[uiSurface].clMin = m_vtSurfaces[uiSurface].clMax = ccl_vec;
#ifdef OSG_ARBITRARY_SPLIT
        m_vtSurfaces[uiSurface].clMinParam = m_vtSurfaces[uiSurface].clMaxParam =
...
#ifdef OSG_ARBITRARY_SPLIT
        if (m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][0] <
            m_vtSurfaces[uiSurface].clMinParam[0])
          m_vtSurfaces[uiSurface].clMinParam[0] =
              m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][0];
        else if (m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][0] >
                 m_vtSurfaces[uiSurface].clMaxParam[0])
          m_vtSurfaces[uiSurface].clMaxParam[0] =
              m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][0];
        if (m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][1] <
            m_vtSurfaces[uiSurface].clMinParam[1])
          m_vtSurfaces[uiSurface].clMinParam[1] =
              m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][1];
        else if (m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][1] >
                 m_vtSurfaces[uiSurface].clMaxParam[1])
          m_vtSurfaces[uiSurface].clMaxParam[1] =
              m_vtSurfaces[uiSurface].vvclEdgeLoops[ui_loop][ui_vertex][1];
#endif
      }
  ...
  m_vtSurfaces[uiSurface].clMin -= Vec3d(m_vtSurfaces[uiSurface].dError,
      m_vtSurfaces[uiSurface].dError, m_vtSurfaces[uiSurface].dError);
  m_vtSurfaces[uiSurface].clMax += Vec3d(m_vtSurfaces[uiSurface].dError,
      m_vtSurfaces[uiSurface].dError, m_vtSurfaces[uiSurface].dError);
#ifdef OSG_ADAPTIVE_QUAD_TREE
  if (m_pclQuadTree != NULL) {
    delete m_pclQuadTree;
...
void CNurbsPatchSurface::CalculateActualTrimming(unsigned int uiSurface) {
  ...
  ////	cl_trimmer.Initialize2( m_clMesh, m_vtSurfaces[ uiSurface ].vvclEdgeLoops, m_vtSurfaces[
  ///uiSurface ].vvclEdgeLoops3D );

#ifdef OSG_FORCE_NO_T_VERTICES
  cl_trimmer.Initialize2(m_clMesh, m_vtSurfaces[uiSurface].vvclEdgeLoops,
      m_vtSurfaces[uiSurface].vvclEdgeLoops3D,
      //                m_vtSurfaces[ uiSurface ].vvclEdgeLoopsNorm,
      m_vtSurfaces[uiSurface].vbReversed, m_vtSurfaces[uiSurface].vbUsed);

#else
  cl_trimmer.Initialize2(m_clMesh, m_vtSurfaces[uiSurface].vvclEdgeLoops,
      m_vtSurfaces[uiSurface].vvclEdgeLoops3D, m_vtSurfaces[uiSurface].vbReversed,
      m_vtSurfaces[uiSurface].vbUsed);
#endif /* OSG_FORCE_NO_T_VERTICES */
       //	cl_trimmer.Initialize2( m_vclSeperateMeshes[ uiSurface ], m_vtSurfaces[ uiSurface
       //].vvclEdgeLoops );
  try {
    ...
    std::cerr << "Par space trimmer2 exception caught: " << cl_err.errtype << " (surface "
              << uiSurface + 1 << ")" << std::endl;
    return;
  }
...
void CNurbsPatchSurface::CalculateGraph(unsigned int uiSurface, bool usedelaunay) {
  ...
  try {
    ...
    std::cerr << "Graph traverser exception caught: " << cl_err.errtype << " (surface "
              << uiSurface + 1 << ")" << std::endl;
    //		m_vtSurfaces[ uiSurface ].ucStatus |= TRIANGULATION_ERROR;
    //		return;
  }
...
void CNurbsPatchSurface::ComputeNormalCone(
    const unsigned int cuiSurface, std::vector<Vec3f>& normals) {
  ...
  double d_distance_squared= cl_center.squareLength();
          ...
          double d_nominator_root= ( d_distance_squared - cl_miniball.squared_radius( ) + 1.0 ) *
  0.5; double d_height= sqrt( 1.0 - ( d_nominator_root * d_nominator_root ) / d_distance_squared );
  ...
  for (ui_loop = 0; ui_loop < cui_loop_cnt; ++ui_loop) {
    ...
    for (ui_curve = 0; ui_curve < ui_curve_cnt; ++ui_curve) {
      ...
      for (ui_pos = 0; ui_pos <= ui_dim; ++ui_pos) {
        ...
        //				vcl_points.push_back( rcl_surf.computewdeCasteljau( cl_param,
        //i_err ) );
        vcl_points.push_back(
            m_vclBSplineSurfaces[cuiSurface].getSurface().compute4D(cl_param, i_err));
      }
...
#ifdef OSG_NURBS_DEBUG
      if (approxdegree != rcl_curve2d.getControlPointVector().size() - 1) {
        std::cerr << "optimdegree: "
                  << m_vtSurfaces[cuiSurface]
                             .vvcl3DCurves[ui_loop][ui_curve]
                             .getControlPointVector()
                             .size() -
                         1
                  << std::endl;
...
#endif /* OSG_NURBS_DEBUG */
       //			rcl_curve2d.write( );
       //			m_vtSurfaces[ cuiSurface ].vvcl3DCurves[ ui_loop ][ ui_curve
       //].write( );
    }
  }
}

void CNurbsPatchSurface::CutCurve(const unsigned int cuiSurface, BezierCurve2D& rclCurve,
    bezier2ddeque& rclCut, std::vector<unsigned int>& rvuiUSeg,
  ...
  std::multimap<double, unsigned int>::iterator itmm_curve_it;
  int                                           i_err;
  unsigned int                                  ui_idx;
  bool                                          b_cut;
  ...
  for (ui_curve = 0; ui_curve < ui_curve_cnt; ++ui_curve) {
    ...
    for (i_u_seg = 0; i_u_seg < ci_u_seg_cnt; ++i_u_seg) {
      ...
      for (ui_int = 0; ui_int < vd_int.size(); ++ui_int) {
        if ((vd_int[ui_int] > 1e-7) && (1.0 - vd_int[ui_int] > 1e-7)) {
  ...
  ui_curve = 0;
  for (itmm_curve_it = mm_curve_sort.begin(); itmm_curve_it != mm_curve_sort.end();
       ++itmm_curve_it) {
