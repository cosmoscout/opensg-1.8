...
bool TTFontStyle::set_ttInstance(TT_Instance* ttInstance, TT_Face* ttFace) {
  ...
  _ttError = TT_Set_Instance_PointSize(*ttInstance, (Int32)osgfloor(64 * getSize()));
  if (_ttError) {
  ...
  setMaxAscent((ttFaceProps.os2->sTypoAscender * (((Real32)ttInstMetrics.y_scale) / 65536)) / 4096);

  setMaxDescent(
      (ttFaceProps.os2->sTypoDescender * (((Real32)ttInstMetrics.y_scale) / 65536)) / 4096);
  ...
  for (i = 0; i < ttFaceProps.num_CharMaps; i++) {
    ...
    if ((ttPlatformID == 3 && ttEncodingID == 1) || (!ttPlatformID && !ttEncodingID)) {
...
void qsortIGlyphs(Int32 numGlyphs, UChar8* indices, std::vector<ImageFontGlyph*> iGlyphs) {
    ...
    if (res[0] && res[1] || current == ' ') {
      if (res[0] + x >= width) {
        ...
        for (j = i + 1; j < numCreated; j++) {
          if (createdIndices[j] && _imageGlyphs[createdIndices[j]]->getImageSize()[0] < spaceLeft) {
