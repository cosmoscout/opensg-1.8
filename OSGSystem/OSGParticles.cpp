...
void Particles::initMethod(void) {
  DrawAction::registerEnterDefault(getClassType(),
      osgTypedMethodFunctor2BaseCPtrRef<Action::ResultE, ParticlesPtr, CNodePtr, Action*>(
...
Particles::Particles(void)
  ...
  _funcglMultiTexCoord3fvARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glMultiTexCoord3fvARB", _extMultitexture);
}
  ...
  if (whichField & PositionsFieldMask) {
    ...
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfPositions.getValue() != NullFC && _sfPositions.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & ColorsFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfColors.getValue() != NullFC && _sfColors.getValue()->findParent(thisP) == -1) {
  ...
  if (whichField & NormalsFieldMask) {
    if (origin & ChangedOrigin::Abstract) {
      if (origin & ChangedOrigin::AbstrCheckValid) {
        ...
        if (_sfNormals.getValue() != NullFC && _sfNormals.getValue()->findParent(thisP) == -1) {
...
void Particles::dump(UInt32, const BitVector) const {
 ...
 */
char* ColTraitBase::formatNames[] = {"GL_BYTE", "GL_UNSIGNED_BYTE", "GL_SHORT", "GL_UNSIGNED_SHORT",
 ...
 */
ColTraitBase::pumpFunc ColTraitBase::ColorFuncs[ColTraitBase::numFormats][4] = {
...
struct ColTraitSingle : public ColTraitBase {
  ...
  static inline void init(Particles* part, DrawActionBase*, dataType&) {
    ...
    if (col != NullFC) {
      ...
      col_func = ColorFuncs[col->getFormat() - formatBase][col->getDimension() - 1];

      if (col_func == NULL) {
        SWARNING << "Particles " << part << "have illegal "
                 << "colors: " << col->getDimension() << "D "
                 << formatNames[col->getFormat() - formatBase] << "!" << std::endl;
        return;
      } else {
...
struct ColTraitParticle : public ColTraitBase {
  ...
  static inline void init(Particles* part, DrawActionBase*, dataType& data) {
    ...
    data.func = ColorFuncs[col->getFormat() - formatBase][col->getDimension() - 1];
...
struct PosTraitNone : public ParticleTraits {
  ...
  static inline void init(Particles*, DrawActionBase*, dataType& data, GeoPositionsPtr& pos) {
  }
...
struct PosTraitGeneric : public ParticleTraits {
  ...
  static inline void vertex(dataType& data, UInt32, Vec4f& dir, Real32 s) {
    glVertex3f(data.p[0] + dir[0] * s, data.p[1] + dir[1] * s, data.p[2] + dir[2] * s);
  }
...
struct TexTraitGeneric : public ParticleTraits {
  ...
  static inline void vertex(dataType& data, UInt32, Real32 u, Real32 v) {
...
struct OSG::ParticlesDrawer {
  ...
  virtual void drawIndexed(
      Particles* part, DrawActionBase* action, Int32* index, UInt32 length) = 0;
...
struct drawViewerQuads : public ParticlesDrawer {
  virtual void drawIndexed(Particles* part, DrawActionBase* action, Int32* index, UInt32 length) {
    ...
    for (UInt32 ii = 0; ii < length; ++ii) {
      ...
      d.setValues(-vnorm[0] - vup[0], -vnorm[1] - vup[1], -vnorm[2] - vup[2], 1);
      colTrait::vertex(colData, i, 0);
      texTrait::vertex(texData, 0, 0, 0);
...
struct GeoTraitArrow : public ParticleTraits {
  ...
  static inline void draw(dataType&, Pnt3f& p, Pnt3f&, Vec3f& dx, Vec3f& dy, Vec3f& dz, Vec3f& s) {
    dz *= s[2];
    dx *= s[0];
    ...
    glVertex3f(p[0] + dz[0] * .5f + dx[0], p[1] + dz[1] * .5f + dx[1], p[2] + dz[2] * .5f + dx[2]);

    glVertex3f(p[0] + dz[0] * .5f + dx[0] * .5f, p[1] + dz[1] * .5f + dx[1] * .5f,
        p[2] + dz[2] * .5f + dx[2] * .5f);

    glVertex3f(p[0] + dz[0] + dx[0] * .5f, p[1] + dz[1] + dx[1] * .5f, p[2] + dz[2] + dx[2] * .5f);
...
struct GeoTraitRectangle : public ParticleTraits {
  ...
  static inline void draw(dataType&, Pnt3f& p, Pnt3f& sp, Vec3f& dx, Vec3f& dy, Vec3f&, Vec3f& s) {
    dx *= s[0] * .5f;

    glNormal3fv((GLfloat*)dy.getValues());

    glVertex3f(p[0] - dx[0], p[1] - dx[1], p[2] - dx[2]);

    glVertex3f(p[0] + dx[0], p[1] + dx[1], p[2] + dx[2]);

    glVertex3f(sp[0] + dx[0], sp[1] + dx[1], sp[2] + dx[2]);
...
struct drawObjects : public ParticlesDrawer {
  virtual void draw(Particles* part, DrawActionBase* action, UInt32 length) {
    ...
    for (UInt32 i = 0; i < length; ++i) {
      ...
      geoTrait::draw(geoData, p, sp, dx, dy, dz, sizeTrait::size(sizeData, i));
    }
...
template <class posTrait, class colTrait, class sizeTrait, class geoTrait>
...
template <class posTrait, class secPosTrait, class colTrait, class texTrait, class sizeTrait,
    class normalTrait>
struct drawShaderQuads : public ParticlesDrawer {
  virtual void drawIndexed(Particles* part, DrawActionBase* action, Int32* index, UInt32 length) {
    Window* win = action->getWindow();

    void(OSG_APIENTRY * _glMultiTexCoord3fvARB)(GLenum which, GLubyte * data) =
    ...
    for (UInt32 ii = 0; ii < length; ++ii) {
      ...
      _glMultiTexCoord3fvARB(GL_TEXTURE1_ARB, (GLubyte*)posTrait::position(posData).getValues());
      _glMultiTexCoord3fvARB(
          GL_TEXTURE2_ARB, (GLubyte*)secPosTrait::position(secPosData).getValues());
...
Int32* Particles::calcIndex(DrawActionBase* action, UInt32& len, Int32* index) {
  // some variables for faster access
  GeoPositionsPtr pos     = getPositions();
  ...
  if (getMode() == ViewDirQuads) {
    if (indices->size() > 0) {
      for (i = 0; i < size; i++) {
        if ((*(indices))[i] < 0 || (*(indices))[i] > Int32(pos->size()))
          continue;
...
Action::ResultE Particles::drawPrimitives(DrawActionBase* action) {
  ...
  if ((size->size() > 1 && size->size() != pos->getSize()) ||
      (col != NullFC && col->getSize() != 1 && col->getSize() != pos->getSize()) ||
      (norm != NullFC && norm->getSize() != 1 && norm->getSize() != pos->getSize())) {
    FWARNING(("Particles::draw: inconsistent attributes "
              "(p:%d s:%d c:%d)!\n",
        pos->getSize(), size->size(), (col != NullFC) ? (int)col->getSize() : -1));
  ...
  if (_sfDrawOrder.getValue() != Particles::Any) {
    if (getDynamic()) {
      ...
      Pnt3f ref(modelview[3][0], modelview[3][1], modelview[3][2]);
      Vec3f refvec(modelview[2][0], modelview[2][1], modelview[2][2]);
...
ParticlesDrawer* Particles::findDrawer(void) {
  ...
  size = (getSizes().size() == getPositions()->getSize()) ? part
                                                          : (getSizes().size() == 1) ? sing : none;
  normal = (getNormals() != NullFC && getNormals()->getSize() == getPositions()->getSize())
               ? part
               : (getNormals() != NullFC && getNormals()->getSize() == 1) ? sing : none;
  secpos =
      (getSecPositions() != NullFC && getSecPositions()->getSize() == getPositions()->getSize())
          ? part
          : (getSecPositions() != NullFC && getSecPositions()->getSize() == 1) ? sing : none;
  color = (getColors() != NullFC && getColors()->getSize() == getPositions()->getSize())
              ? part
              : (getColors() != NullFC && getColors()->getSize() == 1) ? sing : none;
  tex = (getTextureZs().size() == getPositions()->getSize())
            ? part
            : (getTextureZs().size() == 1) ? sing : none;
  ...
  bool posIs3f = (getPositions() != NullFC && getPositions()->getFormat() == GL_FLOAT &&
  ...
  case Points: {
    static ParticlesDrawer* fallback =
        new drawPoints<PosTraitGeneric, ColTraitGeneric, TexTraitGeneric, SizeTraitGeneric>;
  ...
  case Lines: {
    ...
    static ParticlesDrawer* fallback =
        new drawLines<PosTraitGeneric, ColTraitGeneric, TexTraitGeneric, SizeTraitGeneric>;
  ...
  case ViewDirQuads: {
    static ParticlesDrawer* fallback =
        new drawViewDirQuads<PosTraitGeneric, ColTraitGeneric, TexTraitGeneric, SizeTraitGeneric>;

    if (posIs3f && tex == none && size != none) {
      static ParticlesDrawer* pumps[] = {
          new drawViewDirQuads<PosTrait3f, ColTraitParticle, TexTraitNone, SizeTraitParticle>,
          new drawViewDirQuads<PosTrait3f, ColTraitSingle, TexTraitNone, SizeTraitParticle>,
          new drawViewDirQuads<PosTrait3f, ColTraitNone, TexTraitNone, SizeTraitParticle>,

          new drawViewDirQuads<PosTrait3f, ColTraitParticle, TexTraitNone, SizeTraitSingle>,
          new drawViewDirQuads<PosTrait3f, ColTraitSingle, TexTraitNone, SizeTraitSingle>,
          new drawViewDirQuads<PosTrait3f, ColTraitNone, TexTraitNone, SizeTraitSingle>,
      };
  ...
  case ViewerQuads: {
    static ParticlesDrawer* fallback =
        new drawViewerQuads<PosTraitGeneric, ColTraitGeneric, TexTraitGeneric, SizeTraitGeneric>;

    if (posIs3f && tex == none && size != none) {
      static ParticlesDrawer* pumps[] = {
          new drawViewerQuads<PosTrait3f, ColTraitParticle, TexTraitNone, SizeTraitParticle>,
          new drawViewerQuads<PosTrait3f, ColTraitSingle, TexTraitNone, SizeTraitParticle>,
          new drawViewerQuads<PosTrait3f, ColTraitNone, TexTraitNone, SizeTraitParticle>,

          new drawViewerQuads<PosTrait3f, ColTraitParticle, TexTraitNone, SizeTraitSingle>,
          new drawViewerQuads<PosTrait3f, ColTraitSingle, TexTraitNone, SizeTraitSingle>,
          new drawViewerQuads<PosTrait3f, ColTraitNone, TexTraitNone, SizeTraitSingle>,
      };
  ...
  case Arrows: {
    ...
    static ParticlesDrawer* fallback = new drawObjects<PosTraitGeneric, ColTraitGeneric,
        SizeTraitGeneric, NormalTraitGeneric, GeoTraitArrow>;
  ...
  case ViewerArrows: {
    ...
    static ParticlesDrawer* fallback =
        new drawViewerObjects<PosTraitGeneric, ColTraitGeneric, SizeTraitGeneric, GeoTraitArrow>;
  ...
  case Rectangles: {
    ...
    static ParticlesDrawer* fallback = new drawObjects<PosTraitGeneric, ColTraitGeneric,
        SizeTraitGeneric, NormalTraitGeneric, GeoTraitRectangle>;

    return fallback;
  }

  case ShaderQuads: {
    static ParticlesDrawer
        *fallback      = new drawShaderQuads<PosTraitGeneric, PosTraitGeneric, ColTraitGeneric,
            TexTraitGeneric, SizeTraitGeneric, NormalTraitGeneric>,
        *nosecfallback = new drawShaderQuads<PosTraitGeneric, PosTraitNone, ColTraitGeneric,
            TexTraitGeneric, SizeTraitGeneric, NormalTraitGeneric>;
  ...
  case ShaderStrips: {
    static ParticlesDrawer
        *fallback      = new drawShaderStrips<PosTraitGeneric, PosTraitGeneric, ColTraitGeneric,
            TexTraitGeneric, SizeTraitGeneric, NormalTraitGeneric>,
        *nosecfallback = new drawShaderStrips<PosTraitGeneric, PosTraitNone, ColTraitGeneric,
            TexTraitGeneric, SizeTraitGeneric, NormalTraitGeneric>;
  ...
  if (normal)
    return NULL; // make picky compilers happy
  return NULL;
}
