...
static std::string _depth_vp = "varying vec3 lightVec;\n"
...
static std::string _depth_fp = "uniform float sceneDiagLength;\n"
...
bool VarianceShadowMap::initFBO(Window* win) {
  if (!_texChanged) {
    ...
    for (UInt32 i = 0; i < _shadowVP->_lights.size(); i++) {
      ...
      _shadowVP->_shadowImages[i]->set(
          Image::OSG_RGBA_PF, mSize, mSize, 1, 1, 1, 0, 0, Image::OSG_FLOAT16_IMAGEDATA, false);
  ...
  if (_useFBO) {
    ...
    // Attach Renderbuffer to Framebuffer depth Buffer
    glFramebufferRenderbufferEXT(
        GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, _rb_depth2);
  ...
  for (UInt32 i = 0; i < _shadowVP->_lights.size(); ++i) {
    if (_shadowVP->_lightStates[i] != 0) {
      if (_shadowVP->getGlobalShadowIntensity() != 0.0 ||
        ...
        //_shadowVP->_texChunks[i]->activate(action,
        ...
        //_shadowVP->_texChunks[i]->deactivate(action,
        //action->getWindow()->getGLObjectId(_shadowVP->_texChunks[i]->getGLId()));
        ...
        if (_shadowVP->_lights[i].second->getType() == PointLight::getClassType()) {
          ...
          Vec3f  diff       = (_shadowVP->getLightRoot(i)->getVolume().getMax() - center);
          Real32 diffLength = diff.length();

          sceneDiagLength = dirLength + diffLength;
        }

        else if (_shadowVP->_lights[i].second->getType() == SpotLight::getClassType()) {
...
void VarianceShadowMap::createShadowFactorMap(RenderActionBase* action, UInt32 num) {
  ...
  if (_shadowVP->_lights[num].second->getShadowIntensity() != 0.0 ||
    ...
    if (_shadowVP->_lights[num].second->getType() == PointLight::getClassType()) {
      ...
      Vec3f  diff       = (_shadowVP->getLightRoot(num)->getVolume().getMax() - center);
      Real32 diffLength = diff.length();
