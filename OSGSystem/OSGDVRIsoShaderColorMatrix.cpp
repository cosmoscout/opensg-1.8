...
void DVRIsoShader::activate_ColorMatrixShading(DVRVolume* volume, DrawActionBase*) {
  ...
  glPushAttrib(GL_COLOR_BUFFER_BIT | GL_ENABLE_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT |
               GL_POLYGON_BIT | GL_PIXEL_MODE_BIT);
...
void DVRIsoShader::getCoveredScreenRect(
    DVRVolume* volume, DrawActionBase* action, GLfloat screenRect[4]) {
  int vpSize[2];
  ...
  if (tex != NullFC) {
    ...
    action->getCamera()->getProjectionTranslation(prjTranslationMatrix, vpSize[0], vpSize[1]);
    Matrix4f prjMatrix;

    action->getCamera()->getProjection(prjMatrix, vpSize[0], vpSize[1]);
    ...
    Pnt3f minBB(-0.5f * res[0] * slice[0], -0.5f * res[1] * slice[1], -0.5f * res[2] * slice[2]);

    Pnt3f maxBB(-minBB);

    Pnt3f BB[8] = {Pnt3f(minBB[0], minBB[1], minBB[2]), Pnt3f(maxBB[0], minBB[1], minBB[2]),
        Pnt3f(maxBB[0], maxBB[1], minBB[2]), Pnt3f(minBB[0], maxBB[1], minBB[2]),
        Pnt3f(minBB[0], minBB[1], maxBB[2]), Pnt3f(maxBB[0], minBB[1], maxBB[2]),
        Pnt3f(maxBB[0], maxBB[1], maxBB[2]), Pnt3f(minBB[0], maxBB[1], maxBB[2])};
