...
static const Real32 DEF_SPINE[DEF_N_SPINE_POINTS][3] = {{0.f, 0.f, 0.f}, {0.f, 1.f, 0.f}};
...
class ExtrusionSurface {
 ...
 public:
  ...
  ExtrusionSurface(const std::vector<Pnt2f>& crossSection,
      const std::vector<Quaternion>& orientation, const std::vector<Vec2f>& scale,
      const std::vector<Pnt3f>& spine, Real32 creaseAngle = 0.f, bool beginCap = true,
      bool endCap = true, bool ccw = true, bool convex = true, bool buildNormal = true,
 ...
 private:
   ...
   */
  void storeConvexCap(UInt32 spineIndex, bool invertNormal, GeoIndicesUI32Ptr indicesPtr,
      GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr);
   ...
   */
  void storeSweepSurfaceWithNormals(
   ...
   * are not generated.
   */
  void storeSweepSurfaceWithoutNormals(
      GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr);
   ...
   */
  void storeMaps(
      GeoPositions3fPtr positionsPtr, GeoNormals3fPtr normalsPtr, GeoTexCoords2fPtr texCoordsPtr);

  /**
   * Stores a property index in the supplied field container and
   * the property itself into an internal map
   */
  inline void storeVertex(const Vertex& vertex, GeoIndicesUI32Ptr indicesPtr);
   ...
   */
  inline void storePrimitive(GLenum type, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr);
  ...
  inline UInt32 store(PType property, std::map<PType, UInt32, vecless<PType>>& propertyIndexMap);
   ...
   */
  template <typename PType>
  inline void store(const PType property, std::map<PType, UInt32, vecless<PType>>& propertyIndexMap,
   ...
   * otherwise
   */
  inline Vec3f calcTriangleFaceNormal(const Pnt3f& a, const Pnt3f& b, const Pnt3f& c);
   ...
   */
  inline Vec3f calcQuadFaceNormal(const Pnt3f& a, const Pnt3f& b, const Pnt3f& c, const Pnt3f& d);
   ...
   */
  inline void calcVertexNormal(Vertex* vertexPtr, UInt32 faceIndex);
  ...
  void storeFaceNormalGeo(
      GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr);

  // adds vertex normals to geometry
  void storeVertexNormalGeo(
      GeoIndicesUI32Ptr indicesPtr, GeoPLengthsUI32Ptr lensPtr, GeoPTypesUI8Ptr typesPtr);
};
