...
/*! \var Vec3f           ParticlesBase::_mfSizes
    The particle sizes. If not set (1,1,1) will be used, if only one entry         is set, it will
   be used for all particles. If the number of sizes if         equal to the number of positions
   every particle will get its own size.         Most modes only use the X coordinate of the vector.
   Particles with          size[0] == 0 are ignored.
*/
/*! \var GeoPositionsPtr ParticlesBase::_sfSecPositions
    The secondary position of the particle. This information is only used         by a few rendering
   modes, e.g. the streak mode. Usually it represents         the particle's last position.
*/
/*! \var GeoColorsPtr    ParticlesBase::_sfColors
    The particle colors (optional).
*/
/*! \var GeoNormalsPtr   ParticlesBase::_sfNormals
    Most particles will be automatically aligned to the view         direction. If normals are set
   they will be used to define the         direction the particles are facing.
*/
...
/*! \var Real32          ParticlesBase::_mfTextureZs
    The texture z coordinate of the particles. Useful in conjunction with 3D          textures to
   use different texture images on different particles.
*/
/*! \var UInt32          ParticlesBase::_sfDrawOrder
    Define an optional sorting on the particles, see osg::Particles::DrawOrderE         for
   variants. Default is unordered.
*/
/*! \var bool            ParticlesBase::_sfDynamic
    Hint to tell the system whether particles are expected to change position or         not. Is
   used to speed up sorting.
*/
...
/*! \var Int32           ParticlesBase::_sfNumParticles
    Optional number of particles to use. If set to -1, all the particles in          pos, or indices
   if set, will be used.
*/
...
FieldDescription* ParticlesBase::_desc[] = {
    new FieldDescription(SFUInt32::getClassType(), "mode", ModeFieldId, ModeFieldMask, false,
        (FieldAccessMethod)&ParticlesBase::getSFMode),
    new FieldDescription(SFGeoPositionsPtr::getClassType(), "positions", PositionsFieldId,
        PositionsFieldMask, false, (FieldAccessMethod)&ParticlesBase::getSFPositions),
    new FieldDescription(MFVec3f::getClassType(), "sizes", SizesFieldId, SizesFieldMask, false,
        (FieldAccessMethod)&ParticlesBase::getMFSizes),
    new FieldDescription(SFGeoPositionsPtr::getClassType(), "secPositions", SecPositionsFieldId,
        SecPositionsFieldMask, false, (FieldAccessMethod)&ParticlesBase::getSFSecPositions),
    new FieldDescription(SFGeoColorsPtr::getClassType(), "colors", ColorsFieldId, ColorsFieldMask,
        false, (FieldAccessMethod)&ParticlesBase::getSFColors),
    new FieldDescription(SFGeoNormalsPtr::getClassType(), "normals", NormalsFieldId,
        NormalsFieldMask, false, (FieldAccessMethod)&ParticlesBase::getSFNormals),
    new FieldDescription(MFInt32::getClassType(), "indices", IndicesFieldId, IndicesFieldMask,
        false, (FieldAccessMethod)&ParticlesBase::getMFIndices),
    new FieldDescription(MFReal32::getClassType(), "textureZs", TextureZsFieldId,
        TextureZsFieldMask, false, (FieldAccessMethod)&ParticlesBase::getMFTextureZs),
    new FieldDescription(SFUInt32::getClassType(), "drawOrder", DrawOrderFieldId,
        DrawOrderFieldMask, false, (FieldAccessMethod)&ParticlesBase::getSFDrawOrder),
    new FieldDescription(SFBool::getClassType(), "dynamic", DynamicFieldId, DynamicFieldMask, false,
        (FieldAccessMethod)&ParticlesBase::getSFDynamic),
    new FieldDescription(SFUInt32::getClassType(), "pump", PumpFieldId, PumpFieldMask, true,
        (FieldAccessMethod)&ParticlesBase::getSFPump),
    new FieldDescription(SFParticleBSPTree::getClassType(), "bsp", BspFieldId, BspFieldMask, false,
        (FieldAccessMethod)&ParticlesBase::getSFBsp),
    new FieldDescription(SFInt32::getClassType(), "numParticles", NumParticlesFieldId,
        NumParticlesFieldMask, false, (FieldAccessMethod)&ParticlesBase::getSFNumParticles)};

FieldContainerType ParticlesBase::_type("Particles", "MaterialDrawable", NULL,
    (PrototypeCreateF)&ParticlesBase::createEmpty, Particles::initMethod, _desc, sizeof(_desc));
...
void ParticlesBase::executeSync(FieldContainer& other, const BitVector& whichField) {
  this->executeSyncImpl((ParticlesBase*)&other, whichField);
}
#else
void ParticlesBase::executeSync(
    FieldContainer& other, const BitVector& whichField, const SyncInfo& sInfo) {
  this->executeSyncImpl((ParticlesBase*)&other, whichField, sInfo);
}
void ParticlesBase::execBeginEdit(
...
void ParticlesBase::copyToBin(BinaryDataHandler& pMem, const BitVector& whichField) {
...
void ParticlesBase::copyFromBin(BinaryDataHandler& pMem, const BitVector& whichField) {
...
#if !defined(OSG_FIXED_MFIELDSYNC)
void ParticlesBase::executeSyncImpl(ParticlesBase* pOther, const BitVector& whichField) {
...
#else
void ParticlesBase::executeSyncImpl(
    ParticlesBase* pOther, const BitVector& whichField, const SyncInfo& sInfo) {
...
void ParticlesBase::execBeginEditImpl(
    const BitVector& whichField, UInt32 uiAspect, UInt32 uiContainerSize) {
