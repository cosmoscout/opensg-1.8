...
const OSG::BitVector Surface::CurveFieldMask =
    (Surface::NumCurvesFieldMask | Surface::KnotLengthsFieldMask | Surface::DimensionsFieldMask |
        Surface::CurveControlPointsFieldMask | Surface::KnotsFieldMask |
        Surface::CurvesPerLoopFieldMask);

const OSG::BitVector Surface::SurfaceFieldMask =
    (Surface::DimUFieldMask | Surface::DimVFieldMask | Surface::KnotsUFieldMask |
        Surface::KnotsVFieldMask | Surface::ControlPointsFieldMask | Surface::ErrorFieldMask |
        Surface::IsDelaunayFieldMask | Surface::TextureControlPointsFieldMask);

/*! \class osg::Surface
    \ingroup Geometries
This is the OpenSG NURBS surface node. It supports an arbitrary number of trimming curves. The
interface lets you define both rational and nonrational surfaces, and you can also vary rational and
nonrational trimming curves.
...
void Surface::initMethod(void) {
  ...
  RenderAction::registerEnterDefault(getClassType(),
      osgTypedMethodFunctor2BaseCPtrRef<Action::ResultE, SurfacePtr, CNodePtr, Action*>(
...
void Surface::changed(BitVector whichField, UInt32 origin) {
  ...
  {
    if ((whichField & DimUFieldMask) || (whichField & DimVFieldMask) ||
        (whichField & KnotsUFieldMask) || (whichField & KnotsVFieldMask) ||
        (whichField & ControlPointsFieldMask) || (whichField & NumCurvesFieldMask) ||
        (whichField & KnotLengthsFieldMask) || (whichField & DimensionsFieldMask) ||
        (whichField & CurveControlPointsFieldMask) || (whichField & KnotsFieldMask) ||
        (whichField & CurvesPerLoopFieldMask) || (whichField & TextureControlPointsFieldMask)) {
    ...
    } else if ((whichField & ErrorFieldMask) || (whichField & IsDelaunayFieldMask)) {
...
bool Surface::updateError(Pnt3f viewPos, Real64 halfPixelSize) {
  ...
  if (d_max_error < current_error) {
    std::cerr << "retessellating due to too high error: " << d_max_error << " " << current_error
              << std::endl;
    ...
    std::cerr << "retessellating due to too low error: " << d_max_error << " " << current_error
              << std::endl;
    SurfacePtr thisPtr = SurfacePtr(this);
    beginEditCP(thisPtr, Surface::ErrorFieldMask);
    { setError(d_max_error / 2.0); }
...
void Surface::reTessellate(void) {
  ...
  if (_sfTextureControlPoints.getValue() == NullFC) {
    ...
    */
    ...
    _surfacePatch->getTessellation(gverts, norms, texturecoords, tris, getIsDelaunay());
    (void)buildSurface(tris, gverts, norms, texturecoords);
  }
...
Int32 Surface::convertSurface(void) {
  ...
  bool              israt;
  GeoPositions3fPtr pPos    = GeoPositions3fPtr::dcast(_sfControlPoints.getValue());
  GeoPositions4fPtr pRatPos = GeoPositions4fPtr::dcast(_sfControlPoints.getValue());
  ...
  for (i = 0; i < trimloops.size(); ++i) {
    for (j = 0; j < trimloops[i].size(); ++j) {
      ...
      if (acttrimcpsize < 1) {
        SLOG << "Surface::tessellate: wrong trimming curve attributes " << endLog;
        return -1;
      }
      ...
      actcurve.setKnotsAndDimension(actknots, _mfDimensions[actcurveno]);
...
Int32 Surface::tessellateSurface(
  ...
  _min = Pnt3d(surfaceData->clMin[0], surfaceData->clMin[1], surfaceData->clMin[2]);
  _max = Pnt3d(surfaceData->clMax[0], surfaceData->clMax[1], surfaceData->clMax[2]);
...
Int32 Surface::tessellateSurface(std::vector<SimplePolygon>& triangles, std::vector<Pnt3f>& gverts,
  ...
  for (i = 0; i < s; ++i) {
    tmptexcoords.push_back(Pnt2f(pTexPos->getField()[i][0], pTexPos->getField()[i][1]));
  }
...
// internal
// normal surface
Int32 Surface::buildSurface(std::vector<SimplePolygon>& triangles, std::vector<Pnt3f>& gverts,
    std::vector<Vec3f>& norms, std::vector<Pnt2f>& texcoords)

{
  ...
  if (texcoords.size() > 0) {
    beginEditCP(pcl_texcoords, GeoTexCoords2f::GeoPropDataFieldMask);
    {
      ...
      for (ui_vert = 0; ui_vert < cui_verts; ++ui_vert) {
        if (vi_new_idx[ui_vert] >= 0) {
          pcl_texcoords->push_back(Vec2f(texcoords[ui_vert][0], texcoords[ui_vert][1]));
  ...
  beginEditCP(pcl_indices, GeoIndicesUI32::GeoPropDataFieldMask);
  {
    ...
    for (ui_face = 0; ui_face < cui_faces; ++ui_face) {
      pcl_face = &triangles[ui_face];
      if ((norms.size() == 0) || (checkOrient(pcl_face->vertices[0], pcl_face->vertices[1],
  ...
  if (b_show_trimming) {
    if (norms.size() > 0) {
      ...
      {
        beginEditCP(thisPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                                 Geometry::IndicesFieldMask | Geometry::PositionsFieldMask |
                                 Geometry::NormalsFieldMask | Geometry::TexCoordsFieldMask |
                                 Geometry::IndexMappingFieldMask | Geometry::TexCoords1FieldMask);
        {
          setTypes(pcl_type);
          setLengths(pcl_size);
        ...
        endEditCP(thisPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                               Geometry::IndicesFieldMask | Geometry::PositionsFieldMask |
                               Geometry::NormalsFieldMask | Geometry::TexCoordsFieldMask |
                               Geometry::IndexMappingFieldMask | Geometry::TexCoords1FieldMask);
      } else // fatborders, normals but no standard textures (most used case)
      {
        beginEditCP(thisPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                                 Geometry::IndicesFieldMask | Geometry::PositionsFieldMask |
                                 Geometry::NormalsFieldMask | Geometry::IndexMappingFieldMask |
                                 Geometry::TexCoordsFieldMask | Geometry::TexCoords1FieldMask);
        {
          setTypes(pcl_type);
          setLengths(pcl_size);
          setIndices(pcl_indices);
      ...
      if (texcoords.size() > 0) // fatborders and normalmaps and no textures
      {
        beginEditCP(thisPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                                 Geometry::IndicesFieldMask | Geometry::PositionsFieldMask |
                                 Geometry::TexCoordsFieldMask | Geometry::IndexMappingFieldMask |
             ...
             // essentially flat-shading
      {
        beginEditCP(thisPtr, Geometry::TypesFieldMask | Geometry::LengthsFieldMask |
                                 Geometry::IndicesFieldMask | Geometry::PositionsFieldMask |
                                 Geometry::IndexMappingFieldMask | Geometry::TexCoordsFieldMask |
                                 Geometry::TexCoords1FieldMask);
        {
          setTypes(pcl_type);
          setLengths(pcl_size);
...
void Surface::addCurve(
    UInt32 dim, std::vector<Real64>& knots, std::vector<Pnt2f>& controlpoints, bool newloop) {
  ...
  for (UInt32 i = 0; i < cpsize; ++i) {
    ratcontrolpoints.push_back(Pnt3f(controlpoints[i][0], controlpoints[i][1], 1.0f));
  }
 ...
 */
void Surface::addCurve(
    UInt32 dim, std::vector<Real64>& knots, std::vector<Pnt3f>& controlpoints, bool newloop) {
  UInt32 cpsize   = controlpoints.size();
  ...
  for (UInt32 i = 0; i < trimloop.size(); ++i) {
    ...
    for (UInt32 j = 0; j < trimloop[i].size(); ++j) {
      ...
      for (UInt32 kk = 0; kk < v3actcps.size(); ++kk) {
        actcontrolpoints.push_back(Pnt3f(v3actcps[kk][0], v3actcps[kk][1], v3actcps[kk][2]));
      }
...
UInt32 Surface::writetoobj(std::ostream& outfile, UInt32 offset) {
  ...
  for (uicnt = 0; uicnt < uivertsize; uicnt++) {
    ...
    outfile << "v " << ppp.x() << " " << ppp.y() << " " << ppp.z() << std::endl;
  }
  // write out normals
  for (uicnt = 0; uicnt < uivertsize; uicnt++) {
    Pnt3f ppp = pcl_norms->getValue(uicnt);

    outfile << "vn " << ppp.x() << " " << ppp.y() << " " << ppp.z() << std::endl;
  }
  // write out texture coordinates
  for (uicnt = 0; uicnt < uivertsize; uicnt++) {
    Pnt2f ppp = pcl_texcoords->getValue(uicnt);

    outfile << "vt " << ppp.x() << " " << ppp.y() << " " << std::endl;
  }
...
void Surface::dump(UInt32, const BitVector) const {
...
void Surface::onCreate(const Surface* source) {
  ...
  if (Thread::getAspect() != _sfIgnoreGLForAspect.getValue()) {
    ...
    setSurfaceGLId(
        Window::registerGLObject(osgTypedMethodVoidFunctor2ObjCPtrPtr<SurfacePtr, Window, UInt32>(
                                     tmpPtr, &Surface::handleGL),
            1));
