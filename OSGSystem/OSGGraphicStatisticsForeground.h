...
class OSG_SYSTEMLIB_DLLMAPPING GraphicStatisticsForeground
    : public GraphicStatisticsForegroundBase {
 private:
  typedef GraphicStatisticsForegroundBase Inherited;

  /*==========================  PUBLIC  =================================*/
 public:
  enum Type { OSG_ANALOG = 0, OSG_BAR = 1, OSG_CHART = 2, OSG_LINE_CHART = 3, OSG_TEXT = 4 };
  ...
  void addElement(StatElemDescBase& desc, UInt32 displayType, Vec2f pos, Vec2f size,
      Color4f highColor, Color4f lowColor, Color4f currentColor, Real32 minValue = 0.0,
      Real32 maxValue = 0.0, UInt32 Flags = OSG_UNDERFLOW_RESIZE | OSG_OVERFLOW_RESIZE,
      UInt32 historySize = 0, std::string description = "");
 ...
 protected:
  ...
  //! Draws a string
  void drawString(DrawActionBase* base, const std::string& text,
...
#define OSGGRAPHICSTATISTICSFOREGROUND_HEADER_CVSID                                                \
  "@(#)$Id: OSGGraphicStatisticsForeground.h,v 1.1 2002/07/18 16:09:33 jbehr Exp $"
