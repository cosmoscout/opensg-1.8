...
static UInt32 parseUInt32(char*& ptr) {
  UInt32 result = (static_cast<UInt32>(static_cast<UInt8>(ptr[0])) << 24) |
...
static UInt16 parseUInt16(char*& ptr) {
  UInt16 result = (static_cast<UInt32>(static_cast<UInt8>(ptr[0])) << 8) |
...
static bool readImageData(std::istream& is, const PSDHeader& header, UInt16 relevantChannels,
    char* colormap, Int16& transparentIndex, UInt8* data) {
...
static const Char8* psdSuffixArray[] = {"psd"};

PSDImageFileType PSDImageFileType::_the(
    "image/x-photoshop", psdSuffixArray, sizeof(psdSuffixArray), OSG_READ_SUPPORTED);
...
*/
bool PSDImageFileType::read(ImagePtr& image, std::istream& is, const std::string& mimetype) {
  PSDHeader header;
...
std::string PSDImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "8BPS", 4) == 0 ? std::string(getMimeType()) : std::string();
}
...
*/
PSDImageFileType::PSDImageFileType(
    const Char8* mimeType, const Char8* psdSuffixArray[], UInt16 suffixByteCount, UInt32 flags)
