...
Int32 SceneFileHandler::getSuffixList(std::list<const Char8*>& suffixList, UInt32 flags) {
  Int32                 count = 0;
...
NodePtr SceneFileHandler::read(
    std::istream& is, const Char8* fileNameOrExtension, GraphOpSeq* graphOpSeq) {
...
NodePtr SceneFileHandler::read(const Char8* fileName, GraphOpSeq* graphOpSeq) {
  if (!fileName) {
  ...
  if (type) {
    ...
    SINFO << "try to read " << fullFilePath << " as " << type->getName() << std::endl;
    ...
    SWARNING << "could not read " << fullFilePath << "; unknown file format" << std::endl;
  }
...
SceneFileHandler::FCPtrStore SceneFileHandler::readTopNodes(
    const Char8* fileName, GraphOpSeq* graphOpSeq) {
  std::vector<FieldContainerPtr> nodeVec;
...
//
//------------------------------
bool SceneFileHandler::write(
    const NodePtr& node, std::ostream& os, const Char8* fileNameOrExtension, bool compress) {
  bool           retCode = false;
  SceneFileType* type    = getFileType(fileNameOrExtension);
...
void SceneFileHandler::print(void) {
  ...
  for (sI = _suffixTypeMap.begin(); sI != _suffixTypeMap.end(); sI++) {
    ...
    std::cerr << "suffix: " << sI->first.str() << ", type: " << sI->second->front()->getName()
              << " " << rw << std::endl;
  }
}
...
bool SceneFileHandler::addSceneFileType(SceneFileType& fileType) {
  ...
  for (sI = fileType.suffixList().begin(); sI != fileType.suffixList().end(); ++sI) {
    suffix.set(sI->str());
    suffix.toLower();
    ...
    if (smI != the()._suffixTypeMap.end()) {
      if (fileType.doOverride() == true) {
        ...
        lIt = std::find_if(the()._suffixTypeMap[suffix]->begin(),
            the()._suffixTypeMap[suffix]->end(), overrideFinder);

        the()._suffixTypeMap[suffix]->insert(lIt, &fileType);

        SWARNING << "Added an file type with suffix " << suffix << " overriding " << std::endl;
      } else {
        the()._suffixTypeMap[suffix]->push_back(&fileType);

        SWARNING << "Added an file type with suffix " << suffix
                 << " non overriding at the end of the list" << std::endl;
      }
    } else {
...
void SceneFileHandler::readProgress(void* OSG_CHECK_ARG(data)) {
  ...
  while (p < 100 && !the()._readReady) {
    if (!the()._progressData.is->eof() && !the()._progressData.is->bad()) {
