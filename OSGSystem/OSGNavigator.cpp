...
void Navigator::buttonPress(Int16 button, Int16 x, Int16 y) {
  ...
  case TRACKBALL:
    ...
    case RIGHT_MOUSE:
      _currentState = TRANSLATING_Z;
      break;

    case MIDDLE_MOUSE:
      _currentState = TRANSLATING_XY;
    ...
    default:
      FNOTICE(("Navigator: buttonPress, unknown button\n"));
      break;
    }
    break;
  ...
  case FLY:
    ...
    switch (button) {
    case LEFT_MOUSE:
      _currentState = TRANSLATING_ZPLUS;
      break;
    ...
    case RIGHT_MOUSE:
      _currentState = TRANSLATING_ZMINUS;
      break;
...
void Navigator::buttonRelease(Int16, Int16 x, Int16 y) {
  ...
  default:
    FNOTICE(("Navigator: buttonRelease, unknown mode\n"));
    break;
  }
  _currentState = IDLE;
}
...
void Navigator::keyPress(Int16 key, Int16, Int16) {
  ...
  case TRACKBALL:
    ...
    case BACKWARDS:
      ...
      break;
    default:
      FNOTICE(("Navigator: keyPress, unknown key\n"));
    }
    break;
  ...
  case FLY:
    ...
    switch (key) {
    case LEFTROT:
      _flyer.rotate(-_rRotationAngle, 0);
      break;
    case RIGHTROT:
      _flyer.rotate(_rRotationAngle, 0);
      break;
    case LEFT:
      _flyer.right(_rMotionFactor);
      break;
    case RIGHT:
      _flyer.right(-_rMotionFactor);
      break;
  ...
  case WALK:
    ...
    switch (key) {
    case LEFTROT:
      _walker.rotate(-_rRotationAngle, 0);
      break;
    case RIGHTROT:
      _walker.rotate(_rRotationAngle, 0);
      break;
    case LEFT:
      _walker.right(_rMotionFactor);
      break;
    case RIGHT:
      _walker.right(-_rMotionFactor);
      break;
...
void Navigator::moveTo(Int16 x, Int16 y) {
  ...
  Real32 fromY = (2.0f * (winHeight - _lastY - _vp->getPixelBottom()) - height) / height;
  Real32 toX   = (2.0f * (x - _vp->getPixelLeft()) - width) / width;
  Real32 toY   = (2.0f * (winHeight - y - _vp->getPixelBottom()) - height) / height;
  ...
  case TRACKBALL:
    ...
    case TRANSLATING_XY: {
      ...
      calcDeltas(Int16(_lastX), Int16(_lastY), x, y, distanceX, distanceY);
      _trackball.translateXY(distanceX, distanceY);
    } break;

    case TRANSLATING_Z: {
      Real32 distance = osgSgn(toY - fromY) * 100.f * osgpow(osgabs(toY - fromY), 2.f);
...
void Navigator::updateCameraTransformation() {
  ...
  switch (_currentMode) {
  case NONE:
    theMatrix.mult(_NoneMatrix);
    break;
  case TRACKBALL:
    theMatrix.mult(_trackball.getMatrix());
    break;
  case FLY:
    theMatrix.mult(_flyer.getMatrix());
    break;
  case WALK:
    theMatrix.mult(_walker.getMatrix());
    break;
  default:
    FNOTICE(("Navigator: updateCamTrans, unknown mode\n"));
    break;
  }
...
void Navigator::setFrom(Pnt3f new_from) {
  switch (_currentMode) {
  case NONE:
    setNoneMatrix(new_from, (Pnt3f)(_NoneMatrix[3] - _NoneMatrix[2]), (Vec3f)_NoneMatrix[1]);
    break;
  case TRACKBALL:
    _trackball.setFrom(new_from);
    break;
  case FLY:
    _flyer.setFrom(new_from);
    break;
  case WALK:
    _walker.setFrom(new_from);
    break;
  default:
    FNOTICE(("Navigator: setFrom, unknown mode"));
    break;
  }
}
...
void Navigator::setAt(Pnt3f new_at) {
  ...
  case NONE:
    ...
    break;
  case TRACKBALL:
    _trackball.setAt(new_at);
    break;
  case FLY:
    _flyer.setAt(new_at);
    break;
  case WALK:
    _walker.setAt(new_at);
    break;
  default:
    FNOTICE(("Navigator: setAt, unknown mode"));
    break;
  }
}
...
void Navigator::setDistance(Real32 new_distance) {
  ...
  case TRACKBALL:
    _trackball.setDistance(new_distance);
    break;

  case FLY:
    _flyer.forward(new_distance);
    break;

  case WALK:
    _walker.forward(new_distance);
    break;

  default:
    FNOTICE(("Navigator: setDistance, unknown mode"));
    break;
  }
}
...
void Navigator::setUp(Vec3f new_up) {
  switch (_currentMode) {
  case NONE:
    setNoneMatrix((Pnt3f)_NoneMatrix[3], (Pnt3f)(_NoneMatrix[3] - _NoneMatrix[2]), new_up);
    break;

  case TRACKBALL:
    _trackball.setUp(new_up);
    break;
  ...
  case WALK:
    _walker.setUp(new_up);
    break;

  default:
    FNOTICE(("Navigator: setUp, unknown mode"));
    break;
  }
}
...
void Navigator::set(Pnt3f new_from, Pnt3f new_at, Vec3f new_up) {
  switch (_currentMode) {
  case NONE:
    setNoneMatrix(new_from, new_at, new_up);
    break;

  case TRACKBALL:
    _trackball.set(new_from, new_at, new_up);
    break;

  case FLY:
    _flyer.set(new_from, new_at, new_up);
    break;

  case WALK:
    _walker.set(new_from, new_at, new_up);
    break;

  default:
    FNOTICE(("Navigator: set, unknown mode"));
    break;
  }
}
...
void Navigator::set(const Matrix& new_matrix) {
  ...
  case TRACKBALL:
    _trackball.set(new_matrix);
    break;

  case FLY:
    _flyer.set(new_matrix);
    break;

  case WALK:
    _walker.set(new_matrix);
    break;

  default:
    FNOTICE(("Navigator: set(Matrix), unknown mode"));
    break;
  }
}
...
const Matrix& Navigator::getMatrix(void) {
  ...
  case FLY:
    return _flyer.getMatrix();
  case WALK:
    return _walker.getMatrix();
  default:
    FNOTICE(("Navigator: getMatrix, unknown mode"));
    break;
  }
...
const Pnt3f& Navigator::getFrom(void) {
  ...
  case FLY:
    return _flyer.getFrom();
  case WALK:
    return _walker.getFrom();
  default:
    FNOTICE(("Navigator: getFrom, unknown mode"));
    break;
  }
...
const Pnt3f& Navigator::getAt(void) {
  ...
  case FLY:
    return _flyer.getAt();
  case WALK:
    return _walker.getAt();
  default:
    FNOTICE(("Navigator: getAt, unknown mode"));
    break;
  }
...
const Vec3f& Navigator::getUp(void) {
  ...
  case FLY:
    return _flyer.getUp();
  case WALK:
    return _walker.getUp();
  default:
    FNOTICE(("NavigUpor: getUp, unknown mode"));
    break;
  }
...
Real32 Navigator::getDistance(void) {
  ...
  case TRACKBALL:
    distance = _trackball.getDistance();
    break;
...
static void calcCCtoWCMatrix(Matrix& cctowc, const Matrix& view, const ViewportPtr port) {
  ...
  port->getCamera()->getProjection(proj, port->getPixelWidth(), port->getPixelHeight());
  port->getCamera()->getProjectionTranslation(
      projtrans, port->getPixelWidth(), port->getPixelHeight());
...
void Navigator::getIntersectionPoint(Int16 x, Int16 y) {
  ...
  if (_clickNoIntersect) {
    Real32 u =
        (_dir.dot(Pnt3f(0.0f, 0.0f, 0.0f) - line.getPosition())) / (_dir.dot(line.getDirection()));
...
/*! Calculate the real translation that has to be done, so that the
    ...
    screen.
*/
void Navigator::calcDeltas(
    Int16, Int16, Int16 toX, Int16 toY, Real32& distanceX, Real32& distanceY) {
  Matrix view;
  ...
  Real32 u = (_dir.dot(_ip - line2.getPosition())) / (_dir.dot(line2.getDirection()));
