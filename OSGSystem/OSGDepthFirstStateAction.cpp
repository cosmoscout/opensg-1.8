...
StatElemDesc<StatIntElem> DepthFirstStateAction::statStateClones(
    "DepthFirstState::stateClones", "Number of state clones created");
StatElemDesc<StatIntElem> DepthFirstStateAction::statStateRestores(
    "DepthFirstState::statStateRestores", "Number of state restores");
...
DepthFirstStateAction* DepthFirstStateAction::create(void) {
...
DepthFirstStateAction::ResultE DepthFirstStateAction::apply(NodePtr pRoot) {
...
void DepthFirstStateAction::addExtendEvent(ExtendActorBase* pActor, UInt32 actorIndex) {
...
void DepthFirstStateAction::subExtendEvent(ExtendActorBase* pActor, UInt32 actorIndex) {
  ExtendActorStoreIt itEnter  = _extendEnterActors.begin();
...
void DepthFirstStateAction::addBasicEvent(BasicActorBase* pActor, UInt32 actorIndex) {
...
void DepthFirstStateAction::subBasicEvent(BasicActorBase* pActor, UInt32 actorIndex) {
  BasicActorStoreIt itEnter  = _basicEnterActors.begin();
...
void DepthFirstStateAction::startEvent(void) {
...
void DepthFirstStateAction::stopEvent(void) {
  Inherited::stopEvent();
}
...
void DepthFirstStateAction::beginEditStateEvent(ActorBase* pActor, UInt32 actorId) {
...
void DepthFirstStateAction::endEditStateEvent(ActorBase* pActor, UInt32 actorId) {
}
...
DepthFirstStateAction::ResultE DepthFirstStateAction::traverseEnter(void) {
  ResultE              result = NewActionTypes::Continue;
  NodePtr              pNode;
  ...
  while ((_nodeStack.empty() == false) && !(result & NewActionTypes::Quit)) {
    ...
    if ((nodePass == 1) && (multiPasses > 1)) {
      for (; multiPasses > 1; --multiPasses) {
        ...
        _nodeStack.push_back(NodeStackEntry(pNode, _itActiveState, multiPasses));
      }
    }
  ...
  while ((_nodeStack.empty() == false) && !(result & NewActionTypes::Quit)) {
    ...
    if (nodePass > 0) {
      ...
      if ((nodePass == 1) && (multiPasses > 1)) {
        ...
        _nodeStack.push_back(NodeStackEntry(pNode, _itActiveState, -nodePass));
      } else {
...
void DepthFirstStateAction::pushChildren(const NodePtr& pNode, ResultE result) {
  ...
  if (getChildrenListEnabled() == true) {
    for (UInt32 i = 0, size = cl.getSize(); i < size; ++i) {
      if ((cl.getActive(i) == true) && (cl.getChild(i) != NullFC) &&
        ...
        _nodeStack.push_back(NodeStackEntry(cl.getChild(i), _itActiveState, 1));
      }
    }
  } else {
    ...
    for (; itChildren != endChildren; ++itChildren) {
      if ((*itChildren != NullFC) && (((*itChildren)->getTravMask() & getTravMask()) != 0)) {
        ...
        _nodeStack.push_back(NodeStackEntry(*itChildren, _itActiveState, 1));
      }
    }
  }
  ...
  for (UInt32 i = 0, size = ecl.getSize(); i < size; ++i) {
    if ((ecl.getActive(i) == true) && (ecl.getChild(i) != NullFC) &&
      ...
      _nodeStack.push_back(NodeStackEntry(ecl.getChild(i), _itActiveState, 1));
    }
  }
