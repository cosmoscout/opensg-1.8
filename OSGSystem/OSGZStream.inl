...
basic_zip_streambuf<charT, traits>::basic_zip_streambuf(ostream_reference ostream, int level,
    EStrategy strategy, int window_size, int memory_level, size_t buffer_size)
    : _ostream(ostream)
  ...
  _err = deflateInit2(
      &_zip_stream, level, Z_DEFLATED, window_size, memory_level, static_cast<int>(strategy));
...
std::streamsize basic_zip_streambuf<charT, traits>::flush(void) {
  ...
  do {
    ...
    if (_err == Z_OK || _err == Z_STREAM_END) {
      written_byte_size =
          static_cast<std::streamsize>(_output_buffer.size()) - _zip_stream.avail_out;
      ...
      if ((remainder = written_byte_size % sizeof(char_type)) != 0) {
        ...
        memcpy(&(_output_buffer[0]), &(_output_buffer[written_byte_size - remainder]), remainder);
      }
 ...
 */
template <class charT, class traits>
inline unsigned long basic_zip_streambuf<charT, traits>::get_in_size(void) const {
...
basic_unzip_streambuf<charT, traits>::basic_unzip_streambuf(
    istream_reference istream, int window_size, size_t read_buffer_size, size_t input_buffer_size)
    : _istream(istream)
...
basic_unzip_streambuf<charT, traits>::underflow(void) {
  ...
  int num = unzip_from_stream(
      &_buffer[0] + 4, static_cast<std::streamsize>((_buffer.size() - 4) * sizeof(char_type)));
 ...
 */
template <class charT, class traits>
inline void basic_unzip_streambuf<charT, traits>::put_back_from_zip_stream(void) {
...
inline std::streamsize basic_unzip_streambuf<charT, traits>::unzip_from_stream(
  ...
  // updating crc
  _crc = crc32(
      _crc, (byte_buffer_type)buffer, buffer_size - _zip_stream.avail_out / sizeof(char_type));
 ...
 */
template <class charT, class traits>
inline basic_zip_ostream<charT, traits>::basic_zip_ostream(ostream_reference ostream, bool is_gzip,
    int level, EStrategy strategy, int window_size, int memory_level, size_t buffer_size)
    : basic_zip_streambuf<charT, traits>(
          ostream, level, strategy, window_size, memory_level, buffer_size)
...
template <class charT, class traits>
inline basic_zip_ostream<charT, traits>& basic_zip_ostream<charT, traits>::zflush(void) {
...
basic_zip_istream<charT, traits>::basic_zip_istream(
    ...
    : basic_unzip_streambuf<charT, traits>(
          istream, window_size, read_buffer_size, input_buffer_size)
 ...
 */
template <class charT, class traits>
int basic_zip_istream<charT, traits>::check_header(void) {
  ...
  if ((zip_stream.avail_in >= 2) && (this->_input_buffer[0] == detail::gz_magic[0]) &&
