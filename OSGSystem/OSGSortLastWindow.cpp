...
/*! init composition
 */
void SortLastWindow::serverInit(WindowPtr serverWindow, UInt32 id) {
  ...
  if (getComposer() != NullFC) {
    ...
    getComposer()->setup(false, id, serverWindow, clusterWindow);
    getComposer()->open();
  }
}
 ...
 */
void SortLastWindow::serverRender(WindowPtr serverWindow, UInt32 id, RenderActionBase* action) {
  ...
  for (cv = 0, sv = 0; cv < getPort().size(); ++cv) {
    ...
    if (getWidth() && getHeight())
      serverPort->setSize(clientPort->getPixelLeft(), clientPort->getPixelBottom(),
          clientPort->getPixelRight(), clientPort->getPixelTop());
    else
...
/*! swap
 */
void SortLastWindow::serverSwap(WindowPtr window, UInt32 id) {
  if (getComposer() == NullFC) {
...
void SortLastWindow::clientInit(void) {
  getNetwork()->getAspect()->addFieldFilter(Node::getClassType().getId(), Node::VolumeFieldMask);
  getNetwork()->getAspect()->addFieldFilter(Node::getClassType().getId(), Node::TravMaskFieldMask);
  ...
  if (getComposer() != NullFC) {
    ...
    getComposer()->setup(true, getServers().size(), getClientWindow(), clusterWindow);
    getComposer()->open();
    // build node groups
    buildGroups();
  }
  ...
  if (getClientWindow() != NullFC) {
    ...
    if (width != getWidth() || height != getHeight()) {
      beginEditCP(SortLastWindowPtr(this),
          SortLastWindow::WidthFieldMask | SortLastWindow::HeightFieldMask);
...
void SortLastWindow::clientRender(RenderActionBase* action) {
  ...
  if (getServers().size()) {
    Connection* srcConnection = getNetwork()->getConnection(groupId);
...
void SortLastWindow::dump(UInt32, const BitVector) const {
...
void SortLastWindow::collectDrawables(NodePtr& node, DrawableListT& drawables) {
  MaterialPtr mat;
...
void SortLastWindow::splitDrawables(DrawableListT& src, UInt32 groups, bool cut) {
  BoxVolume vol;
  //    Real32                   srcLoad=0;
  Real32                  dst1Load = 0;
  Real32                  dst2Load = 0;
...
void SortLastWindow::setupNodes(UInt32 groupId) {
  ...
  if (getServers().size() == groupId &&
      (getComposer() == NullFC || !getComposer()->getClientRendering())) {
  ...
  for (nI = 0, gnI = 0, gI = 0, group = 0; nI < getGroupNodes().size(); ++nI) {
    ...
    if (group == groupId) {
      if (getGroupNodes()[nI]->getTravMask() != TypeTraits<UInt32>::getMax()) {
        beginEditCP(getGroupNodes()[nI], Node::TravMaskFieldMask);
