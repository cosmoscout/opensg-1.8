...
void BinarySwapComposer::sendToClient(
  ...
  for (ty = bottom; ty <= top; ++ty)
    for (tx = left; tx <= right; ++tx) {
      ...
      if (!tile->empty) {
        ...
        client->put(&(tile->header), tile->colorSize + sizeof(tile->header));
        client->flush();
  ...
  if (lastx == right + 1) {
    ...
    t.header.x = 0x8000;
    t.header.y = 0x8000;
    client->put(&(t.header), sizeof(t.header));
    client->flush();
...
template <class DepthT, class ColorT>
void BinarySwapComposer::recvFromServers(
  ...
  GroupConnection* servers  = clusterWindow()->getNetwork()->getGroupConnection(clusterId());

  Connection::Channel channel;

  while (missing) {
    channel = servers->selectChannel();
    servers->get(&(readTile->header), sizeof(readTile->header));
    ...
    if (!(readTile->header.y & 0x8000)) {
      servers->get(readTile->data, readTile->header.w * readTile->header.h * sizeof(ColorT));
      _statistics.bytesIn += readTile->header.w * readTile->header.h * sizeof(ColorT);
      glRasterPos2f(readTile->header.x * getTileSize(), readTile->header.y * getTileSize());
      glDrawPixels(readTile->header.w, readTile->header.h, colorType, colorFormat, readTile->data);
    }
  }
...
void BinarySwapComposer::writeCombine(DepthT& depthDummy, ColorT& colorDummy) {
  ...
  for (;;) {
    ...
    for (ty = _writeBottom; ty <= _writeTop; ++ty) {
      for (tx = _writeLeft; tx <= _writeRight; ++tx) {
        ...
        if (!tile->empty) {
          if (ty == lasty && tx == lastx)
            tile->header.x ^= 0x8000;
          _swapConnection->put(&(tile->header), tile->dataSize + sizeof(tile->header));
    ...
    if (lastx == _writeRight + 1) {
      ...
      t.header.x = 0x8000;
      t.header.y = 0x8000;
      _swapConnection->put(&(t.header), sizeof(t.header));
      _statistics.bytesOut += sizeof(t.header);
    }
  }
}
...
template <class DepthT, class ColorT>
void BinarySwapComposer::readCombine(DepthT& depth, ColorT& color, UInt32 colorType,
    UInt32 colorFormat, UInt32 left, UInt32 bottom, UInt32 right, UInt32 top, UInt32 level) {
  UInt32      w, h;
  ...
  if (_swapWith >= _usableServers) {
    _barrier->enter(2);
    // send the resulting tile to the client
    sendToClient(depth, color, left, bottom, right, top);
    return;
  }
  ...
  do {
    _swapConnection->get(&(readTile->header), sizeof(readTile->header));
    _statistics.bytesIn += sizeof(readTile->header);
    if (readTile->header.x & 0x8000) {
      last = true;
      readTile->header.x ^= 0x8000;
    ...
    if (!(readTile->header.y & 0x8000)) {
      ...
      if (tile->empty) {
        ...
        _swapConnection->get(tile->data, tile->dataSize);
        _statistics.bytesIn += tile->dataSize;
        if (sizeof(ColorT) == 4) {
          glRasterPos2f(tile->header.x * getTileSize(), tile->header.y * getTileSize());
          glDrawPixels(tile->header.w, tile->header.h, colorType, colorFormat, tile->data);
        }
        tile->empty = false;
      } else {
        // read
        _swapConnection->get(readTile->data, tile->dataSize);
        _statistics.bytesIn += tile->dataSize;
        // depth compose
        srcDepth    = (DepthT*)(readTile->data + tile->colorSize);
        srcDepthEnd = (DepthT*)(readTile->data + tile->dataSize);
        ...
        if (sizeof(ColorT) == 4) {
          ...
          glRasterPos2f(tile->header.x * getTileSize(), tile->header.y * getTileSize());
          glDrawPixels(tile->header.w, tile->header.h, colorType, colorFormat, readTile->data);
          glReadPixels(tile->header.x * getTileSize(), tile->header.y * getTileSize(),
              tile->header.w, tile->header.h, colorType, colorFormat, tile->data);
        } else {
          // no alpha
          while (srcDepth < srcDepthEnd) {
            if (*srcDepth < *dstDepth) {
  ...
  readCombine(
      depth, color, colorType, colorFormat, readLeft, readBottom, readRight, readTop, level << 1);
}

template <class DepthT, class ColorT>
void BinarySwapComposer::startReader(DepthT& depthMax, ColorT& color, UInt32 depthFormat,
    UInt32 colorType, UInt32 colorFormat, ViewportPtr port) {
  ...
  if (!getScreenAlignedBBox(port->getRoot(), port, left, bottom, right, top, front, back)) {
  ...
  for (ty = 0; ty < _tilesY; ++ty) {
    for (tx = 0; tx < _tilesX; ++tx) {
      ...
      if (x > right || (x + w) <= left || y > top || (y + h) <= bottom) {
        tile->empty = true;
        continue;
      }
      tile->empty = false;
      glReadPixels(x, y, w, h, GL_DEPTH_COMPONENT, depthFormat, tile->data + tile->colorSize);
#ifdef OPTIMIZED_BINARY_SWAP
      if (sizeof(ColorT) != 4) {
        depthStart = (DepthT*)(tile->data + tile->colorSize);
        depthEnd   = (DepthT*)(tile->data + tile->colorSize + tile->depthSize);
        // empty if no depth
        while (depthStart < depthEnd && *depthStart == (DepthT)depthMax)
          ++depthStart;
...
#endif
      if (!tile->empty) {
        glReadPixels(x, y, w, h, colorType, colorFormat, tile->data);
        if (sizeof(ColorT) == 4) {
  ...
  _barrier->enter(2);
  readCombine(depthMax, color, colorType, colorFormat, 0, 0, _tilesX - 1, _tilesY - 1, 1);
}

OSG_END_NAMESPACE

#define OSGBINARYSWAPCOMPOSER_INLINE_CVSID "@(#)$Id: $"
