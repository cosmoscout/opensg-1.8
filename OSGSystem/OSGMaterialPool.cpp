...
void MaterialPool::dump(UInt32 uiIndent, const BitVector bvFlags) const {
...
void MaterialPool::sync(void) {
  ...
  if (_mfMaterials.size() == _mats.size()) {
    FINFO((
        "MaterialPool::sync : %u materials already synced, nothing to do.\n", _mfMaterials.size()));
    return;
  }
...
void MaterialPool::add(const NodePtr& root) {
  traverse(root, osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, MaterialPool, NodePtr>(
                     this, &MaterialPool::addMaterialCB));
}
...
void MaterialPool::sub(const MaterialPtr& mat) {
  if (_mats.count(mat) == 0) {
    SWARNING << "MaterialPool(" << this << ")::sub: " << mat << " is not one of my materials!"
             << std::endl;
    return;
  }
...
void MaterialPool::sub(const NodePtr& root) {
  traverse(root, osgTypedMethodFunctor1ObjPtrCPtrRef<Action::ResultE, MaterialPool, NodePtr>(
                     this, &MaterialPool::subMaterialCB));
}
