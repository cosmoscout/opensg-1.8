...
Int32 TTVectorFontGlyph::processArc(FontGlyphContour* glyphContour, TT_Outline outline, Int32 start,
    Int32 contour, Real32** normalStack, Int32& numOnStack, Int32& counter, Int32 step,
    bool dryRun) {
  ...
  while (numOffPoints) {
    ...
    if (s == start && !(outline.flags[start] & 1) && !(outline.flags[start + step] & 1)) {
      endSeg[0] = ((outline.points[s].x + outline.points[s + step].x) / 2) * _ttScale;
      endSeg[1] = ((outline.points[s].y + outline.points[s + step].y) / 2) * _ttScale;
      doFirst   = true;
    } else if (s == start && !(outline.flags[start] & 1) && (outline.flags[start + step] & 1)) {
      endSeg[0] = (outline.points[start + step].x) * _ttScale;
      endSeg[1] = (outline.points[start + step].y) * _ttScale;
      doFirst   = true;
    } else if (doFirst) {
    ...
    if (numOffPoints == 1 && !doFirst) {
      if ((step > 0 && s + step2 <= end) || (step < 0 && s + step2 >= end)) {
      ...
      endSeg[0] = ((outline.points[s + step2].x + outline.points[s + step].x) / 2) * _ttScale;
      endSeg[1] = ((outline.points[s + step2].y + outline.points[s + step].y) / 2) * _ttScale;
    }
    ...
    a = osgMax(
            fabs(osgMax(osgMax(s1[0], s2[0]), endSeg[0]) - osgMin(osgMin(s1[0], s2[0]), endSeg[0])),
            fabs(osgMax(osgMax(s1[1], s2[1]), endSeg[1]) -
                 osgMin(osgMin(s1[1], s2[1]), endSeg[1]))) /
        _precision;
...
Int32 TTVectorFontGlyph::sortContours(TT_Outline outline) {
  ...
  for (i = 0; i < outline.n_contours; i++) {
    ...
    order += (static_cast<Real32>(outline.points[i ? outline.contours[i - 1] + 1 : 0].x) * last[1] -

              static_cast<Real32>(outline.points[i ? outline.contours[i - 1] + 1 : 0].y) * last[0]);
    ...
    while (left) {
      ...
      for (i = 0; i < (Int32)_contours.size(); i++) {
        ...
        if (fabs(cIter->getArea()) > area) {
          if (largestIsClockwise == -1 || (Int32)cIter->isClockwise() == largestIsClockwise) {
      ...
      for (i = 0; i < (Int32)_contours.size(); i++) {
        ...
        if (cIter == 0)
          continue;
        if (((Int32)cIter->isClockwise() == largestIsClockwise) && fabs(cIter->getArea()) < area) {
          doThis                                = contourList[_contours.size() - total];
          contourList[_contours.size() - total] = contourLeftList[i];
          contourLeftList[i]                    = doThis;
          area                                  = fabs(cIter->getArea());
...
void TTVectorFontGlyph::cleanup(void) {
  ...
  memcpy(&(_normalIndices[0]), &(_indices[0]), (_indices.size()) * sizeof(Int32));
...
bool TTVectorFontGlyph::create(void) {
  ...
  while (j < ttOutline.n_contours) {
    ...
    while (i <= ttOutline.contours[j]) {
      if (i < ttOutline.contours[j] && (ttOutline.flags[i] & 1) && !(ttOutline.flags[i + 1] & 1)) {
        numPoints += 2;
        i += processArc(glyphContour, ttOutline, i, j, normalStack, numOnStack, numPoints, 1, true);
  ...
  while (k < ttOutline.n_contours) {
    ...
    while (step > 0 && i <= ttOutline.contours[j] ||
      ...
      if ((step < 0 && i == glyphContour->getStart() && !(ttOutline.flags[i] & 1))) {
        point = _vertexBuffer.allocFloat(3);
        if (step < 0) {
          point[0] = (ttOutline.points[(j ? ttOutline.contours[j - 1] + 1 : 0)].x * _ttScale);
          point[1] = (ttOutline.points[(j ? ttOutline.contours[j - 1] + 1 : 0)].y * _ttScale);
        }
        ...
        glyphContour->addPoint(point, normal);
        pushIt(normalStack, numOnStack, point);
        i += processArc(glyphContour, ttOutline, i, j, normalStack, numOnStack, numPoints, step);
        dontAddFirst = true;
      } else if ((step > 0 && i < ttOutline.contours[j] && (ttOutline.flags[i] & 1) &&
        ...
        if ((step > 0 && i == ttOutline.contours[j] && (ttOutline.flags[i] & 1)) ||
            (step < 0 && i == (j ? ttOutline.contours[j - 1] + 1 : 0) &&
                (ttOutline.flags[i] & 1))) {
          point    = _vertexBuffer.allocFloat(3);
          point[0] = (ttOutline.points[i].x * _ttScale);
