...
/*! Constructor
 */
TileLoadBalancer::TileLoadBalancer(bool useFaceDistribution, bool cutBestDirection)
...
void TileLoadBalancer::update(NodePtr node) {
  ...
  for (TileGeometryLoadLstT::iterator gI = _tileGeometryLoad.begin(); gI != _tileGeometryLoad.end();
       ++gI) {
...
void TileLoadBalancer::balance(ViewportPtr vp, bool shrink, ResultT& result) {
  ...
  for (TileGeometryLoadLstT::iterator l = _tileGeometryLoad.begin(); l != _tileGeometryLoad.end();
       ++l) {
    // update view dependent values
    l->updateView(viewing, projection, rNear, width, height);
    // sort by min values
    if (l->isVisible()) {
  ...
  if (shrink) {
    ...
    if (wmax[0] < wmin[0] || wmax[1] < wmin[1])
  ...
  if (_renderNode.size() > 1) {
    splitRegion(0, _renderNode.size() - 1, visible, wmin, wmax, 0, result);
  } else {
    Region region;
    region.x1 = wmin[0];
    region.y1 = wmin[1];
...
void TileLoadBalancer::setRegionStatistics(ViewportPtr vp, ResultT& result) {
  ...
  vp->getCamera()->getViewing(viewing, width, height);
  vp->getCamera()->getProjection(projection, width, height);
  for (resultI = result.begin(); resultI != result.end(); ++resultI) {
    resultI->culledNodes = 0;
    resultI->faces       = 0;
    resultI->culledFaces = 0;
    resultI->pixel       = 0;
  }
  for (l = _tileGeometryLoad.begin(); l != _tileGeometryLoad.end(); ++l) {
    l->updateView(viewing, projection, rNear, width, height);
    for (resultI = result.begin(); resultI != result.end(); ++resultI) {
      if (!l->isVisible())
        resultI->culledNodes++;
        ...
        wmax[0]        = resultI->x2;
        wmax[1]        = resultI->y2;
        Real32 visible = l->getVisibleFraction(wmin, wmax, vismin, vismax) * l->getFaces();
        if (visible == 0)
          resultI->culledNodes++;
        else {
          resultI->faces += visible;
          resultI->culledFaces += l->getFaces() - visible;
          resultI->pixel += (vismax[0] - vismin[0] + 1) * (vismax[1] - vismin[1] + 1);
        }
      }
    }
  }
...
void TileLoadBalancer::updateSubtree(NodePtr& node, TileGeometryLoadMapT& loadMap) {
  ...
  if (node->getCore() != NullFC && GeometryPtr::dcast(node->getCore()) != NullFC) {
    // is this a new node
    TileGeometryLoadMapT::iterator mI = loadMap.find(node.getFieldContainerId());
    if (mI == loadMap.end()) {
      _tileGeometryLoad.push_back(
          TileGeometryLoad(node.getFieldContainerId(), _useFaceDistribution));
    } else {
      loadMap.erase(mI);
    }
  }
  // handle all childs
  for (MFNodePtr::iterator n = node->getMFChildren()->begin(); n != node->getMFChildren()->end();
       ++n) {
...
/** Splitupt region
 **/
void TileLoadBalancer::splitRegion(UInt32 rnFrom, UInt32 rnTo, RegionLoadVecT& visible,
    Int32 wmin[2], Int32 wmax[2], UInt32 depth, ResultT& result) {
  Int32                    axis, cut;
  Int32                    maxA[2];
  Int32                    minB[2];
  ...
  if (_cutBestDirection)
    ...
    axis = depth & 1;
  // search for best cut
  findBestCut(*renderNodeA, *renderNodeB, visible, wmin, wmax, axis, cut);
  // create new regions
  maxA[axis]     = cut;
  ...
  if (rnFrom != rnToA || rnFromB != rnTo) {
    ...
    for (vi = visible.begin(); vi != visible.end(); vi++) {
      ...
      else if (vi->getLoad()->getMin()[axis] > cut)
 ...
 **/
Real32 TileLoadBalancer::findBestCut(const RenderNode& renderNodeA, const RenderNode& renderNodeB,
    RegionLoadVecT& visible, Int32 wmin[2], Int32 wmax[2], Int32& bestAxis, Int32& bestCut) {
  RegionLoadVecT::iterator vi;
  Int32                    a, f, t, newCut;
  Int32                    minB[2];
  Int32                    maxA[2];
  ...
  for (a = checkAxisFrom; a <= checkAxisTo; ++a) {
    ...
    do {
      ...
      for (vi = visible.begin(); vi != visible.end(); vi++) {
        ...
        } else if (vi->getLoad()->getMin()[a] > newCut) {
