...
std::string SplitGraphOp::usage(void) {
  return "Split: Split large geometries into smaller ones\n"
...
bool SplitGraphOp::splitNode(NodePtr& node, std::vector<NodePtr>& split) {
  ...
  if (geo->getPositions() == NullFC || geo->getPositions()->size() == 0 ||
      geo->getLengths() == NullFC || geo->getLengths()->size() == 0 || geo->getTypes() == NullFC ||
      geo->getTypes()->size() == 0)
    return false;
  ...
  while (!it.isAtEnd()) {
    ...
    default:
      SWARNING << "SplitGraphOp::splitLeave: encountered "
               << "unknown primitive type " << it.getType() << ", ignoring!" << std::endl;
      break;
    }
  ...
  while (!it.isAtEnd()) {
    ...
    case GL_TRIANGLES: {
      ...
      while (it.getLength() - i >= 3) {
        ...
        if (types[geoIndex]->size() > 0 &&
            types[geoIndex]->getValue(types[geoIndex]->size() - 1) == GL_TRIANGLES) {
          int lind;
  ...
  while (!it.isAtEnd()) {
    ...
    case GL_QUADS: {
      ...
      while (it.getLength() - i >= 4) {
        geoIndex = order[ind] / _max_polygons;
        if (types[geoIndex]->size() > 0 &&
            types[geoIndex]->getValue(types[geoIndex]->size() - 1) == GL_QUADS) {
          int lind;
          if ((lind = lens[geoIndex]->size() - 1) >= 0)
