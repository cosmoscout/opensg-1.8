...
bool BINLoader::createFieldContainers(void) {
  UInt32 countTypes = 0, countCurrentType = 0, oldFCId = 0, numOfRoots = 0, currentId = 0, i, j;
  ...
  for (i = 0; i != countTypes; ++i) {
    ...
    for (j = 0; j != countCurrentType; j++) {
      ...
      if (newFCInfo.ptr == NullFC) {
        SFATAL << "Couldn't create unknown FieldContainer with type '" << fcTypeCName << "'!"
               << std::endl;
        return false;
      }
      _countContainers++;
      ...
      if (_fcInfoMap.insert(std::make_pair(oldFCId, newFCInfo)).second == false) {
        std::cerr << "ERROR in BINLoader::createFieldContainers()" << std::endl;
      }
    }
  }
...
void BINLoader::chargeFieldContainers(void) {
  ...
  while (count < mapSize) {
    ...
    SINFO << "loading container " << std::setw(4) << count << "/" << mapSize << "..." << std::endl;
    ...
    if (fcInfoIter == fcInfoEnd) {
      SWARNING << "ERROR in BINLoader::chargeFieldContainers():" << std::endl
               << "no matching container found for ID " << currentFieldContainerOldId << std::endl
    ...
    if (fcInfoIter->second.read) {
      SWARNING << "ERROR in BINLoader::chargeFieldContainers():" << std::endl
               << "original ID: " << currentFieldContainerOldId << std::endl
               << "new ID     : " << fcInfoIter->second.newId << std::endl
    ...
    beginEditCP(
        fcInfoIter->second.ptr, mask, ChangedOrigin::Abstract | ChangedOrigin::AbstrIncRefCount);
