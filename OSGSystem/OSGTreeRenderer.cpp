...
std::string TreeRenderer::_shadow_combine_vp = "varying vec2 texCoord;\n"
                                               "\n"
                                               "void main(void)\n"
                                               "{\n"
...
void TreeRenderer::initialize(Window* win) {
  if (!_initDone) {
    ...
    if (_useFBO) {
      ...
      FNOTICE(("framebuffer objects not supported, try new video drivers. Some shadow modes will "
               "be disabled.\n"));
    }
    ...
    if (!win->hasExtension("GL_ARB_shading_language_100") ||
        ...
        !win->hasExtension("GL_ARB_vertex_shader") || !win->hasExtension("GL_ARB_shader_objects")) {
      _useGLSL = false;
    }
    ...
    if (_useFBO) {
      ...
      glBindFramebufferEXT = (OSGGLBINDFRAMEBUFFEREXTPROC)win->getFunction(_funcBindFramebuffer);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glBindRenderbufferEXT = (OSGGLBINDRENDERBUFFEREXTPROC)win->getFunction(_funcBindRenderbuffer);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      ...
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glGenFramebuffersEXT = (OSGGLGENFRAMEBUFFERSEXTPROC)win->getFunction(_funcGenFramebuffers);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glGenRenderbuffersEXT = (OSGGLGENRENDERBUFFERSEXTPROC)win->getFunction(_funcGenRenderbuffers);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glGenerateMipmapEXT = (OSGGLGENERATEMIPMAPEXTPROC)win->getFunction(_funcGenerateMipmap);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glGetFramebufferAttachmentParameterivEXT =
          (OSGGLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC)win->getFunction(
              _funcGetFramebufferAttachmentParameteriv);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      ...
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glIsFramebufferEXT = (OSGGLISFRAMEBUFFEREXTPROC)win->getFunction(_funcIsFramebuffer);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glIsRenderbufferEXT = (OSGGLISRENDERBUFFEREXTPROC)win->getFunction(_funcIsRenderbuffer);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      ...
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      glDrawBuffersARB = (OSGGLDRAWBUFFERSARBPROC)win->getFunction(_funcDrawBuffers);
      if ((errCode = glGetError()) != GL_NO_ERROR)
        FBOerror = true;
      ...
      if (FBOerror) {
        FNOTICE(("Needed FBO functions could not be initialized (error code %d), FBOs disabled. "
                 "Try new video drivers!\n",
            errCode));
        _useFBO = false;
      }
