...
static const Char8* suffixArray[] = {"nrrd"};

NRRDImageFileType NRRDImageFileType::_the(
    "image/x-nrrd", suffixArray, sizeof(suffixArray), OSG_READ_SUPPORTED | OSG_WRITE_SUPPORTED);
...
static HeadStr headStrVec[] = {{"NRRD0001", MIME_HE}, {"type", TYPE_HE},
    {"dimension", DIMENSION_HE}, {"sizes", SIZES_HE}, {"endian", ENDIAN_HE},
    {"encoding", ENCODING_HE},

    {"unsigned", UNSIGNED_HE}, {"signed", SIGNED_HE},

    {"char", CHAR_HE}, {"short", SHORT_HE},
...
inline unsigned stringTokenizer(
...
bool NRRDImageFileType::read(ImagePtr& image, std::istream& in, const std::string& mimetype) {
  ...
  if (in.good()) {
    ...
    FINFO(("NRRDImageFileType::read: got %d D %s endian %s data.\n", dimension,
        littleEndian ? "little" : "big", rawEncoding ? "raw" : "cooked"));

    image->set(pixelFormat, width, height, depth, 1, 1, 0.0, 0, imageType);
...
*/
NRRDImageFileType::NRRDImageFileType(
