...
void FresnelMaterial::updateFresnel(void) {
  ...
  if (_img->getPixelFormat() != getImage()->getPixelFormat() ||
      _img->getWidth() != getImage()->getWidth() || _img->getHeight() != getImage()->getHeight()) {
    beginEditCP(_img);
...
void FresnelMaterial::changed(BitVector whichField, UInt32 origin) {
  ...
  if ((whichField & ImageFieldMask) || (whichField & IndexFieldMask) ||
      (whichField & ScaleFieldMask) || (whichField & BiasFieldMask)) {
    updateFresnel();
  }
...
void FresnelMaterial::dump(UInt32, const BitVector) const {
