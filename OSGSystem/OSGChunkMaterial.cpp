...
bool ChunkMaterial::subChunk(StateChunkPtr chunk, Int32 slot) {
  ...
  for (i = 0; i < _mfChunks.size(); ++i) {
    if (_mfChunks[i] == chunk &&
        ((i < _mfSlots.size() && _mfSlots[i] == slot) || slot == State::AutoSlotReplace)) {
      subRefCP(chunk);
  ...
  SWARNING << "ChunkMaterial::subChunk(" << this << ") has no chunk " << chunk << " with slot "
           << slot << std::endl;
...
StateChunkPtr ChunkMaterial::find(const FieldContainerType& type, Int32 slot) {
  UInt32 index = 0;
...
void ChunkMaterial::addChunks(StatePtr state) {
  ...
  for (i = 0; i < _mfChunks.size(); ++i)
    state->addChunk(_mfChunks[i], (i < _mfSlots.size()) ? _mfSlots[i] : State::AutoSlotReplace);
}
...
void ChunkMaterial::dump(
  ...
  for (i = 0; i < _mfChunks.size(); ++i)
    SLOG << _mfChunks[i] << " "
         << static_cast<Int32>((i < _mfSlots.size()) ? _mfSlots[i] : State::AutoSlotReplace)
         << std::endl;
}
