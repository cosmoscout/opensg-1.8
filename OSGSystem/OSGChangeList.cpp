...
void ChangeList::addChanged(const FieldContainerPtr& pFieldContainer, BitVector bvWhichField) {
  if (_bReadOnly == true)
    return;
  ...
  // std::bad_alloc doesn't work.
    ...
    SWARNING << "Compacted ChangeList from " << oldSize << " to " << _vChangedFieldContainers.size()
             << " entries." << std::endl;
    _vChangedFieldContainers.push_back(tmpEntry);
  }
}
...
bool ChangeList::merge(const ChangeList& clist) {
  ...
  _vChangedFieldContainers.insert(
      _vChangedFieldContainers.end(), clist.beginChanged(), clist.endChanged());

  _vAddRefdFieldContainers.insert(
      _vAddRefdFieldContainers.end(), clist.beginAddRefd(), clist.endAddRefd());

  _vSubRefdFieldContainers.insert(
      _vSubRefdFieldContainers.end(), clist.beginSubRefd(), clist.endSubRefd());

  _vCreatedFieldContainers.insert(
      _vCreatedFieldContainers.end(), clist.beginCreated(), clist.endCreated());

  _vDestroyedFieldContainers.insert(
      _vDestroyedFieldContainers.end(), clist.beginDestroyed(), clist.endDestroyed());

  return returnValue;
}

bool ChangeList::empty() const {
  return _vChangedFieldContainers.empty() && _vAddRefdFieldContainers.empty() &&
         _vSubRefdFieldContainers.empty() && _vCreatedFieldContainers.empty() &&
         _vDestroyedFieldContainers.empty();
}

void ChangeList::setAspect(UInt32 uiAspectId) {
  if (_vChangedFieldContainers.size() != 0 || _vAddRefdFieldContainers.size() != 0 ||
...
void ChangeList::applyTo(UInt32 uiAspectId) {
  ...
  for (i = 0; i < _vChangedFieldContainers.size(); i++) {
    pTmp = FieldContainerFactory::the()->getContainer(_vChangedFieldContainers[i].first);

    if (pTmp == NullFC)
      continue;

    pTmp.executeSync(_uiAspectId, uiAspectId, _vChangedFieldContainers[i].second);
  }

  for (i = 0; i < _vAddRefdFieldContainers.size(); i++) {
    pTmp = FieldContainerFactory::the()->getContainer(_vAddRefdFieldContainers[i]);
  ...
  for (i = 0; i < _vSubRefdFieldContainers.size(); i++) {
    pTmp = FieldContainerFactory::the()->getContainer(_vSubRefdFieldContainers[i]);
...
void ChangeList::apply(void) {
  ...
  for (i = 0; i < _vChangedFieldContainers.size(); i++) {
    ...
    pTmp.executeSync(_uiAspectId, Thread::getAspect(), _vChangedFieldContainers[i].second);
  }
...
void ChangeList::applySubRefs(void) {
  ...
  while (tmpList.size() != 0) {
    ...
    for (UInt32 i = 0; i < tmpList.size(); i++) {
      ...
      pTmp = FieldContainerFactory::the()->getContainer(tmpList[i]);
...
//                _vChangedFieldContainers[i].first.getFieldContainerId());
  ...
  for (i = 0; i < _vCreatedFieldContainers.size(); i++) {
    ...
    FieldContainerPtr pTmp =
        FieldContainerFactory::the()->getContainer(_vCreatedFieldContainers[i]);
