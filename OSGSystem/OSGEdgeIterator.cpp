...
char EdgeIterator::cvsid[] = "@(#)$Id: OSGEdgeIterator.cpp,v 1.1 2005/01/14 11:24:21 a-m-z Exp $";
...
void EdgeIterator::operator++() {
  ...
  // at end of primitive?
  if (_actPrimIndex >= PrimitiveIterator::getLength() || getType() == GL_LINE_STRIP ||
  ...
  default:
    SWARNING << "EdgeIterator::++: encountered "
...
void EdgeIterator::startPrim(void) {
  ...
  while (!isAtEnd()) {
    ...
    case GL_LINE_STRIP:
    case GL_LINE_LOOP:
      if (PrimitiveIterator::getLength() >= 2)
        return;
      break;
    ...
    case GL_QUAD_STRIP:
      break;
    default:
      SWARNING << "EdgeIterator::startPrim: encountered "
               << "unknown primitive type " << getType() << ", ignoring!" << std::endl;
      break;
    }
