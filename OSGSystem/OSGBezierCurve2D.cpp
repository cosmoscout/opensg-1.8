...
int BezierCurve2D::intersection(DCTPdvector& res, Vec2d a, Vec2d b) {
  ...
  //  DCTPdvector dists( control_points.size() ); // the signed distance of the control points and
  //  the polyline
  ...
  for (i = 0; i <= cpsize; i++) {
    //    std::cerr.precision( DCTP_PRECISION );
    //    std::cerr << "cp[i].x: " << control_points[ i ][0] << " cp[i].y: " << control_points[ i
    //    ][1] << std::endl;
...
//    return 0;
  ...
  */
  ...
  //        std::cerr << " res.size(): " << res.size() << " res[res.size()-1]: " <<
  res[res.size()-1] << std::endl; res.push_back( (s + e) / 2.0 ); // we have a new (unique)
  solution, record it
  //      }
        return 0;
      }
...
// calculate the signed distance between a homogenious point and a line (given with a point and a
// normalvector) returns the signed distance
double BezierCurve2D::pointLineDistancewNormal(Vec3d p, Vec2d a, Vec2d n) {
  ...
  //  t[0] = p[0] - a[0];
  //  t[1] = p[1] - a[1];
  //  std::cerr << " a.x: " << a[0] << " a.y: " << a[1] << " p.x: " << p[0] << " p.y: " << p[1] <<
  //  std::endl;
...
int BezierCurve2D::approximate_sub(DCTPVec2dvector& vertices, double delta) {
  ...
  for (i = 0; i <= n; i++) {
    ...
    ae    = ei - e0 * t1 - en * t2;
    //    ae[0] = control_points[ i ][0] - t1 * control_points[ 0 ][0] - t2 * control_points[ n
    //    ][0]; ae[1] = control_points[ i ][1] - t1 * control_points[ 0 ][1] - t2 * control_points[
    //    n ][1];
  ...
  //  std::cerr << control_points[ control_points.size() - 1 ][0] << " " << control_points[
  //  control_points.size() - 1 ][1] << std::endl;
  if (max_error > delta) {
    ...
    if (!vertices.size()) {
      ...
      tmp[1] = control_points[0][1] / control_points[0][2];
      vertices.push_back(tmp);
      tmp[0] = control_points[control_points.size() - 1][0] /
               control_points[control_points.size() - 1][2];
      tmp[1] = control_points[control_points.size() - 1][1] /
               control_points[control_points.size() - 1][2];
      vertices.push_back(tmp);
    } else {
...
bool BezierCurve2D::reduceDegree(double tol) {
  ...
  for (i = 1; i < n; ++i) {
    b_right[i] = (control_points[i] * n - b_right[i - 1] * i) * (1.0 / (double)(n - i));
  }

  // calculate b_left:
  b_left[n - 1] = control_points[n];
  for (i = n - 1; i > 0; --i) {
    b_left[i - 1] = (control_points[i] * n - b_left[i] * (n - i)) * (1.0 / (double)i);
  }
  ...
  if (n_half != n_half1) {
    ...
    dist4d = homogeniousDistanceSquared(
        control_points[n_half], 0.5 * (b_right[n_half - 1] + b_left[n_half]));
  }
...
double BezierCurve2D::CalculateSupinumSquared() const {
  ...
  sup =
      (control_points[0][0] * control_points[0][0] + control_points[0][1] * control_points[0][1]) /
  ...
  diff = (control_points[n - 1][0] * control_points[n - 1][0] +
             control_points[n - 1][1] * control_points[n - 1][1]) /
...
void BezierCurve2D::CalculateDifferenceCurve(
    const BezierCurve2D& Other, BezierCurve2D& Diff) const {
  BezierCurve2D temp_curve;
  unsigned int  i;
  bool          rational = false;
