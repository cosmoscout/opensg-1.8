...
const BitVector Node::VolumeFieldMask   = (TypeTraits<BitVector>::One << Node::VolumeFieldId);
const BitVector Node::TravMaskFieldMask = (TypeTraits<BitVector>::One << Node::TravMaskFieldId);
const BitVector Node::ParentFieldMask   = (TypeTraits<BitVector>::One << Node::ParentFieldId);
const BitVector Node::ChildrenFieldMask = (TypeTraits<BitVector>::One << Node::ChildrenFieldId);
const BitVector Node::CoreFieldMask     = (TypeTraits<BitVector>::One << Node::CoreFieldId);

FieldDescription* Node::_desc[] = {
    new FieldDescription(SFDynamicVolume::getClassType(), "volume",
        OSG_FC_FIELD_IDM_DESC(VolumeField), false, (FieldAccessMethod)&Node::getSFVolume),
    ...
    new FieldDescription(SFBool::getClassType(), "travMask", OSG_FC_FIELD_IDM_DESC(TravMaskField),
        false, (FieldAccessMethod)&Node::getSFTravMask),

    new FieldDescription(SFNodePtr::getClassType(), "parent", OSG_FC_FIELD_IDM_DESC(ParentField),
        true, (FieldAccessMethod)&Node::getSFParent),

    new FieldDescription(MFNodePtr::getClassType(), "children",
        OSG_FC_FIELD_IDM_DESC(ChildrenField), false, (FieldAccessMethod)&Node::getMFChildren),

    new FieldDescription(SFNodeCorePtr::getClassType(), "core", OSG_FC_FIELD_IDM_DESC(CoreField),
        false, (FieldAccessMethod)&Node::getSFCore)};

FieldContainerType Node::_type("Node", "AttachmentContainer", 0,
    (PrototypeCreateF)&Node::createEmpty, 0, _desc, sizeof(_desc));
...
bool Node::replaceChildBy(const NodePtr& childP, const NodePtr& newChildP) {
...
void Node::subChild(const NodePtr& childP) {
  ...
  if (childIt != _mfChildren.end()) {
    ...
    SWARNING << "Node(" << this << ")::subChild: " << childP << " is not one of my children!"
             << std::endl;
  }
...
void Node::updateVolume(void) {
  if (_sfVolume.getValue().getInstance().isValid() == true || getTravMask() == 0x0000) {
...
void Node::dump(UInt32 uiIndent, const BitVector bvFlags) const {
  UInt32 i;
  ...
  PLOG << "Node"
       << "(" << thisP.getFieldContainerId() << ") : " << _mfChildren.size() << " children | "
...
FieldContainerPtr OSG::deepClone(
    const FieldContainerPtr& src, const std::vector<std::string>& share) {
  if (src == NullFC)
    return NullFC;
    ...
    if (strcmp(fdesc->getCName(), "attachments") == 0) {
      ...
      for (; mapIt != mapEnd; ++mapIt) {
        ...
        for (UInt32 k = 0; k < share.size(); ++k) {
          FieldContainerType* fct = FieldContainerFactory::the()->findType(share[k].c_str());
          if (fc != NullFC && fct != NULL && fc->getType().isDerivedFrom(*fct)) {
            shareit = true;
            break;
          }
        }
...
FieldContainerPtr OSG::deepClone(
    const FieldContainerPtr& src, const std::vector<UInt16>& shareGroupIds) {
...
void OSG::deepCloneAttachments(const AttachmentContainerPtr& src, AttachmentContainerPtr dst,
