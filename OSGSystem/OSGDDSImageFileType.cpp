...
static const Char8* ddsSuffixArray[] = {"dds"};
...
class CSurface {
 ...
 public:
  ...
  inline Int32 get_depth() {
    return depth;
  }
  inline Int32 get_size() {
    return size;
  }
  inline char* get_pixels() {
    return pixels;
  }
...
class CDDSImage {
 public:
  ...
  bool load(
      std::istream& is, bool flipImage = true, bool swapCubeMap = true, bool flipCubeMap = false);
  void clear();

            operator char*();
  CTexture& operator[](Int32 index);

  inline Int32 get_num_images(void) {
  ...
  inline Int32 get_components() {
    return components;
  }
  inline Int32 get_format() {
    return format;
  }
...
bool DDSImageFileType::read(ImagePtr& image, std::istream& is, const std::string& mimetype) {
  ...
  if (ddsImage.load(is, _flipImage, _swapCubeMap, _flipCubeMap) &&
    ...
    SINFO << "cs: " << components << ", f: " << format << ", cd: " << isCompressed
          << ", cm: " << isCubeMap << ", vo: " << isVolume << endLog;
    ...
    for (i = 0; i < ddsImage.get_num_images(); ++i) {
      ...
      if (i) {
        if ((w != width) || (h != height) || (d != depth) && (mm != numMipMaps))
          validImage = false;
      } else {
        width      = w;
        height     = h;
      ...
      for (j = 0; j < mm; ++j) {
        ...
        dataSize += size;
        SINFO << "    " << j << ", " << w << "x" << h << "x" << d << ", size: " << size
              << ", mm: " << mm << endLog;
      }
    }
    if (validImage) {
      image->set(osg::Image::PixelFormat(format), width, height, depth, numMipMaps + 1, 1, 0.0, 0,
          osg::Image::OSG_UINT8_IMAGEDATA, true, ddsImage.get_num_images());
      ...
      if (dataSize == image->getSize()) {
        ...
        SWARNING << "ERROR: Invalid data size; cannot cp dds data" << endLog;
      }
    }
  } else
...
std::string DDSImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "DDS ", 4) == 0 ? std::string(getMimeType()) : std::string();
}
...
bool CDDSImage::load(std::istream& is, bool flipImage, bool swapCubeMap, bool flipCubeMap) {
  ...
  if (ddsh.ddspf.dwFlags & DDS_FOURCC) {
    ...
    case FOURCC_DXT5:
      ...
      break;
    default:
      SWARNING << "ERROR: unknown compressed format(" << ddsh.ddspf.dwFourCC << ")!" << endLog;
      return false;
    }
  ...
  for (Int32 n = 0; n < (cubemap ? 6 : 1); n++) {
    ...
    */
    ...
    if ((format == GL_COMPRESSED_RGB_S3TC_DXT1_EXT) && check_dxt1_alpha_data(img, img.size))
...
Int32 CDDSImage::size_dxtc(Int32 width, Int32 height) {
  Int32 comp(
      ((format == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) || (format == GL_COMPRESSED_RGB_S3TC_DXT1_EXT))
          ? 8
          : 16);
...
void CDDSImage::align_memory(CTexture* surface) {
  ...
  CTexture newSurface(surface->width, surface->height, surface->depth, imagesize);
      ...
      if (hasAlpha) {
        ...
        for (unsigned j = 0; j < 4; j++)
          FNOTICE(("  DXT Col Index: %d %d %d %d\n", ((colBlock[i].row[j] >> 0) & 3),
              ((colBlock[i].row[j] >> 2) & 3), ((colBlock[i].row[j] >> 4) & 3),
