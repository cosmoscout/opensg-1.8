...
static UInt32 parseUInt32(char*& ptr) {
  UInt32 result = (static_cast<UInt32>(static_cast<UInt8>(ptr[0])) << 0) |
...
static UInt16 parseUInt16(char*& ptr) {
  UInt16 result = (static_cast<UInt32>(static_cast<UInt8>(ptr[0])) << 0) |
                  (static_cast<UInt32>(static_cast<UInt8>(ptr[1])) << 8);
  ptr += 2;
  return result;
}
...
static bool readColorMap(std::istream& is, const BITMAPFILEHEADER& fileHeader,
    const BITMAPINFOHEADER& infoHeader, RGB colorMap[256]) {
  if (infoHeader.biBitCount <= 8) {
    ...
    int bytesPerColor =
        (infoHeader.biSize < 40) || (infoHeader.biSize + 14 + 4 * nCols > fileHeader.bfOffBits) ? 3
                                                                                                : 4;
    char buffer[256 * 4];
...
static bool parseUncompressedPaletteImage(std::istream& is, const BITMAPINFOHEADER& infoHeader,
    const RGB colorMap[256], ImagePtr& image) {
...
static bool parseRLEImage(std::istream& is, const BITMAPINFOHEADER& infoHeader,
    const RGB colorMap[256], ImagePtr& image) {
  unsigned int height;
  long         bytesPerLine;
  UChar8*      data;
...
static bool parseTrueColorImage(
    std::istream& is, const BITMAPINFOHEADER& infoHeader, ImagePtr& image) {
...
static void bmp_header1_write(std::ostream& os, unsigned short int filetype,
    unsigned long int filesize, unsigned short int reserved1, unsigned short int reserved2,
...
static void bmp_header2_write(std::ostream& os, unsigned long int size, unsigned long int width,
    long int height, unsigned short int planes, unsigned short int bitsperpixel,
    unsigned long int compression, unsigned long int sizeofbitmap, unsigned long int horzresolution,
    unsigned long int vertresolution, unsigned long int colorsused,
...
static void bmp_24_data_write(
    std::ostream& os, unsigned long int width, long int height, unsigned char* data) {
  int            i;
...
static const Char8* bmpSuffixArray[] = {"bmp", "dib", "rle"};

BMPImageFileType BMPImageFileType::_the(
    "image/bmp", bmpSuffixArray, sizeof(bmpSuffixArray), OSG_READ_SUPPORTED | OSG_WRITE_SUPPORTED);
...
*/
bool BMPImageFileType::read(ImagePtr& image, std::istream& is, const std::string& mimetype) {
...
bool BMPImageFileType::write(const ImagePtr& image, std::ostream& os, const std::string& mimetype) {
  ...
  bmp_header1_write(os, filetype, filesize, reserved1, reserved2, bitmapoffset);
  ...
  bmp_header2_write(os, size, width, height, planes, bitsperpixel, compression, sizeofbitmap,
      horzresolution, vertresolution, colorsused, colorsimportant);
...
std::string BMPImageFileType::determineMimetypeFromStream(std::istream& is) {
  ...
  return strncmp(filecode, "BM", 2) == 0 ? std::string(getMimeType()) : std::string();
}
...
*/
BMPImageFileType::BMPImageFileType(
