...
STLSceneFileType STLSceneFileType::_the(
...
//
//------------------------------
bool STLSceneFileType::readASCII(
    std::istream& is, STLFaceList& theFaces, std::string& theName) const {
  char   token[9];
  int    finished    = 0;
  int    parseResult = 0;
  UInt32 i;
...
//
//------------------------------
bool STLSceneFileType::readBinary(
    std::istream& is, STLFaceList& theFaces, std::string& theName) const {
  if (!is)
    return false;
...
//
//------------------------------
STLSceneFileType::STLSceneFileType(const Char8* suffixArray[], UInt16 suffixByteCount,
