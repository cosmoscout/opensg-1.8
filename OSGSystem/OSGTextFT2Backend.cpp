...
class TextFT2TXFGlyph : public TextTXFGlyph {
 public:
  // Constructor
  TextFT2TXFGlyph(Index glyphIndex, TextFT2TXFFace* face, Real32 scale, FT_GlyphSlot glyphSlot);
...
// Author: pdaehne
//----------------------------------------------------------------------
bool TextFT2Backend::findPath(
    const string& family, TextFace::Style style, string& path, int& index) {
...
// Author: pdaehne
//----------------------------------------------------------------------
TextVectorFace* TextFT2Backend::createVectorFace(const string& family, TextFace::Style style) {
...
// Author: pdaehne
//----------------------------------------------------------------------
TextPixmapFace* TextFT2Backend::createPixmapFace(
...
// Author: pdaehne
//----------------------------------------------------------------------
TextTXFFace* TextFT2Backend::createTXFFace(
...
#endif
      string path =
          end == string::npos ? _pathList.substr(start) : _pathList.substr(start, end - start);
...
void TextFT2Backend::checkFile(const string& fullname) {
  ...
  while (index < numFaces) {
    ...
    if (face->style_flags & FT_STYLE_FLAG_BOLD)
      fontInfo.style = face->style_flags & FT_STYLE_FLAG_ITALIC ? TextFace::STYLE_BOLDITALIC
                                                                : TextFace::STYLE_BOLD;
    else
      fontInfo.style =
          face->style_flags & FT_STYLE_FLAG_ITALIC ? TextFace::STYLE_ITALIC : TextFace::STYLE_PLAIN;
...
static void getFaceInfo(FT_Face face, string& family, TextFace::Style& style) {
  ...
  // Determine the style
  if (face->style_flags & FT_STYLE_FLAG_BOLD)
    style = face->style_flags & FT_STYLE_FLAG_ITALIC ? TextFace::STYLE_BOLDITALIC
                                                     : TextFace::STYLE_BOLD;
  else
    style =
        face->style_flags & FT_STYLE_FLAG_ITALIC ? TextFace::STYLE_ITALIC : TextFace::STYLE_PLAIN;
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (param.horizontal == true) {
      if (param.leftToRight == true) {
        FT_Vector kerning;
        FT_Get_Kerning(_face, previousGlyphIndex, glyphIndex,
            ft_kerning_unscaled /*FT_KERNING_UNSCALED*/, &kerning);
      ...
      {
        FT_Vector kerning;
        FT_Get_Kerning(_face, glyphIndex, previousGlyphIndex,
            ft_kerning_unscaled /*FT_KERNING_UNSCALED*/, &kerning);
...
TextFT2VectorGlyph::TextFT2VectorGlyph(Index glyphIndex, float scale, FT_GlyphSlot glyphSlot)
  ...
  FT_Outline_Funcs func_interface = {moveToFunc, lineToFunc, conicToFunc, cubicToFunc, 0, 0};
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (param.horizontal == true) {
      if (param.leftToRight == true) {
        FT_Vector kerning;
        FT_Get_Kerning(_face, previousGlyphIndex, glyphIndex,
            ft_kerning_default /*FT_KERNING_DEFAULT*/, &kerning);
...
TextFT2PixmapGlyph::TextFT2PixmapGlyph(
    Index glyphIndex, TextFT2PixmapFace* face, FT_GlyphSlot glyphSlot)
    : TextPixmapGlyph() {
  _glyphIndex = glyphIndex;
...
TextFT2TXFFace::TextFT2TXFFace(FT_Face face, const TextTXFParam& param) {
  ...
  for (it = param.getCharacters().begin(); it != param.getCharacters().end(); ++it) {
    ...
    _glyphMap.insert(
        GlyphMap::value_type(*it, new TextFT2TXFGlyph(*it, this, _scale, face->glyph)));
  }
...
TextFT2TXFGlyph::TextFT2TXFGlyph(
    Index glyphIndex, TextFT2TXFFace* face, Real32 scale, FT_GlyphSlot glyphSlot)
    : TextTXFGlyph() {
  _glyphIndex = glyphIndex;
  _scale      = scale;
  _width      = glyphSlot->bitmap.width;
