...
void TextVectorFace::fillGeo(GeometryPtr& geoPtr, const TextLayoutResult& layoutResult,
    Real32 scale, Real32 depth, UInt32 level, Real32 creaseAngle) {
  beginEditCP(geoPtr);
  ...
  for (i = 0; i < numGlyphs; ++i) {
    ...
    if (depth > 0.f) {
      ...
      vector<TextVectorGlyph::Orientation>::const_iterator oriIt =
          glyph.getContourOrientations().begin();
...
// Author: pdaehne
//----------------------------------------------------------------------
NodePtr TextVectorFace::makeNode(const TextLayoutResult& layoutResult, Real32 scale, Real32 depth,
    UInt32 level, Real32 creaseAngle) {
