...
double QuadTreeCreator::computeApproximationError(DCTPFace* f) {
...
int QuadTreeCreator::setInitialLeaves(const beziersurfacematrix& patches,
    const DCTPdvector& intervals_u, const DCTPdvector& intervals_v) {
...
int QuadTreeCreator::finishSubdivisions(DCTPFace* f) {
  ...
  (qtm->faces[idx])->faceinfo                            = new BezierTensorSurface;
  *((BezierTensorSurface*)(qtm->faces[idx++])->faceinfo) = created_surfaces[2];
  ...
  /*		std::cerr << std::endl;
                  ...
                  std::cerr << created_surfaces[ 1 ].getControlPointMatrix( )[ 0 ][ 0 ] <<
     std::endl;*/

  (qtm->faces[idx])->faceinfo                            = new BezierTensorSurface;
  *((BezierTensorSurface*)(qtm->faces[idx++])->faceinfo) = created_surfaces[1];
...
int QuadTreeCreator::createQuadTree(void) {
  ...
  for (dctpfacevector::size_type f = 0; f < qtm->faces.size(); ++f) {
...
double QuadTreeCreator::computeBilinearNorm(DCTPFace* face) {
  BezierTensorSurface*       btsp = (BezierTensorSurface*)face->faceinfo;
