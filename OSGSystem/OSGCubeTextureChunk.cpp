...
void CubeTextureChunk::onCreate(const CubeTextureChunk*) {
  ...
  setGLId(Window::registerGLObject(
      osgTypedMethodVoidFunctor2ObjCPtrPtr<CubeTextureChunkPtr, Window, UInt32>(
          tmpPtr, &CubeTextureChunk::handleGL),
      1));
...
void CubeTextureChunk::dump(UInt32, const BitVector) const {
...
void CubeTextureChunk::handleGL(Window* win, UInt32 idstatus) {
  ...
  if (mode == Window::destroy) {
    ...
    handleTexture(win, id, GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_CUBE_MAP_ARB,
        GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB, mode, getImage());
    ...
    SWARNING << "Win:" << win << "TextureChunk(" << this << "::handleGL: Illegal mode: " << mode
             << " for id " << id << std::endl;
  }
}
...
void CubeTextureChunk::activate(DrawActionBase* action, UInt32 idx) {
  ...
  if (getImage() != NullFC) {
    action->getStatistics()
        ...
        ->add(getGLId(), getImage()->getSize() * 6);
  }
...
void CubeTextureChunk::changeFrom(DrawActionBase* action, StateChunk* old, UInt32 idx) {
  // does the window support cubemaps?
  if (action->getWindow()->hasExtension(_arbCubeTex) == false)
    return;
...
#ifdef GL_NV_point_sprite
  if (idx < ntexcoords) {
    if (oldp->getPointSprite() != getPointSprite() && win->hasExtension(_nvPointSprite)) {
  ...
  if (idx < nteximages) {
    if (oldp->getLodBias() != getLodBias() && win->hasExtension(_extTextureLodBias)) {
      glTexEnvf(GL_TEXTURE_FILTER_CONTROL_EXT, GL_TEXTURE_LOD_BIAS_EXT, getLodBias());
    }
  }
  ...
  if (idx < ntexunits) {
    ...
    if (win->hasExtension(_nvTextureShader)) {
      if (getShaderOperation() != GL_NONE && oldp->getShaderOperation() == GL_NONE) {
        handleTextureShader(win, GL_TEXTURE_CUBE_MAP_ARB);
        if (idx == 0)
          glEnable(GL_TEXTURE_SHADER_NV);
      } else if (getShaderOperation() == GL_NONE && oldp->getShaderOperation() != GL_NONE) {
...
bool CubeTextureChunk::operator==(const StateChunk& other) const {
  CubeTextureChunk const* tother = dynamic_cast<CubeTextureChunk const*>(&other);
  ...
  return TextureChunk::operator==(other) && getPosZImage() == tother->getPosZImage() &&
         getPosYImage() == tother->getPosYImage() && getNegYImage() == tother->getNegYImage() &&
         getPosXImage() == tother->getPosXImage() && getNegXImage() == tother->getNegXImage();
}
