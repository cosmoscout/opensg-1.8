...
Real32 Trackball::projectToSphere(Real32 rRadius, Real32 rX, Real32 rY) {
  ...
  if (d < rRadius * 0.70710678118654752440f) { /* Inside sphere */
...
void Trackball::updateRotation(Real32 rLastX, Real32 rLastY, Real32 rCurrentX, Real32 rCurrentY) {
  Quaternion qCurrVal;
  ...
  if ((osgabs(rLastX - rCurrentX) > Eps) || (osgabs(rLastY - rCurrentY) > Eps)) {
    /*
     * First, figure out z-coordinates for projection of P1 and P2 to
     * deformed sphere
     */

    gP1.setValues(rLastX, rLastY, projectToSphere(_rTrackballSize, rLastX, rLastY));

    gP2.setValues(rCurrentX, rCurrentY, projectToSphere(_rTrackballSize, rCurrentX, rCurrentY));
...
void Trackball::updatePosition(Real32 rLastX, Real32 rLastY, Real32 rCurrentX, Real32 rCurrentY) {
  if (_gTransMode == OSGFree) {
    if (_gTransGen == OSGAbsoluteTranslation) {
      ...
      Vec3f tmpVec((rLastX - rCurrentX) * -_rTranslationScale,
          (rLastY - rCurrentY) * -_rTranslationScale, 0.);
...
void Trackball::updatePositionNeg(
    Real32 /*rLastX*/, Real32 rLastY, Real32 /*rCurrentX*/, Real32 rCurrentY) {
  if (_gTransMode == OSGFree) {
    if (_gTransGen == OSGAbsoluteTranslation) {
      _pVal[2] += (rLastY - rCurrentY) * -_rTranslationScale;
    } else {
      Vec3f tmpVec(0., 0., (rLastY - rCurrentY) * -_rTranslationScale);
...
void Trackball::setTranslationGen(TranslationGen gMode) {
  if (_gTransGen == OSGAbsoluteTranslation && gMode == OSGOrientedTranslation) {
...
void Trackball::setStartPosition(Real32 rX, Real32 rY, Real32 rZ, bool bUpdate) {
...
void Trackball::setStartRotation(Real32 rX, Real32 rY, Real32 rZ, Real32 rW, bool bUpdate) {
