...
struct ValueHandler {
  virtual Real64 getValue(const UInt8* data, UInt32 channels, UInt32 width, UInt32 height, UInt32 c,
      UInt32 x1, UInt32 y1, UInt32 z1, UInt32 x2, UInt32 y2, UInt32 z2, Real64* weightX,
      Real64* weightY, Real64* weightZ)         = 0;
  virtual void   setValue(Real64 value, UInt8* data, UInt32 channels, UInt32 width, UInt32 height,
...
struct ValueHandlerTempl : public ValueHandler {
  virtual Real64 getValue(const UInt8* data, UInt32 channels, UInt32 width, UInt32 height, UInt32 c,
      UInt32 x1, UInt32 y1, UInt32 z1, UInt32 x2, UInt32 y2, UInt32 z2, Real64* weightX,
      Real64* weightY, Real64* weightZ) {
    Real64 result = 0;
    UInt32 x, y, z;

    for (z = 0; z <= (z2 - z1); ++z) {
      for (y = 0; y <= (y2 - y1); ++y) {
        ValueT* valueP = (ValueT*)(data) + (z1 + z) * (height * width * channels) +
                         (y1 + y) * (width * channels) + (x1) * (channels) + c;
        for (x = 0; x <= (x2 - x1); ++x) {
          result += (Real64) * (valueP) * (weightX[x] * weightY[y] * weightZ[z]);
          valueP += channels;
        }
      }
    }
  ...
  virtual void setValue(Real64 value, UInt8* data, UInt32 channels, UInt32 width, UInt32 height,
    ...
    valueP = valueP + z * (width * height * channels) + y * (width * channels) + x * (channels) + c;
...
//----------------------------------------------------------------------------
void ImageScaler::calcContributions(UInt32 axis, Int32 uResSize, Int32 uSrcSize,
    const ImageGenericFilter& filter, std::vector<Contribution> contrib[3]) {
  Real64 dWidth;
  Real64 dFScale      = 1.0;
  ...
  for (Int32 u = 0; u < uResSize; u++) {
    ...
    for (Int32 iSrc = iLeft; iSrc <= iRight; iSrc++) {
      // calculate weight
      Real64 weight = dFScale * filter.filter(dFScale * (dCenter - (Real64)iSrc));
...
/*! rescale image */
//----------------------------------------------------------------------------
bool ImageScaler::scale(ImagePtr& srcImg, ImagePtr& dstImg, Int32 width, Int32 height, Int32 depth,
  ...
  if (!dstImg->set((Image::PixelFormat)srcImg->getPixelFormat(), width, height, depth,
          // create mipmaps from rescaled image
          1, srcImg->getFrameCount(), srcImg->getFrameDelay(), NULL, srcImg->getDataType(), true,
          srcImg->getSideCount())) {
        ...
        for (y = 0; y < height; ++y) {
          ...
          for (x = 0; x < width; ++x) {
            ...
            for (c = 0; c < channels; ++c) {
              value = valueHandler->getValue(srcImgData, channels, srcImgWidth, srcImgHeight, c, x1,
                  y1, z1, x2, y2, z2, weightX, weightY, weightZ);

              valueHandler->setValue(value, dstImgData, channels, width, height, c, x, y, z);
            }
          }
        }
      }
