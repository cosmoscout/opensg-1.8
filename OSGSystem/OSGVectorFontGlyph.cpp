...
void VectorFontGlyph::addPoint(Real32* point, bool OSG_CHECK_ARG(lower)) {
  ...
  end   = (begin + (_BLOCK_ALLOC / 3) >= (Int32)_points.size() ? _points.size() - 1
                                                             : begin + (_BLOCK_ALLOC / 3) - 1);

  if (tmp < 0 || (pointIndex = findPoint(point, begin, end)) < 0) {
    FWARNING(("failed finding point (%f,%f,%f) within point-list of %d points.", point[0], point[1],
        point[2], _numPoints));
...
void VectorFontGlyph::extrude(void) {
  ...
  memcpy(&(_indices[0]) + _numIndices, &(_indices[0]), _numIndices * sizeof(Int32));
...
bool VectorFontGlyph::createTriangles(void) {
  ...
  gluTessCallback(triangulator, (GLenum)GLU_BEGIN, reinterpret_cast<OSGGLUfuncptr>(tessBegin));
  gluTessCallback(triangulator, (GLenum)GLU_VERTEX, reinterpret_cast<OSGGLUfuncptr>(tessVertex));
  gluTessCallback(
      triangulator, (GLenum)GLU_EDGE_FLAG, reinterpret_cast<OSGGLUfuncptr>(tessEdgeFlag));
  gluTessCallback(triangulator, (GLenum)GLU_END, reinterpret_cast<OSGGLUfuncptr>(tessEnd));
  ...
  for (cIter = _contours.begin(); cIter != _contours.end(); cIter++) {
    ...
    memcpy(
        &(_points[0]) + last, &((*cIter)->getPoints()[0]), (totalPoints - last) * sizeof(Real32*));
