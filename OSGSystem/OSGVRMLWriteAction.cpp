...
Char8 VRMLWriteAction::FCInfo::mapChar(Char8 c) {
  ...
  static char badchars[] = {0x22, 0x23, 0x27, 0x2c, 0x2e, 0x5b, 0x5c, 0x5d, 0x7b, 0x7d, 0x7f};
...
void VRMLWriteAction::FCInfo::convertName(Char8*& szName) {
  ...
  if ((szName[0] >= 0x30 && szName[0] <= 0x39) || szName[0] == 0x2b || szName[0] == 0x2d) {
...
void VRMLWriteAction::FCInfo::buildName(const Char8* szTypename, UInt32 uiContainerId) {
  if (_szName != NULL)
    return;
...
void VRMLWriteAction::registerEnterDefault(
...
void VRMLWriteAction::registerLeaveDefault(
    const FieldContainerType& type, const Action::Functor& func) {
...
Action::ResultE VRMLWriteAction::writeGroupEnter(CNodePtr& pGroup, Action* pAction) {
  ...
  if (pWriter->getMode() == VRMLWriteAction::OSGCollectFC) {
    ...
    if (pCoreInfo->getUse() > 0 && pCoreInfo->getWritten() == true) {
      pWriter->printIndent();
      fprintf(pFile, "Group # osg shared %s\n", pCoreInfo->getName());

      //            pWriter->setCurrentUse(true);
    } else if ((pCoreInfo->getName() != NULL) && (pCoreInfo->getName()[0] != '\0')) {
      pWriter->printIndent();
      fprintf(pFile, "DEF %s Group\n", pCoreInfo->getName());

      pCoreInfo->setWritten();
    } else if ((pInfo->getName() != NULL) && (pInfo->getName()[0] != '\0')) {
      pWriter->printIndent();
      fprintf(pFile, "DEF %s Group\n", pInfo->getName());
...
Action::ResultE VRMLWriteAction::writeGroupLeave(CNodePtr& pGroup, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
Action::ResultE VRMLWriteAction::writeComponentTransformEnter(CNodePtr& pGroup, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);

  ComponentTransformPtr pTrans = ComponentTransformPtr::dcast(pGroup.getNode()->getCore());
  ...
  if (pWriter->getMode() == VRMLWriteAction::OSGCollectFC) {
    ...
    if ((pInfo->getName() != NULL) && (pInfo->getWritten() == false) &&
        (pInfo->getName()[0] != '\0')) {
      pWriter->printIndent();
    ...
    pWriter->printIndent();
    fprintf(pFile, "center %f %f %f\n", pTrans->getCenter()[0], pTrans->getCenter()[1],
        pTrans->getCenter()[2]);
    ...
    pWriter->printIndent();
    fprintf(pFile, "scale %f %f %f\n", pTrans->getScale()[0], pTrans->getScale()[1],
        pTrans->getScale()[2]);
    ...
    fprintf(pFile, "translation %f %f %f\n", pTrans->getTranslation()[0],
        pTrans->getTranslation()[1], pTrans->getTranslation()[2]);
...
Action::ResultE VRMLWriteAction::writeComponentTransformLeave(CNodePtr&, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
Action::ResultE VRMLWriteAction::writeTransformEnter(CNodePtr& pGroup, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
Action::ResultE VRMLWriteAction::writeTransformLeave(CNodePtr&, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
void VRMLWriteAction::writePoints(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
...
void VRMLWriteAction::writeNormals(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
...
void VRMLWriteAction::writeColors(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
...
void VRMLWriteAction::writeTexCoords(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
...
void VRMLWriteAction::writeIndex(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
  ...
  if ((pIndex == NullFC) || (pTypes == NullFC) || (pLength == NullFC)) {
    return;
  }

  if (pIndex->size() == 0 || pTypes->size() == 0 || pLength->size() == 0) {
    return;
  }
...
void VRMLWriteAction::writeLineIndex(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
  ...
  if ((pTypes == NullFC) || (pLength == NullFC)) {
    return;
  }

  if (pTypes->size() == 0 || pLength->size() == 0) {
    return;
  }
  ...
  for (it = pGeo->beginPrimitives(); it != pGeo->endPrimitives(); ++it) {
    if (it.getType() == GL_LINES) {
      for (i = 0; i < it.getLength(); i += 2) {
        ...
        fprintf(pFile, "%d, %d, -1,\n", it.getPositionIndex(i), it.getPositionIndex(i + 1));
      }
    ...
    for (it = pGeo->beginPrimitives(); it != pGeo->endPrimitives(); ++it) {
      if (it.getType() == GL_LINES) {
        for (i = 0; i < it.getLength(); i += 2) {
          ...
          fprintf(pFile, "%d, %d, -1,\n", it.getColorIndex(i), it.getColorIndex(i + 1));
        }
      } else if (it.getType() == GL_LINE_STRIP) {
        pWriter->printIndent();
...
void VRMLWriteAction::writeMaterial(GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
  if (pGeo == NullFC)
    return;
  ...
  StateChunkPtr sChunk = st->getChunk(MaterialChunk::getStaticClassId());
  ...
  pWriter->printIndent();
  fprintf(pFile, "diffuseColor %f %f %f\n", mChunk->getDiffuse()[0], mChunk->getDiffuse()[1],
      mChunk->getDiffuse()[2]);
  ...
  if (!mChunk->getLit()) {
    ...
    fprintf(pFile, "emissiveColor %f %f %f\n", mChunk->getEmission()[0], mChunk->getEmission()[1],
  ...
  pWriter->printIndent();
  fprintf(pFile, "specularColor %f %f %f\n", mChunk->getSpecular()[0], mChunk->getSpecular()[1],
  ...
  if (pTChunk != NullFC) {
    if (pWriter->isWritten(pTChunk)) {
      ...
      if (pImage != NullFC) {
        if (pWriter->getOptions() & VRMLWriteAction::OSGPixelTextures) {
          pWriter->printIndent();
          ...
          pWriter->printIndent();
          fprintf(pFile, "image %d %d %u    ", pImage->getWidth(), pImage->getHeight(), pixelsize);
          ...
          const std::string* pFilename = pImage->findAttachmentField("fileName");
          std::string        filename;
          if (pFilename == NULL)
...
bool VRMLWriteAction::writeGeoCommon(NodePtr pNode, GeometryPtr pGeo, FILE* pFile,
    VRMLWriteAction* pWriter, const Char8* setTypename) {
  ...
  if (pCoreInfo->getUse() > 0 && pCoreInfo->getWritten() == true) {
    ...
    if ((pCoreInfo->getName() != NULL) && (pCoreInfo->getName()[0] != '\0')) {
      pWriter->printIndent();
      fprintf(pFile, "geometry DEF %s %s\n", pCoreInfo->getName(), setTypename);

      pCoreInfo->setWritten();
    } else if ((pInfo->getName() != NULL) && (pInfo->getName()[0] != '\0')) {
      pWriter->printIndent();
      fprintf(pFile, "geometry DEF %s %s\n", pInfo->getName(), setTypename);
...
void VRMLWriteAction::writePointSet(
    NodePtr pNode, GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter) {
...
void VRMLWriteAction::writeLineSet(
    NodePtr pNode, GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter, bool) {
...
void VRMLWriteAction::writeFaceSet(
    NodePtr pNode, GeometryPtr pGeo, FILE* pFile, VRMLWriteAction* pWriter, bool) {
...
Action::ResultE VRMLWriteAction::writeGeoEnter(CNodePtr& pGroup, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
  ...
  if (pWriter->getMode() == VRMLWriteAction::OSGCollectFC) {
    ...
    while (pIt != pEnd) {
      if (pIt.getType() == GL_LINES || pIt.getType() == GL_LINE_STRIP ||
    ...
    FINFO(("Geo Stat : %d %d %d\n", uiPointCount, uiLineCount, uiFaceCount));
    ...
    if (uiLineCount != 0) {
      writeLineSet(pNode, pGeo, pFile, pWriter, ((uiPointCount == 0) && (uiFaceCount == 0)));
    }

    if (uiFaceCount != 0) {
      writeFaceSet(pNode, pGeo, pFile, pWriter, ((uiPointCount == 0) && (uiLineCount == 0)));
    }
  }
...
Action::ResultE VRMLWriteAction::writeGeoLeave(CNodePtr&, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
Action::ResultE VRMLWriteAction::writeMatGroupEnter(CNodePtr& pGroup, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);

  MaterialGroupPtr pMatGroup = MaterialGroupPtr::dcast(pGroup.getNode()->getCore());
...
Action::ResultE VRMLWriteAction::writeMatGroupLeave(CNodePtr&, Action* pAction) {
  VRMLWriteAction* pWriter = dynamic_cast<VRMLWriteAction*>(pAction);
...
bool VRMLWriteAction::initializeAction(void) {
  ...
  VRMLWriteAction::registerEnterDefault(
      Group::getClassType(), osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
                                 VRMLWriteAction::writeGroupEnter));

  VRMLWriteAction::registerEnterDefault(ComponentTransform::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          VRMLWriteAction::writeComponentTransformEnter));

  VRMLWriteAction::registerEnterDefault(Transform::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          VRMLWriteAction::writeTransformEnter));

  VRMLWriteAction::registerEnterDefault(
      Geometry::getClassType(), osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
                                    VRMLWriteAction::writeGeoEnter));

  VRMLWriteAction::registerEnterDefault(MaterialGroup::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          VRMLWriteAction::writeMatGroupEnter));

  VRMLWriteAction::registerLeaveDefault(
      Group::getClassType(), osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
                                 &VRMLWriteAction::writeGroupLeave));

  VRMLWriteAction::registerLeaveDefault(ComponentTransform::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          &VRMLWriteAction::writeComponentTransformLeave));

  VRMLWriteAction::registerLeaveDefault(Transform::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          &VRMLWriteAction::writeTransformLeave));

  VRMLWriteAction::registerLeaveDefault(
      Geometry::getClassType(), osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
                                    &VRMLWriteAction::writeGeoLeave));

  VRMLWriteAction::registerLeaveDefault(MaterialGroup::getClassType(),
      osgTypedFunctionFunctor2CPtrRef<Action::ResultE, CNodePtr, Action*>(
          &VRMLWriteAction::writeMatGroupLeave));
...
void VRMLWriteAction::addNodeUse(CNodePtr& pCNode) {
  ...
  NamePtr pNodename = NamePtr::dcast(pNode->findAttachment(Name::getClassType().getGroupId()));

  NamePtr pCorename = NamePtr::dcast(pCore->findAttachment(Name::getClassType().getGroupId()));
  ...
  if (pInfoCore->getUse() > 1) {
    if (pCorename != NullFC) {
      pInfoCore->buildName(pCore->getTypeName(), pCore.getFieldContainerId());
    }
  }
...
void VRMLWriteAction::addContainerUse(FieldContainerPtr& pContainer) {
  ...
  if (pInfo->getUse() > 1) {
    pInfo->buildName(pContainer->getTypeName(), pContainer.getFieldContainerId());
  }
}

VRMLWriteAction::FCInfo* VRMLWriteAction::getInfo(FieldContainerPtr& pContainer) {
  if (pContainer == NullFC)
    return NULL;
...
Action::ResultE VRMLWriteAction::write(NodePtr node) {
  ...
  std::for_each(_vFCInfos.begin(), _vFCInfos.end(), std::mem_fun_ref(&FCInfo::clear));

  _vFCInfos.resize(FieldContainerFactory::the()->getFieldContainerStore()->size());
...
bool VRMLWriteAction::operator==(const VRMLWriteAction& OSG_CHECK_ARG(other)) const {
  return false;
}
...
std::vector<VRMLWriteAction::Functor>* VRMLWriteAction::getDefaultEnterFunctors(void) {
  return _defaultEnterFunctors;
}

std::vector<VRMLWriteAction::Functor>* VRMLWriteAction::getDefaultLeaveFunctors(void) {
  return _defaultLeaveFunctors;
}

Action::ResultE VRMLWriteAction::apply(
