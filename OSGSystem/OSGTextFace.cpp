...
// Author: pdaehne
//----------------------------------------------------------------------
void TextFace::layout(
    const string& utf8Text, const TextLayoutParam& param, TextLayoutResult& result) {
...
//----------------------------------------------------------------------
void TextFace::layout(
    const vector<string>& lines, const TextLayoutParam& param, TextLayoutResult& result) {
  ...
  for (lineIt = lines.begin(), unicodeLineIt = unicodeLines.begin(); lineIt != lines.end();
       ++lineIt, ++unicodeLineIt)
...
//----------------------------------------------------------------------
void TextFace::layout(
    const vector<wstring>& lines, const TextLayoutParam& param, TextLayoutResult& result) {
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextFace::calculateBoundingBox(
    const TextLayoutResult& layoutResult, Vec2f& lowerLeft, Vec2f& upperRight) {
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextFace::justifyLine(const TextLayoutParam& param, const vector<UInt32>& spaceIndices,
  ...
  Real32 factor = 0.5;
  Real32 spaceDelta =
      delta / (static_cast<Real32>(numSpaces) + static_cast<Real32>(numGlyphs) * factor);
  ...
  else if (param.topToBottom == true) {
...
// Author: pdaehne
//----------------------------------------------------------------------
void TextFace::adjustLineOrigin(
    const TextLayoutParam& param, const Vec2f& currPos, TextLayoutResult& layoutResult) const {
  Vec2f offset;
  if (param.horizontal == true) {
    switch (param.minorAlignment) {
    default:
    ...
    case TextLayoutParam::ALIGN_BEGIN:
      ...
      break;
    case TextLayoutParam::ALIGN_MIDDLE:
      offset[1] = -(_horiAscent + _horiDescent) / 2.f;
      break;
    case TextLayoutParam::ALIGN_END:
      offset[1] = -_horiDescent;
      break;
  ...
  } else // param.horizontal == false
  {
    ...
    case TextLayoutParam::ALIGN_BEGIN:
      ...
      break;
    case TextLayoutParam::ALIGN_MIDDLE:
      offset[0] = -(_vertAscent + _vertDescent) / 2.f;
      break;
    case TextLayoutParam::ALIGN_END:
      offset[0] = -_vertDescent;
      break;
