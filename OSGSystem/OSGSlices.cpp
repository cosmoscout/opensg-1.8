...
typedef void(OSG_APIENTRY* PFNGLMULTITEXCOORD3FARBPROC)(
...
void Slices::initMethod(void) {
  ...
  IntersectAction::registerEnterDefault(getClassType(),
      osgTypedMethodFunctor2BaseCPtrRef<Action::ResultE, SlicesPtr, CNodePtr, Action*>(
          &Slices::intersect));
}

//! react to field changes

void Slices::changed(BitVector whichField, UInt32 origin) {
...
void Slices::dump(UInt32, const BitVector) const {
...
void Slices::initEdgeVec(void) {
  ...
  for (i = 0; i < 12; i++)
    _edgeVec[i].line.setValue(
        _pointVec[_edgeVec[i].pointIndexA], _pointVec[_edgeVec[i].pointIndexB]);
}
...
UInt32 Slices::createSlice(const OSG::Plane& plane, Slice& slice) {
  ...
  for (slice.numOfIntersection = i = 0; i < 12; i++) {
    if ((pointInHalfSpace[_edgeVec[i].pointIndexA] != pointInHalfSpace[_edgeVec[i].pointIndexB])) {
      plane.intersect(_edgeVec[i].line, slice.pointVec[slice.numOfIntersection]);
  ...
  if (slice.numOfIntersection > 2) {
    ...
    for (; j++ <= lastButOne; i++) {
      ...
      while ((j <= lastButOne) &&
             ((_edgeVec[slice.edgeVec[i]].edgeTag & _edgeVec[slice.edgeVec[j]].edgeTag) != 0)) {
        i++;
        j++;
      }
      ...
      if (j <= lastButOne) {
        ...
        k = j + 1;
        while ((k <= lastButOne) &&
               ((_edgeVec[slice.edgeVec[i]].edgeTag & _edgeVec[slice.edgeVec[k]].edgeTag) == 0)) {
          k++;
        }
    ...
    slice.ccw = (fabs(winding.x() + plane.getNormal().x()) >= fabs(plane.getNormal().x()));
  }

  return slice.numOfIntersection;
}

void Slices::drawSlices(
    Window* win, const Vec3f& planeNormal, UInt32& triCount, UInt32& vertexCount) {
  bool has_multitexture = win->hasExtension(_arbMultitexture);

  PFNGLMULTITEXCOORD3FARBPROC multiTexCoord3f = NULL;
  if (has_multitexture)
    multiTexCoord3f = (PFNGLMULTITEXCOORD3FARBPROC)win->getFunction(_funcMultiTexCoord3fARB);
  ...
  texSliceNormal.setValues(planeNormal.x() * sliceDistance * ssx,
      planeNormal.y() * sliceDistance * ssy, planeNormal.z() * sliceDistance * ssz);
      ...
      if (createSlice(plane, slice) > 2) {
        ...
        if (slice.ccw) {
          for (i = 0; i < slice.numOfIntersection; i++) {
            ...
            glTexCoord3f(
                (hsx + texPos0.x()) * ssx, (hsy + texPos0.y()) * ssy, (hsz + texPos0.z()) * ssz);

            if (has_multitexture) {
              multiTexCoord3f(GL_TEXTURE1_ARB, (hsx + texPos1.x()) * ssx, (hsy + texPos1.y()) * ssy,
                  (hsz + texPos1.z()) * ssz);

              multiTexCoord3f(GL_TEXTURE2_ARB, (hsx + texPos2.x()) * ssx, (hsy + texPos2.y()) * ssy,
                  (hsz + texPos2.z()) * ssz);
            }
