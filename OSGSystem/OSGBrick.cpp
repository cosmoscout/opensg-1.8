...
void BrickSet::buildBricks3D(DVRVolume* volume, Vec3f brickSize, Int32 overlap, Orientation ori) {
  FDEBUG(("BrickSet::buildBricks3D %d %d %d\n", brickSize[0], brickSize[1], brickSize[2]));
  ...
  if ((brickSize[0] <= overlap) || (brickSize[1] <= overlap) || (brickSize[2] <= overlap)) {
    SFATAL << "BrickSet::buildBricks3D - brickSize too small: " << brickSize[0] << ","
           << brickSize[1] << "," << brickSize[2] << "for overlap " << overlap << std::endl;
  ...
  for (Int32 z = 0; z < nNumBricksZ; z++) {
    ...
    for (Int32 y = 0; y < nNumBricksY; y++) {
      ...
      for (Int32 x = 0; x < nNumBricksX; x++) {
        ...
        m_pBricks[index].init(nXMin, nYMin, nZMin, nBrickSizeX, nBrickSizeY, nBrickSizeZ, xmin,
            ymin, zmin, xmax, ymax, zmax, overlap, ori);
...
void BrickSet::buildBricks2D(DVRVolume* volume, Orientation ori) {
  ...
  for (Int32 index = 0; index < m_nNumBricks; index++) {
    m_pBricks[index].init(resX == 1 ? index : 0, resY == 1 ? index : 0, resZ == 1 ? index : 0, resX,
        resY, resZ, xmin, ymin, zmin, xmax, ymax, zmax, 1, ori);
...
TextureChunkPtr BrickSet::makeTexture(UInt32 internalFormat, UInt32 externalFormat, ImagePtr img) {
  ...
  beginEditCP(chunk);
  {
    ...
    FDEBUG(
        ("TextureManager - makeTexture: intern %d - extern %d\n", internalFormat, externalFormat));
...
void BrickSet::buildBrickTextures(
    ChunkMaterialPtr& material, TextureSet& textures, TextureManager::TextureMode mode) {
      ...
      for (UInt32 tch = 0; tch < textures.size(); tch++) {
        ...
        case TextureManager::TM_2D_Multi: // 2D multi textures
        {
          ...
          textures[tch]->_image->slice(
              resX == 2 ? nXMin : -1, resY == 2 ? nYMin : -1, resZ == 2 ? nZMin : -1, img);
          if (index == 0) {
            m_pBricks[index].texture[texCounter] =
                makeTexture(textures[tch]->_internalFormat, textures[tch]->_externalFormat, img);
          } else {
          ...
          m_pBricks[index].texStage[texCounter] = textures[tch]->_textureStage0;
          ...
          textures[tch]->_image->slice(resX == 2 ? nXMin + 1 : -1, resY == 2 ? nYMin + 1 : -1,
              resZ == 2 ? nZMin + 1 : -1, img);

          m_pBricks[index].texture[texCounter] =
              makeTexture(textures[tch]->_internalFormat, textures[tch]->_externalFormat, img);

          m_pBricks[index].texStage[texCounter] = textures[tch]->_textureStage1;
        ...
        {
          textures[tch]->_image->slice(
              resX == 1 ? nXMin : -1, resY == 1 ? nYMin : -1, resZ == 1 ? nZMin : -1, img);
        ...
        default: {
          textures[tch]->_image->subImage(nXMin, nYMin, nZMin, resX, resY, resZ, img);

          m_pBricks[index].texture[texCounter] =
              makeTexture(textures[tch]->_internalFormat, textures[tch]->_externalFormat, img);

          m_pBricks[index].texStage[texCounter] = textures[texCounter]->_textureStage0;
...
void Brick::init(Int32 minVoxX, Int32 minVoxY, Int32 minVoxZ, Int32 resX, Int32 resY, Int32 resZ,
    Real32 xmin, Real32 ymin, Real32 zmin, Real32 xmax, Real32 ymax, Real32 zmax, Int32 overlap,
    BrickSet::Orientation ori) {

  FINFO(("Brick::init - upper left (%d, %d, %d)\n", minVoxX, minVoxY, minVoxZ));

  FINFO(("Brick::init (%d, %d, %d): %f %f %f -> %f %f %f \n", resX, resY, resZ, xmin, ymin, zmin,
      xmax, ymax, zmax));
  ...
  texScale = Vec3f((texCoorMax[0] - texCoorMin[0]) / xrange,
      (texCoorMax[1] - texCoorMin[1]) / yrange, (texCoorMax[2] - texCoorMin[2]) / zrange);

  texTranslate = Vec3f(texCoorMin[0] - texScale[0] * xmin, texCoorMin[1] - texScale[1] * ymin,
  ...
  for (Int32 i = 0; i < 12; i++) {
    edges[i] = Line(Pnt3f(vertex[eindex[i][0]]), Pnt3f(vertex[eindex[i][1]]));
  }
...
//! Render 2D/3D Slices
void Brick::renderSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, TextureManager::TextureMode textureMode) {
  switch (textureMode) {
  ...
  case TextureManager::TM_2D_Multi: {
    ...
    render2DMultiSlices(volume, action, shader, clipper, Slicer::isBackToFront(dir), sliceDir);
    break;
  }
...
void Brick::renderBrick() {
  ...
  const int eindex[12][2] = {{0, 1}, {2, 3}, {6, 7}, {4, 5}, {1, 2}, {0, 3}, {5, 6}, {4, 7}, {1, 7},
      {2, 4}, {3, 5}, {0, 6}};
...
void Brick::vertToTex(Real32* vx, Real32* vy, Real32* vz, Real32* tx, Real32* ty, Real32* tz) {
...
//! Render 3D Slices
void Brick::render3DSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, const Vec3f& sliceDir) {
  Int32 i, j;
  ...
  Real32 fAlphaCorrect  = 1.f - osgpow(1.f - volume->getBaseAlpha(), 1.0f / fSampleRate);
  ...
  for (nSlice = 0; nSlice < numSlices; nSlice++) {
    ...
    if (numIntersections > 2) {
      ...
      if (doClipping) {
        ...
        Vec3f sliceNormal = (vecIntersectionsSorted[1] - vecIntersectionsSorted[0])
                                .cross(vecIntersectionsSorted[2] - vecIntersectionsSorted[1]);
        ...
        if (clipObjects->getDoContours()) {
          ...
          clipper->clipSlice(volume, slice, sliceDir, fSliceDistance * nSlice, clippedSlice);
          activateTexture(action);
        ...
        if (shader->hasRenderCallback()) {
          ...
          for (i = 0; i < numIntersections; i++) {
            ...
            glTexCoord3f((v[0] * texscale[0] + textrans[0]), (v[1] * texscale[1] + textrans[1]),
                (v[2] * texscale[2] + textrans[2]));
            glVertex3fv(v.getValues());

            FDEBUG(("Brick::render3DSlices V: %f %f %f T: %f %f %f\n", v[0], v[1], v[2],
                (v[0] * texscale[0] + textrans[0]), (v[1] * texscale[1] + textrans[1]),
                (v[2] * texscale[2] + textrans[2])));
          }
...
void Brick::render2DSliceXY(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, const Vec3f& sliceDir, const Real32& zCoord) {
  ...
  if (doClipping) {
    ...
    Real32 dist = clipper->getReferencePlane().distance(Pnt3f(center[0], center[1], zCoord));
    ...
    if (clipObjects->getDoContours()) {
      ...
      clipper->clipSlice(volume, slice, sliceDir, dist, clippedSlice);
    ...
    if ((shader != NullFC) && shader->hasRenderCallback()) {
      ...
      Real32 data[] = {xmin, ymin,
          zCoord, // first vertex
          xmin * texScale[0] + texTranslate[0], ymin * texScale[1] + texTranslate[1],
          multiTexIntFactor,

          xmax, ymin,
          zCoord, // second vertex
          xmax * texScale[0] + texTranslate[0], ymin * texScale[1] + texTranslate[1],
          multiTexIntFactor,

          xmax, ymax,
          zCoord, // third vertex
          xmax * texScale[0] + texTranslate[0], ymax * texScale[1] + texTranslate[1],
          multiTexIntFactor,

          xmin, ymax,
          zCoord, // fourth vertex
          xmin * texScale[0] + texTranslate[0], ymax * texScale[1] + texTranslate[1],
          multiTexIntFactor};

      shader->renderSlice(volume, action, data, 4, 6);

    } else {
      glTexCoord2f(xmin * texScale[0] + texTranslate[0], ymin * texScale[1] + texTranslate[1]);

      glVertex3f(xmin, ymin, zCoord);

      glTexCoord2f(xmax * texScale[0] + texTranslate[0], ymin * texScale[1] + texTranslate[1]);

      glVertex3f(xmax, ymin, zCoord);

      glTexCoord2f(xmax * texScale[0] + texTranslate[0], ymax * texScale[1] + texTranslate[1]);

      glVertex3f(xmax, ymax, zCoord);

      glTexCoord2f(xmin * texScale[0] + texTranslate[0], ymax * texScale[1] + texTranslate[1]);
...
void Brick::render2DSliceXZ(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, const Vec3f& sliceDir, const Real32& yCoord) {
  Real32 xmin = vertex[0][0];
  Real32 xmax = vertex[1][0];
  ...
  if (doClipping) {
    ...
    Real32 dist = clipper->getReferencePlane().distance(Pnt3f(center[0], yCoord, center[2]));
    ...
    if ((shader != NullFC) && shader->hasRenderCallback()) {
      ...
      Real32 data[] = {xmin, yCoord,
          zmin, // first vertex
          xmin * texScale[0] + texTranslate[0], zmin * texScale[2] + texTranslate[2],
          multiTexIntFactor,

          xmax, yCoord,
          zmin, // second vertex
          xmax * texScale[0] + texTranslate[0], zmin * texScale[2] + texTranslate[2],
          multiTexIntFactor,

          xmax, yCoord,
          zmax, // third vertex
          xmax * texScale[0] + texTranslate[0], zmax * texScale[2] + texTranslate[2],
          multiTexIntFactor,

          xmin, yCoord,
          zmax, // fourth vertex
          xmin * texScale[0] + texTranslate[0], zmax * texScale[2] + texTranslate[2],
          multiTexIntFactor};
      ...
      glTexCoord2f(xmin * texScale[0] + texTranslate[0], zmin * texScale[2] + texTranslate[2]);

      glVertex3f(xmin, yCoord, zmin);

      glTexCoord2f(xmax * texScale[0] + texTranslate[0], zmin * texScale[2] + texTranslate[2]);

      glVertex3f(xmax, yCoord, zmin);

      glTexCoord2f(xmax * texScale[0] + texTranslate[0], zmax * texScale[2] + texTranslate[2]);

      glVertex3f(xmax, yCoord, zmax);

      glTexCoord2f(xmin * texScale[0] + texTranslate[0], zmax * texScale[2] + texTranslate[2]);
...
void Brick::render2DSliceYZ(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, const Vec3f& sliceDir, const Real32& xCoord) {
  Real32 ymin = vertex[0][1];
  Real32 ymax = vertex[2][1];
  ...
  if (doClipping) {
    ...
    Real32 dist = clipper->getReferencePlane().distance(Pnt3f(xCoord, center[1], center[2]));
    ...
    if ((shader != NullFC) && shader->hasRenderCallback()) {
      ...
      Real32 data[] = {xCoord, ymin, zmin, ymin * texScale[1] + texTranslate[1],
          zmin * texScale[2] + texTranslate[2], multiTexIntFactor,

          xCoord, ymax, zmin, ymax * texScale[1] + texTranslate[1],
          zmin * texScale[2] + texTranslate[2], multiTexIntFactor,

          xCoord, ymax, zmax, ymax * texScale[1] + texTranslate[1],
          zmax * texScale[2] + texTranslate[2], multiTexIntFactor,

          xCoord, ymin, zmax, ymin * texScale[1] + texTranslate[1],
          zmax * texScale[2] + texTranslate[2], multiTexIntFactor};

      shader->renderSlice(volume, action, data, 4, 6);
    } else {
      glTexCoord2f(ymin * texScale[1] + texTranslate[1], zmin * texScale[2] + texTranslate[2]);

      glVertex3f(xCoord, ymin, zmin);

      glTexCoord2f(ymax * texScale[1] + texTranslate[1], zmin * texScale[2] + texTranslate[2]);

      glVertex3f(xCoord, ymax, zmin);

      glTexCoord2f(ymax * texScale[1] + texTranslate[1], zmax * texScale[2] + texTranslate[2]);

      glVertex3f(xCoord, ymax, zmax);

      glTexCoord2f(ymin * texScale[1] + texTranslate[1], zmax * texScale[2] + texTranslate[2]);
...
void Brick::render2DSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, const Vec3f& sliceDir) {
  // number of additional per vertex attributes
  // (not position + first texture coordinates)
...
void Brick::render2DMultiSlices(DVRVolume* volume, DrawActionBase* action, DVRShaderPtr shader,
    DVRClipper* clipper, bool btf, const Vec3f& sliceDir) {
  DVRVolumeTexturePtr tex = DVRVOLUME_PARAMETER(volume, DVRVolumeTexture);
