...
RemoteAspect::RemoteAspect(UInt32 aspectId)
  ...
  _fieldFilter[CubeTextureChunk::getClassType().getId()]     = CubeTextureChunk::GLIdFieldMask;
  _fieldFilter[VertexProgramChunk::getClassType().getId()]   = VertexProgramChunk::GLIdFieldMask;
  _fieldFilter[FragmentProgramChunk::getClassType().getId()] = FragmentProgramChunk::GLIdFieldMask;
  ...
  for (typeI = FieldContainerFactory::the()->beginTypes();
    ...
    if (typeI->second->isDerivedFrom(Window::getClassType())) {
      _fieldFilter[typeI->second->getId()] = Window::GlObjectEventCounterFieldMask |
  ...
  do {
    ...
    case CREATED: {
      ...
      if (localTypeI == _localType.end()) {
        SWARNING << "Unknown TypeID: " << remoteTypeId << " for remote id " << remoteId
                 << std::endl;
      } else {
    ...
    case DESTROYED: {
      ...
      if (getLocalId(remoteId, localId)) {
        ...
        SWARNING << "Can't destroy unknown FC:" << remoteId << " " << std::endl;
      }
      break;
    }
    ...
    case CHANGED: {
      ...
      if (getLocalId(remoteId, localId)) {
        ...
        */
        ...
        changedFCs.push_back(std::pair<FieldContainerPtr, BitVector>(fcPtr, mask));
      } else {
        char dummy;

        SWARNING << "Can't change unknown FC:" << remoteId << " skip " << len << " bytes."
                 << std::endl;
        while (len--)
...
void RemoteAspect::sendSync(Connection& connection, ChangeList* changeList) {
  ...
  // created fct
  for (createdI = changeList->beginCreated(); createdI != changeList->endCreated(); createdI++) {
  ...
  // only once
  for (changedI = changeList->beginChanged(); changedI != changeList->endChanged(); changedI++) {
    changedMapI = changedMap.find(changedI->first);
    if (changedMapI == changedMap.end()) {
      changedMap.insert(std::pair<UInt32, BitVector>(changedI->first, changedI->second));
    } else {
      changedMapI->second |= changedI->second;
    }
  }

  for (FieldMaskMapT::iterator condensedI = changedMap.begin(); condensedI != changedMap.end();
       ++condensedI) {
    ...
    if (mask) {
      ...
      FDEBUG(("Changed: %s ID:%d Mask:%lld\n", fcPtr->getType().getName().str(),
          fcPtr.getFieldContainerId(), mask))
    }
  }

  // destroy fct
  // needs to be called after the changes (see receiveSync for more info)!!!
  for (destroyedI = changeList->beginDestroyed(); destroyedI != changeList->endDestroyed();
       destroyedI++) {
  ...
  // addref
  for (addRefedI = changeList->beginAddRefd(); addRefedI != changeList->endAddRefd(); ++addRefedI) {
    UInt32 id = (*addRefedI);
  ...
  // subref
  for (subRefedI = changeList->beginSubRefd(); subRefedI != changeList->endSubRefd(); ++subRefedI) {
    UInt32 id = (*subRefedI);
 ...
 *  \see receiveSync
 */
void RemoteAspect::registerCreated(const FieldContainerType& type, const Functor& func) {
  while (type.getId() >= _createdFunctors.size()) {
    _createdFunctors.push_back(
        osgTypedFunctionFunctor2CPtrRef<bool, FieldContainerPtr, RemoteAspect*>(
 ...
 *  \see receiveSync
 */
void RemoteAspect::registerDestroyed(const FieldContainerType& type, const Functor& func) {
 ...
 *  \see receiveSync
 */
void RemoteAspect::registerChanged(const FieldContainerType& type, const Functor& func) {
...
void RemoteAspect::handleFCMapping(Connection& connection) {
  ...
  for (remoteFCI = _remoteFC.begin(); remoteFCI != _remoteFC.end(); ++remoteFCI) {
 ...
 */
bool RemoteAspect::getLocalId(UInt32 remoteId, UInt32& localId) {
  UInt64                fullRemoteId = getFullRemoteId(remoteId);
...
bool RemoteAspect::_defaultCreatedFunction(FieldContainerPtr& fcp, RemoteAspect*) {
  FDEBUG(("Created:%s %d\n", fcp->getType().getName().str(), fcp.getFieldContainerId()))
  return true;
}

/*! Default destroyed functor
 */
bool RemoteAspect::_defaultDestroyedFunction(FieldContainerPtr& fcp, RemoteAspect*) {
