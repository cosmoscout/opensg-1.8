...
class OSG_SYSTEMLIB_DLLMAPPING CNurbsPatchSurface {
 ...
 public:
  ...
  //	std::vector< NodePtr >* GetNodes( std::ofstream &meshout, std::ofstream &rclLinesOut,
  //std::ofstream &rclLinesOut2 );
  ...
  // set surface
  void setSurface(
      BSplineTrimmedSurface* clSurface, std::vector<Pnt2f>& texturecps, bool bUseTextures = false);
  // set error
  void setError(double dError);
  // get tessellated geometry
  void getTessellation(std::vector<Pnt3f>& gverts, std::vector<Vec3f>& norms,
  ...
  void getTessellation(std::vector<Pnt3f>& gverts, std::vector<Pnt2f>& texturecoords,
      std::vector<SimplePolygon>& tris, bool usedelaunay);
  ...
  void calculatePointsNormalsAndTextureCoords(std::vector<Pnt3f>& gverts,
      std::vector<SimplePolygon>& tris, std::vector<Vec3f>& norms,
      std::vector<Pnt2f>& texturecoords);
  void calculatePointsAndTextureCoords(std::vector<Pnt3f>& gverts, std::vector<SimplePolygon>& tris,
      std::vector<Pnt2f>& texturecoords);
 ...
 private:
  ...
  void convertPoints(std::vector<Pnt3f>& gverts, std::vector<SimplePolygon>& tris);

  // compute the normalcone (needed for backface culling)
  void ComputeNormalCone(const unsigned int cuiSurface, std::vector<Vec3f>& normals);

  void Gen3DLoops(const unsigned int cuiSurface);
  void CutCurve(const unsigned int cuiSurface, BezierCurve2D& rclCurve, bezier2ddeque& rclCut,
      std::vector<unsigned int>& rvuiUSeg, std::vector<unsigned int>& rvuiVSeg);
