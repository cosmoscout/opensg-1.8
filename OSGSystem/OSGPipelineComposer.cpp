...
void PipelineComposer::dump(UInt32, const BitVector) const {
...
void PipelineComposer::open() {
  ...
  _clusterWindow->getNetwork()->connectAllGroupToPoint(clusterId(), "StreamSock");
  ...
  if (getStatistics()) {
    if (isClient()) {
      ...
      printf("DepthAndColor    : %10d\n", serverCount() * _composeTilesX * _composeTilesY -
                                              _statistics.occluded - _statistics.noDepth -
                                              _statistics.noGeo);
    } else {
 ...
 */
bool PipelineComposer::GroupInfoOrder::operator()(const GroupInfo* a, const GroupInfo* b) {
  if (a->depth.max > b->depth.max || (a->depth.max == b->depth.max && a->id > b->id))
    return true;
  else
    return false;
}
