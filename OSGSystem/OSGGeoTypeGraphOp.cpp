...
void GeoTypeGraphOp::setParams(const std::string params) {
  ...
  if (ps("filter", filter)) {
    _filter = 0;
    if (filter.find("Nor") != std::string::npos || filter.find("nor") != std::string::npos) {
      _filter |= Geometry::NormalsFieldMask;
    }
    if (filter.find("Ind") != std::string::npos || filter.find("ind") != std::string::npos) {
      _filter |= Geometry::IndicesFieldMask;
    }
    if (filter.find("Len") != std::string::npos || filter.find("len") != std::string::npos) {
...
std::string GeoTypeGraphOp::usage(void) {
  return "GeoType: convert the types of a Geometry's attributes\n"
