...
StatElemDesc<StatTimeElem> RenderAction::statDrawTime("drawTime", "time for draw tree traversal");
StatElemDesc<StatIntElem>  RenderAction::statNMaterials("NMaterials", "number of material changes");
StatElemDesc<StatIntElem>  RenderAction::statNMatrices("NMatrices", "number of matrix changes");
StatElemDesc<StatIntElem>  RenderAction::statNLights("NLights", "number of light changes");
StatElemDesc<StatIntElem>  RenderAction::statNGeometries("NGeometries", "number of Geometry nodes");
StatElemDesc<StatIntElem>  RenderAction::statNTransGeometries(
    "NTransGeometries", "number of transformed Geometry nodes");
StatElemDesc<StatIntOnceElem> RenderAction::statNTextures("NTextures", "number of texture changes");
StatElemDesc<StatIntOnceElem> RenderAction::statNTexBytes(
    "NTexBytes", "sum of all used textures' sizes (approx., in bytes)");
StatElemDesc<StatStringElem> RenderAction::statNOcclusionMode(
    "OcclusionMode", "occlusion culling mode");
StatElemDesc<StatIntElem> RenderAction::statNOcclusionTests(
    "NOcclusionTests", "number of occlusion tests");
StatElemDesc<StatIntElem> RenderAction::statNOcclusionCulled(
    "NOcclusionCulled", "number of objects culled via occlusion culling");
...
void RenderAction::registerEnterDefault(
  ...
  while (type.getId() >= _vDefaultEnterFunctors->size()) {
    _vDefaultEnterFunctors->push_back(osgTypedFunctionFunctor2CPtrRef<ResultE, CNodePtr, Action*>(
...
void RenderAction::registerLeaveDefault(
  ...
  while (type.getId() >= _vDefaultLeaveFunctors->size()) {
    _vDefaultLeaveFunctors->push_back(osgTypedFunctionFunctor2CPtrRef<ResultE, CNodePtr, Action*>(
...
RenderAction::RenderAction(void)
  ...
  _funcGenQueriesARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glGenQueriesARB", _arbOcclusionQuery);
  _funcDeleteQueriesARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glDeleteQueriesARB", _arbOcclusionQuery);
  _funcBeginQueryARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glBeginQueryARB", _arbOcclusionQuery);
  _funcEndQueryARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glEndQueryARB", _arbOcclusionQuery);
  _funcGetQueryObjectuivARB =
      Window::registerFunction(OSG_DLSYM_UNDERSCORE "glGetQueryObjectuivARB", _arbOcclusionQuery);

  // we can't include OSGCGChunk here because it is in Contrib ...
  StateChunkPtr cgChunk =
  ...
  StateChunkPtr cgfxChunk =
      StateChunkPtr::dcast(FieldContainerFactory::the()->createFieldContainer("CGFXChunk"));
  if (cgfxChunk != NullFC) {
    _cgfxChunkId = cgfxChunk->getClass()->getId();
    subRefCP(cgfxChunk);
  }

  StateChunkPtr shlChunk =
      StateChunkPtr::dcast(FieldContainerFactory::the()->createFieldContainer("SHLChunk"));
  if (shlChunk != NullFC) {
...
void RenderAction::dropGeometry(Geometry* pGeo) {
  ...
  if (!_stateSorting ||
      (sortKey == Material::NoStateSorting && (!_bSortTrans || !pMat->isTransparent()))) {
...
void RenderAction::popVisibility(void) {
  ...
  if (!_visibilityStack.empty())
    ...
    SWARNING << "RenderAction::popVisibility: visibility stack is empty looks"
                "like a pushVisibility and popVisibility mismatch!"
             << std::endl;
}
...
void RenderAction::activateLocalLights(DrawTreeNode* pRoot) {
  ...
  for (UInt32 i = light_id; i < _activeLightsCount; ++i) {
    ...
    glLightfv(GL_LIGHT0 + i, GL_DIFFUSE, black.getValuesRGBA());
...
bool RenderAction::isOccluded(DrawTreeNode* pRoot) {
  ...
  UInt32 pos_size = 0;
  if ((_bOcclusionCulling || _bSmallFeatureCulling) && pRoot->getNode()->getCore() != NullFC) {
...
void RenderAction::deleteOcclusionQueriesPool(void) {
  ...
  for (std::set<UInt32>::iterator it = _hier_occlusions.begin(); it != _hier_occlusions.end();
       ++it) {
...
void RenderAction::drawHierarchicalMultiFrameOcclusionBB(const Matrix& view, NodePtr node) {
    ...
    if (uiNextMatrix != 0 && uiNextMatrix != _uiActiveMatrix) {
      ...
      for (int i = 0; i < 4; i++) {
        fprintf(stderr, "% 5.2f % 5.2f % 5.2f % 5.2f\n", pRoot->getMatrixStore().second[i][0],
            pRoot->getMatrixStore().second[i][1], pRoot->getMatrixStore().second[i][2],
...
Action::ResultE RenderAction::start(void) {
  ...
  if (_glGenQueriesARB == NULL && _window->hasExtension(_arbOcclusionQuery)) {
    _glGenQueriesARB =
        (void(OSG_APIENTRY*)(GLsizei, GLuint*))_window->getFunction(_funcGenQueriesARB);
    _glDeleteQueriesARB =
        (void(OSG_APIENTRY*)(GLsizei, GLuint*))_window->getFunction(_funcDeleteQueriesARB);
    _glBeginQueryARB =
        (void(OSG_APIENTRY*)(GLenum, GLuint))_window->getFunction(_funcBeginQueryARB);
    _glEndQueryARB          = (void(OSG_APIENTRY*)(GLenum))_window->getFunction(_funcEndQueryARB);
    _glGetQueryObjectuivARB = (void(OSG_APIENTRY*)(GLuint, GLenum, GLuint*))_window->getFunction(
        _funcGetQueryObjectuivARB);
  }
  ...
  if (_viewport != NULL) {
    if (_camera != NULL) {
      ...
      _camera->getViewing(
          _currMatrix.second, _viewport->getPixelWidth(), _viewport->getPixelHeight());
...
Action::ResultE RenderAction::stop(ResultE res) {
  ...
  while (matRootsIt != _pMatRoots.end() || transMatRootsIt != _pTransMatRoots.end()) {
  ...
  if (_bOcclusionCulling && (_occlusionCullingMode & OcclusionMultiFrame)) {
    if (_occlusionCullingMode == OcclusionHierarchicalMultiFrame) {
      ...
      for (std::vector<NodePtr>::iterator it = _occluded_nodes.begin(); it != _occluded_nodes.end();
           ++it) {
        NodePtr occluded = (*it);
    ...
    if (_occlusionCullingMode == OcclusionHierarchicalMultiFrame) {
      ...
      Matrix view;
      if (_camera != NULL) {
        _camera->getViewing(view, _viewport->getPixelWidth(), _viewport->getPixelHeight());
      }
  ...
  if (!_ownStat) {
    ...
    getStatistics()->getElem(statNLights)->set(_uiNumLightChanges);
    ...
    getStatistics()->getElem(statNOcclusionTests)->set(_uiNumOcclusionTests);
    getStatistics()->getElem(statNOcclusionCulled)->set(_uiNumOcclusionCulled);

    if (_bOcclusionCulling) {
      if (_occlusionCullingMode == OcclusionStopAndWait) {
        getStatistics()->getElem(statNOcclusionMode)->set("Stop And Wait");
      } else if (_occlusionCullingMode == OcclusionMultiFrame) {
        getStatistics()->getElem(statNOcclusionMode)->set("Multi Frame");
      } else if (_occlusionCullingMode == OcclusionHierarchicalMultiFrame) {
        getStatistics()->getElem(statNOcclusionMode)->set("Hier. Multi Frame");
      }
    } else {
      getStatistics()->getElem(statNOcclusionMode)->set("Off");
    }
  }
...
void RenderAction::push_matrix(const Matrix& matrix) {
  ...
  for (int i = 0; i < 4; i++) {
    fprintf(stderr, "% 5.2f % 5.2f % 5.2f % 5.2f\n", _currMatrix.second[i][0],
        _currMatrix.second[i][1], _currMatrix.second[i][2], _currMatrix.second[i][3]);
  }
#endif
