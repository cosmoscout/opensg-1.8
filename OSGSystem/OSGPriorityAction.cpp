...
StatElemDesc<StatIntElem> PriorityAction::statStateClones(
    "PriorityAction::stateClones", "Number of state clones created");
StatElemDesc<StatIntElem> PriorityAction::statStateRestores(
    "PriorityAction::statStateRestores", "Number of state restores");
...
PriorityAction* PriorityAction::create(void) {
...
PriorityAction::ResultE PriorityAction::apply(NodePtr pRoot) {
  ...
  pqPush(NodeQueueEntry(pRoot, PriorityTypeTraits::getZeroElement(), 1, _itActiveState));
...
void PriorityAction::addExtendEvent(ExtendActorBase* pActor, UInt32 actorIndex) {
...
void PriorityAction::subExtendEvent(ExtendActorBase* pActor, UInt32 actorIndex) {
  ExtendActorStoreIt itEnter  = _extendEnterActors.begin();
...
void PriorityAction::addBasicEvent(BasicActorBase* pActor, UInt32 actorIndex) {
...
void PriorityAction::subBasicEvent(BasicActorBase* pActor, UInt32 actorIndex) {
  BasicActorStoreIt itEnter  = _basicEnterActors.begin();
...
void PriorityAction::startEvent(void) {
...
void PriorityAction::stopEvent(void) {
  Inherited::stopEvent();
}
...
void PriorityAction::beginEditStateEvent(ActorBase* pActor, UInt32 actorId) {
...
void PriorityAction::endEditStateEvent(ActorBase* pActor, UInt32 actorId) {
}
...
PriorityAction::ResultE PriorityAction::traverseEnter(void) {
  ResultE              result = NewActionTypes::Continue;
  NodePtr              pNode;
  ...
  while ((pqEmpty() == false) && !(result & NewActionTypes::Quit)) {
    ...
    if ((nodePass == 1) && (multiPasses > 1)) {
      for (; multiPasses > 1; --multiPasses) {
        ...
        pqPush(NodeQueueEntry(pNode, nodePrio, multiPasses, _itActiveState));
      }
    }
...
void PriorityAction::enqueueChildren(const NodePtr& pNode, ResultE result) {
  ...
  if (getChildrenListEnabled() == true) {
    for (UInt32 i = 0, size = cl.getSize(); i < size; ++i) {
      if ((cl.getActive(i) == true) && (cl.getChild(i) != NullFC) &&
        ...
        pqPush(NodeQueueEntry(cl.getChild(i), cl.getPriority(i), 1, _itActiveState));
      }
    }
  } else {
    ...
    for (; itChildren != endChildren; ++itChildren) {
      if ((*itChildren != NullFC) && (((*itChildren)->getTravMask() & getTravMask()) != 0)) {
        ...
        pqPush(
            NodeQueueEntry(*itChildren, PriorityTypeTraits::getZeroElement(), 1, _itActiveState));
      }
    }
  }
  ...
  for (UInt32 i = 0, size = ecl.getSize(); i < size; ++i) {
    if ((ecl.getActive(i) == true) && (ecl.getChild(i) != NullFC) &&
      ...
      pqPush(NodeQueueEntry(ecl.getChild(i), ecl.getPriority(i), 1, _itActiveState));
    }
  }
