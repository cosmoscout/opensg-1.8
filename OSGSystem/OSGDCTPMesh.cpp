...
int DCTPMesh::directEdge(Vec3d& from, Vec3d& into) {
  ...
  dctpvertexset::iterator v1, v2, vend = vertices.end();
  v1 = vertices.find(from_vp);
  if (v1 == vend) {
    delete from_vp;
    delete into_vp;
    return -1;
  ...
  //        std::cerr << "v1->coords: " << (*v1)->coords << " v2->coords: " << (*v2)->coords <<
  //        std::endl;
  delete from_vp;
  delete into_vp;
  ...
  for (std::vector<DCTPEdge*>::iterator i = (*v1)->edges.begin(); i != eend; ++i) {
    ...
    if (DCTPVecIsEqual(tv1->coords, (*v1)->coords) && DCTPVecIsEqual(tv2->coords, (*v2)->coords)) {
      ++(*i)->orientation;
      return 0;
    }
    if (DCTPVecIsEqual(tv1->coords, (*v2)->coords) && DCTPVecIsEqual(tv2->coords, (*v1)->coords)) {
      --(*i)->orientation;
      return 0;
    }
  }
...
DCTPVertex* DCTPMesh::SplitEdge(DCTPEdge* edge, double t) {
  ...
  if (!siv.second) {
    ...
    if (nv->edges.size() > 0) {
      std::cerr
          << "DCTPMesh::SplitEdge: error: inserting already existing splitvertex with edges..."
          << std::endl;
...
//  std::cerr << "old: " << v1->coords << " - " << nv->coords << std::endl;
  ...
  if (!sie1.second) {
    std::cerr << "DCTPMesh::SplitEdge: error: reinserting already existing splitedge (this "
                 "shouldn't happen)"
              << std::endl;
    return NULL;
  }
...
//  std::cerr << "new: " << nv->coords << " - " << v2->coords << std::endl;
  ...
  if (!sie.second) {
    delete ne;
    ne = *(sie.first);
    std::cerr << "DCTPMesh::SplitEdge: error: inserting already existing splitedge (this shouldn't "
                 "happen)"
              << std::endl;
    return NULL;
  }
...
int DCTPMesh::MoveVertex(DCTPVertex* vert, Vec3d& newpos) {
  ...
  if (!vinsert.second) {
    std::cerr << "DCTPMesh::MoveVertex: error: reinserting already existing vertex (this shouldn't "
                 "happen) "
...
void DCTPMesh::buildNewFaces(
    DCTPFace* f, DCTPVertex* np, DCTPVertex* sp, DCTPVertex* wp, DCTPVertex* ep) {
  // set up middle point
...
#ifdef OSG_UNION_TRI_QUAD
  quad_save[0] = f->orig_face[0];
  quad_save[1] = f->orig_face[1];
  quad_save[2] = f->orig_face[2];
  quad_save[3] = f->orig_face[3];
#else
  quad_save[0] = f->orig_quad[0];
  quad_save[1] = f->orig_quad[1];
  quad_save[2] = f->orig_quad[2];
  quad_save[3] = f->orig_quad[3];
#endif
...
#ifdef OSG_UNION_TRI_QUAD
  f->orig_face[1] = np;
  f->orig_face[2] = middlev;
  f->orig_face[3] = wp;
#else
  f->orig_quad[1] = np;
  f->orig_quad[2] = middlev;
  f->orig_quad[3] = wp;
...
#ifdef OSG_UNION_TRI_QUAD
  f->orig_face[0] = np;
  f->orig_face[1] = quad_save[1];
  f->orig_face[2] = ep;
  f->orig_face[3] = middlev;
#else
  f->orig_quad[0] = np;
  f->orig_quad[1] = quad_save[1];
  f->orig_quad[2] = ep;
  f->orig_quad[3] = middlev;
  f->orig_triangle[0] = f->orig_triangle[1] = f->orig_triangle[2] = NULL;
#endif
  // lower right
  vertsvec.clear();
  ...
  f = buildQuadFace(vertsvec, NULL);
#ifdef OSG_UNION_TRI_QUAD
  f->orig_face[0] = middlev;
  f->orig_face[1] = ep;
  f->orig_face[2] = quad_save[2];
  f->orig_face[3] = sp;
#else
  f->orig_quad[0] = middlev;
  f->orig_quad[1] = ep;
  f->orig_quad[2] = quad_save[2];
  f->orig_quad[3] = sp;
  f->orig_triangle[0] = f->orig_triangle[1] = f->orig_triangle[2] = NULL;
#endif
  // lower left
...
#ifdef OSG_UNION_TRI_QUAD
  f->orig_face[0] = wp;
  f->orig_face[1] = middlev;
  f->orig_face[2] = sp;
  f->orig_face[3] = quad_save[3];
#else
  f->orig_quad[0] = wp;
  f->orig_quad[1] = middlev;
  f->orig_quad[2] = sp;
  f->orig_quad[3] = quad_save[3];
  f->orig_triangle[0] = f->orig_triangle[1] = f->orig_triangle[2] = NULL;
#endif
}
...
int DCTPMesh::SubdivideQuadNS(DCTPFace* f, double dRatio) {
  ...
  e[1] = f->orig_face[2]->coords[1] +
         (f->orig_face[1]->coords[1] - f->orig_face[2]->coords[1]) * dRatio;
...
//  f->orig_face[ 2 ] = quad_save[ 2 ]; f->orig_face[ 3 ] = quad_save[ 3 ];
  ...
  f = buildQuadFace(vertsvec, NULL);
#ifdef OSG_UNION_TRI_QUAD
  f->orig_face[0] = quad_save[0];
  f->orig_face[1] = quad_save[1];
  f->orig_face[2] = ep;
  f->orig_face[3] = wp;
#else
  f->orig_quad[0] = quad_save[0];
  f->orig_quad[1] = quad_save[1];
  f->orig_quad[2] = ep;
  f->orig_quad[3] = wp;
  f->orig_triangle[0] = f->orig_triangle[1] = f->orig_triangle[2] = NULL;
...
#ifdef OSG_UNION_TRI_QUAD
  n[0] = f->orig_face[0]->coords[0] +
         (f->orig_face[1]->coords[0] - f->orig_face[0]->coords[0]) * dRatio;
...
DCTPEdge* DCTPMesh::getEdgeFromFace(DCTPFace* f, DCTPVertex* v1, DCTPVertex* v2) {
  ...
  for (std::vector<DCTPEdge*>::iterator i = f->edges.begin(); i != ee; ++i) {
    DCTPVertex *tv1, *tv2;
    (*i)->getVertices(tv1, tv2);
    if ((DCTPVecIsEqual(v1->coords, tv1->coords) && DCTPVecIsEqual(v2->coords, tv2->coords)) ||
        (DCTPVecIsEqual(v1->coords, tv2->coords) && DCTPVecIsEqual(v2->coords, tv1->coords)))
      return *i;
  }
...
void DCTPMesh::setupSides(
    DCTPFace* f, std::vector<DCTPVertex*>* sides, double dXRatio, double dYRatio) {
#ifdef OSG_UNION_TRI_QUAD
  ...
  for (std::vector<DCTPVertex*>::iterator i = f->vertices.begin(); i != ve; ++i) {
    ...
    if (osgabs((*i)->coords[1] - f->orig_face[0]->coords[1]) < DCTP_EPS) {
      // left
      if (((*i)->coords[0] > f->orig_face[0]->coords[0]) && ((*i)->coords[0] < middle[0]))
#else
    if (osgabs((*i)->coords[1] - f->orig_quad[0]->coords[1]) < DCTP_EPS) {
      // left
      if (((*i)->coords[0] > f->orig_quad[0]->coords[0]) && ((*i)->coords[0] < middle[0]))
#endif
        sides[0].push_back(*i);
        // right
#ifdef OSG_UNION_TRI_QUAD
      else if (((*i)->coords[0] > middle[0]) && ((*i)->coords[0] < f->orig_face[1]->coords[0]))
#else
      else if (((*i)->coords[0] > middle[0]) && ((*i)->coords[0] < f->orig_quad[1]->coords[0]))
#endif
        sides[1].push_back(*i);
    } // end if
    // right up and down
    ...
    if (osgabs((*i)->coords[0] - f->orig_face[1]->coords[0]) < DCTP_EPS) {
      // up
      if (((*i)->coords[1] < f->orig_face[1]->coords[1]) && ((*i)->coords[1] > middle[1]))
#else
    if (osgabs((*i)->coords[0] - f->orig_quad[1]->coords[0]) < DCTP_EPS) {
      // up
      if (((*i)->coords[1] < f->orig_quad[1]->coords[1]) && ((*i)->coords[1] > middle[1]))
#endif
        sides[2].push_back(*i);
        // down
#ifdef OSG_UNION_TRI_QUAD
      else if (((*i)->coords[1] < middle[1]) && ((*i)->coords[1] > f->orig_face[2]->coords[1]))
#else
      else if (((*i)->coords[1] < middle[1]) && ((*i)->coords[1] > f->orig_quad[2]->coords[1]))
#endif
        sides[3].push_back(*i);
    } // end if
    ...
    if (osgabs((*i)->coords[1] - f->orig_face[2]->coords[1]) < DCTP_EPS) {
      // right
      if (((*i)->coords[0] < f->orig_face[2]->coords[0]) && ((*i)->coords[0] > middle[0]))
#else
    if (osgabs((*i)->coords[1] - f->orig_quad[2]->coords[1]) < DCTP_EPS) {
      // right
      if (((*i)->coords[0] < f->orig_quad[2]->coords[0]) && ((*i)->coords[0] > middle[0]))
#endif
        sides[4].push_back(*i);
        // left
#ifdef OSG_UNION_TRI_QUAD
      else if (((*i)->coords[0] < middle[0]) && ((*i)->coords[0] > f->orig_face[3]->coords[0]))
#else
      else if (((*i)->coords[0] < middle[0]) && ((*i)->coords[0] > f->orig_quad[3]->coords[0]))
#endif
        sides[5].push_back(*i);
    } // end if
    // left up and down
    ...
    if (osgabs((*i)->coords[0] - f->orig_face[3]->coords[0]) < DCTP_EPS) {
      // down
      if (((*i)->coords[1] > f->orig_face[3]->coords[1]) && ((*i)->coords[1] < middle[1]))
#else
    if (osgabs((*i)->coords[0] - f->orig_quad[3]->coords[0]) < DCTP_EPS) {
      // down
      if (((*i)->coords[1] > f->orig_quad[3]->coords[1]) && ((*i)->coords[1] < middle[1]))
#endif
        sides[6].push_back(*i);
        // up
#ifdef OSG_UNION_TRI_QUAD
      else if (((*i)->coords[1] > middle[1]) && ((*i)->coords[1] < f->orig_face[0]->coords[1]))
#else
      else if (((*i)->coords[1] > middle[1]) && ((*i)->coords[1] < f->orig_quad[0]->coords[1]))
#endif
        sides[7].push_back(*i);
    } // end if
  }   // end for
