...
*/
ImageFileType* ImageFileHandler::getFileType(
    const char* mimeType, const char* fileName, bool validateHeader) {
  std::string                                     suffix;
  ...
  if (validateHeader) {
    ...
    if (fileName && *fileName && type != NULL && !type->validateHeader(fileName, implemented)) {
...
ImageFileType* ImageFileHandler::getDefaultType(void) {
  ...
  std::map<std::string, ImageFileType*>::iterator sI = _suffixTypeMap.find(dSuffix);
...
*/
Int32 ImageFileHandler::getSuffixList(std::list<const Char8*>& suffixList, UInt32 flags) {
  Int32                                           count = 0;
  std::map<std::string, ImageFileType*>::iterator sI;
...
will try to use the fileName suffix to determine the mimeType
*/
bool ImageFileHandler::read(ImagePtr& image, const char* fileName, const char* mimeType) {
  bool        retCode = false;
  ...
  if (type) {
    FDEBUG(("try to image read %s as %s\n", fullFilePath.c_str(), type->getMimeType()));
    ...
    if (retCode) {
      ...
      if (fullFilePath[0] != '/' && fullFilePath[0] != '\\' && fullFilePath[1] != ':') {
        ...
        if (base.size() < 2 || (base[0] != '/' && base[0] != '\\' && base[1] != ':')) {
    ...
    SWARNING << "could not read " << fullFilePath << "; unknown image format" << std::endl;
  }
...
will try to use the fileName suffix to determine the mimeType
*/
bool ImageFileHandler::write(const ImagePtr& image, const char* fileName, const char* mimeType) {
  bool               retCode = false;
  ImageFileType*     type;
  ...
  if ((type = getFileType(mimeType, fileName))) {
    SINFO << "try to write " << fileName << " as " << type->getMimeType() << std::endl;
    retCode = type->write(image, fileName);
  } else {
    SWARNING << "can't write " << fileName << "; unknown image format" << std::endl;
  }
...
bool ImageFileHandler::read(ImagePtr& image, std::istream& is, const std::string& mimeType) {
...
bool ImageFileHandler::write(const ImagePtr& image, std::ostream& os, const std::string& mimeType) {
  ImageFileType* type = getFileType(mimeType.c_str());
...
will try to use the fileName suffix to determine the mimeType
*/
UInt64 ImageFileHandler::restore(ImagePtr& image, const UChar8* buffer, Int32 memSize) {
...
will try to use the fileName suffix to determine the mimeType
*/
UChar8* ImageFileHandler::store(const ImagePtr& image, UInt64& memSize, const char* mimeType) {
  ImageFileType* type = 0;
  UChar8*        mem  = 0;
...
bool ImageFileHandler::addImageFileType(ImageFileType& fileType) {
  ...
  for (sI = fileType.getSuffixList().begin(); sI != fileType.getSuffixList().end(); ++sI) {
    suffix.assign(sI->str());
    normalizeSuffix(suffix);
    smI = _the->_suffixTypeMap.find(suffix);
    if (smI != _the->_suffixTypeMap.end()) {
      SWARNING << "Can't add an image file type with suffix " << suffix << " a second time"
               << std::endl;
    } else {
  ...
  if (tIt != _the->_typeMap.end()) {
    SWARNING << "Can't add an image file type with mimetype " << mimetype << " a second time"
             << std::endl;
  } else
