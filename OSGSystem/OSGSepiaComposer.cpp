...
void SepiaComposer::initMethod(void) {
}
...
void SepiaComposer::changed(BitVector whichField, UInt32 origin) {
  Inherited::changed(whichField, origin);
}

void SepiaComposer::dump(UInt32, const BitVector) const {
...
void SepiaComposer::open(void) {
  ...
  if (isDisplay) {
    ...
    SepiaLockAndBuildDescriptors(
        _pam, _pbuffer, NULL, bfsize, isDisplay, &_descvirtaddr, &_descphysaddr);
  } else {
    ...
    SepiaLockAndBuildDescriptors(
        _pam, _pbuffer, _zbuffer, bfsize, isDisplay, &_descvirtaddr, &_descphysaddr);
  }
#endif
}
...
void SepiaComposer::composeWindow(void) {
  ...
  // display node ?
  if (isClient()) {
    SepiaStartTransfer(_pam, _fromNode, SEPIA_NO_NODE, _descphysaddr);
    usleep(1000);
    ...
    glPixelZoom(1.0, 1.0);
    glDrawPixels(width, height, GL_RGBA, GL_UNSIGNED_BYTE, _pbuffer);
    glPopMatrix();
    ...
    SepiaStartTransfer(_pam, _fromNode, _toNode, _descphysaddr);
