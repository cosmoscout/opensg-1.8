...
void BINWriter::addToIdMap(FieldContainerPtr fcPtr) {
  ...
  if (_fcIdMap.find(fcPtr->getType().getCName()) == _fcIdMap.end()) {
    ...
    if (!(_fcIdMap
                .insert(
                    std::pair<std::string, std::vector<UInt32>>(fcPtr->getType().getCName(), idVec))
                .second)) {
      SWARNING << "BINWriter::addToIdMap(FieldContainerPtr):" << std::endl
               << "ERROR while inserting into _fcIdMap" << std::endl;
    };
  }
...
void BINWriter::doIndexFC(FieldContainerPtr fieldConPtr) {
  ...
  // insert into map
  std::pair<std::map<UInt32, FCInfo>::iterator, bool> element =
      _fcMap.insert(std::pair<UInt32, FCInfo>(fieldConPtr.getFieldContainerId(), FCInfo()));
  ...
  for (i = 1; i <= fcType.getNumFieldDescs(); ++i) {
    ...
    if (!fDesc->isInternal()) {
      ...
      if (strstr(fType.getCName(), "Ptr") != NULL) {
        ...
        if (fieldPtr->getCardinality() == FieldType::SINGLE_FIELD) {
          ...
          UInt32 j;
          for (j = 0; j < ((MFFieldContainerPtr*)fieldPtr)->size(); ++j) {
      ...
      if (strcmp(fDesc->getCName(), "attachments") == 0) {
        AttachmentMap::const_iterator mapIt  = ((SFAttachmentMap*)fieldPtr)->getValue().begin();
        AttachmentMap::const_iterator mapEnd = ((SFAttachmentMap*)fieldPtr)->getValue().end();
    ...
    if (fDesc->isInternal() && strcmp(fDesc->getCName(), "parent") != 0 &&
    ...
    if (fieldPtr->getCardinality() == FieldType::MULTI_FIELD && fieldPtr->isEmpty() == true) {
      continue;
    }

    // add field to fieldmask
    fcInfo.mask = (fcInfo.mask | fcType.getFieldDescription(i)->getFieldMask());
  }
...
void BINWriter::doWriteIndexedFC(void) {
  ...
  for (; i != _fcMap.end(); ++i, ++count) {
    SINFO << "writing container " << std::setw(4) << count << "/" << mapSize << "..." << std::endl;
