 ...
 */
void MultiDisplayWindow::dump(UInt32, const BitVector) const {
 ...
 */
void MultiDisplayWindow::serverInit(WindowPtr, UInt32) {
}
 ...
 */
void MultiDisplayWindow::serverRender(WindowPtr serverWindow, UInt32 id, RenderActionBase* action) {
  ...
  Real64 scaleCHeight =
      ((height - getYOverlap()) * (getVServers() - 1) + height) / (float)getHeight();
  ...
  for (cv = 0, sv = 0; cv < getPort().size(); cv++) {
    ...
    // set viewport size
    beginEditCP(serverPort, Viewport::LeftFieldMask | Viewport::BottomFieldMask |
                                Viewport::RightFieldMask | Viewport::TopFieldMask);
    ...
    endEditCP(serverPort, Viewport::LeftFieldMask | Viewport::BottomFieldMask |
                              Viewport::RightFieldMask | Viewport::TopFieldMask);
...
void MultiDisplayWindow::clientInit(void) {
  ...
  if (getManageClientViewports() == false || getClientWindow() == NullFC)
    return;
  ...
  if (getPort().size() == getClientWindow()->getPort().size()) {
    for (UInt32 v = 0; v < getPort().size() && !changed; v++) {
      ...
      if (vp->getRoot() != cvp->getRoot() || vp->getLeft() != cvp->getLeft() ||
          vp->getRight() != cvp->getRight() || vp->getBottom() != cvp->getBottom() ||
          vp->getTop() != cvp->getTop() || vp->getBackground() != cvp->getBackground() ||
 ...
 */
void MultiDisplayWindow::updateViewport(ViewportPtr& serverPort, ViewportPtr& clientPort) {
  bool equal, found;
  ...
  UInt32                    fcount =
      osgMin(serverPort->getType().getNumFieldDescs(), clientPort->getType().getNumFieldDescs());

  for (UInt32 i = 1; i <= fcount; ++i) {
    const FieldDescription* fdesc = type.getFieldDescription(i);
    // ignore attachments
    if (strcmp(fdesc->getCName(), "parent") == 0 || strcmp(fdesc->getCName(), "camera") == 0)
      continue;
      ...
      if (dst_field->getCardinality() == FieldType::SINGLE_FIELD) {
        ...
        if (strcmp(fdesc->getCName(), "foregrounds") == 0) {
          ...
          while (sFgndIt != serverPort->getForegrounds().end()) {
            ...
            if (filterFgnd != NullFC && !filterFgnd->getServer().empty())
          ...
          if (sFgndBag.size() != clientPort->getForegrounds().size()) {
            ...
            while (sFgndIt != sFgndBag.end() && cFgndIt != clientPort->getForegrounds().end() &&
                   *sFgndIt == *cFgndIt) {
              ++sFgndIt;
              ++cFgndIt;
            }

            if (sFgndIt != sFgndBag.end() || cFgndIt != clientPort->getForegrounds().end())
              equal = false;
          }
        } else {
